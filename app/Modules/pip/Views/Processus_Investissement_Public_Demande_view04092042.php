										...
										<div class="tab-content">
													...
													<div class="card-body">
														<div class="row">
															<div class="col-12">
																...
																<div class="row">
																	<div class="col-12">
																		<button class="input-group-addon btn btn-secondary mt-5 float-right mb-3" style="display: none;" type="button" id="descr_livrable_save"><?= lang('messages_lang.bouton_ajouter') ?><span id="loading_save_livrable"></button>
																	</div>
																</div>
															</div>
															...
															<div class="card-body">
																<div class="row">
																	...
																	<div class="col-12 my-4">
																		<button type="button" class="btn btn-primary RISQUE_PROJET_SAVE" style="float: right;"> <?= lang('messages_lang.bouton_ajouter') ?>
																		<span id="loading_risque"></span> </button>
																	</div>
											...
											<div id="crm_livrable" class="container tab-pane">
												...
												<div class="row mt-3">
													...
													<div class="col-6 col-md-3">
														<div class="form-group">
															<label for="CRM_livrable_an3"><?= $annees[1]->ANNEE_DESCRIPTION ?> <font color="red">*</font></label>
															<div class="input-group">
																<input type="hidden" id="CRM_livrable_annee_3" name="CRM_livrable_annee_3" value="<?= $annees[1]->ANNEE_BUDGETAIRE_ID ?>">
												...
												<div class="row">
													<div class="col-12 col-md-3 pt-4" style="float: right;<?php if(isset($oldValues)): ?>display: none;<?php endif; ?>">
														<button class="btn btn-primary cm_livrable_save_button"> <?= lang('messages_lang.bouton_ajouter') ?><span id="loading_save_cadre"></span> </button>
													</div>
												</div>
															...
															<div class="col-md-12">
																<table id="mytable12" class="table table-bordered">
																	<thead>
																		...
																		<th><?= $annees[1]->ANNEE_DESCRIPTION ?><br><div id="cible3"></div></th>
																	</thead>
																	<tbody>
																	</tbody>
																</table>
														...
														<div class="row" id="financement" <?php if(!isset($oldValues[0]->EST_CO_FINANCE)): ?>style="display: none;"<?php endif; ?>>
															...
															<div class="col-12 col-md-3">
																<div class="form-group">
																	...
																	<select class="form-control select2" autofocus name="bailleur" id="bailleur" onchange="">
																		<option value="" selected disabled>-- <?= lang('messages_lang.selection_message') ?> --</option>
																		<?php 
																		if(isset($bailleurs)): 
															...
															<div class="col-12 col-md-3">
																<div class="form-group">
																	...
																	<select class="form-control select2" name="devise_financement" id="devise_financement" onchange="change_montant()">
																		<option value="" selected disabled>-- <?= lang('messages_lang.selection_message') ?> --</option>
															...
															<div>
																...
																<div class="row">
																	...
																	<div class="col-12 col-md-4">
																		<div class="form-group">
																			<label for="SFP_an3"><?= $annees[1]->ANNEE_DESCRIPTION ?> <font color="red">*</font></label>
																			<div class="input-group">
																				<input type="hidden" id="annee_sfp_3" name="annee_sfp_3" value="<?= $annees[1]->ANNEE_BUDGETAIRE_ID ?>">
																				<input type="text" name="SFP_an3" id="SFP_an3" value="<?= $annee[1] ?? '' ?>" class="form-control">
																			</div>
																		</div>
																	</div>
																</div>
															...
															<div class="col-12 my-4 d-flex justify-content-end">
																<button class="btn btn-primary sfp_save_button mb-4" style="<?php if(isset($oldValues[0]->EST_CO_FINANCE) && $oldValues[0]->EST_CO_FINANCE == '0'): ?>display: none;<?php endif; ?>"> <?= lang('messages_lang.bouton_ajouter') ?><span id="loading_save_sf"> </button>
															</div>
														</div>
													</div>
												</div>
				...
				if(tab_projet.length)
				{
					...
					$.ajax(
					{
						...
						success: function(response)
						{
							...
							{
								$('.cartographie').before('<div class="alert alert-danger mt-2"><?=lang('messages_lang.province_commune_ont_deja_ete_ajoutees')?></div>')
	...
	$('#descr_livrable_save').click(function()
	{
		...
		if(success && count)
		{
			...
			$.ajax(
			{
				...
				beforeSend: function()
		        {
		          $('#loading_save_livrable').html("<i class='fa fa-spinner fa-pulse fa-1x fa-fw'></i>");
		          $('#descr_livrable_save').attr('disabled',true);
		        },

				success: function(response)
				{
					$('#descr_livrable_save').attr('disabled',false);
					$('#loading_save_livrable').html("");
					let count = $('#u_demande_livrable tbody').children().length
...
<script>
	...
	{		
		...
		if(success)
		{
			$('#loading_risque').html("<i class='fa fa-spinner fa-pulse fa-1x fa-fw'></i>");
			$('.RISQUE_PROJET_SAVE').attr('disabled',true);	
			...
			$.ajax(
			{
				...
				success: function(response)
				{
					$('.RISQUE_PROJET_SAVE').attr('disabled',false);
					$('#loading_risque').html("");
					removeFeedBack()
	...
	document.querySelector(".cm_livrable_save_button").addEventListener('click', (ev) =>
	{
		...
		if(success && count)
		{
			...
			$.ajax(
			{
				beforeSend: function()
		        {
		          $('#loading_save_cadre').html("<i class='fa fa-spinner fa-pulse fa-1x fa-fw'></i>");
		          $('.cm_livrable_save_button').attr('disabled',true);
		        },
				type: 'POST',
				...
				success: function(response)
				{
					$('#loading_save_cadre').html("");
		          	$('.cm_livrable_save_button').attr('disabled',false);
					let id = response
	...
	const checksfp = (ev) =>
	{
		...
		if(success && count)
		{
			...
			$.ajax(
			{
				...
				beforeSend: function()
		        {
		          $('#loading_save_sf').html("<i class='fa fa-spinner fa-pulse fa-1x fa-fw'></i>");
		          $('#sfp_save_button').attr('disabled',true);
		        },
				success: function(response)
				{
					$('#loading_save_sf').html("");
		          	$('#sfp_save_button').attr('disabled',false);
					let id = response
