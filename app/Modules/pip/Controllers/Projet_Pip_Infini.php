 ...
 * sonia@mediabox.bi
 * 02/01/2024
 * Liste des projet PIP incomplet
*/
  ...
  class Projet_Pip_Infini extends BaseController
  {
  	protected $library;
  	protected $ModelPs;
  	protected $session;
  	...
  	function liste_pip_infini()
  	{
  		...
  		return view('App\Modules\pip\Views\Projet_Pip_Infini_List_View',$data);
  	}

  	function liste_projet_infini()
  	{
  		$session  = \Config\Services::session();
  		...
  		foreach($fetch_cov_frais as $info)
  		{
  			...
  			$post[]=$DATE_INSERTION;
  			$corriger = lang('messages_lang.action_corriger_pip');
  			$supprimer = lang('messages_lang.supprimer_action');
  			...
  			<li><a class="dropdown-item" href="'.base_url('pip/Processus_Investissement_Public/demande/update/'.$info->ID_DEMANDE).'">'.$corriger.'</a></li>
  			<li><a class="dropdown-item"  onclick="confirmation('.$info->ID_DEMANDE_INFO_SUPP.')">'.$supprimer.'</a></li>
  	...
  	function annuler_projet_incomplet($id)
  	{
  		$session  = \Config\Services::session();
