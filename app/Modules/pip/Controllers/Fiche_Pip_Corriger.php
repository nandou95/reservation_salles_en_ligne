...
class Fiche_Pip_Corriger extends BaseController
{
  protected $library;
  protected $ModelPs;
  protected $session;
  ...
  function liste_pip_corriger()
  {
    ...
    return view('App\Modules\pip\Views\Fiche_Pip_Corriger_List_View',$data);
  }

  function liste_projet_corriger()
  {
    $session  = \Config\Services::session();
    ...
    $query_principal="SELECT ID_DOC_COMPILATION, PATH_DOC_COMPILER,proc_etape.DESCR_ETAPE ,CODE_PIP, DATE_COMPILATION,STATUT FROM pip_document_compilation fiche JOIN proc_etape ON proc_etape.ETAPE_ID=fiche.ETAPE_ID WHERE STATUT=1";
    ...
    foreach($fetch_cov_frais as $info)
    {
      ...
      $projets='<center><a onclick="get_pip_corriger('.$info->ID_DOC_COMPILATION.')" href="javascript:;" ><button class="btn btn-primary"><b style="color:white;">'.$get_nbre_proj['nbre'].'</b></button></a></center>';
      ...
      $action ="<a class='btn btn-primary btn-sm' title='{$detail}' href='".base_url("pip/Processus_Investissement_Public/correction_compilation/".$info->ID_DOC_COMPILATION)."' ><i class='fa fa-list'></a>";
      $post[]= $action;
      $data[]=$post;  
    }
  ...
  function get_corriger_pip($id=0)
  {
    $db = db_connect();
    $session  = \Config\Services::session();
    $user_id ='';
