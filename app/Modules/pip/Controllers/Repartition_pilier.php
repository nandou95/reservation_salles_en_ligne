...
class Repartition_pilier extends BaseController
{
	protected $session;
	...
	public function index()
	{
		...
		return view('App\Modules\pip\Views\Repartition_pilier_list_view',$data); 
	}

	//Debut de la recuperation des projets par ministere 
	public function list_pilier()
	{
		$db = db_connect();
		$session  = \Config\Services::session();
		...
		$order_by = '';
		$order_column = array(1, 'DESCR_PILIER',1,1,1,1,1);
		$order_by = isset($_POST['order']) ? ' ORDER BY ' . $order_column[$_POST['order']['0']['column']] . '  ' . $_POST['order']['0']['dir'] : ' ORDER BY ID_PILIER DESC';

		$search = !empty($_POST['search']['value']) ? (' AND (DESCR_PILIER LIKE "%' . $var_search . '%")') : '';
		...
		$requetedebase1= "SELECT DISTINCT pili.DESCR_PILIER, fina.ANNE_UN, fina.ANNE_DEUX, fina.ANNE_TROIS, fina.TOTAL_TRIENNAL FROM pilier pili JOIN pip_demande_infos_supp info JOIN pip_demande_source_financement fina JOIN user_users user ON pili.ID_PILIER = info.ID_PILIER AND info.ID_DEMANDE_INFO_SUPP = fina.ID_DEMANDE_INFO_SUPP  WHERE 1 ".$user_connecte.""; 
		...
		$requetedebase3= "SELECT SUM(ANNE_UN) AS TOT_ANNE1, SUM(ANNE_DEUX) AS TOT_ANNE2,  SUM(ANNE_TROIS) AS TOT_ANNE3, SUM(TOTAL_TRIENNAL) AS TOT_TOT_TRI FROM pilier pili JOIN pip_demande_infos_supp info JOIN pip_demande_source_financement fina JOIN user_users user ON pili.ID_PILIER = info.ID_PILIER AND info.ID_DEMANDE_INFO_SUPP = fina.ID_DEMANDE_INFO_SUPP AND user.INSTITUTION_ID = info.INSTITUTION_ID WHERE 1 ".$user_connecte."";
