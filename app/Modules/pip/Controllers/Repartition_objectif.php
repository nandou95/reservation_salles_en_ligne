...
class Repartition_objectif extends BaseController
{
	protected $session;
	...
	public function index()
	{
		...
		return view('App\Modules\pip\Views\Repartition_objectif_list_view',$data); 
	}
	...
	public function list_objectif()
	{
		...
		$order_by = '';
		$order_column = array(1, 'DESCR_OBJECTIF_STRATEGIC',1,1,1,1,1);
		$order_by = isset($_POST['order']) ? ' ORDER BY ' . $order_column[$_POST['order']['0']['column']] . '  ' . $_POST['order']['0']['dir'] : ' ORDER BY ID_OBJECT_STRATEGIQUE DESC';

		$search = !empty($_POST['search']['value']) ? (' AND (DESCR_OBJECTIF_STRATEGIC LIKE "%' . $var_search . '%")') : '';
		...
		$requetedebase1= "SELECT DISTINCT obj.DESCR_OBJECTIF_STRATEGIC, fina.ANNE_UN, fina.ANNE_DEUX, fina.ANNE_TROIS, fina.TOTAL_TRIENNAL FROM objectif_strategique obj JOIN pip_demande_infos_supp info JOIN pip_demande_source_financement fina JOIN user_users user ON obj.ID_OBJECT_STRATEGIQUE = info.ID_OBJECT_STRATEGIQUE AND info.ID_DEMANDE_INFO_SUPP = fina.ID_DEMANDE_INFO_SUPP AND user.INSTITUTION_ID = info.INSTITUTION_ID WHERE 1 ".$user_connecte.""; 
		...
		$requetedebase3= "SELECT SUM(ANNE_UN) AS TOT_ANNE1, SUM(ANNE_DEUX) AS TOT_ANNE2,  SUM(ANNE_TROIS) AS TOT_ANNE3, SUM(TOTAL_TRIENNAL) AS TOT_TOT_TRI FROM objectif_strategique obj JOIN pip_demande_infos_supp info JOIN pip_demande_source_financement fina JOIN user_users user ON obj.ID_OBJECT_STRATEGIQUE = info.ID_OBJECT_STRATEGIQUE AND info.ID_DEMANDE_INFO_SUPP = fina.ID_DEMANDE_INFO_SUPP AND user.INSTITUTION_ID = info.INSTITUTION_ID WHERE 1 ".$user_connecte." GROUP BY TOTAL_TRIENNAL";
