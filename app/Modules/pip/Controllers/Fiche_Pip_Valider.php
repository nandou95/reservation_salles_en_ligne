...
* sonia@mediabox.bi
* 04/01/2024
* Liste des fiches des pip valider
*/
...
class Fiche_Pip_Valider extends BaseController
{
  protected $library;
  protected $ModelPs;
  protected $session;
  ...
  function liste_pip_Valider()
  {
    ...
    return view('App\Modules\pip\Views\Fiche_Pip_Valider_List_View',$data);
  }

  function liste_projet_valider()
  {
    $session  = \Config\Services::session();
    ...
    $query_principal = "SELECT ID_DOC_COMPILATION, PATH_DOC_COMPILER,proc_etape.DESCR_ETAPE ,CODE_PIP, DATE_COMPILATION,STATUT FROM pip_document_compilation fiche JOIN proc_etape ON proc_etape.ETAPE_ID=fiche.ETAPE_ID WHERE STATUT=2";
    ...
    $order_by='';
    $order_column='';
    $order_column= array(1,'CODE_PIP','PATH_DOC_COMPILER',1,'proc_etape.DESCR_ETAPE','DATE_COMPILATION',1);
    ...
    {
      $nbre_proj = 'SELECT COUNT(ID_PRO_COMPILER) as nbre,ID_DEMANDE FROM pip_projet_compiler WHERE 1 AND ID_DOC_COMPILATION = '.$info->ID_DOC_COMPILATION;
      $nbre_proj = "CALL `getTable`('" . $nbre_proj . "');";
      $get_nbre_proj = $this->ModelPs->getRequeteOne($nbre_proj);

      $projets='<center><a onclick="get_pip_valider('.$info->ID_DOC_COMPILATION.')" href="javascript:;" ><button class="btn btn-primary"><b style="color:white;">'.$get_nbre_proj['nbre'].'</b></button></a></center>';

      $documa = (!empty($info->PATH_DOC_COMPILER)) ? '<a onclick="pip_doc('.$info->ID_DOC_COMPILATION.')" href="javascript:;"><span style="font-size: 30px;color:red;" class="fa fa-file-pdf"></span></a>' : 'N/A' ;
      ...
      $action ="<a class='btn btn-primary btn-sm' title='{$detail}' href='".base_url("pip/Processus_Investissement_Public/details/".$get_nbre_proj['ID_DEMANDE'])."' ><i class='fa fa-list'></a>";
      $post[] = $action;
      $data[]=$post;  
    }
