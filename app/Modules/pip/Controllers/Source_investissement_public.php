...
class Source_investissement_public extends BaseController
{
	protected $session;
	...
	// Affichage de la liste des investissement public
	public function index()
	{
		...
		return view('App\Modules\pip\Views\Source_investissement_public_list_view',$data); 
	}

	//Debut de la recuperation des investissement public
	public function list_investissement()
	{
		$db = db_connect();
		$session  = \Config\Services::session();
		...
		$order_by = '';
		$order_column = array(1, 'NOM_SOURCE_FINANCE',1,1,1,1,1,1);
		$order_by = isset($_POST['order']) ? ' ORDER BY ' . $order_column[$_POST['order']['0']['column']] . '  ' . $_POST['order']['0']['dir'] : ' ORDER BY ID_SOURCE_FINANCE_BAILLEUR DESC';

		$search = !empty($_POST['search']['value']) ? (' AND (NOM_SOURCE_FINANCE LIKE "%' . $var_search . '%")') : '';
		...
		$requetedebase1= "SELECT DISTINCT bailleur.CODE_BAILLEUR, bailleur.NOM_SOURCE_FINANCE, fina.ANNE_UN, fina.ANNE_DEUX, fina.ANNE_TROIS, fina.TOTAL_TRIENNAL FROM pip_source_financement_bailleur bailleur JOIN pip_demande_infos_supp info JOIN pip_demande_source_financement fina JOIN user_users user ON bailleur.ID_SOURCE_FINANCE_BAILLEUR  = fina.ID_SOURCE_FINANCE_BAILLEUR AND info.ID_DEMANDE_INFO_SUPP = fina.ID_DEMANDE_INFO_SUPP AND user.INSTITUTION_ID = info.INSTITUTION_ID  WHERE 1 ".$user_connecte.""; 
		...
		$requetedebasefilter = $requetedebase1 . ' ' . $conditionsfilter;

		$requetedebase3= "SELECT SUM(fina.ANNE_UN) AS TOT_ANNE1, SUM(fina.ANNE_DEUX) AS TOT_ANNE2,  SUM(fina.ANNE_TROIS) AS TOT_ANNE3, SUM(fina.TOTAL_TRIENNAL) AS TOT_TOT_TRI FROM pip_source_financement_bailleur bailleur JOIN pip_demande_infos_supp info JOIN pip_demande_source_financement fina JOIN user_users user ON bailleur.ID_SOURCE_FINANCE_BAILLEUR  = fina.ID_SOURCE_FINANCE_BAILLEUR AND info.ID_DEMANDE_INFO_SUPP = fina.ID_DEMANDE_INFO_SUPP AND user.INSTITUTION_ID = info.INSTITUTION_ID  WHERE 1 ".$user_connecte.""; 
		...
		foreach ($fetch_data1 as $row)
		{
			...
			//Calcul du pourcentage des trois annees par pilier
			$totTot3ans = !empty($fetch_actions3['TOT_TOT_TRI'])?$fetch_actions3['TOT_TOT_TRI']:$tot3ans; 
