 ...
 * sonia@mediabox.bi
 * 02/01/2024
 * Liste des projet PIP a corrige
*/
  ...
  class Projet_Pip_Corrige extends BaseController
  {
  	protected $library;
  	protected $ModelPs;
  	protected $session;
  	...
  	function liste_pip_corrige()
  	{
  		...
  		return view('App\Modules\pip\Views\Projet_Pip_Corrige_List_View',$data);
  	}

  	function liste_projet_corrige()
  	{
  		$session  = \Config\Services::session();
  		...
  		foreach($fetch_cov_frais as $info)
  		{
  			...
  			$post[]=$DATE_INSERTION;
  			$action='';
  			$detail = lang('messages_lang.detail_title');
  			$correction = lang('messages_lang.correction_title');
  			if($info->IS_CORRECTION == 1)
  			{
  				$action ="<a class='btn btn-primary btn-sm' title='{$detail}' href='".base_url("pip/Processus_Investissement_Public/details/".$info->ID_DEMANDE)."' ><i class='fa fa-arrow-up'></a>";
  			}
  			else if($info->IS_CORRECTION == 2)
  			{
  				$action ="<a class='btn btn-primary btn-sm' title='{$correction}' href='".base_url("pip/Processus_Investissement_Public/demande/update/".$info->ID_DEMANDE)."' ><i class='fa fa-arrow-up'></a>";
  			}
