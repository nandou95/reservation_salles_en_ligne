  ...
  //get liste de classification economique
	public function listing($value = 0)
	{
		...
		foreach($fetch_actions as $row)
		{
			$sub_array = array();
			//execution par ptba
			$get_exec = "SELECT ptba_tache.PTBA_TACHE_ID,SUM(ebet.MONTANT_ENG_BUDGETAIRE) AS MONTANT_ENGAGE,SUM(ebet.MONTANT_ENG_JURIDIQUE) AS MONTANT_JURIDIQUE,SUM(ebet.MONTANT_LIQUIDATION) AS MONTANT_LIQUIDATION,SUM(ebet.MONTANT_ORDONNANCEMENT) AS MONTANT_ORDONNANCEMENT,SUM(ebet.MONTANT_PAIEMENT) AS PAIEMENT,SUM(ebet.MONTANT_DECAISSEMENT) AS MONTANT_DECAISSEMENT FROM execution_budgetaire_titre_decaissement ebtd JOIN execution_budgetaire exec ON ebtd.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID JOIN execution_budgetaire_execution_tache ebet ON ebet.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=ebet.PTBA_TACHE_ID WHERE ebtd.ETAPE_DOUBLE_COMMANDE_ID NOT IN(42) AND ebet.PTBA_TACHE_ID=".$row->PTBA_TACHE_ID." ".$critere_date;
	...
	function exporter($INSTITUTION_ID=0,$PROGRAMME_ID=0,$DATE_DEBUT=0,$DATE_FIN=0,$ANNEE_BUDGETAIRE_ID=0,$GRANDE_MASSE_ID=0,$is_action=0)
	{
      ...
      if($mont_transf_inst['INSTITUTION_ID']==$mont_recep_inst['INSTITUTION_ID'])
      {
      	...
      	$CREDIT_APRES_TRANSFERT_INST = floatval($BUDGET_VOTE_INST) + (floatval($MONTANT_RECEPTION_INST) - floatval($MONTANT_TRANSFERT_INST));
			...
			$params_infos_inst='SELECT SUM(ebet.MONTANT_ENG_BUDGETAIRE) AS MONTANT_ENGAGE,SUM(ebet.MONTANT_ENG_JURIDIQUE) AS MONTANT_JURIDIQUE,SUM(ebet.MONTANT_LIQUIDATION) AS MONTANT_LIQUIDATION,SUM(ebet.MONTANT_ORDONNANCEMENT) AS MONTANT_ORDONNANCEMENT,SUM(ebet.MONTANT_PAIEMENT) AS MONTANT_PAIEMENT,SUM(ebet.MONTANT_DECAISSEMENT) AS MONTANT_DECAISSEMENT FROM execution_budgetaire_titre_decaissement ebtd JOIN execution_budgetaire exec ON ebtd.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID JOIN execution_budgetaire_execution_tache ebet ON ebet.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=ebet.PTBA_TACHE_ID JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID WHERE ebtd.ETAPE_DOUBLE_COMMANDE_ID NOT IN(42) AND ptba.INSTITUTION_ID='.$key->INSTITUTION_ID.' '.$critere_date.' '.$critere_annee.' GROUP BY ptba.INSTITUTION_ID';
		  ...
		  $progr= $this->ModelPs->getRequete($callpsreq,$bindprogr);
			foreach($progr as $valueprog)
			{
				...
				$params_infos_prog='SELECT SUM(ebet.MONTANT_ENG_BUDGETAIRE) AS MONTANT_ENGAGE_PROG,SUM(ebet.MONTANT_ENG_JURIDIQUE) AS MONTANT_JURIDIQUE_PROG,SUM(ebet.MONTANT_LIQUIDATION) AS MONTANT_LIQUIDATION_PROG,SUM(ebet.MONTANT_ORDONNANCEMENT) AS MONTANT_ORDONNANCEMENT_PROG,SUM(ebet.MONTANT_PAIEMENT) AS MONTANT_PAIEMENT_PROG,SUM(ebet.MONTANT_DECAISSEMENT) AS MONTANT_DECAISSEMENT_PROG FROM execution_budgetaire_titre_decaissement ebtd JOIN execution_budgetaire exec ON ebtd.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID JOIN execution_budgetaire_execution_tache ebet ON ebet.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=ebet.PTBA_TACHE_ID JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID WHERE ebtd.ETAPE_DOUBLE_COMMANDE_ID NOT IN(42) AND ptba.INSTITUTION_ID='.$key->INSTITUTION_ID.' AND ptba.PROGRAMME_ID= '.$valueprog->PROGRAMME_ID.' '.$critere_date.' '.$critere_annee;
				...
				if($is_action==1)
				{
					...
					foreach ($actions as $key_action) 
					{
			  		...
			  		// debut execution action ------------------------------------------------------
						$params_infos_action='SELECT SUM(ebet.MONTANT_ENG_BUDGETAIRE) AS MONTANT_ENGAGE_ACTIO,SUM(ebet.MONTANT_ENG_JURIDIQUE) AS MONTANT_JURIDIQUE_ACTIO,SUM(ebet.MONTANT_LIQUIDATION) AS MONTANT_LIQUIDATION_ACTIO,SUM(ebet.MONTANT_ORDONNANCEMENT) AS MONTANT_ORDONNANCEMENT_ACTIO,SUM(ebet.MONTANT_PAIEMENT) AS MONTANT_PAIEMENT_ACTIO,SUM(ebet.MONTANT_DECAISSEMENT) AS MONTANT_DECAISSEMENT_ACTIO FROM execution_budgetaire_titre_decaissement ebtd JOIN execution_budgetaire exec ON ebtd.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID JOIN execution_budgetaire_execution_tache ebet ON ebet.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=ebet.PTBA_TACHE_ID JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID WHERE ebtd.ETAPE_DOUBLE_COMMANDE_ID NOT IN(42) AND ptba.INSTITUTION_ID='.$key->INSTITUTION_ID.' AND ptba.PROGRAMME_ID='.$valueprog->PROGRAMME_ID.' AND ptba.ACTION_ID ='.$key_action->ACTION_ID.' '.$critere_date.' '.$critere_annee;
			  		...
			  		//Debut Gestion des grandes masses
						...
						foreach($grandmasse as $masses)
						{
				  		...
				  		//debut gestion du montant vote par grande masse
							...
							$params_infos_gm="SELECT SUM(ebet.MONTANT_ENG_BUDGETAIRE) AS MONTANT_ENGAGE_GM,SUM(ebet.MONTANT_ENG_JURIDIQUE) AS MONTANT_JURIDIQUE_GM,SUM(ebet.MONTANT_LIQUIDATION) AS MONTANT_LIQUIDATION_GM,SUM(ebet.MONTANT_ORDONNANCEMENT) AS MONTANT_ORDONNANCEMENT_GM,SUM(ebet.MONTANT_PAIEMENT) AS MONTANT_PAIEMENT_GM,SUM(ebet.MONTANT_DECAISSEMENT) AS MONTANT_DECAISSEMENT_GM FROM execution_budgetaire_titre_decaissement ebtd JOIN execution_budgetaire exec ON ebtd.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID JOIN execution_budgetaire_execution_tache ebet ON ebet.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=ebet.PTBA_TACHE_ID JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID WHERE ebtd.ETAPE_DOUBLE_COMMANDE_ID NOT IN(42) AND ptba.INSTITUTION_ID=".$key->INSTITUTION_ID." AND ptba.PROGRAMME_ID=".$valueprog->PROGRAMME_ID." AND ptba.ACTION_ID=".$key_action->ACTION_ID." AND ptba.GRANDE_MASSE_ID=".$masses->GRANDE_MASSE_ID." ".$critere_annee."".$critere_date;
	...
	function export_pdf($INSTITUTION_ID=0,$PROGRAMME_ID=0,$DATE_DEBUT=0,$DATE_FIN=0,$ANNEE_BUDGETAIRE_ID=0,$GRANDE_MASSE_ID=0,$is_action=0)
	{
      ...
      if($mont_transf_inst['INSTITUTION_ID']==$mont_recep_inst['INSTITUTION_ID'])
      {
      	...
      	$CREDIT_APRES_TRANSFERT_INST = floatval($BUDGET_VOTE_INST) + (floatval($MONTANT_RECEPTION_INST) - floatval($MONTANT_TRANSFERT_INST));
			...
			{
			  // debut execution programme ------------------------------------------------------
				$params_infos_prog='SELECT SUM(ebet.MONTANT_ENG_BUDGETAIRE) AS MONTANT_ENGAGE_PROG,SUM(ebet.MONTANT_ENG_JURIDIQUE) AS MONTANT_JURIDIQUE_PROG,SUM(ebet.MONTANT_LIQUIDATION) AS MONTANT_LIQUIDATION_PROG,SUM(ebet.MONTANT_ORDONNANCEMENT) AS MONTANT_ORDONNANCEMENT_PROG,SUM(ebet.MONTANT_PAIEMENT) AS MONTANT_PAIEMENT_PROG,SUM(ebet.MONTANT_DECAISSEMENT) AS MONTANT_DECAISSEMENT_PROG FROM execution_budgetaire_titre_decaissement ebtd JOIN execution_budgetaire exec ON ebtd.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID JOIN execution_budgetaire_execution_tache ebet ON ebet.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID LEFT JOIN transfert_historique_transfert histo ON histo.PTBA_TACHE_ID_TRANSFERT=ebet.PTBA_TACHE_ID JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=ebet.PTBA_TACHE_ID JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID WHERE ebtd.ETAPE_DOUBLE_COMMANDE_ID NOT IN(42) AND ptba.INSTITUTION_ID='.$key->INSTITUTION_ID.' AND ptba.PROGRAMME_ID= '.$valueprog->PROGRAMME_ID.' '.$critere_date.' '.$critere_annee;
				$infos_sup_prog=$this->ModelPs->getRequeteOne('CALL getTable("'.$params_infos_prog.'")');
