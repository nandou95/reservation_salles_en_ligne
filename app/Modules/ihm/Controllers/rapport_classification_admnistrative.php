...
class rapport_classification_admnistrative extends BaseController
{
  ...
  public function export($INSTITUTION_ID='',$SOUS_TUTEL_ID='',$PROGRAMME_ID='',$ACTION_ID='',$ANNEE_BUDGETAIRE_ID='',$DATE_DEBUT='',$DATE_FIN='',$activ='')
  {
    ...
    {
      $params_infos='SELECT SUM(ebet.MONTANT_ENG_BUDGETAIRE) AS MONTANT_ENGAGE,SUM(ebet.MONTANT_ENG_JURIDIQUE) AS MONTANT_JURIDIQUE,SUM(ebet.MONTANT_LIQUIDATION) AS MONTANT_LIQUIDATION,SUM(ebet.MONTANT_ORDONNANCEMENT) AS MONTANT_ORDONNANCEMENT ,SUM(ebet.MONTANT_PAIEMENT) AS PAIEMENT ,SUM(ebet.MONTANT_DECAISSEMENT) AS MONTANT_DECAISSEMENT FROM execution_budgetaire_titre_decaissement ebtd JOIN execution_budgetaire exec ON ebtd.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID JOIN execution_budgetaire_execution_tache ebet ON ebet.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=ebet.PTBA_TACHE_ID JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID WHERE ebtd.ETAPE_DOUBLE_COMMANDE_ID NOT IN(42) AND ptba.INSTITUTION_ID='.$key->INSTITUTION_ID.' '.$critere_annee.''.$critere_date.'';
