...
class Detail_Programme extends BaseController
{
  ...
  function index($id)
  {
    ...
    else
    {
      ...
      //vote
      $bind_montant_vote = $this->getBindParms('SUM(`T1`) as T1,SUM(`T2`) as T2,SUM(`T3`) as T3,SUM(`T4`) as T4','ptba','`PROGRAMME_ID`="'.$data['programme']['PROGRAMME_ID'].'" ','SUM(`T1`)');
      ...
      //execute 
      $mont_exe = "SELECT SUM(racc.MONTANT_RACCROCHE) as EXECUTEE,tranche.CODE_TRANCHE FROM execution_budgetaire_raccrochage_activite_new racc JOIN op_tranches tranche ON tranche.TRANCHE_ID=racc.TRIMESTRE_ID JOIN ptba ON ptba.PTBA_ID=racc.PTBA_ID JOIN inst_institutions_programmes prog ON prog.PROGRAMME_ID=ptba.PROGRAMME_ID WHERE 1 AND prog.CODE_PROGRAMME ='".$data['programme']['CODE_PROGRAMME']."' GROUP BY tranche.CODE_TRANCHE";
      ...
      $nbre_activite = 'SELECT ptba.PTBA_ID, prog.`PROGRAMME_ID`,ptba.CODES_PROGRAMMATIQUE,ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE AS CODE_NOMENCLATURE_BUDGETAIRE,ptba.ACTIVITES,ptba.T1 AS T1,ptba.T2 AS T2,ptba.T3 AS T3,ptba.T4 AS T4 FROM `inst_institutions_programmes` prog JOIN ptba ON ptba.PROGRAMME_ID=prog.`PROGRAMME_ID` JOIN inst_institutions_ligne_budgetaire ligne_budg ON ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE_ID=ptba.CODE_NOMENCLATURE_BUDGETAIRE_ID WHERE 1 AND MD5(prog.`PROGRAMME_ID`) = "'.$id.'"';
      ...
      $nbre_ligne= 'SELECT ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE,progr.PROGRAMME_ID,progr.CODE_PROGRAMME,SUM(ptba.T1) AS T1,SUM(ptba.T2) AS T2,SUM(ptba.T3) AS T3,SUM(ptba.T4) AS T4 FROM inst_institutions_programmes progr JOIN ptba ON ptba.PROGRAMME_ID=progr.PROGRAMME_ID JOIN inst_institutions_ligne_budgetaire ligne_budg ON ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE_ID=ptba.CODE_NOMENCLATURE_BUDGETAIRE_ID WHERE 1 AND MD5(progr.PROGRAMME_ID) ="'.$id.'" GROUP BY progr.PROGRAMME_ID,progr.CODE_PROGRAMME,ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE';
      ...
      //fin
      return view('App\Modules\ptba\Views\Detail_Programme_View',$data);
    }
  }
  ...
  function liste_action($id)
  {
    ...
    $search = !empty($_POST['search']['value']) ?  (" AND (prog.`CODE_PROGRAMME` LIKE '%$var_search%' OR act.CODE_ACTION LIKE '%$var_search%' OR prog.OBJECTIF_DU_PROGRAMME LIKE '%$var_search%')"):'';
    ...
    {
      $montant_exe ="SELECT act.CODE_ACTION,SUM(racc.MONTANT_RACCROCHE) AS MONTANT_RACCROCHE FROM execution_budgetaire_raccrochage_activite_new racc JOIN ptba ON ptba.PTBA_ID=racc.PTBA_ID JOIN inst_institutions_actions act ON act.ACTION_ID=ptba.ACTION_ID WHERE 1 AND act.CODE_ACTION= '".$info->CODE_ACTION."' GROUP BY act.CODE_ACTION";
  ...
  function liste_activite($id)
  {
    ...
    $query_principal="SELECT ptba.PTBA_ID, prog.`PROGRAMME_ID`,ptba.CODES_PROGRAMMATIQUE,ligne.CODE_NOMENCLATURE_BUDGETAIRE,ptba.CODES_PROGRAMMATIQUE,ptba.ACTIVITES,ptba.T1 AS T1,ptba.T2 AS T2,ptba.T3 AS T3,ptba.T4 AS T4 FROM inst_institutions_programmes prog JOIN ptba ON ptba.PROGRAMME_ID=prog.PROGRAMME_ID JOIN inst_institutions_ligne_budgetaire ligne ON ligne.CODE_NOMENCLATURE_BUDGETAIRE_ID=ptba.CODE_NOMENCLATURE_BUDGETAIRE_ID WHERE 1 AND prog.`PROGRAMME_ID`=".$id."";
    ...
    $order_by = isset($_POST['order']) ? ' ORDER BY '. $order_column[$_POST['order']['0']['column']] .' '.$_POST['order']['0']['dir'] : ' ORDER BY prog.CODES_PROGRAMMATIQUE ASC';
  ...
  public function  liste_ligne_budget($id)
  {
    ...
    foreach($fetch_cov_frais as $info)
    {
      $montant ="SELECT ligne.CODE_NOMENCLATURE_BUDGETAIRE,SUM(racc.MONTANT_RACCROCHE) AS MONTANT_RACCROCHE FROM execution_budgetaire_raccrochage_activite_new racc JOIN ptba ON ptba.PTBA_ID=racc.PTBA_ID JOIN inst_institutions_ligne_budgetaire ligne ON ligne.CODE_NOMENCLATURE_BUDGETAIRE_ID=ptba.CODE_NOMENCLATURE_BUDGETAIRE_ID WHERE 1 AND ligne.CODE_NOMENCLATURE_BUDGETAIRE='".$info->CODE_NOMENCLATURE_BUDGETAIRE."' GROUP BY ligne.CODE_NOMENCLATURE_BUDGETAIRE";
