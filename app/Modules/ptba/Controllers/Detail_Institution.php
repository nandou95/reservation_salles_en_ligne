...
class Detail_Institution extends BaseController
{
  ...
  function index($id=0)
  {
    ...
    $bind_id_exist = $this->getBindParms('INSTITUTION_ID','inst_institutions','MD5(INSTITUTION_ID)="'.$id.'"','1');
    ...
    if(!empty($id_exist))
    {
      ...
      //print_r($data['get_institution']['CODE_INSTITUTION']);die();
      ...
      //montant voté
      $bind_montant_vote = $this->getBindParms('SUM(`T1`) as T1,SUM(`T2`) as T2,SUM(`T3`) as T3,SUM(`T4`) as T4','ptba','INSTITUTION_ID="'.$data['get_institution']['INSTITUTION_ID'].'"','SUM(`T1`)');
      ...
      $mont_exe ="SELECT SUM(racc.MONTANT_RACCROCHE) as EXECUTEE,tranche.CODE_TRANCHE FROM execution_budgetaire_raccrochage_activite_new racc JOIN op_tranches tranche ON tranche.TRANCHE_ID=racc.TRIMESTRE_ID   WHERE 1 AND MD5(racc.INSTITUTION_ID)='".$id."' GROUP BY tranche.CODE_TRANCHE";
      ...
      $nbre_action = 'SELECT inst.INSTITUTION_ID,inst.CODE_INSTITUTION,inst_institutions_actions.CODE_ACTION,inst_institutions_actions.LIBELLE_ACTION,inst_institutions_actions.OBJECTIF_ACTION,SUM(ptba.T1) AS T1,SUM(ptba.T2) AS T2,SUM(ptba.T3) AS T3,SUM(ptba.T4) AS T4 FROM inst_institutions inst JOIN ptba ON ptba.INSTITUTION_ID=inst.INSTITUTION_ID JOIN inst_institutions_actions ON inst_institutions_actions.ACTION_ID=ptba.ACTION_ID WHERE 1 AND MD5(inst.INSTITUTION_ID) = "'.$id.'" GROUP BY inst.INSTITUTION_ID,inst.CODE_INSTITUTION,inst_institutions_actions.CODE_ACTION,inst_institutions_actions.LIBELLE_ACTION,inst_institutions_actions.OBJECTIF_ACTION';
      ...
      $nbre_activite = 'SELECT ptba.PTBA_ID, inst.INSTITUTION_ID,inst.CODE_INSTITUTION,ptba.CODES_PROGRAMMATIQUE,ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE,ptba.CODES_PROGRAMMATIQUE,ptba.ACTIVITES, T1,T2,T3, T4 FROM inst_institutions inst JOIN ptba ON ptba.INSTITUTION_ID=inst.INSTITUTION_ID JOIN  inst_institutions_ligne_budgetaire ligne_budg ON ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE_ID  = ptba.CODE_NOMENCLATURE_BUDGETAIRE_ID WHERE 1 AND MD5(inst.INSTITUTION_ID) ="'.$id.'"';
      ...
      $nbre_ligne= 'SELECT inst.INSTITUTION_ID,inst.CODE_INSTITUTION,SUM(ptba.T1) AS T1,SUM(ptba.T2) AS T2,SUM(ptba.T3) AS T3,SUM(ptba.T4),ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE AS T4 FROM inst_institutions inst JOIN ptba ON ptba.INSTITUTION_ID=inst.INSTITUTION_ID JOIN  inst_institutions_ligne_budgetaire ligne_budg ON ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE_ID  = ptba.CODE_NOMENCLATURE_BUDGETAIRE_ID WHERE 1 AND MD5(inst.INSTITUTION_ID) ="'.$id.'" GROUP BY inst.INSTITUTION_ID,inst.CODE_INSTITUTION,ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE';
      ...
      return view('App\Modules\ptba\Views\Detail_Institution_View',$data);
  ...
  public function detail_rapport()
  {
    ...
    {
    // BUDGET VOTE
      $query_principal="SELECT ptba.`ACTIVITES` AS NAME1,`T1`,`T2`,`T3`,`T4`,inst_institutions_sous_tutel.DESCRIPTION_SOUS_TUTEL AS DESCRIPTION_INSTITUTION  FROM ptba JOIN  inst_institutions_ligne_budgetaire ligne_budg ON ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE_ID  = ptba.CODE_NOMENCLATURE_BUDGETAIRE_ID JOIN inst_institutions_sous_tutel ON  SUBSTR(ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE,5,3)=inst_institutions_sous_tutel.CODE_SOUS_TUTEL";
    ...
    {
    // BUDGET EXECUTE
      $query_principal="SELECT `MONTANT_RACCROCHE` AS MONTANT,DESCRIPTION_INSTITUTION,ptba.ACTIVITES AS NAME1  FROM `execution_budgetaire_raccrochage_activite_new` racc JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=racc.INSTITUTION_ID JOIN ptba ON ptba.PTBA_ID=racc.PTBA_ID WHERE 1 ";
      if($_POST['order']['0']['column']!=0)
      {
  ...
  public function  liste_programme($id)
  {
    ...
    $query_principal="SELECT progr.PROGRAMME_ID,inst.INSTITUTION_ID,inst.CODE_INSTITUTION,progr.CODE_PROGRAMME,progr.INTITULE_PROGRAMME,progr.OBJECTIF_DU_PROGRAMME,SUM(ptba.T1) AS T1,SUM(ptba.T2) AS T2,SUM(ptba.T3) AS T3,SUM(ptba.T4) AS T4,exe.LIQUIDATION FROM inst_institutions inst JOIN ptba ON ptba.INSTITUTION_ID=inst.INSTITUTION_ID JOIN  inst_institutions_ligne_budgetaire ligne_budg ON ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE_ID  = ptba.CODE_NOMENCLATURE_BUDGETAIRE_ID JOIN execution_budgetaire_new exe ON ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE=exe.IMPUTATION JOIN inst_institutions_programmes progr ON progr.INSTITUTION_ID=inst.INSTITUTION_ID WHERE 1 AND inst.INSTITUTION_ID = ".$id." GROUP BY progr.PROGRAMME_ID,inst.INSTITUTION_ID,inst.CODE_INSTITUTION,progr.CODE_PROGRAMME,progr.INTITULE_PROGRAMME,progr.OBJECTIF_DU_PROGRAMME";
    ...
    $order_by='';
    $order_column='';
    $order_column= array('progr.CODE_PROGRAMME','progr.INTITULE_PROGRAMME','ptba.OBJECTIF_PROGRAMME','ptba.T1','exe.LIQUIDATION');
    ...
    $search = !empty($_POST['search']['value']) ?  (" AND (progr.CODE_PROGRAMME LIKE '%$var_search%' OR progr.INTITULE_PROGRAMME LIKE '%$var_search%' OR progr.OBJECTIF_PROGRAMME LIKE '%$var_search%' OR ptba.T1 LIKE '%$var_search%' OR exe.LIQUIDATION LIKE '%$var_search%')"):'';
    ...
    {
      $montant_exe ="SELECT pro.CODE_PROGRAMME,SUM(racc.MONTANT_RACCROCHE) AS MONTANT_RACCROCHE FROM execution_budgetaire_raccrochage_activite_new racc JOIN ptba ON ptba.PTBA_ID=racc.PTBA_ID JOIN inst_institutions_programmes pro ON pro.PROGRAMME_ID=ptba.PROGRAMME_ID WHERE 1 AND pro.CODE_PROGRAMME= '".$info->CODE_PROGRAMME."'  GROUP BY pro.CODE_PROGRAMME";
  ...
  { 
    ...
    $query_principal="SELECT inst.INSTITUTION_ID,inst.CODE_INSTITUTION,act.ACTION_ID,act.CODE_ACTION,act.LIBELLE_ACTION,act.OBJECTIF_ACTION,SUM(ptba.T1) AS T1,SUM(ptba.T2) AS T2,SUM(ptba.T3) AS T3,SUM(ptba.T4) AS T4 FROM inst_institutions inst JOIN ptba ON ptba.INSTITUTION_ID=inst.INSTITUTION_ID JOIN inst_institutions_actions act ON act.ACTION_ID=ptba.ACTION_ID WHERE 1 AND inst.INSTITUTION_ID = ".$id." GROUP BY inst.INSTITUTION_ID,inst.CODE_INSTITUTION,act.CODE_ACTION,act.LIBELLE_ACTION,act.OBJECTIF_ACTION";
    $query_principal = str_replace('\\', '', $query_principal);
    $var_search= !empty($_POST['search']['value']) ? $_POST['search']['value'] : null;
    $limit='LIMIT 0,10';
    if($_POST['length'] != -1)
    ...
    $order_by = isset($_POST['order']) ? ' ORDER BY '.$order_column[$_POST['order']['0']['column']] .'  '.$_POST['order']['0']['dir'] : ' ORDER BY ptba.CODE_ACTION ASC';

    $search = !empty($_POST['search']['value']) ?  (" AND (act.CODE_ACTION LIKE '%$var_search%' OR act.LIBELLE_ACTION LIKE '%$var_search%' OR act.OBJECTIF_ACTION LIKE '%$var_search%' OR ptba.T1 LIKE '%$var_search%' OR LIKE '%$var_search%')"):'';
    ...
    foreach($fetch_cov_frais as $info)
    {
      $montant_exe ="SELECT act.CODE_ACTION,SUM(racc.MONTANT_RACCROCHE) AS MONTANT_RACCROCHE FROM execution_budgetaire_raccrochage_activite_new racc JOIN ptba ON ptba.PTBA_ID=racc.PTBA_ID JOIN inst_institutions_actions ACT ON ptba.ACTION_ID=act.ACTION_ID WHERE 1 AND ptba.ACTION_ID= '".$info->ACTION_ID."' GROUP BY act.CODE_ACTION";
  ...
  public function  liste_activite($id)
  {
    ...
    $query_principal="SELECT ptba.PTBA_ID, inst.INSTITUTION_ID,inst.CODE_INSTITUTION,ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE,ptba.CODES_PROGRAMMATIQUE,ptba.CODES_PROGRAMMATIQUE,ptba.ACTIVITES,T1, T2,T3, T4 FROM inst_institutions inst JOIN ptba ON ptba.INSTITUTION_ID=inst.INSTITUTION_ID JOIN  inst_institutions_ligne_budgetaire ligne_budg ON ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE_ID  = ptba.CODE_NOMENCLATURE_BUDGETAIRE_ID WHERE 1 AND inst.INSTITUTION_ID =".$id."";
    $var_search= !empty($_POST['search']['value']) ? $_POST['search']['value'] : null;
    $limit='LIMIT 0,10';
    if($_POST['length'] != -1)
    {
    ...
    $order_by='';
    $order_column='';
    $order_column= array('ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE','ptba.CODES_PROGRAMMATIQUE','ptba.ACTIVITES','ptba.T1');

    $order_by = isset($_POST['order']) ? ' ORDER BY '. $order_column[$_POST['order']['0']['column']] .' '.$_POST['order']['0']['dir'] : ' ORDER BY ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE ASC';

    $search = !empty($_POST['search']['value']) ?  (" AND (ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE LIKE '%$var_search%' OR ptba.CODES_PROGRAMMATIQUE LIKE '%$var_search%' OR ptba.ACTIVITES LIKE '%$var_search%' OR ptba.T1 LIKE '%$var_search%')"):'';
    ...
    foreach($fetch_cov_frais as $info)
    {
      $montant_execute ="SELECT PTBA_ID,MONTANT_RACCROCHE FROM execution_budgetaire_raccrochage_activite_new WHERE 1 AND PTBA_ID =".$info->PTBA_ID;
  ...
  public function  liste_ligne_budget($id)
  {
    ...
    /*print_r($INSTITUTION_ID);exit();*/
    $session  = \Config\Services::session();
    if($session->get('SESSION_SUIVIE_PTBA_PTBA_INSTITUTION')!=1)
    {
      return redirect('Login_Ptba/homepage');
    }
    $query_principal="SELECT inst.INSTITUTION_ID,inst.CODE_INSTITUTION,SUM(ptba.T1) AS T1,SUM(ptba.T2) AS T2,SUM(ptba.T3) AS T3,SUM(ptba.T4) AS T4, ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE FROM inst_institutions inst JOIN ptba ON ptba.INSTITUTION_ID=inst.INSTITUTION_ID JOIN  inst_institutions_ligne_budgetaire ligne_budg ON ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE_ID  = ptba.CODE_NOMENCLATURE_BUDGETAIRE_ID WHERE 1 AND inst.INSTITUTION_ID =".$INSTITUTION_ID." GROUP BY inst.INSTITUTION_ID,inst.CODE_INSTITUTION,ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE";
    $query_principal = str_replace('\\','',$query_principal);
    $var_search= !empty($_POST['search']['value']) ? $_POST['search']['value'] : null;
    $limit='LIMIT 0,10';
    if($_POST['length'] != -1)
    ...
    $order_by='';
    $order_column='';
    $order_column= array('ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE','ptba.T1','1');

    $order_by = isset($_POST['order']) ? ' ORDER BY '. $order_column[$_POST['order']['0']['column']] .'  '.$_POST['order']['0']['dir'] : ' ORDER BY ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE ASC';

    $search = !empty($_POST['search']['value']) ?  (" AND (ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE LIKE '%$var_search%' OR ptba.T1 LIKE '%$var_search%')"):'';
    ...
    foreach($fetch_cov_frais as $info)
    {
      $montant ="SELECT ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE,SUM(racc.MONTANT_RACCROCHE) AS MONTANT_RACCROCHE FROM execution_budgetaire_raccrochage_activite_new racc JOIN ptba ON ptba.PTBA_ID=racc.PTBA_ID JOIN  inst_institutions_ligne_budgetaire ligne_budg ON ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE_ID  = ptba.CODE_NOMENCLATURE_BUDGETAIRE_ID WHERE 1 AND ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE= '".$info->CODE_NOMENCLATURE_BUDGETAIRE."' GROUP BY ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE";
