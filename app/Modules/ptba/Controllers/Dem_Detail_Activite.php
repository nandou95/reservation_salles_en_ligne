...
class Dem_Detail_Activite extends BaseController
{
  ...
  function index($id)
  {
    ...
    ///montant voté
    $bind_montant_vote = $this->getBindParms('SUM(`T1`) as T1, SUM(`T2`) as T2, SUM(`T3`) as T3, SUM(`T4`) as T4', 'ptba','MD5(PTBA_ID)="'.$id.'"','SUM(`T1`)');
    ...
    ///quantité voté
    $bind_quant_vote = $this->getBindParms('SUM(`QT1`) as QT1, SUM(`QT2`) as QT2, SUM(`QT3`) as QT3, SUM(`QT4`) as QT4', 'ptba', 'MD5(PTBA_ID)="'.$id.'"', 'SUM(`QT1`)');
    ...
    return view('App\Modules\ptba\Views\Dem_Detail_Activite_View',$data);
  }
