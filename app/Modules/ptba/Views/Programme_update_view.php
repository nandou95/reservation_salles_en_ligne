              ...
              <div style="box-shadow: rgba(100, 100, 111, 0.25) 0px 7px 29px 0px" class="card">
                <div class="card-header">
                  <div class="row col-md-12">
                    <div class="col-md-6">
                      <h1 class="header-title text-black">Modification d'un programme</h1>
                    </div>
                    <div class="col-md-6" style="float: right;">
                      <a href="<?=base_url('ptba/Programme')?>" style="float: right;margin: 4px" class="btn btn-primary"><i class="fa fa-list text-light" aria-hidden="true"></i> Liste </a>
                    </div>
                  </div>
                </div>

                <div class="car-body">
                  <form name="myform" id="myform" action="<?=base_url('ptba/Programme/update')?>" method="POST" enctype="multipart/form-data">
                    <div class="card-body">
                      <div class="row">
                        <input type="hidden" name="PROGRAMME_ID" id="PROGRAMME_ID" value="<?=$program['PROGRAMME_ID']?>">
                        <div class="col-md-6">
                          <div class="form-group">
                            <label><?=lang('messages_lang.label_institution')?> <span style="color: red;">*</span></label>
...
<script type="text/javascript">
  function save_educ()
  {
    ...
    if(INSTITUTION_ID=='')
    {
      $('#error_INSTITUTION_ID').html('<?=lang('messages_lang.champ_obligatoire')?>');
      statut = 1;
    }

    if(CODE_PROGRAMME=='')
    {
      $('#error_CODE_PROGRAMME').html('<?=lang('messages_lang.champ_obligatoire')?>');
      statut = 1;
    }

    if(INTITULE_PROGRAMME=='')
    {
      $('#error_INTITULE_PROGRAMME').html('<?=lang('messages_lang.champ_obligatoire')?>');
      statut = 1;
    }

    if(OBJECTIF_DU_PROGRAMME  == '')
    {
      $('#error_OBJECTIF_DU_PROGRAMME').html('<?=lang('messages_lang.champ_obligatoire')?>');
      statut = 1;
    }

    if(statut==2)
    {
      url = "<?= base_url('ptba/Programme/update') ?>";
      $.post(url,
      {
      ...
      function(data)
      {
        if(data.status==true) 
        {
          window.location.href= "<?=base_url('ptba/Programme')?>";
        }
      });
    }
  }
