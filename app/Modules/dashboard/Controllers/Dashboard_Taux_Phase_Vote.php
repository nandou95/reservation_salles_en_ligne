...
if(!empty($PAP_ACTIVITE_ID))
   ...
   $KEY2=7;
      ...
      foreach ($decaissement_req11 as $value)
      {
        ...
        $taux2=("SELECT SUM(exec.ENG_BUDGETAIRE) as taux FROM execution_budgetaire exec JOIN execution_budgetaire_tache_detail detail ON exec.EXECUTION_BUDGETAIRE_ID=detail.EXECUTION_BUDGETAIRE_ID JOIN execution_budgetaire_execution_tache exec_tache ON exec_tache.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID LEFT JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec_tache.PTBA_TACHE_ID JOIN inst_institutions inst ON inst.INSTITUTION_ID=ptba.INSTITUTION_ID WHERE 1 ".$cond." ".$cond_exec." ".$cond_trim." ".$cond_pri1." ");
        $taux1=$this->ModelPs->getRequeteOne(' CALL getTable("'.$taux.'")');
        $taux21=$this->ModelPs->getRequeteOne(' CALL getTable("'.$taux2.'")');
        $pourc_taux1 = ($taux1['taux'] > 0) ? $taux1['taux'] : 1 ;
        $pourc_taux2 = ($taux21['taux'] > 0) ? $taux21['taux'] : 1 ;
