...
/* @author NIYONGABO EMERY emery@mediabox.bi (79408602)
...
class Dashboard_TCD_Taux_Engagement extends BaseController
{
  protected $session;
  protected $ModelPs;
  ...
  {  
    ...
    if(!empty($session->get('SESSION_SUIVIE_PTBA_USER_ID')))
    {
      ...
      $user_connect=("SELECT user_affectaion.`INSTITUTION_ID`,inst_institutions.NIVEAU_VISION inst_institutions FROM user_affectaion LEFT JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=user_affectaion.INSTITUTION_ID WHERE USER_ID=".$user_id." ");
    ...
    //print_r($inst_connect);die();
    return view('App\Modules\dashboard\Views\Dashboard_TCD_Taux_Engagement_View',$data);
  }
  # fonction pour les details
  
  function detail_tcd_taux_engagements() 
      {
    $data=$this->urichk();
    $db = db_connect(); 
    $session  = \Config\Services::session();
    ...
    $query_principal="//SELECT ligne.CODE_NOMENCLATURE_BUDGETAIRE AS service ,".$totaux." as vote,exec.EXECUTION_BUDGETAIRE_ID,exec.DATE_ENG_JURIDIQUE,exec.DATE_DEMANDE,inst.DESCRIPTION_INSTITUTION AS INTITULE_MINISTERE,programme.INTITULE_PROGRAMME, actions.LIBELLE_ACTION,pap_activites.DESC_PAP_ACTIVITE,pap_activites.RESULTAT_PAP_ACTIVITE, exec.ENG_BUDGETAIRE AS ENG_BUDGETAIRE,exec.LIQUIDATION  AS LIQUIDATION, exec.ENG_JURIDIQUE AS ENG_JURIDIQUE, exec.DECAISSEMENT AS DECAISSEMENT,exec.ORDONNANCEMENT AS ORDONNANCEMENT,exec.PAIEMENT AS PAIEMENT FROM execution_budgetaire exec LEFT JOIN execution_budgetaire_execution_tache exec_tache ON exec.EXECUTION_BUDGETAIRE_ID=exec_tache.EXECUTION_BUDGETAIRE_ID LEFT JOIN ptba_tache ptba ON exec_tache.PTBA_TACHE_ID=ptba.PTBA_TACHE_ID LEFT JOIN inst_institutions inst ON inst.INSTITUTION_ID=ptba.INSTITUTION_ID LEFT JOIN inst_institutions_ligne_budgetaire ligne ON ligne.CODE_NOMENCLATURE_BUDGETAIRE_ID =ptba.CODE_NOMENCLATURE_BUDGETAIRE_ID LEFT JOIN inst_institutions_programmes programme ON programme.PROGRAMME_ID=ptba.PROGRAMME_ID LEFT JOIN inst_institutions_actions actions ON actions.ACTION_ID = ptba.ACTION_ID LEFT JOIN pap_activites ON pap_activites.PAP_ACTIVITE_ID=ptba.PAP_ACTIVITE_ID   WHERE 1 ".$cond." ".$cond_trim." ";
    ...
    if($_POST['order']['0']['column']!=0) {
      ...
      $order_by = isset($_POST['order']) ? ' ORDER BY '.$_POST['order']['0']['column'] .'  '.$_POST['order']['0']['dir'] : ' ORDER BY exec.EXECUTION_BUDGETAIRE_ID  DESC'; 
    }
    $search = !empty($_POST['search']['value']) ? ("AND (
      inst.DESCRIPTION_INSTITUTION  LIKE '%$var_search%' OR programme.INTITULE_PROGRAMME LIKE '%$var_search%' OR actions.LIBELLE_ACTION LIKE '%$var_search%' OR RESULTAT_PAP_ACTIVITE LIKE '%$var_search%')"  ) : '';
  ...
  function liste_institution_phase_engage() 
  {
    $data=$this->urichk();
    $db = db_connect(); 
    $session  = \Config\Services::session();
...
$order_by = '';
$order_column = array('','exec.EXECUTION_BUDGETAIRE_ID','');
...
$search = !empty($_POST['search']['value']) ? (' AND (exec_tache.PTBA_TACHE_ID LIKE "%' . $var_search . '%" OR ligne.CODE_NOMENCLATURE_BUDGETAIRE LIKE "%' . $var_search . '%" OR inst.DESCRIPTION_INSTITUTION LIKE "%' . $var_search . '%" OR programme.INTITULE_PROGRAMME LIKE "%' . $var_search . '%" OR pap_activites.DESC_PAP_ACTIVITE LIKE "%' . $var_search . '%" OR pap_activites.RESULTAT_PAP_ACTIVITE LIKE "%' . $var_search . '%" )') : '';

$conditions = $critaire.' '.$search.' '.$group.' '.$order_by.' '.$limit;
$conditionsfilter = $critaire.' '.$search.' '.$group;
$requetedebase = "SELECT  ligne.CODE_NOMENCLATURE_BUDGETAIRE as service,".$totaux." as vote,inst.DESCRIPTION_INSTITUTION AS INTITULE_MINISTERE,ptba.DESC_TACHE,ptba.PTBA_TACHE_ID,programme.INTITULE_PROGRAMME,actions.LIBELLE_ACTION,pap_activites.DESC_PAP_ACTIVITE,pap_activites.RESULTAT_PAP_ACTIVITE, exec.ENG_BUDGETAIRE AS ENG_BUDGETAIRE,exec.LIQUIDATION AS LIQUIDATION, exec.ENG_JURIDIQUE AS ENG_JURIDIQUE, exec.DECAISSEMENT AS DECAISSEMENT,exec.ORDONNANCEMENT AS ORDONNANCEMENT,exec.PAIEMENT AS PAIEMENT FROM  execution_budgetaire exec LEFT JOIN execution_budgetaire_execution_tache exec_tache ON exec.EXECUTION_BUDGETAIRE_ID=exec_tache.EXECUTION_BUDGETAIRE_ID LEFT JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec_tache.PTBA_TACHE_ID LEFT JOIN inst_institutions inst ON inst.INSTITUTION_ID=ptba.INSTITUTION_ID LEFT JOIN inst_institutions_ligne_budgetaire ligne ON ligne.CODE_NOMENCLATURE_BUDGETAIRE_ID =ptba.CODE_NOMENCLATURE_BUDGETAIRE_ID LEFT JOIN inst_institutions_programmes programme ON programme.PROGRAMME_ID=ptba.PROGRAMME_ID LEFT JOIN inst_institutions_actions actions ON actions.ACTION_ID = ptba.ACTION_ID LEFT JOIN pap_activites ON pap_activites.PAP_ACTIVITE_ID=ptba.PAP_ACTIVITE_ID WHERE 1".$cond." ".$cond_trim."";
...
foreach ($fetch_projets as $row) 
{
  ...
  if (strlen($row->service) < 30){
    $engagement[] ='<center><font color="#000000" size=2><label>'.$row->service.'</label></font> </center>';
  }else{
    $engagement[] ='<center><font color="#000000" size=2><label>'.mb_substr($row->service, 0, 30).'...<a class="btn-sm" data-toggle="tooltip" title="'.$row->service.'"><i class="fa fa-eye"></i></a></label></font> </center>';
  }
  if (strlen($row->DESC_PAP_ACTIVITE) < 13){
    $engagement[] ='<center><font color="#000000" size=2><label>'.$row->DESC_PAP_ACTIVITE.'</label></font> </center>';
  }else{
...
public function get_rapport()
{
  ...
  $name_table= " LEFT JOIN inst_institutions inst ON inst.INSTITUTION_ID=ptba.INSTITUTION_ID";
  // ptba.
  $cond_program='';
  $cond_pri='';
  $cond_pri1='';
  ...
  $code_inst =  substr($code_key2,0,-1);
  $cond_pri.=' AND inst.INSTITUTION_ID IN ('.$code_inst.')';
  $cond_pri1.=' AND ptba.INSTITUTION_ID IN ('.$code_inst.')';
  ...
  $engage=("SELECT SUM(ENG_BUDGETAIRE) AS engage FROM execution_budgetaire exec LEFT JOIN execution_budgetaire_execution_tache exec_tache ON exec.EXECUTION_BUDGETAIRE_ID=exec_tache.EXECUTION_BUDGETAIRE_ID LEFT JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec_tache.PTBA_TACHE_ID LEFT JOIN inst_institutions inst ON inst.INSTITUTION_ID=ptba.INSTITUTION_ID WHERE 1 ".$cond_trim." ".$cond."");


  $liquide=("SELECT  SUM(LIQUIDATION) AS liquide FROM execution_budgetaire exec LEFT JOIN execution_budgetaire_execution_tache exec_tache ON exec.EXECUTION_BUDGETAIRE_ID=exec_tache.EXECUTION_BUDGETAIRE_ID LEFT JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec_tache.PTBA_TACHE_ID LEFT JOIN inst_institutions inst ON inst.INSTITUTION_ID=ptba.INSTITUTION_ID WHERE 1 ".$cond_trim." ".$cond."");


  $decaissement=("SELECT  SUM(DECAISSEMENT) as decaissement FROM execution_budgetaire exec LEFT JOIN execution_budgetaire_execution_tache exec_tache ON exec.EXECUTION_BUDGETAIRE_ID=exec_tache.EXECUTION_BUDGETAIRE_ID LEFT JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec_tache.PTBA_TACHE_ID LEFT JOIN inst_institutions inst ON inst.INSTITUTION_ID=ptba.INSTITUTION_ID WHERE 1 ".$cond_trim." ".$cond." ");

  $jurdique=("SELECT  SUM(ENG_JURIDIQUE) as jurdique FROM execution_budgetaire exec LEFT JOIN execution_budgetaire_execution_tache exec_tache ON exec.EXECUTION_BUDGETAIRE_ID=exec_tache.EXECUTION_BUDGETAIRE_ID LEFT JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec_tache.PTBA_TACHE_ID LEFT JOIN inst_institutions inst ON inst.INSTITUTION_ID=ptba.INSTITUTION_ID WHERE 1 ".$cond_trim." ".$cond."  ");

  $ordonence=("SELECT  SUM(ORDONNANCEMENT) as ordonence FROM execution_budgetaire exec LEFT JOIN execution_budgetaire_execution_tache exec_tache ON exec.EXECUTION_BUDGETAIRE_ID=exec_tache.EXECUTION_BUDGETAIRE_ID LEFT JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec_tache.PTBA_TACHE_ID LEFT JOIN inst_institutions inst ON inst.INSTITUTION_ID=ptba.INSTITUTION_ID WHERE 1 ".$cond_trim." ".$cond." ");

  $paiement=("SELECT  SUM(PAIEMENT) as paiement FROM execution_budgetaire exec LEFT JOIN execution_budgetaire_execution_tache exec_tache ON exec.EXECUTION_BUDGETAIRE_ID=exec_tache.EXECUTION_BUDGETAIRE_ID LEFT JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec_tache.PTBA_TACHE_ID LEFT JOIN inst_institutions inst ON inst.INSTITUTION_ID=ptba.INSTITUTION_ID WHERE 1 ".$cond_trim." ".$cond."");
  ...
  $paie_sum= ($paiement_req['paiement']>0) ? $paiement_req['paiement'] : 0 ;
  ...
  $data_liquide="{name:'Liquidation :: ".number_format($liquide_sum,0,',',' ')." BIF',color:'#a33558', y:(".$liquide_sum/$pourc_taux.")*100,key2:2,key3:".$IS_PRIVATE."},";
  ...
  $data_ordonence="{name:'Ordonnancement (".number_format($ordonence_sum,0,',',' ')." BIF) ', y:(".$ordonence_sum/$pourc_taux.")*100,key2:5,key3:".$IS_PRIVATE."},";
  ...
  $data_engage11="{name:'Engagement budgétaire :: ".number_format($engage_sum,0,',',' ')." BIF', y:(".$engage_sum/$pourc_taux.")*100,key2:1,key3:".$IS_PRIVATE."},";
  $data_liquide11="{name:'Liquidation :: ".number_format($liquide_sum,0,',',' ')." BIF',color:'#a33558', y:(".$liquide_sum/$pourc_taux.")*100,key2:2,key3:".$IS_PRIVATE."},";
  $data_decaissement11="{name:'Décaissement :: ".number_format($decaissement_sum,0,',',' ')." BIF', y:(".$decaissement_sum/$pourc_taux.")*100,key2:3,key3:".$IS_PRIVATE."},";
  $data_jurdique11="{name:'Engagement juridique :: ".number_format($jurdique_sum,0,',',' ')." BIF', y:(".$jurdique_sum/$pourc_taux.")*100,key2:4,key3:".$IS_PRIVATE."},";
  $data_ordonence11="{name:'Ordonnancement :: ".number_format($ordonence_sum,0,',',' ')." BIF ', y:(".$ordonence_sum/$pourc_taux.")*100,key2:5,key3:".$IS_PRIVATE."},";
  $data_paie11="{name:'Paiement :: ".number_format($paie_sum,0,',',' ')." BIF ', y:(".$paie_sum/$pourc_taux.")*100,key2:6,key3:".$IS_PRIVATE."},";
...
Highcharts.chart('container2', {
  chart: {
    ...
    title: {
      text:  '".lang('messages_lang.titre_taux_engagement')."',
      },
      subtitle:{
       text: ''
       },
                                    ...
                                    $(\"#trim\").html(\"".lang('messages_lang.budget_deuxieme')."\");
                                      ...
                                      $(\"#trim\").html(\"".lang('messages_lang.budget_troisieme')."\");
                                        ...
                                        $(\"#mytable\").DataTable({
                                          ...
                                          \"oreder\":[],
                                          \"ajax\":{
                                           url:\"".base_url('dashboard/Dashboard_TCD_Taux_Engagement/detail_tcd_taux_engagement')."\",
                                           type:\"POST\",
                                           data:{
                                            key:this.key,
                                            key3:this.key3,
                                        ...
                                        credits: {
                                          ...
                                          yAxis: {
                                            ...
                                            series: [
                                            {
                                             colorByPoint: true,
                                             name:'Taux',
                                             data: [".$data_decaissement11.$data_paie11.$data_ordonence11.$data_liquide11.$data_jurdique11.$data_engage11."]
                                             }]
    ...
    if ($SOUS_TUTEL_ID != '')
    {
      $program_sect="SELECT DISTINCT inst_institutions_programmes.INTITULE_PROGRAMME,inst_institutions_programmes.PROGRAMME_ID FROM  inst_institutions_programmes LEFT JOIN inst_institutions ON  inst_institutions.INSTITUTION_ID=inst_institutions_programmes.INSTITUTION_ID LEFT JOIN ptba_tache ON ptba_tache.PROGRAMME_ID=inst_institutions_programmes.PROGRAMME_ID WHERE 1 AND SUBSTRING(ptba_tache.CODE_NOMENCLATURE_BUDGETAIRE, 5,3)='".$SOUS_TUTEL_ID."' ".$cond_program."  ORDER BY inst_institutions_programmes.CODE_PROGRAMME ASC";
    ...
    $ligne_budgetaires= '<option selected="" disabled="">'.lang("messages_lang.selection_message").'</option>';
                     if ($ACTION_ID != '')
                     {
             $ligne_budgetaire_sect='SELECT DISTINCT inst_institutions_ligne_budgetaire.LIBELLE_CODE_NOMENCLATURE_BUDGETAIRE,inst_institutions_ligne_budgetaire.CODE_NOMENCLATURE_BUDGETAIRE_ID FROM  ptba_tache RIGHT LEFT JOIN inst_institutions_ligne_budgetaire ON inst_institutions_ligne_budgetaire.CODE_NOMENCLATURE_BUDGETAIRE_ID=ptba_tache.CODE_NOMENCLATURE_BUDGETAIRE_ID WHERE 1 AND ptba_tache.ACTION_ID='.$ACTION_ID;
             ...
             $ligne_budgetaire_sect_req = $this->ModelPs->getRequete('CALL getTable("'.$ligne_budgetaire_sect.'")');
                ...
                $ligne_activite= '<option selected="" disabled="">'.lang("messages_lang.selection_message").'</option>';
                   if ($LIGNE_BUDGETAIRE != '')
                    {
                 $ligne_activites_sect='SELECT DISTINCT  pap_activites.PAP_ACTIVITE_ID,pap_activites.DESC_PAP_ACTIVITE FROM  ptba_tache RIGHT LEFT JOIN pap_activites ON pap_activites.PAP_ACTIVITE_ID=ptba.PAP_ACTIVITE_ID WHERE 1 AND pap_activites.CODE_NOMENCLATURE_BUDGETAIRE_ID='.$LIGNE_BUDGETAIRE;
...
function exporter($TYPE_INSTITUTION_ID='',$INSTITUTION_ID='',$SOUS_TUTEL_ID='',$IS_PRIVATE='',$PROGRAMME_ID='',$ACTION_ID='',$LIGNE_BUDGETAIRE='',$PAP_ACTIVITE_ID='')
      {
    ...
    if($SOUS_TUTEL_ID!=0)
    {
      $cond3="AND ptba.SOUS_TUTEL_ID='".$SOUS_TUTEL_ID."'";
    }
    ...
    $getRequete="SELECT ptba.CODE_NOMENCLATURE_BUDGETAIRE service,BUDGET_T1+BUDGET_T2+BUDGET_T3+BUDGET_T4 as vote,inst.DESCRIPTION_INSTITUTION AS INTITULE_MINISTERE,ptba.PTBA_TACHE_ID, programme.INTITULE_PROGRAMME,actions.LIBELLE_ACTION,pap_activites.DESC_PAP_ACTIVITE,pap_activites.RESULTAT_PAP_ACTIVITE,exec.ENG_BUDGETAIRE AS ENG_BUDGETAIRE,exec.LIQUIDATION AS LIQUIDATION, exec.ENG_JURIDIQUE AS ENG_JURIDIQUE, exec.DECAISSEMENT AS DECAISSEMENT,exec.ORDONNANCEMENT AS ORDONNANCEMENT,exec.PAIEMENT AS PAIEMENT FROM execution_budgetaire exec LEFT JOIN execution_budgetaire_execution_tache exec_tache ON exec_tache.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID LEFT JOIN ptba_tache ptba ON exec_tache.PTBA_TACHE_ID=ptba.PTBA_TACHE_ID LEFT JOIN inst_institutions inst ON inst.INSTITUTION_ID=ptba.INSTITUTION_ID LEFT JOIN inst_institutions_programmes programme ON programme.PROGRAMME_ID=ptba.PROGRAMME_ID LEFT JOIN inst_institutions_actions actions ON actions.ACTION_ID=ptba.ACTION_ID LEFT JOIN pap_activites ON pap_activites.PAP_ACTIVITE_ID=ptba.PAP_ACTIVITE_ID  WHERE 1 ".$cond." ".$cond_trim." ";
    ...
    $writer->save('world.xlsx');
     return $this->response->download('world.xlsx', null)->setFileName('TCD sur engagement.xlsx');
