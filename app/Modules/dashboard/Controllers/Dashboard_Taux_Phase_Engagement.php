 ...
 *emery@mediabox.bi
 * Tableau de bord "Taux d'exécution par phase en référence au budget engagé" 
 le 27/02/2023
 Adapté à la version 3 par Claude claude@mediabox.bi le 05/12/2024
 */ 
  ...
  public function get_rapport()
    {
    	...
    	if(! empty($INSTITUTION_ID))
    	{
    		...
    		$name_table= " JOIN inst_institutions inst ON inst.INSTITUTION_ID=ptba.INSTITUTION_ID JOIN  inst_institutions_sous_tutel sous_tutel ON sous_tutel.INSTITUTION_ID=inst.INSTITUTION_ID WHERE sous_tutel.INSTITUTION_ID=".$INSTITUTION_ID." ";
    		$format=" {point.y:.2f} %";
    		$type="column";
    		...
    		if (! empty($cond_sy_req['INSTITUTION_ID'])) {
    			...
    			$cond2= ' AND inst.`INSTITUTION_ID`='.$cond_sy_req['INSTITUTION_ID'];
    		}
    	...
    	if(! empty($SOUS_TUTEL_ID))
    	{
    		...
    		$id_decl= "sous_tutel.CODE_SOUS_TUTEL";
    		$name_table1= "  JOIN inst_institutions inst ON inst.INSTITUTION_ID=ptba.INSTITUTION_ID";
    	...
    	$engage11=("SELECT ".$name_decl." AS name,".$id_decl." as ID,SUM(exec.ENG_BUDGETAIRE) as engage  FROM `execution_budgetaire` exec JOIN execution_budgetaire_execution_tache exec_tache ON exec.EXECUTION_BUDGETAIRE_ID = exec_tache.EXECUTION_BUDGETAIRE_ID JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec_tache.PTBA_TACHE_ID ".$name_table."   ".$cond_trim." ".$cond." GROUP BY ".$name_decl.",".$id_decl." ");

    	$liquide11=("SELECT ".$name_decl." AS name,".$id_decl." as ID,SUM(exec.LIQUIDATION) as liquide FROM `execution_budgetaire` exec JOIN execution_budgetaire_execution_tache exec_tache ON exec.EXECUTION_BUDGETAIRE_ID = exec_tache.EXECUTION_BUDGETAIRE_ID JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec_tache.PTBA_TACHE_ID".$name_table."   ".$cond_trim." ".$cond." GROUP BY ".$name_decl.",".$id_decl."");
    	$decaissement11=("SELECT ".$name_decl." AS name,".$id_decl." as ID, SUM(exec.DECAISSEMENT) as decaissement FROM `execution_budgetaire` exec JOIN execution_budgetaire_execution_tache exec_tache ON exec.EXECUTION_BUDGETAIRE_ID = exec_tache.EXECUTION_BUDGETAIRE_ID JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec_tache.PTBA_TACHE_ID".$name_table."   ".$cond_trim." ".$cond." GROUP BY ".$name_decl.",".$id_decl."");
    	$jurdique11=("SELECT ".$name_decl." AS name,".$id_decl." as ID,SUM(exec.ENG_JURIDIQUE) as jurdique FROM  `execution_budgetaire` exec JOIN execution_budgetaire_execution_tache exec_tache ON exec.EXECUTION_BUDGETAIRE_ID = exec_tache.EXECUTION_BUDGETAIRE_ID JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec_tache.PTBA_TACHE_ID".$name_table." ".$cond_trim."   GROUP BY ".$name_decl.",".$id_decl."");
    	$ordonence11=("SELECT ".$name_decl." AS name,".$id_decl." as ID,SUM(exec.ORDONNANCEMENT) as ordonence FROM `execution_budgetaire` exec JOIN execution_budgetaire_execution_tache exec_tache ON exec.EXECUTION_BUDGETAIRE_ID = exec_tache.EXECUTION_BUDGETAIRE_ID JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec_tache.PTBA_TACHE_ID".$name_table."  ".$cond_trim." ".$cond."   GROUP BY ".$name_decl.",".$id_decl."");
    	$paiement11=("SELECT ".$name_decl." AS name,".$id_decl." as ID,SUM(exec.PAIEMENT) as paie FROM `execution_budgetaire` exec JOIN execution_budgetaire_execution_tache exec_tache ON exec.EXECUTION_BUDGETAIRE_ID = exec_tache.EXECUTION_BUDGETAIRE_ID JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec_tache.PTBA_TACHE_ID".$name_table."  ".$cond_trim." ".$cond."   GROUP BY ".$name_decl.",".$id_decl."");
    	...
    	foreach ($engage_req11 as $value)
    	{
    		...
    		$taux2=("SELECT SUM(exec.ENG_BUDGETAIRE) as taux FROM `execution_budgetaire` exec JOIN execution_budgetaire_execution_tache exec_tache ON exec.EXECUTION_BUDGETAIRE_ID = exec_tache.EXECUTION_BUDGETAIRE_ID JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec_tache.PTBA_TACHE_ID JOIN inst_institutions inst ON inst.INSTITUTION_ID=ptba.INSTITUTION_ID WHERE 1 ".$cond." ".$cond1." ".$cond_trim." ");
    		$categorie.="'";
    	...
    	{
    		$pourcent=0;
    		$taux=("SELECT ".$totaux." AS taux FROM `ptba_tache` ptba JOIN inst_institutions inst ON inst.INSTITUTION_ID=ptba.INSTITUTION_ID WHERE 1 ".$cond." ".$cond2." ");
    	...
    	$taux=("SELECT ".$totaux." AS vote FROM ptba_tache ptba JOIN inst_institutions inst ON inst.INSTITUTION_ID=ptba.INSTITUTION_ID WHERE 1 ".$cond." ".$cond_pri1."  ");
    	$taux1=$this->ModelPs->getRequeteOne(' CALL getTable("'.$taux.'")');
   ...
   $cond_trim=" " ;
		...
		$query_principal="SELECT ligne.CODE_NOMENCLATURE_BUDGETAIRE service,".$totaux." as total,ptba.PTBA_TACHE_ID,exec.EXECUTION_BUDGETAIRE_ID,exec.DATE_ENG_JURIDIQUE,exec.DATE_DEMANDE,inst.DESCRIPTION_INSTITUTION AS INTITULE_MINISTERE,programme.INTITULE_PROGRAMME, actions.LIBELLE_ACTION,pap_activites.DESC_PAP_ACTIVITE,pap_activites.RESULTAT_PAP_ACTIVITE, exec.ENG_BUDGETAIRE AS ENG_BUDGETAIRE,exec.LIQUIDATION  AS LIQUIDATION, exec.ENG_JURIDIQUE AS ENG_JURIDIQUE, exec.DECAISSEMENT AS DECAISSEMENT,exec.ORDONNANCEMENT AS ORDONNANCEMENT,exec.PAIEMENT AS PAIEMENT,exec.DATE_DEMANDE,exec.DATE_ENG_JURIDIQUE FROM execution_budgetaire exec  JOIN execution_budgetaire_execution_tache exec_tache ON exec.EXECUTION_BUDGETAIRE_ID = exec_tache.EXECUTION_BUDGETAIRE_ID JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec_tache.PTBA_TACHE_ID  LEFT JOIN inst_institutions inst ON inst.INSTITUTION_ID=ptba.INSTITUTION_ID JOIN programme ON programme.PROGRAMME_ID=ptba.PROGRAMME_ID JOIN inst_institutions_actions actions ON actions.ACTION_ID=ptba.ACTION_ID JOIN pap_activites ON pap_activites.PAP_ACTIVITE_ID=ptba_tache.PAP_ACTIVITE_ID   WHERE 1 ".$cond." ".$cond_trim." ";
     ...
     $critere="";  
		...
		inst.DESCRIPTION_INSTITUTION LIKE '%$var_search%' OR programme.INTITULE_PROGRAMME LIKE '%$var_search%' OR actions.LIBELLE_ACTION LIKE '%$var_search%' OR DESC_PAP_ACTIVITE LIKE '%$var_search%'  OR RESULTAT_PAP_ACTIVITE LIKE '%$var_search%')") : '';
   ...
   ######liste des entites responsable
    ...
    if ($inst_conn>0){
		...
		if (! empty($SOUS_TUTEL_ID)) 
		{
		 $cond.=" AND ptba.SOUS_TUTEL_ID, 5,3)='".$SOUS_TUTEL_ID."'";
		}
...
$requetedebase = "SELECT exec.EXECUTION_BUDGETAIRE_ID,ligne.CODE_NOMENCLATURE_BUDGETAIRE service,".$totaux." as vote,inst.DESCRIPTION_INSTITUTION AS INTITULE_MINISTERE,ptba.DESC_TACHE,ligne.CODE_NOMENCLATURE_BUDGETAIRE,ptba.PTBA_TACHE_ID, programme.INTITULE_PROGRAMME,actions.LIBELLE_ACTION,pap_activites.DESC_PAP_ACTIVITE,pap_activites.RESULTAT_PAP_ACTIVITE,exec.ENG_BUDGETAIRE AS ENG_BUDGETAIRE,exec.LIQUIDATION AS LIQUIDATION, exec.ENG_JURIDIQUE AS ENG_JURIDIQUE, exec.DECAISSEMENT AS DECAISSEMENT,exec.ORDONNANCEMENT AS ORDONNANCEMENT,exec.PAIEMENT AS PAIEMENT FROM execution_budgetaire exec LEFT JOIN execution_budgetaire_execution_tache exec_tache ON exec.EXECUTION_BUDGETAIRE_ID=exec_tache.EXECUTION_BUDGETAIRE_ID JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec_tache.PTBA_TACHE_ID LEFT JOIN inst_institutions inst ON inst.INSTITUTION_ID=ptba.INSTITUTION_ID JOIN inst_institutions_programmes programme ON programme.PROGRAMME_ID=ptba.PROGRAMME_ID JOIN inst_institutions_actions actions ON actions.ACTION_ID=ptba.ACTION_ID JOIN pap_activites ON pap_activites.PAP_ACTIVITE_ID=ptba.PAP_ACTIVITE_ID join inst_institutions_ligne_budgetaire ligne ON  ligne.CODE_NOMENCLATURE_BUDGETAIRE_ID=ptba.CODE_NOMENCLATURE_BUDGETAIRE_ID WHERE 1 ".$cond." ".$cond_trim."";
...
return $this->response->setJSON($output);
    ...
    $LIGNE_BUDGETAIRE=$LIGNE_BUDGETAIRE;
        ...
        $getRequete="SELECT exec.EXECUTION_BUDGETAIRE_ID,ligne.CODE_NOMENCLATURE_BUDGETAIRE service,".$totaux." as vote,inst.DESCRIPTION_INSTITUTION AS INTITULE_MINISTERE,ptba.DESC_TACHE,ligne.CODE_NOMENCLATURE_BUDGETAIRE,ptba.PTBA_TACHE_ID, programme.INTITULE_PROGRAMME,actions.LIBELLE_ACTION,pap_activites.DESC_PAP_ACTIVITE,pap_activites.RESULTAT_PAP_ACTIVITE,exec.ENG_BUDGETAIRE AS ENG_BUDGETAIRE,exec.LIQUIDATION AS LIQUIDATION, exec.ENG_JURIDIQUE AS ENG_JURIDIQUE, exec.DECAISSEMENT AS DECAISSEMENT,exec.ORDONNANCEMENT AS ORDONNANCEMENT,exec.PAIEMENT AS PAIEMENT FROM execution_budgetaire exec JOIN execution_budgetaire_execution_tache exec_tache ON exec_tache.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID LEFT JOIN   ptba_tache ptba ON exec_tache.PTBA_TACHE_ID=ptba.PTBA_TACHE_ID LEFT JOIN inst_institutions inst ON inst.INSTITUTION_ID=ptba.INSTITUTION_ID JOIN inst_institutions_programmes programme ON programme.PROGRAMME_ID=ptba.PROGRAMME_ID JOIN inst_institutions_actions actions ON actions.ACTION_ID=ptba.ACTION_ID JOIN pap_activites ON pap_activites.PAP_ACTIVITE_ID=ptba.PAP_ACTIVITE_ID join inst_institutions_ligne_budgetaire ligne ON  ligne.CODE_NOMENCLATURE_BUDGETAIRE_ID=ptba.CODE_NOMENCLATURE_BUDGETAIRE_ID WHERE 1 ".$cond." ".$cond_trim." ";
