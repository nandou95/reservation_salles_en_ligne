...
class Dashboard_Comparaison_Vote_Execution extends BaseController
{
	protected $session;
	protected $ModelPs;
  ...
  */
    public function getBindParmsLimit($columnselect, $table, $where, $orderby, $Limit)
  {
    ...
    if($session->get('SESSION_SUIVIE_PTBA_TABLEAU_BORD_BUDGET')!=1)
    {
      return redirect('Login_Ptba/homepage');
       ...
       //Selection de l'année budgétaire
		...
		return view('App\Modules\dashboard\Views\Dashboard_Comparaison_Vote_Execution_View',$data);
	}
	...
	public function get_rapport()
	{
		...
		$format=" {point.y:.3f} %";
		$type="column";
		$table="inst_types_institution";
		$condition="inst_institutions.TYPE_INSTITUTION_ID";
		$cond2='';
		$name_table= " RIGHT JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=ptba_tache.INSTITUTION_ID";
		if(! empty($TYPE_INSTITUTION_ID))
		{
			$titr_deux=''.lang("messages_lang.par_type_institution").'';
			$titr_deux2=''.lang("messages_lang.par_type_institution").'';
		  ...
		  $condition="inst_institutions.INSTITUTION_ID";
		  $cond2=' AND inst_institutions.TYPE_INSTITUTION_ID='.$TYPE_INSTITUTION_ID;
		...
		if(! empty($INSTITUTION_ID))
		{
			...
			$name_table= " RIGHT JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=ptba_tache.INSTITUTION_ID RIGHT JOIN inst_institutions_programmes ON inst_institutions_programmes.PROGRAMME_ID=ptba_tache.PROGRAMME_ID";
			$table="inst_institutions_programmes";
		  $condition="inst_institutions_programmes.PROGRAMME_ID";
			$cond.=" AND ptba_tache.INSTITUTION_ID='".$INSTITUTION_ID."'";
			$titr_deux=''.lang("messages_lang.par_programme").'';
			$titr_deux2=''.lang("messages_lang.par_programme").'';
			$type="column";
		$cond2=' AND inst_institutions_programmes.INSTITUTION_ID='.$INSTITUTION_ID;
			$KEY2=3;
		}
		$cond33='';
		$cond333="";
		...
		if(! empty($PROGRAMME_ID))
		{
			...
			$name_table= " RIGHT JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=ptba_tache.INSTITUTION_ID RIGHT JOIN inst_institutions_actions ON inst_institutions_actions.ACTION_ID=ptba_tache.ACTION_ID";
			$cond.=" AND ptba_tache.PROGRAMME_ID='".$PROGRAMME_ID."'";
		  ...
		  $condition="inst_institutions_actions.ACTION_ID";
			$type="column";
		 $cond2=' AND inst_institutions_actions.PROGRAMME_ID='.$PROGRAMME_ID;
			$format=" {point.y:.3f} %";
    ...
    $name_decl= "inst_institutions_ligne_budgetaire.CODE_NOMENCLATURE_BUDGETAIRE";
    $name_table= " RIGHT JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=ptba_tache.INSTITUTION_ID RIGHT JOIN inst_institutions_ligne_budgetaire ON ptba_tache.CODE_NOMENCLATURE_BUDGETAIRE_ID =inst_institutions_ligne_budgetaire.CODE_NOMENCLATURE_BUDGETAIRE_ID";
     $table="inst_institutions_ligne_budgetaire";
     $condition="inst_institutions_ligne_budgetaire.CODE_NOMENCLATURE_BUDGETAIRE_ID";
     $cond.=" AND ptba_tache.ACTION_ID=".$ACTION_ID."";
     $cond333.=" AND ptba_tache.ACTION_ID=".$ACTION_ID."";
     $type="column";
     $titr_deux=''.lang("messages_lang.par_activite").'';
     $titr_deux2='';
     $cond2=' AND inst_institutions_ligne_budgetaire.INSTITUTION_ID='.$INSTITUTION_ID;
     $format=" {point.y:.3f} %";
     $KEY2=5;
   }

	 $budget=("SELECT ".$name_decl." AS Name,".$id_decl." AS ID,".$totaux." AS enga FROM ptba_tache ".$name_table." WHERE 1 ".$cond."   GROUP BY ".$name_decl.",".$id_decl." ORDER BY enga ASC ");
		...
		foreach ($budget_req as $value)
		{
			...
			$mont=($value->enga>0) ? $value->enga : 0;
			if ($taux1['taux']>0) {
				$pourcent=($value->enga/$taux1['taux'])*100;
			}
			$data_budget_req.="{name:'".$this->str_replacecatego($value->Name)." (".number_format($value->enga,0,',',' ')." BIF)', y:".$mont.",color:'#000080',key:'".$this->str_replacecatego($value->ID)."',key2:".$KEY2."},";
			$data_total=$data_total+$value->enga;
		} 

    #########
   $activites_exec=("SELECT ".$name_decl." AS Name,".$id_decl." AS ID,SUM(ENG_BUDGETAIRE) AS enga FROM execution_budgetaire RIGHT JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID ".$name_table." WHERE 1 ".$cond."   GROUP BY ".$name_decl.",".$id_decl." ORDER BY enga ASC ");
    $activite_req=$this->ModelPs->getRequete(' CALL getTable("'.$activites_exec.'")');
		$data_budget_execution='';
		$data_total_execution=0;
		...
		$data_budget_execution.="{name:'".$this->str_replacecatego($value->Name)." (".number_format($value->enga,0,',',' ')." BIF)', y:".$mont.",color:'green',key:'".$this->str_replacecatego($value->ID)."',key2:".$KEY2."},";
		$data_total_execution=$data_total_execution+$value->enga;
		} 
	    	  ###############
		$vote_angagement=("SELECT ".$name_decl." AS Name,".$id_decl." AS ID,(SELECT SUM(ENG_BUDGETAIRE) FROM execution_budgetaire RIGHT JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID ".$name_table." WHERE ".$condition."=ID  ".$cond.") AS angagement,(SELECT ".$totaux." FROM ptba_tache ".$name_table." WHERE ".$condition."=ID  ".$cond.") AS VOTE FROM ".$table." WHERE 1  ".$cond2."  GROUP BY ".$name_decl.",".$id_decl." ORDER BY ".$id_decl." ASC ");

    $vote_angagements=$this->ModelPs->getRequete(' CALL getTable("'.$vote_angagement.'")');
		$data_vote_execution='';
		$data_total_execution_vote=0;

		foreach ($vote_angagements as $value)
		{
			$pourcent=0;
			$taux=("SELECT ".$totaux." AS taux FROM `ptba_tache` JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=ptba_tache.INSTITUTION_ID WHERE 1 ".$cond." ".$cond_affectations1." ");
			$taux1=$this->ModelPs->getRequeteOne(' CALL getTable("'.$taux.'")');
			$execution=($value->angagement>0) ? $value->angagement : 0;
			$vote=($value->VOTE>0) ? $value->VOTE : 1;
			$pourcent=($execution/$vote)*100;
		$data_vote_execution.="{name:'".$this->str_replacecatego($value->Name)." (".number_format($value->angagement,0,',',' ')." BIF)', y:".$pourcent.",color:'green',key:'".$this->str_replacecatego($value->ID)."',key2:".$KEY2."},";
		$data_total_execution_vote=$data_total_execution_vote+$value->angagement;
		} 
																	...
																	$(\"#titre\").html(\"".lang("messages_lang.budget_vote_detail")."\");
																		...
																		$(\"#titre\").html(\"".lang("messages_lang.budget_vote_detail")."\");
																			...
																			$(\"#mytable\").DataTable({
																				...
																				\"bDestroy\": true,
																				\"ajax\":{
																					url:\"".base_url('dashboard/Dashboard_Comparaison_Vote_Execution/detail_comparaisons_vote')."\",
																					type:\"POST\",
																					data:{
																						key:this.key,
																						key2:this.key2,
	 ...
	 $rapp1="<script type=\"text/javascript\">
		Highcharts.chart('container1', {
			...
			chart: {
				...
				title: {
				text: 'Budget execute <br> ".number_format($data_total_execution,0,',',' ')." BIF',
					},  
					subtitle: {
						text: ''
						},
																	...
																	$(\"#titre\").html(\"".lang("messages_lang.budget_vote_detail")."\");
																		...
																		$(\"#titre\").html(\"".lang("messages_lang.budget_vote_detail")."\");
																			...
																			$(\"#mytable\").DataTable({
																				...
																				\"bDestroy\": true,
																				\"ajax\":{
																					url:\"".base_url('dashboard/Dashboard_Comparaison_Vote_Execution/detail_comparaisons_execution')."\",
																					type:\"POST\",
																					data:{
																					key:this.key,
																					key2:this.key2,
...
###detail du rapport des projets vs montant par axe stratégique 
	function detail_comparaisons_votes() 
	   {
	...
	$var_search = !empty($_POST['search']['value']) ? $_POST['search']['value'] : null; 
	$query_principal="SELECT inst_institutions.DESCRIPTION_INSTITUTION AS `INTITULE_MINISTERE`,inst_institutions_programmes.`INTITULE_PROGRAMME`,".$totaux." AS MONTANT,inst_institutions_actions.`LIBELLE_ACTION`,ptba_tache.DESC_TACHE,ptba_tache.RESULTAT_ATTENDUS_TACHE, date_format(execution_budgetaire.DATE_DEMANDE,'%d-%m-%Y') as dat,pap_activites.DESC_PAP_ACTIVITE,REPLACE(RTRIM(ptba_tache.BUDGET_ANNUEL),' ','') AS name FROM ptba_tache LEFT JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=ptba_tache.INSTITUTION_ID JOIN inst_institutions_programmes ON inst_institutions_programmes.PROGRAMME_ID=ptba_tache.PROGRAMME_ID JOIN inst_institutions_actions ON inst_institutions_actions.ACTION_ID=ptba_tache.ACTION_ID LEFT JOIN execution_budgetaire ON execution_budgetaire.PTBA_TACHE_ID=ptba_tache.PTBA_TACHE_ID LEFT JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_ID=execution_budgetaire.EXECUTION_BUDGETAIRE_ID JOIN pap_activites ON  pap_activites.PAP_ACTIVITE_ID=ptba_tache.PAP_ACTIVITE_ID  WHERE 1 ".$cond." ";
		...
		foreach ($fetch_data as $row)
		{
			 ...
			 if (strlen($row->DESC_PAP_ACTIVITE) > 12){ 
					$sub_array[] = mb_substr($row->INTITULE_MINISTERE, 0, 11) .'...<a class="btn-sm" data-toggle="tooltip" title="'.$row->INTITULE_MINISTERE.'"><i class="fa fa-eye"></i></a>';
					$sub_array[] = mb_substr($row->INTITULE_PROGRAMME, 0, 11) .'...<a class="btn-sm" data-toggle="tooltip" title="'.$row->INTITULE_PROGRAMME.'"><i class="fa fa-eye"></i></a>';
					$sub_array[] = mb_substr($row->LIBELLE_ACTION, 0, 8) .'...<a class="btn-sm" data-toggle="tooltip" title="'.$row->LIBELLE_ACTION.'"><i class="fa fa-eye"></i></a>';
				  $sub_array[] = mb_substr($row->DESC_PAP_ACTIVITE, 0, 11) .'...<a class="btn-sm" data-toggle="tooltip" title="'.$row->DESC_PAP_ACTIVITE.'"><i class="fa fa-eye"></i></a>';
				  $sub_array[] = mb_substr($row->RESULTAT_ATTENDUS_TACHE, 0, 11) .'...<a class="btn-sm" data-toggle="tooltip" title="'.$row->RESULTAT_ATTENDUS_TACHE.'"><i class="fa fa-eye"></i></a>';
			 	$sub_array[] = mb_substr($row->DESC_TACHE, 0, 11) .'...<a class="btn-sm" data-toggle="tooltip" title="'.$row->DESC_TACHE.'"><i class="fa fa-eye"></i></a>';
			   }else{
			   ...
			   $sub_array[] ='<center><font color="#000000" size=2><label>'.$row->DESC_PAP_ACTIVITE.'</label></font> </center>';
				$sub_array[] ='<center><font color="#000000" size=2><label>'.$row->RESULTAT_ATTENDUS_TACHE.'</label></font> </center>';
				$sub_array[] ='<center><font color="#000000" size=2><label>'.$row->DESC_TACHE.'</label></font> </center>';
			   }
			$sub_array[] ='<center><font color="#000000" size=2><label>'.number_format($row->MONTANT,0,',',' ').'</label></font> </center>';
			$data[] = $sub_array;
		   }
		$recordsTotal ="CALL `getTable`('" . $query_principal . "');";
		$recordsFiltered ="CALL `getTable`('" . $query_filter . "');";
...
###detail du rapport des projets vs montant par axe stratégique 
	function detail_comparaisons_executions() 
	   {
		$KEY=$this->request->getPost('key');
		$KEY2=$this->request->getPost('key2');
		$TYPE_INSTITUTION_ID=$this->request->getPost('TYPE_INSTITUTION_ID');
	...
	$var_search = !empty($_POST['search']['value']) ? $_POST['search']['value'] : null; 
	$query_principal="SELECT inst_institutions.DESCRIPTION_INSTITUTION AS `INTITULE_MINISTERE`,inst_institutions_programmes.`INTITULE_PROGRAMME`,execution_budgetaire.ENG_BUDGETAIRE AS MONTANT,inst_institutions_actions.`LIBELLE_ACTION`,ptba_tache.DESC_TACHE,ptba_tache.RESULTAT_ATTENDUS_TACHE, date_format(execution_budgetaire.DATE_DEMANDE,'%d-%m-%Y') as dat,pap_activites.DESC_PAP_ACTIVITE,REPLACE(RTRIM(execution_budgetaire.ENG_BUDGETAIRE),' ','') AS name FROM ptba_tache  JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=ptba_tache.INSTITUTION_ID JOIN inst_institutions_programmes ON inst_institutions_programmes.PROGRAMME_ID=ptba_tache.PROGRAMME_ID JOIN inst_institutions_actions ON inst_institutions_actions.ACTION_ID=ptba_tache.ACTION_ID JOIN execution_budgetaire ON execution_budgetaire.PTBA_TACHE_ID=ptba_tache.PTBA_TACHE_ID JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_ID=execution_budgetaire.EXECUTION_BUDGETAIRE_ID JOIN pap_activites ON  pap_activites.PAP_ACTIVITE_ID=ptba_tache.PAP_ACTIVITE_ID  WHERE 1 ".$cond." ";
