...
$excute_grande=("SELECT if(inst_grande_masse.GRANDE_MASSE_ID in (6,7,8),7,inst_grande_masse.GRANDE_MASSE_ID) as GRANDE_MASSE_ID,if(inst_grande_masse.GRANDE_MASSE_ID in (6,7,8),'transferts et subsides',DESCRIPTION_GRANDE_MASSE) as name FROM `inst_grande_masse` JOIN ptba_tache ON ptba_tache.GRANDE_MASSE_ID=inst_grande_masse.GRANDE_MASSE_ID JOIN execution_budgetaire_execution_tache exec_tache ON exec_tache.PTBA_TACHE_ID= ptba_tache.PTBA_TACHE_ID JOIN execution_budgetaire ON execution_budgetaire.EXECUTION_BUDGETAIRE_ID=exec_tache.EXECUTION_BUDGETAIRE_ID WHERE 1 GROUP BY name,inst_grande_masse.GRANDE_MASSE_ID ORDER BY name ASC ");
...
foreach ($excute_grande_req as $value)
{
 ...
 $engage1=("SELECT SUM(exec.ENG_BUDGETAIRE) as engage FROM execution_budgetaire exec JOIN execution_budgetaire_execution_tache exec_tache ON exec.EXECUTION_BUDGETAIRE_ID = exec_tache.EXECUTION_BUDGETAIRE_ID JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec_tache.PTBA_TACHE_ID ".$name_table."  WHERE 1 ".$cond_trim." ".$cond."  ".$cond_gr."");
 ...
 $decaissement1=("SELECT SUM(exec.DECAISSEMENT) as decaissement FROM  execution_budgetaire exec JOIN execution_budgetaire_execution_tache exec_tache ON exec.EXECUTION_BUDGETAIRE_ID = exec_tache.EXECUTION_BUDGETAIRE_ID JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec_tache.PTBA_TACHE_ID ".$name_table."  WHERE 1 ".$cond_trim." ".$cond."  ".$cond_gr." ");
