 ...
 */ 
 ...
 class Dashbord_Avancement_Statut extends BaseController
 {
  protected $session;
  protected $ModelPs;
  ...
  return view('App\Modules\dashboard\Views\Dashbord_Avancement_Statut_View',$data);
}
...
public function get_rapport()
{
  ...
  function exporter($TYPE_INSTITUTION_ID='',$INSTITUTION_ID='',$SOUS_TUTEL_ID='',$PROGRAMME_ID='',$ACTION_ID='',$LIGNE_BUDGETAIRE='',$IS_PRIVATE='' ,$PAP_ACTIVITE_ID='')
  {
    ...
    $getRequete="SELECT ptba_tache.INSTITUTION_ID,".$totaux." AS VOTE,execution_budgetaire.ENG_BUDGETAIRE,execution_budgetaire.ENG_JURIDIQUE,execution_budgetaire.LIQUIDATION,execution_budgetaire.ORDONNANCEMENT,execution_budgetaire.PAIEMENT,execution_budgetaire.DECAISSEMENT,inst_institutions.DESCRIPTION_INSTITUTION AS INTITULE_MINISTERE,pnd_pilier.DESCR_PILIER,pnd_axe.DESCR_AXE_PND,ptba_tache.DESC_TACHE,inst_institutions_ligne_budgetaire.CODE_NOMENCLATURE_BUDGETAIRE,inst_institutions_programmes.`INTITULE_PROGRAMME`,inst_institutions_actions.LIBELLE_ACTION,pap_activites.DESC_PAP_ACTIVITE,pap_activites.RESULTAT_PAP_ACTIVITE,ptba_tache.GRANDE_MASSE_ID,DESCRIPTION_GRANDE_MASSE,pap_activites.DESC_PAP_ACTIVITE,pap_activites.RESULTAT_PAP_ACTIVITE FROM  ptba_tache  JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=ptba_tache.INSTITUTION_ID JOIN inst_institutions_programmes ON inst_institutions_programmes.PROGRAMME_ID=ptba_tache.PROGRAMME_ID JOIN inst_institutions_actions ON inst_institutions_actions.ACTION_ID=ptba_tache.ACTION_ID JOIN inst_grande_masse ON inst_grande_masse.GRANDE_MASSE_ID = ptba_tache.GRANDE_MASSE_ID JOIN pap_activites ON pap_activites.PAP_ACTIVITE_ID=ptba_tache.PAP_ACTIVITE_ID JOIN pnd_pilier ON pnd_pilier.ID_PILIER=ptba_tache.ID_PILIER JOIN inst_institutions_ligne_budgetaire ON inst_institutions_ligne_budgetaire.CODE_NOMENCLATURE_BUDGETAIRE_ID=ptba_tache.CODE_NOMENCLATURE_BUDGETAIRE_ID JOIN execution_budgetaire ON execution_budgetaire.PTBA_TACHE_ID=ptba_tache.PTBA_TACHE_ID JOIN pnd_axe ON pnd_axe.AXE_PND_ID=ptba_tache.AXE_PND_ID WHERE 1 ";
    ...
    return redirect('dashboard/Dashbord_Avancement_Statut');
  }
