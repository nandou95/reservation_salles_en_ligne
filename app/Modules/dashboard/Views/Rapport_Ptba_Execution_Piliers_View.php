...
<script type="text/javascript">
 ...
 var IS_PRIVATE = document.getElementById('IS_PRIVATE5').value;
    ...
    $.ajax({
      url : "<?=base_url('dashboard/Rapport_Ptba_Execution_Piliers/get_Rapport_Pilier')?>",
      type : "GET",
      dataType: "JSON",
      cache:false,
      data:{
...
<script>
  function liste()
  {
    ...
    $("#mytable2").DataTable(
    {
      ...
      "ajax":
      {
        url:"<?= base_url('dashboard/Rapport_Ptba_Execution_Piliers/listing_budget')?>",
        type:"POST", 
        data:
        {
          TYPE_INSTITUTION_ID:TYPE_INSTITUTION_ID,
...
function exporter()
{
  if (document.getElementById('IS_PRIVATE1').checked){
    ...
    document.getElementById("btnexport").href = "<?=base_url('dashboard/Rapport_Ptba_Execution_Piliers/exporter/')?>"+'/'+TYPE_INSTITUTION_ID+'/'+INSTITUTION_ID+'/'+PROGRAMME_ID+'/'+ACTION_ID+'/'+LIGNE_BUDGETAIRE+'/'+IS_PRIVATE+'/'+PAP_ACTIVITE_ID;
}
</script>