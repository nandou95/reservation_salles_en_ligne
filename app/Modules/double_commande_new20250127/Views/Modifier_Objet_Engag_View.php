                  ...
                  <form id="my_form" action="<?= base_url('double_commande_new/Modifier_Objet_Engag/save') ?>" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row col-md-12">
                      <div class="col-6">
                        ...
                        <select class="form-control select2" name="EXECUTION_BUDGETAIRE_DETAIL_ID" id="EXECUTION_BUDGETAIRE_DETAIL_ID" onchange="get_info()">
                          <option value=""><?=lang('messages_lang.selectionner_transmission_du_bordereau')?></option>
                          <?php  
                          foreach ($BEs as $keys)
                          {
                            if($keys->EXECUTION_BUDGETAIRE_DETAIL_ID==set_value('EXECUTION_BUDGETAIRE_DETAIL_ID'))
                            { 
                              echo '<option value="'.$keys->EXECUTION_BUDGETAIRE_DETAIL_ID.'" selected>
                                '.$keys->NUMERO_BON_ENGAGEMENT.'</option>';
                            }
                            else
                            {
                              echo '<option value="'.$keys->EXECUTION_BUDGETAIRE_DETAIL_ID .'">
                                '.$keys->NUMERO_BON_ENGAGEMENT.'</option>';
                            }
                          }
                          ?>
                        ...
                        <?php if (isset($validation)) : ?>
                          <?= $validation->getError('EXECUTION_BUDGETAIRE_DETAIL_ID'); ?>
                        <?php endif ?>
                        <span id="error_EXECUTION_BUDGETAIRE_DETAIL_ID" class="text-danger"></span>
                      </div>
              ...
              <table class="table m-b-0 m-t-20">
                <tbody>
                  <tr>
                    ...
                    <td id="EXECUTION_BUDGETAIRE_DETAIL_ID_VERIFY" class="text-dark"></td>
                  </tr>
                  <tr>
...
<script>
  function save()
  {
    ...
    $('#error_EXECUTION_BUDGETAIRE_DETAIL_ID').val('');
    ...
    {
      statut=2
      $('#error_EXECUTION_BUDGETAIRE_DETAIL_ID').text('<?=lang('messages_lang.message_champs_obligatoire')?>');
    }
    ...
    if(statut==1)
    {
      var EXECUTION_BUDGETAIRE_DETAIL_ID_VERIFY=$('#EXECUTION_BUDGETAIRE_DETAIL_ID option:selected').text()
      $('#EXECUTION_BUDGETAIRE_DETAIL_ID_VERIFY').text(EXECUTION_BUDGETAIRE_DETAIL_ID_VERIFY);
      if (TITRE_CREANCE!='')
      {
  ...
  function get_info()
  {
    ...
    $.ajax(
    {
      url: "<?=base_url()?>/double_commande_new/Modifier_Objet_Engag/get_info/" + EXECUTION_BUDGETAIRE_DETAIL_ID,
      type: "GET",
      dataType: "JSON",
      beforeSend:function()
      {
