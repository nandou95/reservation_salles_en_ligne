  ...
  <table class="table m-b-0 m-t-20">
   ...
   <!--    <--?php if(!empty($get_info['MONTANT_EN_DEVISE'])){ ?>
      ...
      <?php if(!empty($get_info['PATH_LETTRE_OTB'])){?>
        <tr>
          <td>&nbsp;<strong><?=$get_info['DESC_BUDGETAIRE_TYPE_DOCUMENT']?></strong></td>
          <td>
                                    ...
                                    <?php if (!empty($get_info['PATH_RAPPORT_COMPTE_GENERAL_TRESOR'])) { ?>
                                      ...
                                      <?php if (!empty($get_info['PATH_BALANCE_COMPTE'])) { ?>
                                        <embed id="pdf16" style="display:none;" src="<?=base_url('uploads/double_commande_new/'.$get_info['PATH_BALANCE_COMPTE'])?>" type="application/pdf" width="100%" height="600px">
                                          <?PHP } ?>
                                          <?php if (!empty($get_info['PIECE_SUPPL']) AND !empty($get_info['PIECE_DET'])) { ?>
                                        <embed id="pdf23" style="display:none;" src="<?=base_url().'/'.$get_info['PIECE_DET']?>" type="application/pdf" width="100%" height="600px">
                                          <?PHP } ?>
                                          <?php if (!empty($get_info['PIECE_DET']) OR empty($get_info['PIECE_SUPPL'])) { ?>
                                        <embed id="pdf23" style="display:none;" src="<?=base_url().'/'.$get_info['PIECE_DET']?>" type="application/pdf" width="100%" height="600px">
                                          <?PHP } ?>
                                          <?php if (!empty($get_info['PIECE_SUPPL']) OR empty($get_info['PIECE_DET'])) { ?>
                                        <embed id="pdf23" style="display:none;" src="<?=base_url().'/'.$get_info['PIECE_SUPPL']?>" type="application/pdf" width="100%" height="600px">
                                          <?PHP } ?>
