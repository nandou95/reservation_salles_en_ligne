                                ...
                                <div class="card-body">
                                    ...
                                    <div class="card-body" style="margin-top: -20px">
                                        ...
                                        <form enctype='multipart/form-data' name="phase_comptabe" id="phase_comptabe" action="<?= base_url('double_commande_new/Phase_Comptable_Directeur_Comptable/save') ?>" method="post">
                                            ...
                                            <input type="hidden" name="status_bordereau" id="status_bordereau">
                                                ...
                                                <div class="col-md-12">
                                                    <div class="row">
                                                        <div class="col-md-6">
                                                                ...
                                                                <input type="text" oninput="this.value = this.value.toUpperCase();checkbord()" class="form-control" name="NUM_BORDEREAU_TRANSMISSION" id="NUM_BORDEREAU_TRANSMISSION">
                                                        ...
                                                        <div class="col-md-6">
                                                                ...
                                                                <input type="text" class="form-control" name="VERIFICATION" oninput="this.value = this.value.toUpperCase();checkbord()" id="VERIFICATION" onpaste="return false">
                                                    ...
                                                    <div class="row">
                                                        ...
                                                        <div class="col-md-6">
                                                                ...
                                                                <select class="form-control select2" name="EXECUTION_BUDGETAIRE_DETAIL_ID[]" id="EXECUTION_BUDGETAIRE_DETAIL_ID" multiple>
                                                                    <?php
                                                                    foreach ($get_titre_decaissement as $value) {
                                                                        if ($value->EXECUTION_BUDGETAIRE_DETAIL_ID == set_value('EXECUTION_BUDGETAIRE_DETAIL_ID')) { ?>
                                                                            <option value="<?= $value->EXECUTION_BUDGETAIRE_DETAIL_ID ?>" selected><?= $value->NUMERO_TITRE_DECAISSEMNT ?></option>
                                                                        <?php } else {
                                                                        ?>
                                                                            <option value="<?= $value->EXECUTION_BUDGETAIRE_DETAIL_ID ?>"><?= $value->NUMERO_TITRE_DECAISSEMNT ?></option>
                                                                    <?php
                                                                        }
                                                                    }
                                                                    ?>
                                                    ...
                                                    <div class="row">
                                                        ...
                                                        <div class="col-md-6">
                                                                ...
                                                                <input type="hidden" name="ORIGINE_DESTINATION" id="ORIGINE_DESTINATION" value="<?= $get_origine_destination['ID_ORIGINE_DESTINATION'] ?>">
                                                        ...
                                                        <div class="col-md-6">
                                                                ...
                                                                <input type="date" min="<?=date('Y-m-d', strtotime($date_trans['DATE_TRANSMISSION']))?>" class="form-control" name="DATE_TRANSMISSION" id="DATE_TRANSMISSION" value="<?=set_value('DATE_TRANSMISSION')?>" max="<?=Date('Y-m-d')?>" onkeypress="return false" onblur="this.type='date'">
...
<script type="text/javascript">
    function save_phase() {
        ...
        if (EXECUTION_BUDGETAIRE_DETAIL_ID == '') {
        ...
        var status_bordereau=$('#status_bordereau').val();
        if(status_bordereau!=1)
        {
          $('#error_NUM_BORDEREAU_TRANSMISSION').html('<?= lang('messages_lang.messg_bord_exist')?>');
          statut = 1;
        }
        ...
        if(statut == 2)
        {
            ...
            var EXECUTION_BUDGETAIRE_DETAIL_ID = $('#EXECUTION_BUDGETAIRE_DETAIL_ID option:selected').toArray().map(item => item.text).join();
            $('#EXECUTION_BUDGETAIRE_RACCROCHAGE_NEW_ID_MODAL').html(EXECUTION_BUDGETAIRE_DETAIL_ID);
...
<script>
  ...
  {
    var NUM_BORDEREAU_TRANSMISSION = $('#NUM_BORDEREAU_TRANSMISSION').val();        
    var ID_ORIGINE_DESTINATION = $('#ORIGINE_DESTINATION').val();        
    ...
    $.ajax({
      url: "<?=base_url('/double_commande_new/Phase_Comptable_Directeur_Comptable/checkbord')?>",
      type: "POST",
      dataType: "JSON",
      data: form,
      processData: false,
      ...
      success: function(data)
      {
        if (data.status==0)
        {
          $('#status_bordereau').val(data.status);
          $('#error_NUM_BORDEREAU_TRANSMISSION').html('<?= lang('messages_lang.messg_bord_exist')?>');
        }
        else
        {
