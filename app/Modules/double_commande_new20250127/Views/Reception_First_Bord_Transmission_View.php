                  ...
                  <form id="my_form" action="<?= base_url('double_commande_new/Reception_First_Bord_Transmission/save') ?>" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row col-md-12">
                      ...
                      <div class="col-6">
                        ...
                        <input autocomplete="off" type="text" name="NUM_BORDEREAU_TRANSMISSION" id="NUM_BORDEREAU_TRANSMISSION" class="form-control" maxlength="25" oninput="checkbord()">
                      ...
                      <div class="col-6">
                        ...
                        <input onkeyup="verify_bon()" autocomplete="off" type="text" name="CONFIRM_NUM_BORDEREAU_TRANSMISSION" id="CONFIRM_NUM_BORDEREAU_TRANSMISSION" oninput="checkbord()" class="form-control" maxlength="25" onpaste="return false">
                      ...
                      <div class="col-md-6">
                        ...
                        <select class="form-control select2" multiple name="EXECUTION_BUDGETAIRE_DETAIL_ID[]" id="EXECUTION_BUDGETAIRE_DETAIL_ID">
                          <option value=""><?= lang('messages_lang.selectionner_transmission_du_bordereau') ?></option>
                          <?php  foreach ($bon_engagement as $keys) { ?>
                            <?php if($keys->EXECUTION_BUDGETAIRE_DETAIL_ID==set_value('EXECUTION_BUDGETAIRE_DETAIL_ID')) { ?>
                              <option value="<?=$keys->EXECUTION_BUDGETAIRE_DETAIL_ID ?>" selected>
                              ...
                              <?php }else{?>
                               <option value="<?=$keys->EXECUTION_BUDGETAIRE_DETAIL_ID ?>">
                                <?=$keys->NUMERO_BON_ENGAGEMENT.' ('. lang("messages_lang.labelle_montant").' = '. $ord = ($keys->DEVISE_TYPE_ID == 1) ? number_format($keys->MONTANT_ORDONNANCEMENT,2,',',' ') : number_format($keys->MONTANT_ORDONNANCEMENT_DEVISE,2,',',' ') ; ?>)</option>
                              <?php } }?>
                            </select>
                        ...
                        <div class="row col-md-12">
                          ...
                          <div class="col-md-6">
                            ...
                            <div class="">
                              ...
                              <input type="date" class="form-control" id="DATE_RECEPTION" name="DATE_RECEPTION" min="<?= date('Y-m-d') ?>" max="<?= date('Y-m-d') ?>" onkeypress="return false" onchange="get_date_min_trans()">
                        ...
                        <div class="row col-md-12">
                          <div class="col-md-6">
                            ...
                            <input type="date" class="form-control" name="DATE_TRANSMISSION" value="<?= set_value('DATE_TRANSMISSION') ?>" max="<?= date('Y-m-d') ?>" id="DATE_TRANSMISSION" onkeypress="return false" onblur="this.type='date'">
                            <?php if (isset($validation)) : ?>
...
<script type="text/javascript">
  function save()
  {
    ...
    {
      $('#error_num_bord').html('<?= lang('messages_lang.messg_bord_exist')?>');
      statut = 1;
    }
    ...
    if (statut == 2)
    {
      ...
      var RACCROCHAGE = $('#EXECUTION_BUDGETAIRE_DETAIL_ID option:selected').toArray().map(item => `<li>${item.text}</li>`).join('');
...
<script>
  function checkbord()
  {
    ...
    $.ajax({
      url: "<?=base_url('/double_commande_new/Reception_First_Bord_Transmission/checkbord')?>",
      type: "POST",
      dataType: "JSON",
      data: form,
      processData: false,
