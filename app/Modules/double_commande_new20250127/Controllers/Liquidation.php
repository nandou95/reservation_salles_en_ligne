...
class Liquidation extends BaseController
{
	...
	public function getOne($idmd5='',$dist='')
	{
		...
		$inforacc = 'SELECT EXECUTION_BUDGETAIRE_DETAIL_ID FROM execution_budgetaire_tache_detail WHERE md5(EXECUTION_BUDGETAIRE_DETAIL_ID)="'.$idmd5.'"';
		...
		$id=$resultatracc['EXECUTION_BUDGETAIRE_DETAIL_ID'];
		$data=$this->urichk();
		$callpsreq = "CALL `getRequete`(?,?,?,?);";
		if($id=='' || $dist=='')
		{
		...
		$infoAffiche  = 'SELECT progr.INTITULE_PROGRAMME,actions.CODE_ACTION,progr.CODE_PROGRAMME,actions.LIBELLE_ACTION,tache.DESC_TACHE,act.DESC_PAP_ACTIVITE,inst.DESCRIPTION_INSTITUTION AS INTITULE_MINISTERE,inst.CODE_INSTITUTION AS CODE_MINISTERE,ligne.CODE_NOMENCLATURE_BUDGETAIRE,ligne.LIBELLE_CODE_NOMENCLATURE_BUDGETAIRE,exec.SOUS_TUTEL_ID,exec.INSTITUTION_ID,exec.LIQUIDATION, exec.NUMERO_BON_ENGAGEMENT,exec.EXECUTION_BUDGETAIRE_ID,exec.MARCHE_PUBLIQUE, exec.COMMENTAIRE,exec.LIQUIDATION_TYPE_ID AS ID_TYPE_LIQUIDATION,exec.ENG_JURIDIQUE_DEVISE AS MONTANT_RACCROCHE_JURIDIQUE_DEVISE,det.MONTANT_LIQUIDATION_DEVISE AS MONTANT_RACCROCHE_LIQUIDATION_DEVISE,det.MONTANT_LIQUIDATION AS MONTANT_RACCROCHE_LIQUIDATION,exec.ENG_JURIDIQUE AS MONTANT_RACCROCHE_JURIDIQUE,exec.ENG_BUDGETAIRE AS MONTANT_RACCROCHE,det.ETAPE_DOUBLE_COMMANDE_ID,det.EXECUTION_BUDGETAIRE_DETAIL_ID, det.COUR_DEVISE, exec.DEVISE_TYPE_ID AS TAUX_ECHANGE_ID,det.DATE_LIQUIDATION,det.MOTIF_LIQUIDATION, det.PATH_PV_RECEPTION_LIQUIDATION, det.PATH_FACTURE_LIQUIDATION,det.TITRE_CREANCE,det.DATE_CREANCE,det.MONTANT_CREANCE,exec.ENG_BUDGETAIRE_DEVISE AS MONTANT_RACCROCHE_DEVISE, supp.TAUX_TVA_ID, supp.EXONERATION, supp.DATE_FIN_CONTRAT, supp.NBRE_JR_CONTRAT, supp.DATE_DEBUT_CONTRAT, supp.MONTANT_AMENDE_PAYER, supp.NBRE_JR_RETARD, supp.DATE_LIVRAISON_CONTRAT,det.COMMENTAIRE FROM execution_budgetaire_tache_detail det JOIN execution_budgetaire exec ON exec.EXECUTION_BUDGETAIRE_ID=det.EXECUTION_BUDGETAIRE_ID JOIN ptba_tache tache ON tache.PTBA_TACHE_ID=exec.PTBA_TACHE_ID LEFT JOIN pap_activites act ON act.PAP_ACTIVITE_ID=exec.PAP_ACTIVITE_ID JOIN inst_institutions_programmes progr ON progr.PROGRAMME_ID=exec.PROGRAMME_ID JOIN inst_institutions inst ON inst.INSTITUTION_ID=exec.INSTITUTION_ID JOIN inst_institutions_actions actions ON actions.ACTION_ID=exec.ACTION_ID JOIN execution_budgetaire_tache_info_suppl supp ON supp.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID JOIN  inst_institutions_ligne_budgetaire ligne ON ligne.CODE_NOMENCLATURE_BUDGETAIRE_ID=exec.CODE_NOMENCLATURE_BUDGETAIRE_ID WHERE det.EXECUTION_BUDGETAIRE_DETAIL_ID = '.$id;
		...
		if (!empty($getProfil))
		{
			foreach ($getProfil as $value)
			{
				...
				if ($prof_id == $value->PROFIL_ID || $prof_id==1)
				{
					...
					$bind_date_histo  = 'SELECT DATE_TRANSMISSION,DATE_INSERTION FROM execution_budgetaire_tache_detail_histo  WHERE 1 AND EXECUTION_BUDGETAIRE_DETAIL_ID='.$data['info']['EXECUTION_BUDGETAIRE_DETAIL_ID'].' ORDER BY DATE_INSERTION DESC';
	...
	public function getOne_conf($idmd5='',$dist='')
	{
		...
		$info = 'SELECT EXECUTION_BUDGETAIRE_DETAIL_ID FROM execution_budgetaire_tache_detail WHERE md5(EXECUTION_BUDGETAIRE_DETAIL_ID)="'.$idmd5.'"';
		...
		$id=$resultatdetail['EXECUTION_BUDGETAIRE_DETAIL_ID'];
		// print_r($id);die();
		$data=$this->urichk();
		$callpsreq = "CALL `getRequete`(?,?,?,?);";
		if($id=='' || $dist=='')
	...
	function generer_doc_liquidation($EXECUTION_BUDGETAIRE_DETAIL_ID)
	{
		...
		$html="<html>";
		$html.="<center><b><u>".lang('messages_lang.label_piece_justificative')."</u></b></center><br>";
		...
		$html.='<br><p></strong>'.$users['PROFIL_DESCR'].' : <strong>'.$users['NOM'].' '.$users['PRENOM'].'</strong></p>';
		$html.="</html>";
  	...
  	if($session->get('SESSION_SUIVIE_PTBA_LIQUIDATION') !=1)
    {
		...
		$infoAffiche  = 'SELECT progr.INTITULE_PROGRAMME,actions.CODE_ACTION,progr.CODE_PROGRAMME,actions.LIBELLE_ACTION,act.DESC_PAP_ACTIVITE,tache.DESC_TACHE,inst.DESCRIPTION_INSTITUTION AS INTITULE_MINISTERE,inst.CODE_INSTITUTION AS CODE_MINISTERE,ligne.CODE_NOMENCLATURE_BUDGETAIRE,ligne.LIBELLE_CODE_NOMENCLATURE_BUDGETAIRE,exec.SOUS_TUTEL_ID,exec.INSTITUTION_ID, exec.LIQUIDATION, exec.NUMERO_BON_ENGAGEMENT,exec.EXECUTION_BUDGETAIRE_ID,exec.MARCHE_PUBLIQUE, exec.COMMENTAIRE,exec.LIQUIDATION_TYPE_ID AS ID_TYPE_LIQUIDATION,exec.ENG_BUDGETAIRE_DEVISE AS MONTANT_RACCROCHE_JURIDIQUE_DEVISE,exec.LIQUIDATION_DEVISE AS MONTANT_RACCROCHE_LIQUIDATION_DEVISE, exec.LIQUIDATION AS MONTANT_RACCROCHE_LIQUIDATION, exec.ENG_JURIDIQUE  AS MONTANT_RACCROCHE_JURIDIQUE,exec.ENG_BUDGETAIRE AS MONTANT_RACCROCHE,exec.DEVISE_TYPE_ID AS TAUX_ECHANGE_ID,supp.TAUX_TVA_ID, supp.EXONERATION, supp.DATE_FIN_CONTRAT, supp.NBRE_JR_CONTRAT, supp.DATE_DEBUT_CONTRAT, supp.MONTANT_AMENDE_PAYER, supp.NBRE_JR_RETARD, supp.DATE_LIVRAISON_CONTRAT,exec.COMMENTAIRE FROM execution_budgetaire exec JOIN inst_institutions_ligne_budgetaire ligne ON ligne.CODE_NOMENCLATURE_BUDGETAIRE_ID=exec.CODE_NOMENCLATURE_BUDGETAIRE_ID JOIN ptba_tache tache ON tache.PTBA_TACHE_ID=exec.PTBA_TACHE_ID LEFT JOIN pap_activites act ON act.PAP_ACTIVITE_ID=exec.PAP_ACTIVITE_ID JOIN inst_institutions_programmes progr ON progr.PROGRAMME_ID=exec.PROGRAMME_ID JOIN inst_institutions inst ON inst.INSTITUTION_ID=exec.INSTITUTION_ID JOIN inst_institutions_actions actions ON actions.ACTION_ID=exec.ACTION_ID JOIN execution_budgetaire_tache_info_suppl supp ON exec.EXECUTION_BUDGETAIRE_ID=supp.EXECUTION_BUDGETAIRE_ID WHERE exec.EXECUTION_BUDGETAIRE_ID ='.$id;
		$infoAffiche = "CALL `getTable`('" . $infoAffiche . "');";
		$data['info'] = $this->ModelPs->getRequeteOne($infoAffiche);
		##############################################################

		$taux='SELECT DEVISE_TYPE_HISTO_ID,TAUX FROM devise_type_hist WHERE DEVISE_TYPE_ID ='.$data['info']['TAUX_ECHANGE_ID'].' AND IS_ACTIVE =1';
		$taux = "CALL `getTable`('".$taux."');";
		$data['get_taux'] = $this->ModelPs->getRequeteOne($taux);
		...
		if (!empty($getProfil))
		{
          ...
          //Le min de la date de réception
					$bind_date_histo  = 'SELECT DATE_TRANSMISSION,hist.DATE_INSERTION FROM execution_budgetaire_tache_detail_histo hist JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_DETAIL_ID=hist.EXECUTION_BUDGETAIRE_DETAIL_ID JOIN execution_budgetaire exec ON exec.EXECUTION_BUDGETAIRE_ID=det.EXECUTION_BUDGETAIRE_ID WHERE hist.ETAPE_DOUBLE_COMMANDE_ID='.$data['ETAPE_ID'].' AND exec.EXECUTION_BUDGETAIRE_ID='.$id.' ORDER BY hist.EXECUTION_BUDGETAIRE_DETAIL_HISTO_ID DESC LIMIT 1';
					$bind_date_histoRqt = "CALL `getTable`('" . $bind_date_histo . "');";
					$data['date_trans']= $this->ModelPs->getRequeteOne($bind_date_histoRqt);
          ...
          //Le min de la date de réception
		      ...
		      //retour pour correction
			      ...
			      //la correction (obseravtion,date transmission) motif rejet apres rejet dans l'input
						$bindparams=$this->getBindParms('OBSERVATION,DATE_TRANSMISSION,DATE_RECEPTION,MOTIF_REJET','execution_budgetaire_tache_detail_histo','EXECUTION_BUDGETAIRE_DETAIL_ID='.$data['info']['EXECUTION_BUDGETAIRE_DETAIL_ID'].'','EXECUTION_BUDGETAIRE_DETAIL_HISTO_ID DESC');
						$data['data_correction'] = $this->ModelPs->getRequeteOne($callpsreq, $bindparams);
			            #################################    

	      	  //Récuperation de l'étape précedent
						$bind_etap_prev = $this->getBindParms('ETAPE_DOUBLE_COMMANDE_ID AS ETAPE_ID','execution_budgetaire_tache_detail_histo','EXECUTION_BUDGETAIRE_DETAIL_ID='.$data['info']['EXECUTION_BUDGETAIRE_DETAIL_ID'].'','EXECUTION_BUDGETAIRE_DETAIL_HISTO_ID DESC');
			      ...
			      //get motif rejet de la table historique_raccrochage_operation_verification_motif 
						$motif_rejet  = 'SELECT DISTINCT tpes.TYPE_ANALYSE_MOTIF_ID,tpes.DESC_TYPE_ANALYSE_MOTIF FROM budgetaire_type_analyse_motif tpes JOIN execution_budgetaire_histo_operation_verification_motif motif ON motif.TYPE_ANALYSE_MOTIF_ID=tpes.TYPE_ANALYSE_MOTIF_ID WHERE 1 AND EXECUTION_BUDGETAIRE_DETAIL_ID='.$data['info']['EXECUTION_BUDGETAIRE_DETAIL_ID'].' AND ETAPE_DOUBLE_COMMANDE_ID='.$etap_prev['ETAPE_ID'].'';
	  ...
	  if($gdc!=1 AND $ced!=1)
    {
		...
		if (!empty($histo_liquidation))
		{
	        ...
	        $table = '<div class="table-responsive">
	        		 <h4>Historique liquidation ('.$type_monnaie.')</h4>
	...
	public function getInfoDetail($value='')
	{
        ...
        if (!$file_2 || !$file_2->isValid())
        {
          ...
          $PATH_FACTURE_LIQUIDATION = $this->request->getPost('PATH_FACTURE_LIQUIDATION');
		    ...
		    $columsinsert="USER_ID,PATH_PV_RECEPTION_LIQUIDATION,PATH_FACTURE_LIQUIDATION";
		    $datacolumsinsert=$USER_ID.",'".$PATH_PV_RECEPTION_LIQUIDATION."','".$PATH_FACTURE_LIQUIDATION."'";
		    $TEMPO_FILE_LIQUIDATION_ID   = $this->save_all_table($insertIntoTable,$columsinsert,$datacolumsinsert);

		    $sql_file='SELECT PATH_PV_RECEPTION_LIQUIDATION,PATH_FACTURE_LIQUIDATION FROM execution_budgetaire_tempo_file_liquidation WHERE TEMPO_FILE_LIQUIDATION_ID='.$TEMPO_FILE_LIQUIDATION_ID.' AND USER_ID='.$USER_ID.'';
	...
	public function getInfoDetail2($value='')
	{
      $session  = \Config\Services::session();
	  $gdc = $session->get('SESSION_SUIVIE_PTBA_LIQUIDATION');
	  $ced = $session->get('SESSION_SUIVIE_PTBA_CONFIRM_LIQUIDATION');
	  if($gdc!=1 AND $ced!=1)
	  {
       ...
       //get type operation
       $sql_operation  = 'SELECT DESCRIPTION FROM budgetaire_type_operation_validation WHERE ID_OPERATION='.$ID_OPERATION.'';
       $sql_operation = "CALL getTable('" . $sql_operation . "');";
       $get_operation= $this->ModelPs->getRequeteOne($sql_operation);
	    ################################################################     ###############
       ...
       if ($ID_OPERATION==1) {
		    	...
		    	//get etape_retour_correction
       	$etape_retour  = 'SELECT ETAPE_RETOUR_CORRECTION_ID,DESCRIPTION_ETAPE_RETOUR FROM budgetaire_etape_retour_correction WHERE ETAPE_RETOUR_CORRECTION_ID='.$ETAPE_RETOUR_CORRECTION_ID.'';
       	$etape_retourRqt = "CALL `getTable`('" . $etape_retour . "');";

			// print_r($etape_retourRqt);die();
       	$get_etape_retour= $this->ModelPs->getRequeteOne($etape_retourRqt);
       	$infos = '<tr>
       	<td style="width:250px ;"><font style="float:left;"><i class="fa fa-comment"> </i>&nbsp;'.lang('messages_lang.label_motif_rejet').' </font></td>
       	<td><strong><font style="float:left;">'.$MOTIF_REJET.'</font></strong></td>
       	</tr>
       	<tr>
       	<td style="width:250px ;"><font style="float:left;"><i class="fa fa-comment"> </i>&nbsp;'.lang('messages_lang.label_etape').' </font></td>
       	<td><strong><font style="float:left;">'.$get_etape_retour['DESCRIPTION_ETAPE_RETOUR'].'</font></strong></td>
       	</tr>';
       }
       ...
       <tbody>
       <tr>
       <td style="width:250px ;"><font style="float:left;"><i class="fa fa-list"> </i>&nbsp;'.lang('messages_lang.label_type_operation').'</font></td>
       <td><strong><font style="float:left;">'.$get_operation['DESCRIPTION'].'</font></strong></td>
       </tr>
       '.$infos.'
       <tr>
       <td style="width:250px ;"><font style="float:left;"><i class="fa fa-list"> </i>&nbsp;'.lang('messages_lang.labelle_observartion').'</font></td>
       <td><strong><font style="float:left;">'. $OBSERVATION.'</font></strong></td>
       </tr>
       <tr>
       <td style="width:250px ;"><font style="float:left;"><i class="fa fa-calendar"> </i>&nbsp;'.lang('messages_lang.labelle_d_reception').'</font></td>
  ...
  // insert data liquidation
	public function add($value='')
	{
		...
		if($MARCHE_PUBLIQUE==1)
		{
			...
			$table = 'execution_budgetaire_tache_info_suppl';
			$where='EXECUTION_BUDGETAIRE_ID='.$EXECUTION_BUDGETAIRE_ID;
			$data='DATE_LIVRAISON_CONTRAT="'.$DATE_LIVRAISON_CONTRAT.'",TAUX_TVA_ID='.$TAUX_TVA_ID.',EXONERATION='.$EXONERATION.'';
			$this->update_all_table($table,$data,$where);
            ####################################################################
			...
			$data1='PATH_PV_RECEPTION_LIQUIDATION="'.$PATH_PV_RECEPTION.'", PATH_FACTURE_LIQUIDATION="'.$PATH_FACTURE.'"';
        ...
        {
        	//get devise a jour
        	$devise_a_jour = $this->getBindParms('DEVISE_TYPE_HISTO_ID','devise_type_hist',' DEVISE_TYPE_ID='.$TYPE_MONTANT_ID,'1');
        	...
        	$table1 = 'execution_budgetaire_tache_detail';
        	$where1 ='EXECUTION_BUDGETAIRE_DETAIL_ID='.$EXECUTION_BUDGETAIRE_DETAIL_ID;
        	$data1 = "MONTANT_LIQUIDATION_DEVISE=".$MONTANT_DEVISE.",DEVISE_TYPE_HISTO_LIQUI_ID=".$DEVISE_TYPE_HISTO_LIQUI_ID.",MONTANT_LIQUIDATION='".$LIQUIDATION."',TITRE_CREANCE='".$TITRE_CREANCE."',DATE_CREANCE='".$DATE_CREANCE."',DATE_LIQUIDATION='".$DATE_LIQUIDATION."',MOTIF_LIQUIDATION='".$MOTIF_LIQUIDATION."',ETAPE_DOUBLE_COMMANDE_ID=".$NEXT_ETAPE_ID;
        	$this->update_all_table($table1,$data1,$where1);
			##########################################################################
        }
        else
        {
        	...
        	$table1 = 'execution_budgetaire_tache_detail';
        	$where1='EXECUTION_BUDGETAIRE_DETAIL_ID='.$EXECUTION_BUDGETAIRE_DETAIL_ID;
        	$data1="MONTANT_CREANCE=".$MONTANT_CREANCE.", MONTANT_LIQUIDATION=".$MONTANT_CREANCE.", TITRE_CREANCE='".$TITRE_CREANCE."', DATE_CREANCE='".$DATE_CREANCE."', DATE_LIQUIDATION='".$DATE_LIQUIDATION."', MOTIF_LIQUIDATION='".$MOTIF_LIQUIDATION."', ETAPE_DOUBLE_COMMANDE_ID=".$NEXT_ETAPE_ID;
        	$this->update_all_table($table1,$data1,$where1);
			##########################################################################
        }
        ...
        $columsinsert2="EXECUTION_BUDGETAIRE_DETAIL_ID,USER_ID,ETAPE_DOUBLE_COMMANDE_ID, OBSERVATION, DATE_TRANSMISSION, DATE_RECEPTION";
        $datacolumsinsert2=$EXECUTION_BUDGETAIRE_DETAIL_ID.", ".$USER_ID.", ".$ETAPE_ID.", '".$OBSERVATION."','".$DATE_TRANSMISSION."', '".$DATE_RECEPTION."'";
	...
	public function insert($value='')
	{
      ...
      foreach ($MOTIF_REJET as $key)
      {
      	$columsinsert="TYPE_ANALYSE_MOTIF_ID,ETAPE_DOUBLE_COMMANDE_ID,EXECUTION_BUDGETAIRE_DETAIL_ID";
    		$datacolumsinsert=$key.",".$ETAPE_ID.",".$EXECUTION_BUDGETAIRE_DETAIL_ID;
    		$this->save_all_table($table5,$columsinsert,$datacolumsinsert);
      }
    }
    ...
    else if($TYPE_OPERATION_ID == 2)
    {
    	...
    	if ($MONTANT_RACCROCHE_LIQUIDATION<500000000)
    	{
	      //récuperer les etapes et mouvements
		    ...
		    $next_move = $this->getBindParms('ETAPE_DOUBLE_COMMANDE_ID,MOUVEMENT_DEPENSE_ID','execution_budgetaire_etape_double_commande','ETAPE_DOUBLE_COMMANDE_ID='.$get_next_step['ETAPE_DOUBLE_COMMANDE_SUIVANT_ID'],'ETAPE_DOUBLE_COMMANDE_ID ASC');
		    $get_next_move = $this->ModelPs->getRequeteOne($psgetrequete, $next_move);
		    $MOUVEMENT_NEXT_ID =  $get_next_move['MOUVEMENT_DEPENSE_ID'];// mouve qui va suivre
	    }
	    else
	    {
    		//récuperer les etapes et mouvements
    ...
    if ($TYPE_OPERATION_ID == 2)
    {
        ...
        //liquidation
    	$idmd5 = md5($EXECUTION_BUDGETAIRE_ID);
    	$detail=$this->detail_new(md5($EXECUTION_BUDGETAIRE_DETAIL_ID));
    	...
    	// $html.='<p>'.lang('messages_lang.labelle_credit_restant_apres_engagement').' : <strong>'.number_format($MontantRestant,'0',',',' ').'</strong></p>';
    	...
    	// if (!empty($creditTransfert))
    	// {
    	// 	$html.='<p>'.lang('messages_lang.labelle_credit_transfere_ligne_budgetaire').' : <strong>'.number_format($creditTransfert,'0',',',' ').'</strong></p>';
    	// }
    	...
    	// if (!empty($cred_act))
    	// {
    	// 	$html.='<p>'.lang('messages_lang.labelle_credit_activite_apres_transfert').' : <strong>'.number_format($cred_act,'0',',',' ').'</strong></p>';
    	// }
    	...
    	if(!empty($get_info['DATE_LIVRAISON_CONTRAT']))
    	{
       	...
       	//utilisateur
    	$users  = 'SELECT NOM,PRENOM,PROFIL_DESCR FROM user_users JOIN user_profil ON user_users.PROFIL_ID=user_profil.PROFIL_ID WHERE user_users.PROFIL_ID=66';
    	$users = "CALL `getTable`('" . $users . "');";
    	$users = $this->ModelPs->getRequeteOne($users);

    	$html.='<br><p></strong>'.session()->get("SESSION_SUIVIE_PTBA_PROFIL_DESCRIPTION").' : <strong>'.session()->get("SESSION_SUIVIE_PTBA_NOM").' '.session()->get("SESSION_SUIVIE_PTBA_PRENOM").'</strong></p>';
    	...
    	$filename = "<embed src=".base_url(''.$PATH_PIECE_JUSTIFICATIVE.'')." scrolling='auto' height='500px' width='100%' frameborder='0'>";

			// $downloadfile = "<a href='".base_url(''.$PATH_PIECE_JUSTIFICATIVE.'')."' download  class='btn btn-success'><center><i class='fa fa-download'></i>&nbsp; ".lang('messages_lang.labelle_telecharger_piece_justif')." </center></a>";

    	$message = '<div class="alert alert-success">'.lang('messages_lang.message_success').'</div>';
    }

    echo json_encode(array('fichier'=>$filename,'message'=>$message));
	}
    ...
    if ($MARCHE_PUBLIQUE==1)
    {
        ...
        //mise a jour dans la table execution_budgetaire_raccrochage_activite_detail
			$table1 = 'execution_budgetaire_tache_detail';
			$where1='EXECUTION_BUDGETAIRE_DETAIL_ID='.$EXECUTION_BUDGETAIRE_DETAIL_ID;
			$data1='PATH_PV_RECEPTION_LIQUIDATION="'.$PATH_PV_RECEPTION.'", PATH_FACTURE_LIQUIDATION="'.$PATH_FACTURE_LIQUIDATION.'"';
			$this->update_all_table($table1,$data1,$where1);
		}
	...
	public function deleteFile($value='')
	{
		...
		$GETDATA_DELETE = $this->request->getPost('GETDATA_DELETE');

		if ($GETDATA_DELETE==1)
		{
    	//Delete file dans le dossier
			$delete_file_folder = 'SELECT PATH_PIECE_JUSTIFICATIVE FROM execution_budgetaire_raccrochage_activite_info_suppl_new WHERE EXECUTION_BUDGETAIRE_RACCROCHAGE_ID='.$EXECUTION_BUDGETAIRE_RACCROCHAGE_ID.' ';
			$file_folder = $this->ModelPs->getRequeteOne("CALL `getTable`('" . $delete_file_folder . "')");

			if (!empty($file_folder))
			{
				$PATH_PIECE_JUSTIFICATIVE = $file_folder['PATH_PIECE_JUSTIFICATIVE'];
				unlink($PATH_PIECE_JUSTIFICATIVE);
			}
		  ##############################################################################################

			$PATH_PIECE_JUSTIFICATIVE = NULL;
		...
		{
  		//Delete file dans le dossier
			$delete_file_folder = 'SELECT PATH_PV_RECEPTION_LIQUIDATION,PATH_FACTURE_LIQUIDATION FROM execution_budgetaire_tempo_file_liquidation WHERE USER_ID='.$USER_ID.' ';
			$file_folder = $this->ModelPs->getRequeteOne("CALL `getTable`('" . $delete_file_folder . "')");

			if (!empty($file_folder))
			{
				$PATH_PV_RECEPTION_LIQUIDATION = $file_folder['PATH_PV_RECEPTION_LIQUIDATION'];
				unlink($PATH_PV_RECEPTION_LIQUIDATION);

				$PATH_FACTURE_LIQUIDATION = $file_folder['PATH_FACTURE_LIQUIDATION'];
				unlink($PATH_FACTURE_LIQUIDATION);
			}
	    ###################################################################################
			$sql_file='DELETE FROM execution_budgetaire_tempo_file_liquidation WHERE USER_ID='.$USER_ID.'';
  ...
  public function listing_liquidation_rejeter()
  {
    ...
    $order_by = '';
  	$order_column = array('racc.NUMERO_BON_ENGAGEMENT','ligne.CODE_NOMENCLATURE_BUDGETAIRE',1, 'act.DESC_PAP_ACTIVITE',1, 'exec.COMMENTAIRE','exec.ENG_BUDGETAIRE','exec.ENG_JURIDIQUE' , 1);
  ...
  // insert data partiel
	public function add_partiel($value='')
	{
		...
		//deja liquide sur l'execution
		$getLiquid  = 'SELECT LIQUIDATION , LIQUIDATION_DEVISE AS MONTANT_RACCROCHE_LIQUIDATION_DEVISE FROM execution_budgetaire WHERE EXECUTION_BUDGETAIRE_ID='.$EXECUTION_BUDGETAIRE_ID.'';
		$getLiquid = "CALL `getTable`('" . $getLiquid . "');";
		$getLiquid= $this->ModelPs->getRequeteOne($getLiquid);
	  ...
	  ##########################################################################
		...
		$det='SELECT exec.DEVISE_TYPE_ID AS TAUX_ECHANGE_ID,exec.TRIMESTRE_ID,exec.DATE_DEMANDE AS DATE_ENGAGEMENT_BUDGETAIRE,exec.DATE_ENG_JURIDIQUE AS DATE_ENGAGEMENT_JURIDIQUE,det.COUR_DEVISE,det.DATE_COUR_DEVISE,exec.QTE_RACCROCHE,exec.ANNEE_BUDGETAIRE_ID,UNITE,exec.COMMENTAIRE FROM execution_budgetaire_tache_detail det JOIN execution_budgetaire exec ON exec.EXECUTION_BUDGETAIRE_ID=det.EXECUTION_BUDGETAIRE_ID JOIN devise_type dev ON dev.DEVISE_TYPE_ID=exec.DEVISE_TYPE_ID WHERE exec.EXECUTION_BUDGETAIRE_ID='.$EXECUTION_BUDGETAIRE_ID.' ORDER BY det.EXECUTION_BUDGETAIRE_DETAIL_ID DESC LIMIT 1';
		...
		if ($TYPE_MONTANT_ID!=1)
		{
			...
			$SOMME_MONTANT_DEVISE = $MONTANT_DEVISE+$getLiquid['MONTANT_RACCROCHE_LIQUIDATION_DEVISE'];
		  $SOMME_LIQUIDATION=$SOMME_MONTANT_DEVISE*$get_donDet['COUR_DEVISE'];
			$LIQUIDATION = number_format($SOMME_LIQUIDATION,'0',',','');
			...
			$racc  = 'SELECT ENG_JURIDIQUE_DEVISE AS MONTANT_RACCROCHE_JURIDIQUE_DEVISE,ENG_BUDGETAIRE_DEVISE AS MONTANT_RACCROCHE_DEVISE FROM execution_budgetaire WHERE EXECUTION_BUDGETAIRE_ID = '.$EXECUTION_BUDGETAIRE_ID;
			$getracc = "CALL `getTable`('" . $racc . "');";
			$get_racc= $this->ModelPs->getRequeteOne($getracc);
			################################################################

			$table1 = 'execution_budgetaire_tache_detail';
			$columsinsert1="EXECUTION_BUDGETAIRE_ID, MONTANT_LIQUIDATION_DEVISE, MONTANT_LIQUIDATION, TITRE_CREANCE, DATE_CREANCE, DATE_LIQUIDATION, MOTIF_LIQUIDATION, ETAPE_DOUBLE_COMMANDE_ID,PATH_PV_RECEPTION_LIQUIDATION,PATH_FACTURE_LIQUIDATION,COUR_DEVISE,DATE_COUR_DEVISE,COMMENTAIRE";
			$datacolumsinsert1=$EXECUTION_BUDGETAIRE_ID.", ".$MONTANT_DEVISE.", '".$LIQUIDATION."', '".$TITRE_CREANCE."', '".$DATE_CREANCE."', '".$DATE_LIQUIDATION."', '".$MOTIF_LIQUIDATION."', ".$NEXT_ETAPE_ID.",'".$PATH_PV_RECEPTION."','".$PATH_FACTURE."','".$get_donDet['COUR_DEVISE']."','".$get_donDet['DATE_COUR_DEVISE']."','".$COMMENTAIRE."'";
			$id_det=$this->save_all_table($table1,$columsinsert1,$datacolumsinsert1);
		}
		else
		{
			$MONTANT_RACCROCHE_LIQUIDATION = $this->request->getPost('MONTANT_RACCROCHE_LIQUIDATION');
			$SOMME_MONTANT_CREANCE = floatval($MONTANT_CREANCE)+$getLiquid['LIQUIDATION'];
			...
			//recuperation de l'engagement juridik et budgetaire
			$racc='SELECT ENG_BUDGETAIRE AS MONTANT_RACCROCHE, ENG_JURIDIQUE AS MONTANT_RACCROCHE_JURIDIQUE FROM execution_budgetaire WHERE EXECUTION_BUDGETAIRE_ID = '.$EXECUTION_BUDGETAIRE_ID;
			$getracc = "CALL `getTable`('" . $racc . "');";
			$get_racc= $this->ModelPs->getRequeteOne($getracc);
			###################################################################################

			$table1 = 'execution_budgetaire_tache_detail';
			$columsinsert1="EXECUTION_BUDGETAIRE_ID,MONTANT_CREANCE, MONTANT_LIQUIDATION, TITRE_CREANCE, DATE_CREANCE, DATE_LIQUIDATION, MOTIF_LIQUIDATION, ETAPE_DOUBLE_COMMANDE_ID,PATH_PV_RECEPTION_LIQUIDATION,PATH_FACTURE_LIQUIDATION,COUR_DEVISE,DATE_COUR_DEVISE,COMMENTAIRE";
			$datacolumsinsert1=$EXECUTION_BUDGETAIRE_ID.", ".$MONTANT_CREANCE.", ".$MONTANT_CREANCE.", '".$TITRE_CREANCE."', '".$DATE_CREANCE."', '".$DATE_LIQUIDATION."', '".$MOTIF_LIQUIDATION."', ".$NEXT_ETAPE_ID.",'".$PATH_PV_RECEPTION."','".$PATH_FACTURE."','".$get_donDet['COUR_DEVISE']."','null','".$COMMENTAIRE."'";
			$id_det=$this->save_all_table($table1,$columsinsert1,$datacolumsinsert1);
		}
		...
		$table2 = 'execution_budgetaire_tache_detail_histo';
		$columsinsert2="EXECUTION_BUDGETAIRE_DETAIL_ID, USER_ID,ETAPE_DOUBLE_COMMANDE_ID,OBSERVATION,DATE_TRANSMISSION, DATE_RECEPTION";
		$datacolumsinsert2=$id_det.", ".$USER_ID.", ".$ETAPE_ID.", '".$OBSERVATION."', '".$DATE_TRANSMISSION."', '".$DATE_RECEPTION."'";
		$this->save_all_table($table2,$columsinsert2,$datacolumsinsert2);
    ...
    if ($TYPE_MONTANT_ID!=1)
    {
    	...
    	$SOMME_MONTANT_DEVISE = $MONTANT_DEVISE+floatval($getLiquid['MONTANT_RACCROCHE_LIQUIDATION_DEVISE'])-floatval($MONTANT_DEVISE_LIQUIDATION);

    	$SOMME_LIQUIDATION = number_format($SOMME_MONTANT_DEVISE*$COUT_DEVISE,'0',',','');
    	...
    	$table1 = 'execution_budgetaire_tache_detail';
    	$where1='EXECUTION_BUDGETAIRE_DETAIL_ID='.$EXECUTION_BUDGETAIRE_DETAIL_ID;
    	$data1="MONTANT_LIQUIDATION_DEVISE=".$MONTANT_DEVISE.", MONTANT_LIQUIDATION='".$LIQUIDATION."', TITRE_CREANCE='".$TITRE_CREANCE."', DATE_CREANCE='".$DATE_CREANCE."', DATE_LIQUIDATION='".$DATE_LIQUIDATION."', MOTIF_LIQUIDATION='".$MOTIF_LIQUIDATION."', ETAPE_DOUBLE_COMMANDE_ID=".$NEXT_ETAPE_ID."";
    	$this->update_all_table($table1,$data1,$where1);
    }
    else
    {
    	...
    	$SOMME_MONTANT_CREANCE = $MONTANT_CREANCE+$getLiquid['LIQUIDATION']-floatval($MONTANT_RACCROCHE_LIQUIDATION);

    	$SOMME_LIQUIDATION = $SOMME_MONTANT_CREANCE;
    	// print_r($MONTANT_CREANCE.'+'.$getLiquid['LIQUIDATION'].'-'.floatval($MONTANT_RACCROCHE_LIQUIDATION).'='.$SOMME_LIQUIDATION);die();
	...
	// insert data validation 
	public function confirme_partielle($value='')
	{
		$session  = \Config\Services::session();
		if(empty($session->get('SESSION_SUIVIE_PTBA_USER_ID')))
		{
		...
		if($session->get('SESSION_SUIVIE_PTBA_CONFIRM_LIQUIDATION') !=1)
    {
      return redirect('Login_Ptba/homepage'); 
		...
		$EXECUTION_BUDGETAIRE_RACCROCHAGE_DETAIL_ID = $this->request->getPost('EXECUTION_BUDGETAIRE_RACCROCHAGE_DETAIL_ID');
    ...
    if($TYPE_OPERATION_ID == 1)
    {
    	$tableLiquidTermin = 'execution_budgetaire_raccrochage_activite_new';
			$whereLiquidTermin='EXECUTION_BUDGETAIRE_RACCROCHAGE_ID='.$EXECUTION_BUDGETAIRE_RACCROCHAGE_ID;
			$dataLiquidTermin='IS_FINISHED=0 ';
			$this->update_all_table($tableLiquidTermin,$dataLiquidTermin,$whereLiquidTermin);
      ...
      // insert dans la table historique_raccrochage_operation_verification_motif
			$MOTIF_REJET = $this->request->getPost('MOTIF_REJET[]');
			$table5 = 'historique_raccrochage_operation_verification_motif_new';
			foreach ($MOTIF_REJET as $key)
			{
				$columsinsert="TYPE_ANALYSE_MOTIF_ID, ETAPE_ID, EXECUTION_BUDGETAIRE_RACCROCHAGE_DETAIL_ID";
				$datacolumsinsert=$key.", ".$ETAPE_ID.", ".$EXECUTION_BUDGETAIRE_RACCROCHAGE_DETAIL_ID." ";
				$this->save_all_table($table5,$columsinsert,$datacolumsinsert);
			}

			//soustration dans la table execution_budgetaire_raccrochage_activite_new
			$racc  = 'SELECT MONTANT_RACCROCHE_LIQUIDATION_DEVISE, MONTANT_RACCROCHE_LIQUIDATION FROM execution_budgetaire_raccrochage_activite_new WHERE EXECUTION_BUDGETAIRE_RACCROCHAGE_ID = '.$EXECUTION_BUDGETAIRE_RACCROCHAGE_ID;
			$getracc = "CALL `getTable`('" . $racc . "');";
			$get_exc= $this->ModelPs->getRequeteOne($getracc);

			$detail  = 'SELECT MONTANT_CREANCE, MONTANT_RACCROCHE_LIQUIDATION, MONTANT_RACCROCHE_LIQUIDATION_DEVISE FROM execution_budgetaire_raccrochage_activite_detail WHERE EXECUTION_BUDGETAIRE_RACCROCHAGE_DETAIL_ID = '.$EXECUTION_BUDGETAIRE_RACCROCHAGE_DETAIL_ID;
			$getdetail = "CALL `getTable`('" . $detail . "');";
			$get_detail= $this->ModelPs->getRequeteOne($getdetail);

			//si type montant est ero, dollar...
			if ($TYPE_MONTANT_ID!=1)
			{
				$LIQUIDATION = $get_exc['MONTANT_RACCROCHE_LIQUIDATION_DEVISE']-$get_detail['MONTANT_RACCROCHE_LIQUIDATION_DEVISE'];

				$table = 'execution_budgetaire_raccrochage_activite_new';
				$where='EXECUTION_BUDGETAIRE_RACCROCHAGE_ID='.$EXECUTION_BUDGETAIRE_RACCROCHAGE_ID;
				$data='MONTANT_RACCROCHE_LIQUIDATION_DEVISE='.$LIQUIDATION;
				$this->update_all_table($table,$data,$where);
			}
			else
			{
				$LIQUIDATION = $get_exc['MONTANT_RACCROCHE_LIQUIDATION']-$get_detail['MONTANT_RACCROCHE_LIQUIDATION'];

				$table = 'execution_budgetaire_raccrochage_activite_new';
				$where='EXECUTION_BUDGETAIRE_RACCROCHAGE_ID='.$EXECUTION_BUDGETAIRE_RACCROCHAGE_ID;
				$data='MONTANT_RACCROCHE_LIQUIDATION='.$LIQUIDATION;
				$this->update_all_table($table,$data,$where);
			}
			########################################################################################

			// update dans la table execution_budgetaire pour mettre le montant de la liquidation
			$table3 = 'execution_budgetaire_new';
			$where3='EXECUTION_BUDGETAIRE_ID='.$EXECUTION_BUDGETAIRE_ID;
			$data3='LIQUIDATION="'.$LIQUIDATION.'" ';
			$this->update_all_table($table3,$data3,$where3);

      //mise a jour dans la table execution_budgetaire_brut
			$execution_budgtaire  = 'SELECT EXECUTION_BUDGETAIRE_BRUT_ID FROM execution_budgetaire_new WHERE EXECUTION_BUDGETAIRE_ID = '.$EXECUTION_BUDGETAIRE_ID;
			$getExecution_budgtaire = "CALL `getTable`('" . $execution_budgtaire . "');";
			$get_exc= $this->ModelPs->getRequeteOne($getExecution_budgtaire);

			$EXECUTION_BUDGETAIRE_BRUT_ID  = $get_exc['EXECUTION_BUDGETAIRE_BRUT_ID']; 
			$table4 = 'execution_budgetaire_brut_new';
			$where4='EXECUTION_BUDGETAIRE_BRUT_ID ='.$EXECUTION_BUDGETAIRE_BRUT_ID;
			$data4='LIQUIDATION="'.$LIQUIDATION.'" ';
			$this->update_all_table($table4,$data4,$where4);
        #################################################################################
    }
	    ...
	    {
	    	 $this->gestion_rejet_ptba($EXECUTION_BUDGETAIRE_RACCROCHAGE_DETAIL_ID);
	    	$tableLiquidTermin = 'execution_budgetaire_raccrochage_activite_new';
			$whereLiquidTermin='EXECUTION_BUDGETAIRE_RACCROCHAGE_ID='.$EXECUTION_BUDGETAIRE_RACCROCHAGE_ID;
			$dataLiquidTermin='IS_FINISHED=2 ';
			$this->update_all_table($tableLiquidTermin,$dataLiquidTermin,$whereLiquidTermin);
		...
		$table2 = 'historique_raccrochage_activite_detail';
		$columsinsert2="EXECUTION_BUDGETAIRE_RACCROCHAGE_DETAIL_ID, USER_ID, ETAPE_ID, OBSERVATION, TYPE_RACCROCHAGE_ID, DATE_TRANSMISSION, DATE_RECEPTION, TYPE_OPERATION_ID";
		$datacolumsinsert2=$EXECUTION_BUDGETAIRE_RACCROCHAGE_DETAIL_ID.", ".$USER_ID.", ".$ETAPE_ID.", '".$OBSERVATION."', ".$TYPE_RACCROCHAGE_ID.", '".$DATE_TRANSMISSION."', '".$DATE_RECEPTION."', ".$TYPE_OPERATION_ID;
		$this->save_all_table($table2,$columsinsert2,$datacolumsinsert2);
    	######################################################################################

	  	//mise à jour dans la table execution_budgetaire_raccrochage_activite
		$table1 = 'execution_budgetaire_raccrochage_activite_detail';
		$where1='EXECUTION_BUDGETAIRE_RACCROCHAGE_DETAIL_ID='.$EXECUTION_BUDGETAIRE_RACCROCHAGE_DETAIL_ID;
		$data1="ETAPE_DOUBLE_COMMANDE_ID=".$NEXT_ETAPE_ID.", MOUVEMENT_DEPENSE_ID=".$MOUVEMENT_NEXT_ID." ";
		$this->update_all_table($table1,$data1,$where1);
		###############################################################################
		...
		if ($TYPE_OPERATION_ID == 2)
		{
	        ...
	        //liquidation
	        $idmd5 = md5($EXECUTION_BUDGETAIRE_RACCROCHAGE_ID);
	 		$detail=$this->detail_new(md5($EXECUTION_BUDGETAIRE_RACCROCHAGE_DETAIL_ID));

			$get_info = $detail['get_info'];
			$montantvote = $detail['montantvote'];
			$MontantRestant = $detail['MontantRestant'];
			$creditVote = $detail['creditVote'];
			$creditTransfert = $detail['creditTransfert'];
			$montant_reserve = $detail['montant_reserve'];
			...
			if (!empty($get_info['IMPUTATION']))
			{
				$html.="<p>".lang('messages_lang.labelle_ligne_budgtaire')." : <strong>".$get_info['IMPUTATION']."</strong></p>";
			}

			if (!empty($get_info['CODE_MINISTERE']))
			{
				$html.="<p>".lang('messages_lang.labelle_institution')." : <strong>".$get_info['CODE_MINISTERE'].'&nbsp;&nbsp;'.$get_info['INTITULE_MINISTERE']."</strong></p>";
			}
	        ...
	        if(!empty($get_info['CODES_PROGRAMMATIQUE']))
	        {
	        	$html.='<p>'.lang('messages_lang.labelle_code_programme').' : <strong>'.$get_info['CODES_PROGRAMMATIQUE'].'</strong></p>';
	        }

	        if (!empty($get_info['ACTIVITES']))
	        {
	        	$html.='<p>Activité : <strong>'.$get_info['ACTIVITES'].'</strong></p>';
	        }
			...
			if (!empty($get_info['MONTANT_RACCROCHE']))
	        {
	        	$html.='<p>'.lang('messages_lang.labelle_montant_engagement_budgetaire').' : <strong>'.number_format($get_info['MONTANT_RACCROCHE'],'0',',',' ').'</strong></p>';
	        }
	        ...
	        if (!empty($get_info['DATE_ENG_BUDGETAIRE']))
	        {
	        	$html.='<p>'.lang('messages_lang.table_date_engaget').' : <strong>'.$retVal = (!empty($get_info['DATE_ENG_BUDGETAIRE'])) ? date('d/m/Y',strtotime($get_info['DATE_ENG_BUDGETAIRE'])) : 'N/A'.'</strong></p>';
	        }

	        //récuperer les fournisseurs/acquéreurs
	        $prestataire  = 'SELECT NOM_PRESTATAIRE,PRENOM_PRESTATAIRE,DESC_TYPE_BENEFICIAIRE FROM prestataire JOIN execution_budgetaire_raccrochage_activite_info_suppl_new ON prestataire.PRESTATAIRE_ID=execution_budgetaire_raccrochage_activite_info_suppl_new.PRESTATAIRE_ID JOIN type_beneficiaire ON type_beneficiaire.TYPE_BENEFICIAIRE_ID=prestataire.TYPE_BENEFICIAIRE_ID WHERE md5(EXECUTION_BUDGETAIRE_RACCROCHAGE_ID)="'.$idmd5.'" ';
			$prestataire = "CALL `getTable`('" . $prestataire . "');";
			$prestataire = $this->ModelPs->getRequeteOne($prestataire);
	        ...
	        if (!empty($get_info['MONTANT_RACCROCHE_JURIDIQUE']))
	        {
	        	$html.='<p>'.lang('messages_lang.label_mont_juridique').' : <strong>'.number_format($get_info['MONTANT_RACCROCHE_JURIDIQUE'],'0',',',' ').'</strong></p>';
	        }
	        ...
	        //type_liquidation
			$type_liquidation  = 'SELECT ID_TYPE_LIQUIDATION,DESCRIPTION_LIQUIDATION FROM type_liquidation WHERE ID_TYPE_LIQUIDATION='.$get_info['ID_TYPE_LIQUIDATION'].'';
			$type_liquidationRqt = "CALL `getTable`('" . $type_liquidation . "');";
			$get_type_liquidation = $this->ModelPs->getRequeteOne($type_liquidationRqt);
			...
			{	
				if(!empty($get_info['MONTANT_DEVISE_LIQUIDATION']))
		       	{
		        	$html.='<p>'.lang('messages_lang.label_montant_devise').' : <strong>'.number_format($get_info['MONTANT_DEVISE_LIQUIDATION'],'0',',',' ').'</strong></p>';
		       	}

				if(!empty($get_info['COUR_DEVISE_LIQUIDATION']))
		       	{
		        	$html.='<p>'.lang('messages_lang.label_cours_devise').' : <strong>'.number_format($get_info['COUR_DEVISE_LIQUIDATION'],'0',',',' ').'</strong></p>';
		       	}
	       	...
	       	$html.='<br><br><p></strong>'.session()->get("SESSION_SUIVIE_PTBA_PROFIL_DESCRIPTION").' : <strong>'.session()->get("SESSION_SUIVIE_PTBA_NOM").' '.session()->get("SESSION_SUIVIE_PTBA_PRENOM").'</strong></p>';
	...
	function listing_liquid_partiel()
	{
		...
		$requetedebase= "SELECT exec.EXECUTION_BUDGETAIRE_ID,exec.NUMERO_BON_ENGAGEMENT,exec.COMMENTAIRE,exec.ENG_BUDGETAIRE,exec.ENG_BUDGETAIRE_DEVISE,ligne.CODE_NOMENCLATURE_BUDGETAIRE,ligne.LIBELLE_CODE_NOMENCLATURE_BUDGETAIRE,act.DESC_PAP_ACTIVITE,tache.DESC_TACHE,exec.ENG_JURIDIQUE,exec.ENG_JURIDIQUE_DEVISE,exec.LIQUIDATION,exec.LIQUIDATION_DEVISE,exec.DEVISE_TYPE_ID FROM execution_budgetaire exec JOIN ptba_tache tache ON tache.PTBA_TACHE_ID=exec.PTBA_TACHE_ID LEFT JOIN pap_activites act ON act.PAP_ACTIVITE_ID=exec.PAP_ACTIVITE_ID JOIN inst_institutions_ligne_budgetaire ligne ON ligne.CODE_NOMENCLATURE_BUDGETAIRE_ID=exec.CODE_NOMENCLATURE_BUDGETAIRE_ID WHERE exec.LIQUIDATION_TYPE_ID=1 AND IS_FINISHED !=1 ".$institution." ";
		...
		foreach ($fetch_data as $row)
		{
		    ...
		    if ($row->DEVISE_TYPE_ID==1)
		    {
		    	$ENG_BUDGETAIRE=number_format($row->ENG_BUDGETAIRE,'2',',',' ');
		    	$ENG_JURIDIQUE=number_format($row->ENG_JURIDIQUE,'2',',',' ');
		    	$MONTANT_LIQUIDATION=number_format($row->LIQUIDATION,'2',',',' ');	
		    }
		    else
		    {
		    	$ENG_BUDGETAIRE=number_format($row->ENG_BUDGETAIRE_DEVISE,'2',',',' ');
		    	$ENG_JURIDIQUE=number_format($row->ENG_JURIDIQUE_DEVISE,'2',',',' ');
		    	$MONTANT_LIQUIDATION=number_format($row->LIQUIDATION_DEVISE,'2',',',' ');
		    }
