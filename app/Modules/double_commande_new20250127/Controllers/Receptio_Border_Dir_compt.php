...
class Receptio_Border_Dir_compt extends BaseController
{
  ...
  function liste_titre_decaissement($id=0)
  {
    ...
    $query_principal="SELECT titre.BORDEREAU_TRANSMISSION_BON_TITRE_ID, titre.NUMERO_DOCUMENT, det.MONTANT_ORDONNANCEMENT,stat.DESC_STATUT_DOCUMENT_BORDEREAU_TRANSMISSION FROM execution_budgetaire_bordereau_transmission_bon_titre titre JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_DETAIL_ID=titre.EXECUTION_BUDGETAIRE_DETAIL_ID JOIN statut_document_bordereau_transmission stat ON stat.STATUT_DOCUMENT_BORDEREAU_TRANSMISSION_ID=titre.STATUT_DOCUMENT_BORDEREAU_TRANSMISSION_ID WHERE titre.BORDEREAU_TRANSMISSION_ID = ".$id." AND det.ETAPE_DOUBLE_COMMANDE_ID=22 AND TYPE_DOCUMENT_ID=2 AND titre.STATUT_DOCUMENT_BORDEREAU_TRANSMISSION_ID=1";
    ...
    $order_by='';
    $order_column='';
    $order_column= array('titre.NUMERO_DOCUMENT','det.MONTANT_ORDONNANCEMENT','stat.DESC_STATUT_DOCUMENT_BORDEREAU_TRANSMISSION');
