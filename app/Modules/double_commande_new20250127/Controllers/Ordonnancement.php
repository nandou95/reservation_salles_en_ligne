...
class Ordonnancement extends BaseController
{
  ...
  function save_newMotif()
  {
    ...
    $DESCRIPTION_MOTIF=addslashes($DESCRIPTION_MOTIF);
    $MARCHE_PUBLIQUE = 0;
  ...
  function vue_ordonnance_ministre($EXEC_BUDGET_DET_ID=0)
  {
    $session  = \Config\Services::session();
    $user_id ='';
    ...
    ################################ DETAILS D'INFOS ############################
    $getdemanande = "SELECT execdet.EXECUTION_BUDGETAIRE_DETAIL_ID,exec.EXECUTION_BUDGETAIRE_ID,exec.SOUS_TUTEL_ID, exec.NUMERO_BON_ENGAGEMENT,exec.DATE_BON_ENGAGEMENT,exec.ENG_BUDGETAIRE,exec.ENG_JURIDIQUE,execdet.MONTANT_LIQUIDATION,exec.ENG_BUDGETAIRE_DEVISE,exec.ENG_JURIDIQUE_DEVISE,execdet.MONTANT_LIQUIDATION_DEVISE,inst.DESCRIPTION_INSTITUTION,inst.CODE_INSTITUTION,progr.INTITULE_PROGRAMME,actions.CODE_ACTION,progr.CODE_PROGRAMME,actions.LIBELLE_ACTION,act.DESC_PAP_ACTIVITE,ligne.CODE_NOMENCLATURE_BUDGETAIRE,exec.DATE_DEMANDE,exec.DATE_DEMANDE,execdet.DATE_LIQUIDATION,execdet.ETAPE_DOUBLE_COMMANDE_ID,exec.INSTITUTION_ID,exec.SOUS_TUTEL_ID,exec.DEVISE_TYPE_ID,exec.LIQUIDATION_TYPE_ID FROM execution_budgetaire exec JOIN execution_budgetaire_tache_detail execdet ON exec.EXECUTION_BUDGETAIRE_ID=execdet.EXECUTION_BUDGETAIRE_ID LEFT JOIN pap_activites act ON act.PAP_ACTIVITE_ID=exec.PAP_ACTIVITE_ID JOIN inst_institutions_programmes progr ON progr.PROGRAMME_ID=exec.PROGRAMME_ID JOIN inst_institutions inst ON inst.INSTITUTION_ID=exec.INSTITUTION_ID JOIN inst_institutions_actions actions ON actions.ACTION_ID=exec.ACTION_ID JOIN inst_institutions_ligne_budgetaire ligne ON ligne.CODE_NOMENCLATURE_BUDGETAIRE_ID=exec.CODE_NOMENCLATURE_BUDGETAIRE_ID JOIN execution_budgetaire_tache_info_suppl info ON exec.EXECUTION_BUDGETAIRE_ID=info.EXECUTION_BUDGETAIRE_ID WHERE 1 AND MD5(execdet.EXECUTION_BUDGETAIRE_DETAIL_ID)='".$EXEC_BUDGET_DET_ID."'";
    ...
    if (!empty($getProfil))
    {
      foreach ($getProfil as $value)
      {
        ...
        if ($prof_id == $value->PROFIL_ID || $prof_id==1)
        {
          ...
          $bind_date_histo = $this->getBindParms('EXECUTION_BUDGETAIRE_DETAIL_ID,DATE_TRANSMISSION,DATE_INSERTION','execution_budgetaire_tache_detail_histo','MD5(EXECUTION_BUDGETAIRE_DETAIL_ID)="'.$EXEC_BUDGET_DET_ID.'"','DATE_INSERTION DESC');
          ...
          $detail=$this->detail_new($EXEC_BUDGET_DET_ID);
  ...
  function vue_ordonnance_dgbudget($EXEC_BUDGET_DET_ID=0)
  {
    $session  = \Config\Services::session();
    $user_id ='';
    if(!empty($session->get('SESSION_SUIVIE_PTBA_USER_ID')))
    ...
    ################################ DETAILS D'INFOS ############################
    $getdemanande = "SELECT execdet.EXECUTION_BUDGETAIRE_DETAIL_ID,exec.EXECUTION_BUDGETAIRE_ID,exec.SOUS_TUTEL_ID, exec.NUMERO_BON_ENGAGEMENT,exec.DATE_BON_ENGAGEMENT,exec.ENG_BUDGETAIRE,exec.ENG_JURIDIQUE,execdet.MONTANT_LIQUIDATION,inst.DESCRIPTION_INSTITUTION,inst.CODE_INSTITUTION,progr.INTITULE_PROGRAMME,actions.CODE_ACTION,progr.CODE_PROGRAMME,actions.LIBELLE_ACTION, act.DESC_PAP_ACTIVITE,ligne.CODE_NOMENCLATURE_BUDGETAIRE,exec.DATE_DEMANDE,exec.DATE_DEMANDE AS DATE_ENGAGEMENT_BUDGETAIRE,execdet.DATE_LIQUIDATION,execdet.ETAPE_DOUBLE_COMMANDE_ID,exec.ENG_BUDGETAIRE_DEVISE,exec.INSTITUTION_ID,exec.SOUS_TUTEL_ID,exec.ENG_JURIDIQUE_DEVISE,execdet.MONTANT_LIQUIDATION_DEVISE,exec.LIQUIDATION_TYPE_ID ,exec.DEVISE_TYPE_ID FROM execution_budgetaire exec JOIN execution_budgetaire_tache_detail execdet ON exec.EXECUTION_BUDGETAIRE_ID=execdet.EXECUTION_BUDGETAIRE_ID LEFT JOIN pap_activites act ON act.PAP_ACTIVITE_ID=exec.PAP_ACTIVITE_ID JOIN inst_institutions_programmes progr ON progr.PROGRAMME_ID=exec.PROGRAMME_ID JOIN inst_institutions inst ON inst.INSTITUTION_ID=exec.INSTITUTION_ID JOIN inst_institutions_actions actions ON actions.ACTION_ID=exec.ACTION_ID JOIN inst_institutions_ligne_budgetaire ligne ON ligne.CODE_NOMENCLATURE_BUDGETAIRE_ID=exec.CODE_NOMENCLATURE_BUDGETAIRE_ID JOIN execution_budgetaire_tache_info_suppl info ON exec.EXECUTION_BUDGETAIRE_ID=info.EXECUTION_BUDGETAIRE_ID WHERE 1 AND MD5(execdet.EXECUTION_BUDGETAIRE_DETAIL_ID)='".$EXEC_BUDGET_DET_ID."'";
    ...
    if($this->validation->withRequest($this->request)->run())
    {
      if($OPERATION_ID == 1)
      {
        ...
        if($ETAPE_RETOUR_CORRECTION_ID == 2)
        {
          ...
          $this->gestion_rejet_ptba($EXEC_BUDGET_ID);
        }
        ...
        foreach($TYPE_ANALYSE_MOTIF_ID as $value)
        {
          ...
          $datatoinsert_histo_motif = "".$value.",".$ETAPE_ID.",".$EXEC_BUDGET_DET_ID."";
        ...
        if($TYPE_MONNAIE != 1)
        {
          ...
          $datatomodifie4= 'MONTANT_ORDONNANCEMENT="'.$MONTANT_EN_BIF.'",DATE_ORDONNANCEMENT="'.$DATE_HEURE_ORDONNANCE.'", MONTANT_ORDONNANCEMENT_DEVISE='.$MONTANT_EN_DEVISE.', ETAPE_DOUBLE_COMMANDE_ID='.$NEXT_ETAPE_ID;
        ...
        else
        {
          ...
          $table4='execution_budgetaire_tache_detail';
          $conditions4='EXECUTION_BUDGETAIRE_DETAIL_ID ='.$EXEC_BUDGET_DET_ID;
          $datatomodifie4= 'MONTANT_ORDONNANCEMENT="'.$MONTANT_EN_BIF.'",DATE_ORDONNANCEMENT="'.$DATE_HEURE_ORDONNANCE.'",ETAPE_DOUBLE_COMMANDE_ID='.$NEXT_ETAPE_ID;
          $this->update_all_table($table4,$datatomodifie4,$conditions4);
      ...
      $datatoinsert_histo="".$EXEC_BUDGET_DET_ID.",".$USER_ID_ENG.",".$ETAPE_ID.",'".$DATE_RECEPTION."','".$DATE_TRANSMISSION."','".str_replace("'", "\'", $COMMENTAIRE)."'";
  ...
  function update_etapeDG()
  {
    ...
    if($this->validation->withRequest($this->request)->run())
    {
      ...
      $data = $this->urichk();
      ################################ DETAILS D'INFOS ############################
      $getdemanande = "SELECT execdet.EXECUTION_BUDGETAIRE_DETAIL_ID,exec.EXECUTION_BUDGETAIRE_ID,exec.SOUS_TUTEL_ID, exec.NUMERO_BON_ENGAGEMENT,exec.DATE_BON_ENGAGEMENT,exec.ENG_BUDGETAIRE,exec.ENG_JURIDIQUE,execdet.MONTANT_LIQUIDATION,inst.DESCRIPTION_INSTITUTION,inst.CODE_INSTITUTION,progr.INTITULE_PROGRAMME,actions.CODE_ACTION,progr.CODE_PROGRAMME,actions.LIBELLE_ACTION, act.DESC_PAP_ACTIVITE,ligne.CODE_NOMENCLATURE_BUDGETAIRE,exec.DATE_DEMANDE,exec.DATE_DEMANDE AS DATE_ENGAGEMENT_BUDGETAIRE,execdet.DATE_LIQUIDATION,execdet.ETAPE_DOUBLE_COMMANDE_ID,exec.ENG_BUDGETAIRE_DEVISE,exec.INSTITUTION_ID,exec.SOUS_TUTEL_ID,exec.ENG_JURIDIQUE_DEVISE,execdet.MONTANT_LIQUIDATION_DEVISE,exec.LIQUIDATION_TYPE_ID ,exec.DEVISE_TYPE_ID FROM execution_budgetaire exec JOIN execution_budgetaire_tache_detail execdet ON exec.EXECUTION_BUDGETAIRE_ID=execdet.EXECUTION_BUDGETAIRE_ID LEFT JOIN pap_activites act ON act.PAP_ACTIVITE_ID=exec.PAP_ACTIVITE_ID JOIN inst_institutions_programmes progr ON progr.PROGRAMME_ID=exec.PROGRAMME_ID JOIN inst_institutions inst ON inst.INSTITUTION_ID=exec.INSTITUTION_ID JOIN inst_institutions_actions actions ON actions.ACTION_ID=exec.ACTION_ID JOIN inst_institutions_ligne_budgetaire ligne ON ligne.CODE_NOMENCLATURE_BUDGETAIRE_ID=exec.CODE_NOMENCLATURE_BUDGETAIRE_ID JOIN execution_budgetaire_tache_info_suppl info ON exec.EXECUTION_BUDGETAIRE_ID=info.EXECUTION_BUDGETAIRE_ID WHERE 1 AND execdet.EXECUTION_BUDGETAIRE_DETAIL_ID=".$EXEC_BUDGET_DET_ID;
      ...
      if (!empty($getProfil))
      {
        foreach ($getProfil as $value)
        {
          if ($prof_id == $value->PROFIL_ID || $prof_id==1)
          {
            ...
            //Le min de la date de réception
            $bind_date_histo = $this->getBindParms('EXECUTION_BUDGETAIRE_DETAIL_ID,DATE_TRANSMISSION,DATE_INSERTION','execution_budgetaire_tache_detail_histo','EXECUTION_BUDGETAIRE_DETAIL_ID='.$EXEC_BUDGET_DET_ID,'DATE_INSERTION DESC');
            $bind_date_histo = str_replace('\\','',$bind_date_histo);
            $data['date_trans'] = $this->ModelPs->getRequeteOne($psgetrequete, $bind_date_histo);
            ...
            $EXEC_BUDG_RAC_ID = md5($data['EXEC_BUDGET_DET_ID']);
            $detail=$this->detail_new($EXEC_BUDG_RAC_ID);
