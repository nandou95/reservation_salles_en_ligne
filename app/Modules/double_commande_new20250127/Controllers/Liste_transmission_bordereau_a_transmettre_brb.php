	...
	*/
		...
		if($session->get('SESSION_SUIVIE_PTBA_TRANSMISSION_BRB') !=1)
    {
      return redirect('Login_Ptba/homepage'); 
		...
		$order_by = "";
		$order_column = array('NUMERO_TITRE_DECAISSEMNT','MONTANT_ORDONNANCEMENT','DESCRIPTION_INSTITUTION',1);
		$search = !empty($_POST['search']['value']) ? (' AND (NUMERO_TITRE_DECAISSEMNT LIKE "%' . $var_search . '%" OR det.MONTANT_ORDONNANCEMENT LIKE "%' . $var_search . '%" OR inst.DESCRIPTION_INSTITUTION LIKE "%' . $var_search . '%")') : '';	
		
		$query_principal="SELECT det.EXECUTION_BUDGETAIRE_DETAIL_ID,det.NUMERO_TITRE_DECAISSEMNT,dev.DEVISE_TYPE_ID,dev.DESC_DEVISE_TYPE,det.MONTANT_ORDONNANCEMENT,det.MONTANT_ORDONNANCEMENT_DEVISE,inst.DESCRIPTION_INSTITUTION FROM execution_budgetaire_tache_detail det JOIN execution_budgetaire exec ON exec.EXECUTION_BUDGETAIRE_ID=det.EXECUTION_BUDGETAIRE_ID JOIN inst_institutions inst ON inst.INSTITUTION_ID=exec.INSTITUTION_ID JOIN devise_type dev ON dev.DEVISE_TYPE_ID=exec.DEVISE_TYPE_ID WHERE det.ETAPE_DOUBLE_COMMANDE_ID=27";
