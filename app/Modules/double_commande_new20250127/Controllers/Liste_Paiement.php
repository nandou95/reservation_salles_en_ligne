...
class Liste_Paiement extends BaseController
{
  ...
  function index()
  {
    ...
    return view('App\Modules\double_commande_new\Views\Liste_Paiement_View', $data);
  }
  ...
  public function listing()
  {
    ...
    if (!empty($ETAPE_DOUBLE_COMMANDE_ID)) {
      $crit_etap = " AND exec_detail.ETAPE_DOUBLE_COMMANDE_ID=" . $ETAPE_DOUBLE_COMMANDE_ID;
    } else {
      $crit_etap = "AND exec_detail.ETAPE_DOUBLE_COMMANDE_ID IN (SELECT DISTINCT ETAPE_DOUBLE_COMMANDE_ID FROM execution_budgetaire_etape_double_commande_profil WHERE PROFIL_ID=" . $profil_id . ")";
      if ($profil_id==1)
      {
        $crit_etap = "";
      }
    ...
    $order_column = array('NUMERO_BON_ENGAGEMENT', 'NUMERO_TITRE_DECAISSEMNT', 'pap_activites.DESC_PAP_ACTIVITE', 'ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE', 'exec.ENG_BUDGETAIRE', 1, 'exec.ENG_JURIDIQUE', 1, 'exec_detail.MONTANT_LIQUIDATION', 1, 'exec_detail.MONTANT_ORDONNANCEMENT', 1, 1);
    ...
    $search = !empty($_POST['search']['value']) ?  (" AND (exec.NUMERO_BON_ENGAGEMENT LIKE '%$var_search%' OR pap_activites.DESC_PAP_ACTIVITE LIKE '%$var_search%' OR exec_detail.NUMERO_TITRE_DECAISSEMNT LIKE '%$var_search%' OR exec.ENG_BUDGETAIRE LIKE '%$var_search%')") : '';
    ...
    $requetedebase = "SELECT exec_detail.EXECUTION_BUDGETAIRE_DETAIL_ID,pap_activites.DESC_PAP_ACTIVITE,exec.ENG_BUDGETAIRE,exec.ENG_JURIDIQUE,exec_detail.MONTANT_LIQUIDATION,exec_detail.MONTANT_ORDONNANCEMENT,ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE,exec.COMMENTAIRE,exec_detail.ETAPE_DOUBLE_COMMANDE_ID,exec.NUMERO_BON_ENGAGEMENT,exec_detail.NUMERO_TITRE_DECAISSEMNT,dc.DESC_ETAPE_DOUBLE_COMMANDE,dc.ETAPE_DOUBLE_COMMANDE_ID FROM execution_budgetaire_tache_detail exec_detail JOIN execution_budgetaire exec ON exec.EXECUTION_BUDGETAIRE_ID=exec_detail.EXECUTION_BUDGETAIRE_ID LEFT JOIN pap_activites ON pap_activites.PAP_ACTIVITE_ID = exec.PAP_ACTIVITE_ID JOIN inst_institutions_ligne_budgetaire ligne_budg ON ligne_budg.CODE_NOMENCLATURE_BUDGETAIRE_ID=exec.CODE_NOMENCLATURE_BUDGETAIRE_ID JOIN execution_budgetaire_etape_double_commande dc ON dc.ETAPE_DOUBLE_COMMANDE_ID = exec_detail.ETAPE_DOUBLE_COMMANDE_ID JOIN execution_budgetaire_etape_double_commande_profil etp_prof ON dc.ETAPE_DOUBLE_COMMANDE_ID=etp_prof.ETAPE_DOUBLE_COMMANDE_ID WHERE dc.MOUVEMENT_DEPENSE_ID = 5 AND etp_prof.PROFIL_ID=".$profil_id." AND exec_detail.ETAPE_DOUBLE_COMMANDE_ID NOT IN(17,21,22,26,27)";
      ...
      if (!empty($getProfil))
      {
        foreach ($getProfil as $value)
        {
          if ($prof_id == $value->PROFIL_ID || $prof_id==1)
          {
            ...
            $bouton = "<a class='btn btn-primary btn-sm'  title='' href='" . base_url('double_commande_new/'. $step . "/" . md5($row->EXECUTION_BUDGETAIRE_DETAIL_ID)) . "' ><span class='fa fa-edit'></span></a>";
          }
        }
      }
