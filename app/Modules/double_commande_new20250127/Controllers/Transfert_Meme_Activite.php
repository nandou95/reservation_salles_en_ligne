...
class Transfert_Meme_Activite extends BaseController
{
  ...
  public function add()
  {
    ...
    $gettranches='SELECT DESC_TRIMESTRE,TRIMESTRE_ID  FROM trimestre WHERE 1 AND TRIMESTRE_ID!=5';
    $data['tranches'] = $this->ModelPs->getRequete("CALL `getTable`('" . $gettranches . "')");

    $dataa=$this->converdate();
    $TRANCHE_ID=$dataa['TRIMESTRE_ID'];
    //$gettrimdest='SELECT DESC_TRIMESTRE,TRIMESTRE_ID  FROM trimestre WHERE 1 AND TRIMESTRE_ID!=5 AND TRIMESTRE_ID>='.$TRANCHE_ID;
    $gettrimdest='SELECT DESC_TRIMESTRE,TRIMESTRE_ID  FROM trimestre WHERE 1 AND TRIMESTRE_ID!=5';
    $data['gettrimdest'] = $this->ModelPs->getRequete("CALL `getTable`('".$gettrimdest."')");
  ...
  //montant par trim destination
  public function getMontantDest($value='')
  {
    $session  = \Config\Services::session();
    if($session->get('SESSION_SUIVIE_PTBA_DOUBLE_COMMANDE_TRANSFERT') !=1)
    {
    ...
    $TRIMESTRE_ID = $this->request->getPost('TRIMESTRE_ID_DESTINATION');
    ...
    $output = array(
      "MONTANT_VOTE" => $MONTANT_VOTE,"MONTANT_RESTANT"=>$MONTANT_RESTANT
    );
    return $this->response->setJSON($output);
  }
  ...
  public function getMontantAnnuel($value='')
  {
    ...
    $dataa=$this->converdate();
    $TRANCHE_ID=$dataa['TRIMESTRE_ID'];
    //$gettrimdest='SELECT DESC_TRIMESTRE,TRIMESTRE_ID  FROM trimestre WHERE 1 AND TRIMESTRE_ID NOT IN (5,'.$TRIMESTRE_ID.') AND TRIMESTRE_ID>='.$TRANCHE_ID;
    $gettrimdest='SELECT DESC_TRIMESTRE,TRIMESTRE_ID  FROM trimestre WHERE 1 AND TRIMESTRE_ID NOT IN (5,'.$TRIMESTRE_ID.')';
    $gettrimdest = $this->ModelPs->getRequete("CALL `getTable`('".$gettrimdest."')");

    $html = '<option value="">Sélectionner</option>';
    foreach ($gettrimdest as $key)
    {
      $html .= '<option value="'.$key->TRIMESTRE_ID.'">'.$key->DESC_TRIMESTRE.'</option>';
    }

    $output = array(
      "MONTANT_TRIMESTRE_SELECTION" => $MONTANT_TRIMESTRE_SELECTION,"html"=>$html
    );
    return $this->response->setJSON($output);
  }
  ...
  public function getInfoDetail($value='')
  {
    ...
    $sql_tranche_dest='SELECT DESC_TRIMESTRE FROM trimestre WHERE 1 AND TRIMESTRE_ID='.$TRIMESTRE_ID_DESTINATION.'';
    $tranches_dest = $this->ModelPs->getRequeteOne("CALL `getTable`('" . $sql_tranche_dest . "')");
    ...
    <td style="width:350px ;"><font style="float:left;"><i class="fa fa-credit-card"></i>&nbsp;'.lang('messages_lang.label_term_dest').'</font></td>
    <td><strong><font style="float:left;">'.$tranches_dest['DESC_TRIMESTRE'].'</font></strong></td>
    </tr>
  ...
  public function send_data($value='')
  {
    ...
    if($this->validation->withRequest($this->request)->run())
    {
      ...
      $datacolumsinsert=$TYPE_OPERATION_ID.",".$USER_ID.",".$MONTANT_TRANSFERT.",".$PTBA_TACHE_ID.",".$MONTANT_TRANSFERT.",".$PTBA_TACHE_ID.",".$TRIMESTRE_ID.",".$TRIMESTRE_ID_DESTINATION.",'".$AUTORISATION_TRANSFERT."'";
      ...
      else if ($TRIMESTRE_ID_DESTINATION==2)
      {
        //cas receptrice
      ...
      else if ($TRIMESTRE_ID_DESTINATION==3)
      {
        //cas receptrice
      ...
      else if ($TRIMESTRE_ID_DESTINATION==4)
      {
        //cas receptrice
