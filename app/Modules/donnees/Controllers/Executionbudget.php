...
class Executionbudget extends BaseController
{
  protected $session;
  protected $ModelPs;
  ...
  public function index()
  {
    ...
    $data=$this->urichk();
    return view('App\Modules\donnees\Views\Executionbudget_view',$data);
  }
  ...
  public function importfile()
  {
    ...
    $bind_parms=$this->getBindParms('EXERCICE_ID' ,'op_exercice','ANNEE_DEBUT<='.$annee.' AND ANNEE_FIN>='.$annee,'EXERCICE_ID ASC');
    $resultat=$this->ModelPs->getRequeteOne($callpsreq, $bind_parms);
    $EXERCICE_ID=$resultat['EXERCICE_ID'];
      ...
      { 
        ...
        if($IMPUTATION_NUMBER==26)
        {
          ...
          if(!empty($resultatinst) && !empty($LIBELLE))
          {
            ...
            $columsinsertbrut="IMPUTATION,LIBELLE,CREDIT_VOTE,TRANSFERTS_CREDITS,CREDIT_APRES_TRANSFERT,ENG_BUDGETAIRE,ENG_JURIDIQUE,LIQUIDATION,ORDONNANCEMENT,PAIEMENT,DECAISSEMENT,EXERCICE_ID,DATE_DEMANDE,IS_TRANSFERTS,INSTITUTION_ID,SOUS_TUTEL_ID";
            $valuecolumsinsertbrut="'".$IMPUTATION."','".$LIBELLE."',".$CREDIT_VOTE.",".$TRANSFERTS_CREDITS.",".$CREDIT_APRES_TRANSFERT.",".$ENG_BUDGETAIRE.",".$ENG_JURIDIQUE.",".$LIQUIDATION.",".$ORDONNANCEMENT.",".$PAIEMENT.",".$DECAISSEMENT.",".$EXERCICE_ID.",'".$DATE_DEMANDE."',".$IS_TRANSFERTS.",".$INSTITUTION_ID.",".$SOUS_TUTEL_ID;
            ...
            if(!empty($EXECUTION_BUDGETAIRE_BRUT_ID))
            {
              if($EXECUTION_BUDGETAIRE_BRUT_ID>0)
              {
                ...
                $columsinserte="EXECUTION_BUDGETAIRE_BRUT_ID,IMPUTATION,LIBELLE,CREDIT_VOTE,TRANSFERTS_CREDITS,CREDIT_APRES_TRANSFERT,ENG_BUDGETAIRE,ENG_JURIDIQUE,LIQUIDATION,ORDONNANCEMENT,PAIEMENT,DECAISSEMENT,EXERCICE_ID,DATE_DEMANDE,IS_TRANSFERTS,INSTITUTION_ID,SOUS_TUTEL_ID";
                $valuecolumsinserte=$EXECUTION_BUDGETAIRE_BRUT_ID.",'".$IMPUTATION."','".$LIBELLE."',".$CREDIT_VOTE.",".$TRANSFERTS_CREDITS.",".$CREDIT_APRES_TRANSFERT.",".$ENG_BUDGETAIRE.",".$ENG_JURIDIQUE.",".$LIQUIDATION.",".$ORDONNANCEMENT.",".$PAIEMENT.",".$DECAISSEMENT.",".$EXERCICE_ID.",'".$DATE_DEMANDE."',".$IS_TRANSFERTS.",".$INSTITUTION_ID.",".$SOUS_TUTEL_ID;
    ...
    return redirect('donnees/Executionbudget');
  }
}
?>