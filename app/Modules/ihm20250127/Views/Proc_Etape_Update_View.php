                ...
                <div class="card-header">
                  <div class="col-md-12 d-flex">
                    <div class="col-md-9" style="float: left;">
                      <h1 class="header-title text-black"><?= lang('messages_lang.labelle_et_mod')?></h1></div>
                    ...
                    <form action="<?=base_url('ihm/Proc_Etape/update')?>" id="Myform" method="POST" enctype="multipart/form-data">
                      <div class="card-body">
                        <input type="hidden" name="ETAPE_ID" value="<?=$etape['ETAPE_ID']?>">
                        <div class="row">
                            <div class="col-md-6">
                                <label for=""><?= lang('messages_lang.labelle_et_etapes')?><span style="color: red;">*</span></label>
                                <input type="text" class="form-control" id="ETAPE" name="ETAPE"  value="<?=$etape['DESCR_ETAPE']?>">
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-control" id="PROCESS_ID" name="PROCESS_ID" value="<?=set_value('PROCESS_ID')?>">
                              ...
                              { 
                                if($value->PROCESS_ID==$etape['PROCESS_ID']){?>
                          ...
                          <div class="col-md-6">
                                ...
                                <select class="form-control select2 PROFIL_ID" name="PROFIL_ID[]"           id="inscompSelected" multiple>
                                  ...
                                  <?php foreach($profil as $p): ?>
                                    <option value="<?= $p->PROFIL_ID ?>"
                                      <?php if(!empty($profil_etape)): ?>
                                        <?php foreach($profil_etape as $pe): ?>
                                          <?= ($pe->PROFIL_ID==$p->PROFIL_ID) ? 'selected' : '' ?>
                                        <?php endforeach; ?>
                                      <?php endif; ?>
                                    ><?= $p->PROFIL_DESCR ?></option>
                                  <?php endforeach; ?>
                                </select>
                                <?php if (isset($validation)) : ?>
                            ...
                            <div class="col-md-4" id="SAVE">
                              <button type="button"  style="float:right;" class="btn btn-primary float-end envoi" id="btnSave"  onclick="save_user()"> <i class="fa fa-save" aria-hidden="true"></i>&nbsp;<?= lang('messages_lang.labelle_et_select_btn_mod')?></button>
                            </div>
                          </div>
                        </form>
                      </div> 
