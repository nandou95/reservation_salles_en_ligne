...
class Rapport_Suivi_Evaluation extends BaseController
{
  ...
  function liste()
  {
    ...
    foreach ($fetch_infos as $row)
    {
      ...
      $params_infos=$this->getBindParms('SUM(exec.ENG_BUDGETAIRE) AS MONTANT_ENGAGE,SUM(exec.ENG_JURIDIQUE) AS MONTANT_JURIDIQUE,SUM(exec.LIQUIDATION) AS MONTANT_LIQUIDATION,SUM(exec.DECAISSEMENT) AS MONTANT_DECAISSEMENT,SUM(exec.ORDONNANCEMENT) AS MONTANT_ORDONNANCEMENT,SUM(exec.PAIEMENT) AS PAIEMENT','execution_budgetaire exec JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=exec.PTBA_TACHE_ID JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID','det.ETAPE_DOUBLE_COMMANDE_ID NOT IN(5,9,13) AND exec.PTBA_TACHE_ID='.$row->PTBA_TACHE_ID.$critere_tranche.$critere_anne.$critere_date,'1');
      ...
      $params_qte_realise=$this->getBindParms('SUM(exec.ENG_BUDGETAIRE) resultat_realise,SUM(exec.QTE_RACCROCHE) QTE_REALISE','execution_budgetaire exec JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=exec.PTBA_TACHE_ID JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID','det.ETAPE_DOUBLE_COMMANDE_ID NOT IN(5,9,13) AND ptba_tache.PTBA_TACHE_ID='.$row->PTBA_TACHE_ID.$critere_tranche,'1');
  ...
  function exporter_filtre($RESPONSABLE='',$PROGRAMME_ID='',$ACTION_ID='',$CODE_NOMENCLATURE_BUDGETAIRE_ID='',$TRIMESTRE_ID='',$ANNEE_BUDGETAIRE_ID='',$DATE_DEBUT='',$DATE_FIN='')
  {
    ...
    {
      $params_infos=$this->getBindParms('SUM(exec.ENG_BUDGETAIRE) AS MONTANT_ENGAGE,SUM(exec.ENG_JURIDIQUE) AS MONTANT_JURIDIQUE,SUM(exec.LIQUIDATION) AS MONTANT_LIQUIDATION,SUM(exec.DECAISSEMENT) AS MONTANT_DECAISSEMENT,SUM(exec.ORDONNANCEMENT) AS MONTANT_ORDONNANCEMENT,SUM(exec.PAIEMENT) AS PAIEMENT','execution_budgetaire exec JOIN ptba_tache pt ON pt.PTBA_TACHE_ID=exec.PTBA_TACHE_ID JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID','det.ETAPE_DOUBLE_COMMANDE_ID NOT IN(5,9,13) AND pt.STRUTURE_RESPONSABLE_TACHE_ID='.$key->STRUTURE_RESPONSABLE_TACHE_ID.' ','1');
      $params_infos=str_replace('\"','"',$params_infos);
      $infos_sup=$this->ModelPs->getRequeteOne($callpsreq,$params_infos);
      ...
      ///recuperer le montant,qte realise par trimestre
      $params_qte_realise=$this->getBindParms('SUM(exec.ENG_BUDGETAIRE) resultat_realise,SUM(exec.QTE_RACCROCHE) QTE_REALISE','execution_budgetaire exec JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=exec.PTBA_TACHE_ID JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID','det.ETAPE_DOUBLE_COMMANDE_ID NOT IN(5,9,13) AND ptba_tache.STRUTURE_RESPONSABLE_TACHE_ID='.$key->STRUTURE_RESPONSABLE_TACHE_ID.' '.$critere_tranche,'1');
      $params_qte_realise=str_replace('\"','"',$params_qte_realise);
      $qte_realise=$this->ModelPs->getRequeteOne($callpsreq,$params_qte_realise);
      $RESULTAT_REALISE=!empty($qte_realise['resultat_realise']) ? $qte_realise['resultat_realise'] : '0';
      $MONTANT_ENGAGE=!empty($infos_sup['MONTANT_ENGAGE']) ? $infos_sup['MONTANT_ENGAGE'] : '0';
      ...
      foreach ($programmes as $key_program)
      {
        ...
        ///recuperer le montant,qte realise par trimestre
        $params_qte_realise_pgm=$this->getBindParms('SUM(exec.ENG_BUDGETAIRE) resultat_realise,SUM(exec.QTE_RACCROCHE) QTE_REALISE','execution_budgetaire exec JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=exec.PTBA_TACHE_ID JOIN inst_institutions_programmes prog ON ptba_tache.PROGRAMME_ID=prog.PROGRAMME_ID JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID','det.ETAPE_DOUBLE_COMMANDE_ID NOT IN(5,9,13) AND ptba_tache.STRUTURE_RESPONSABLE_TACHE_ID='.$key->STRUTURE_RESPONSABLE_TACHE_ID.' AND ptba_tache.PROGRAMME_ID='.$key_program->PROGRAMME_ID.' '.$critere_tranche,'1');
        ...
        foreach ($actions as $key_action)
        {
          ...
          ///recuperer le montant,qte realise par trimestre
          $params_qte_realise_action=$this->getBindParms('SUM(exec.ENG_BUDGETAIRE) resultat_realise,SUM(exec.QTE_RACCROCHE) QTE_REALISE','execution_budgetaire exec JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=exec.PTBA_TACHE_ID JOIN inst_institutions_programmes prog ON ptba_tache.PROGRAMME_ID=prog.PROGRAMME_ID JOIN inst_institutions_actions act ON ptba_tache.ACTION_ID=act.ACTION_ID JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID','det.ETAPE_DOUBLE_COMMANDE_ID NOT IN(5,9,13) AND ptba_tache.STRUTURE_RESPONSABLE_TACHE_ID='.$key->STRUTURE_RESPONSABLE_TACHE_ID.' AND ptba_tache.PROGRAMME_ID='.$key_program->PROGRAMME_ID.' AND ptba_tache.ACTION_ID='.$key_action->ACTION_ID.' '.$critere_tranche,'1');
          ...
          { 
            ...
            ///recuperer le montant,qte realise par trimestre
            $params_qte_realise_imput=$this->getBindParms('SUM(exec.ENG_BUDGETAIRE) resultat_realise,SUM(exec.QTE_RACCROCHE) QTE_REALISE','execution_budgetaire exec JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=exec.PTBA_TACHE_ID JOIN inst_institutions_programmes prog ON ptba_tache.PROGRAMME_ID=prog.PROGRAMME_ID JOIN inst_institutions_actions act ON ptba_tache.ACTION_ID=act.ACTION_ID JOIN inst_institutions_ligne_budgetaire ligne ON ptba_tache.CODE_NOMENCLATURE_BUDGETAIRE_ID=ligne.CODE_NOMENCLATURE_BUDGETAIRE_ID JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID','det.ETAPE_DOUBLE_COMMANDE_ID NOT IN(5,9,13) AND ptba_tache.STRUTURE_RESPONSABLE_TACHE_ID='.$key->STRUTURE_RESPONSABLE_TACHE_ID.' AND ptba_tache.PROGRAMME_ID='.$key_program->PROGRAMME_ID.' AND ptba_tache.ACTION_ID='.$key_action->ACTION_ID.' AND ptba_tache.CODE_NOMENCLATURE_BUDGETAIRE_ID='.$key_code->CODE_NOMENCLATURE_BUDGETAIRE_ID.' '.$critere_tranche,'1');
  ...
  function exporter_word($RESPONSABLE='',$PROGRAMME_ID='',$ACTION_ID='',$CODE_NOMENCLATURE_BUDGETAIRE_ID='',$TRIMESTRE_ID='',$ANNEE_BUDGETAIRE_ID='',$DATE_DEBUT='',$DATE_FIN='')
  {
    ...
    foreach ($responsable as $key)
    {
      $params_infos=$this->getBindParms('SUM(exec.ENG_BUDGETAIRE) AS MONTANT_ENGAGE,SUM(exec.ENG_JURIDIQUE) AS MONTANT_JURIDIQUE,SUM(exec.LIQUIDATION) AS MONTANT_LIQUIDATION,SUM(exec.DECAISSEMENT) AS MONTANT_DECAISSEMENT,SUM(exec.ORDONNANCEMENT) AS MONTANT_ORDONNANCEMENT,SUM(exec.PAIEMENT) AS PAIEMENT','execution_budgetaire exec JOIN ptba_tache pt ON pt.PTBA_TACHE_ID=exec.PTBA_TACHE_ID','pt.STRUTURE_RESPONSABLE_TACHE_ID='.$key->STRUTURE_RESPONSABLE_TACHE_ID.' ','1');
      $params_infos=str_replace('\"', '"', $params_infos);
      $infos_sup=$this->ModelPs->getRequeteOne($callpsreq, $params_infos);
      ...
      ///recuperer le montant,qte realise par trimestre
      $params_qte_realise=$this->getBindParms('SUM(exec.ENG_BUDGETAIRE) resultat_realise,SUM(exec.QTE_RACCROCHE) QTE_REALISE','execution_budgetaire exec JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=exec.PTBA_TACHE_ID','1 AND ptba_tache.STRUTURE_RESPONSABLE_TACHE_ID='.$key->STRUTURE_RESPONSABLE_TACHE_ID.'  '.$critere_tranche,'1');
      $params_qte_realise=str_replace('\"','"',$params_qte_realise);
      $qte_realise=$this->ModelPs->getRequeteOne($callpsreq,$params_qte_realise);
      ...
      foreach ($programmes as $key_program)
      {
        ...
        foreach ($actions as $key_action)
        {
          $params_infos_sup_act=$this->getBindParms('SUM(exec.ENG_BUDGETAIRE) AS MONTANT_ENGAGE,SUM(exec.ENG_JURIDIQUE) AS MONTANT_JURIDIQUE,SUM(exec.LIQUIDATION) AS MONTANT_LIQUIDATION,SUM(exec.DECAISSEMENT) AS MONTANT_DECAISSEMENT,SUM(exec.ORDONNANCEMENT) AS MONTANT_ORDONNANCEMENT,SUM(exec.PAIEMENT) AS PAIEMENT','execution_budgetaire exec JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=exec.PTBA_TACHE_ID','ptba_tache.STRUTURE_RESPONSABLE_TACHE_ID='.$key->STRUTURE_RESPONSABLE_TACHE_ID.' AND ptba_tache.PROGRAMME_ID='.$key_program->PROGRAMME_ID.' AND ptba_tache.ACTION_ID='.$key_action->ACTION_ID.' ','1');
          $params_infos_sup_act=str_replace('\"', '"', $params_infos_sup_act);
          $infos_sup_action=$this->ModelPs->getRequeteOne($callpsreq, $params_infos_sup_act);
