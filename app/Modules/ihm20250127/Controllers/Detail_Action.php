...
class Detail_Action extends BaseController
{
  ...
  function index($id=0)
  {
    ...
    else
    {
      ...
      // montant voté
      $bind_montant_vote = $this->getBindParms('SUM(`T1`) as T1,SUM(`T2`) as T2,SUM(`T3`) as T3,SUM(`T4`) as T4','ptba JOIN inst_institutions_actions act ON ptba.ACTION_ID=act.ACTION_ID','act.CODE_ACTION="'.$data['action']['CODE_ACTION'].'"','SUM(`T1`)');
      ...
      $mont_exe = "SELECT COUNT(DISTINCT racc.PTBA_ID) as activites,SUM(racc.MONTANT_RACCROCHE) as EXECUTEE,tranche.CODE_TRANCHE FROM execution_budgetaire_raccrochage_activite racc JOIN ptba ON ptba.PTBA_ID=racc.PTBA_ID LEFT JOIN op_tranches tranche ON racc.TRIMESTRE_ID=tranche.TRANCHE_ID JOIN inst_institutions_actions act ON ptba.ACTION_ID=act.ACTION_ID WHERE 1 AND ptba.ACTION_ID =".$data['action']['ACTION_ID']." GROUP BY tranche.CODE_TRANCHE";
      ...
      // Total des activités
      $query_principal = 'SELECT act.ACTION_ID,ptba.PTBA_ID FROM inst_institutions_actions act LEFT JOIN ptba ON act.ACTION_ID=ptba.ACTION_ID JOIN inst_institutions_ligne_budgetaire ligne ON ptba.CODE_NOMENCLATURE_BUDGETAIRE_ID=ligne.CODE_NOMENCLATURE_BUDGETAIRE_ID JOIN execution_budgetaire_raccrochage_activite exe ON ptba.PTBA_ID=exe.PTBA_ID WHERE 1 AND MD5(act.ACTION_ID) ="'.$id.'"';
      ...
      return view('App\Modules\ihm\Views\Detail_Action_View',$data);
    }
  }
  ...
  function liste_activite($id)
  {
    ...
    $query_principal = "SELECT act.ACTION_ID, ligne.CODE_NOMENCLATURE_BUDGETAIRE, ptba.CODES_PROGRAMMATIQUE, ptba.ACTIVITES,ptba.T1,ptba.T2,ptba.T3,ptba.T4,exe.MONTANT_RACCROCHE, ptba.PTBA_ID FROM inst_institutions_actions act LEFT JOIN ptba ON act.ACTION_ID=ptba.ACTION_ID JOIN inst_institutions_ligne_budgetaire ligne ON ptba.CODE_NOMENCLATURE_BUDGETAIRE_ID=ligne.CODE_NOMENCLATURE_BUDGETAIRE_ID JOIN execution_budgetaire_raccrochage_activite exe ON ptba.PTBA_ID=exe.PTBA_ID WHERE 1 AND act.`ACTION_ID` =".$id;
    ...
    $order_by='';
    $order_column='';
    $order_column= array('ligne.CODE_NOMENCLATURE_BUDGETAIRE','ligne.CODE_NOMENCLATURE_BUDGETAIRE','ptba.CODES_PROGRAMMATIQUE','ptba.ACTIVITES',1,1,1);
    ...
    foreach($fetch_cov_frais as $info)
    {
      ...
      $action .="<li>
      <a href='".base_url("ihm/Detail_Activite/".$info->PTBA_ID)."'><label>&nbsp;&nbsp;".lang('messages_lang.details_prog_budg')."</label></a>
      </li></ul>";
      $post[]= $action;
      $data[]=$post;  
    }
