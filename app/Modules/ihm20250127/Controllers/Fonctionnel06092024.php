...
class Fonctionnel extends BaseController
{
  ...
  function exporter($DIVISION_ID=0,$GROUPE_ID=0,$CLASSE_ID=0,$TRIMESTRE_ID=0,$DATE_DEBUT=0,$DATE_FIN=0,$ANNEE_BUDGETAIRE_ID=0)
  {
    ...
    foreach ($getData as $key)
    {
      ...
      $params_infos_division=$this->getBindParms('SUM(exec.ENG_BUDGETAIRE) AS MONTANT_ENGAGE,SUM(exec.ENG_JURIDIQUE) AS MONTANT_JURIDIQUE,SUM(exec.LIQUIDATION) AS MONTANT_LIQUIDATION,SUM(exec.DECAISSEMENT) AS MONTANT_DECAISSEMENT,SUM(exec.ORDONNANCEMENT) AS MONTANT_ORDONNANCEMENT,SUM(exec.PAIEMENT) AS PAIEMENT','execution_budgetaire exec JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=exec.PTBA_TACHE_ID',' ptba_tache.DIVISION_ID='.$key->DIVISION_ID.' '.$critere_tranche.''.$critere_anne.''.$critere_date,'1');
      ...
      foreach ($groupes as $key_grp)
      {
        ...
        $params_infos_group=$this->getBindParms('SUM(exec.ENG_BUDGETAIRE) AS MONTANT_ENGAGE,SUM(exec.ENG_JURIDIQUE) AS MONTANT_JURIDIQUE,SUM(exec.LIQUIDATION) AS MONTANT_LIQUIDATION,SUM(exec.DECAISSEMENT) AS MONTANT_DECAISSEMENT,SUM(exec.ORDONNANCEMENT) AS MONTANT_ORDONNANCEMENT,SUM(exec.PAIEMENT) AS PAIEMENT','execution_budgetaire exec JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=exec.PTBA_TACHE_ID',' ptba_tache.DIVISION_ID='.$key->DIVISION_ID.' AND ptba_tache.GROUPE_ID='.$key_grp->GROUPE_ID.' '.$critere_tranche.''.$critere_anne.''.$critere_date,'1');
        ...
        foreach ($classes as $key_classe)
        {
          ...
          $params_infos_classe=$this->getBindParms('SUM(exec.ENG_BUDGETAIRE) AS MONTANT_ENGAGE,SUM(exec.ENG_JURIDIQUE) AS MONTANT_JURIDIQUE,SUM(exec.LIQUIDATION) AS MONTANT_LIQUIDATION,SUM(exec.DECAISSEMENT) AS MONTANT_DECAISSEMENT,SUM(exec.ORDONNANCEMENT) AS MONTANT_ORDONNANCEMENT,SUM(exec.PAIEMENT) AS PAIEMENT','execution_budgetaire exec JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=exec.PTBA_TACHE_ID',' ptba_tache.DIVISION_ID='.$key->DIVISION_ID.' AND ptba_tache.GROUPE_ID='.$key_grp->GROUPE_ID.' AND ptba_tache.CLASSE_ID='.$key_classe->CLASSE_ID.' '.$critere_tranche.''.$critere_anne.''.$critere_date,'1');
