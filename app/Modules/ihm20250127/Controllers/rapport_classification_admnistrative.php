                ...
                foreach($imputations as $key_imput)
                {
                  ...
                  if ($activ==1)
                  {
                    ...
                    if(!empty($taches))
                    {
                      ...
                      {
                        $params_infos_5='SELECT SUM(exec.ENG_BUDGETAIRE) AS MONTANT_ENGAGE,SUM(exec.ENG_JURIDIQUE) AS MONTANT_JURIDIQUE,SUM(exec.LIQUIDATION) AS MONTANT_LIQUIDATION,SUM(exec.ORDONNANCEMENT) AS MONTANT_ORDONNANCEMENT,SUM(exec.PAIEMENT) AS PAIEMENT,SUM(exec.DECAISSEMENT) AS MONTANT_DECAISSEMENT FROM execution_budgetaire exec JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec.PTBA_TACHE_ID JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID WHERE det.ETAPE_DOUBLE_COMMANDE_ID NOT IN(5,9,13) AND ptba.INSTITUTION_ID='.$key->INSTITUTION_ID.' AND ptba.PROGRAMME_ID='.$key_progr->PROGRAMME_ID.' AND ptba.ACTION_ID ='.$key_action->ACTION_ID.' AND ptba.CODE_NOMENCLATURE_BUDGETAIRE_ID ='.$key_imput->CODE_NOMENCLATURE_BUDGETAIRE_ID.' AND ptba.PTBA_TACHE_ID='.$key_tache->PTBA_TACHE_ID.''.$critere_annee.' '.$critere_date_act.'';
