  ...
  //get liste de classification economique
	public function listing($value = 0)
	{
		...
		foreach($fetch_actions as $row)
		{
			$sub_array = array();
			//execution par ptba
			$get_exec = "SELECT exec.PTBA_TACHE_ID,SUM(exec.ENG_BUDGETAIRE) AS MONTANT_ENGAGE,SUM(exec.ENG_JURIDIQUE) AS MONTANT_JURIDIQUE,SUM(exec.LIQUIDATION) AS MONTANT_LIQUIDATION,SUM(exec.ORDONNANCEMENT) AS MONTANT_ORDONNANCEMENT,SUM(exec.PAIEMENT) AS PAIEMENT,SUM(exec.DECAISSEMENT) AS MONTANT_DECAISSEMENT FROM execution_budgetaire exec WHERE PTBA_TACHE_ID=".$row->PTBA_TACHE_ID." ".$critere_date;
	...
	function exporter($INSTITUTION_ID=0,$PROGRAMME_ID=0,$DATE_DEBUT=0,$DATE_FIN=0,$ANNEE_BUDGETAIRE_ID=0,$GRANDE_MASSE_ID=0,$is_action=0)
	{
      ...
      if($mont_transf_inst['INSTITUTION_ID']==$mont_recep_inst['INSTITUTION_ID'])
      {
      	...
      	$CREDIT_APRES_TRANSFERT_INST = floatval($BUDGET_VOTE_INST) + (floatval($MONTANT_RECEPTION_INST) - floatval($MONTANT_TRANSFERT_INST));
			...
			$params_infos_inst='SELECT SUM(exec.ENG_BUDGETAIRE) AS MONTANT_ENGAGE,SUM(exec.ENG_JURIDIQUE) AS MONTANT_JURIDIQUE,SUM(exec.LIQUIDATION) AS MONTANT_LIQUIDATION,SUM(exec.ORDONNANCEMENT) AS MONTANT_ORDONNANCEMENT,SUM(exec.PAIEMENT) AS MONTANT_PAIEMENT,SUM(exec.DECAISSEMENT) AS MONTANT_DECAISSEMENT FROM execution_budgetaire exec JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec.PTBA_TACHE_ID WHERE ptba.INSTITUTION_ID='.$key->INSTITUTION_ID.' '.$critere_date.' '.$critere_annee.' GROUP BY ptba.INSTITUTION_ID';
			  		...
			  		//Debut Gestion des grandes masses
						...
						foreach($grandmasse as $masses)
						{
				  		...
				  		//debut gestion du montant vote par grande masse
							...
							$params_infos_gm="SELECT SUM(exec.ENG_BUDGETAIRE) AS MONTANT_ENGAGE_GM,SUM(exec.ENG_JURIDIQUE) AS MONTANT_JURIDIQUE_GM,SUM(exec.LIQUIDATION) AS MONTANT_LIQUIDATION_GM,SUM(exec.ORDONNANCEMENT) AS MONTANT_ORDONNANCEMENT_GM,SUM(exec.PAIEMENT) AS MONTANT_PAIEMENT_GM,SUM(exec.DECAISSEMENT) AS MONTANT_DECAISSEMENT_GM FROM execution_budgetaire exec JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec.PTBA_TACHE_ID WHERE ptba.INSTITUTION_ID=".$key->INSTITUTION_ID." AND ptba.PROGRAMME_ID=".$valueprog->PROGRAMME_ID." AND ptba.ACTION_ID=".$key_action->ACTION_ID." AND ptba.GRANDE_MASSE_ID=".$masses->GRANDE_MASSE_ID." ".$critere_annee."".$critere_date;
	...
	function export_pdf($INSTITUTION_ID=0,$PROGRAMME_ID=0,$DATE_DEBUT=0,$DATE_FIN=0,$ANNEE_BUDGETAIRE_ID=0,$GRANDE_MASSE_ID=0,$is_action=0)
	{
      ...
      if($mont_transf_inst['INSTITUTION_ID']==$mont_recep_inst['INSTITUTION_ID'])
      {
      	...
      	$CREDIT_APRES_TRANSFERT_INST = floatval($BUDGET_VOTE_INST) + (floatval($MONTANT_RECEPTION_INST) - floatval($MONTANT_TRANSFERT_INST));
			...
			{
			  // debut execution programme ------------------------------------------------------
				$params_infos_prog='SELECT SUM(exec.ENG_BUDGETAIRE) AS MONTANT_ENGAGE_PROG,SUM(exec.ENG_JURIDIQUE) AS MONTANT_JURIDIQUE_PROG,SUM(exec.LIQUIDATION) AS MONTANT_LIQUIDATION_PROG,SUM(exec.ORDONNANCEMENT) AS MONTANT_ORDONNANCEMENT_PROG,SUM(exec.PAIEMENT) AS MONTANT_PAIEMENT_PROG,SUM(exec.DECAISSEMENT) AS MONTANT_DECAISSEMENT_PROG FROM execution_budgetaire exec LEFT JOIN transfert_historique_transfert histo ON histo.PTBA_TACHE_ID_TRANSFERT=exec.PTBA_TACHE_ID JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec.PTBA_TACHE_ID WHERE ptba.INSTITUTION_ID='.$key->INSTITUTION_ID.' AND ptba.PROGRAMME_ID= '.$valueprog->PROGRAMME_ID.' '.$critere_date.' '.$critere_annee;
				$infos_sup_prog=$this->ModelPs->getRequeteOne('CALL getTable("'.$params_infos_prog.'")');
			      ...
			      if($mont_transf_action['ACTION_ID']==$mont_recep_action['ACTION_ID'])
			      {
			      	...
			      	$CREDIT_APRES_TRANSFERT_ACTIO = floatval($BUDGET_VOTE_ACTION) + (floatval($MONTANT_RECEPTION_ACTION) - floatval($MONTANT_TRANSFERT_ACTION));
						...
						foreach($grandmasse as $masses)
						{
							...
							//debut gestion des execution par grande masse
							$params_infos_gm="SELECT SUM(exec.ENG_BUDGETAIRE) AS MONTANT_ENGAGE_GM,SUM(exec.ENG_JURIDIQUE) AS MONTANT_JURIDIQUE_GM,SUM(exec.LIQUIDATION) AS MONTANT_LIQUIDATION_GM,SUM(exec.ORDONNANCEMENT) AS MONTANT_ORDONNANCEMENT_GM,SUM(exec.PAIEMENT) AS MONTANT_PAIEMENT_GM,SUM(exec.DECAISSEMENT) AS MONTANT_DECAISSEMENT_GM FROM execution_budgetaire exec JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec.PTBA_TACHE_ID LEFT JOIN transfert_historique_transfert histo ON histo.PTBA_TACHE_ID_TRANSFERT=exec.PTBA_TACHE_ID WHERE ptba.INSTITUTION_ID=".$key->INSTITUTION_ID." AND ptba.PROGRAMME_ID=".$valueprog->PROGRAMME_ID." AND ptba.ACTION_ID=".$key_action->ACTION_ID." AND ptba.GRANDE_MASSE_ID=".$masses->GRANDE_MASSE_ID." ".$critere_annee."".$critere_date;
