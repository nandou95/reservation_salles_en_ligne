...
class Rapport_Suivi_Evaluation extends BaseController
{
  ...
  function liste()
  {
    ...
    foreach ($fetch_infos as $row)
    {
      ...
      $params_qte_realise=$this->getBindParms('SUM(exec.ENG_BUDGETAIRE) resultat_realise,SUM(exec.QTE_RACCROCHE) QTE_REALISE','execution_budgetaire exec JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=exec.PTBA_TACHE_ID','1 AND ptba_tache.PTBA_TACHE_ID='.$row->PTBA_TACHE_ID.$critere_tranche,'1');
