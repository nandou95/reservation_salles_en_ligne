...
*Titre: Execution budgtaire raccrocher
...
class Exec_Budget_Raccrocher extends BaseController
{
 ...
 */
		...
		if($session->get('SESSION_SUIVIE_PTBA_RACCROCHAGE_FINANCIER')!=1 && $session->get('SESSION_SUIVIE_PTBA_RACCROCHAGE_QUANTITE')!=1)
    {
		...
		return view('App\Modules\demande_new\Views\Exec_Budget_Raccrocher_View',$data);   
	}
	...
	function listing($value = 0)
	{
		...
		foreach ($fetch_data as $row)
		{
			...
			if ($quantite==1) 
			{
				$progr_row= "<a  title='Quantité' href='".base_url("demande_new/Exec_Budget_Raccrocher/editQuantite/".$row->EXECUTION_BUDGETAIRE_RACCROCHAGE_ID)."' >".$row->CODES_PROGRAMMATIQUE."</a>";
			}
			else
			{
			...
			$BTN_TRAITE = "<a class='btn btn-primary btn-sm' title='Quantité' href='".base_url("demande_new/Exec_Budget_Raccrocher/editQuantite/".$row->EXECUTION_BUDGETAIRE_RACCROCHAGE_ID)."' > <i class='fa fa-link text-light'></i></a>";
			if($quantite==1) 
			{
				$action="".$BTN_TRAITE."";
				$sub_array[]=$action;
		...
		if($session->get('SESSION_SUIVIE_PTBA_RACCROCHAGE_FINANCIER')!=1 && $session->get('SESSION_SUIVIE_PTBA_RACCROCHAGE_QUANTITE')!=1)
    {
		...
		if($this->validation->withRequest($this->request)->run())
		{
			...
			return redirect('demande/Exec_Budget_Raccrocher');
		}
		else
		{
