<?php
/**HABIMANA NANDOU
*Titre: EXECUTION BUDGETAIRE RECCROCHE
*Numero de telephone: +257 71483905
*Email pro: nandou@mediabox.bi
*Date: 19 sept 2023
* 
* modifie par SONIA MUNEZERO
* TEL  65165772 / +989397728740
...
class Execution_deja_raccroche extends BaseController
{
		...
		if($session->get('SESSION_SUIVIE_PTBA_RACCROCHAGE_FINANCIER')!=1 && $session->get('SESSION_SUIVIE_PTBA_RACCROCHAGE_QUANTITE')!=1)
    {
		...
		return view('App\Modules\demande_new\Views\Execution_deja_raccroche_List_view',$data);   
	}
	...
	public function listing($value = 0)
	{
		...
		{
			$racc = 'SELECT COUNT(EXECUTION_BUDGETAIRE_RACCROCHAGE_ID) as nbre FROM `execution_budgetaire_raccrochage_activite_new` WHERE 1 AND EXECUTION_BUDGETAIRE_ID = '.$row->EXECUTION_BUDGETAIRE_ID;
			...
			if (mb_strlen($row->LIBELLE) > 8){ 
				$LIBELLE =  mb_substr($row->LIBELLE, 0, 8) .'...<a class="btn-sm" title="Afficher"  onclick="show_modal('.$row->EXECUTION_BUDGETAIRE_ID.')"><i class="fa fa-eye"></i></a>';
		...
		if($session->get('SESSION_SUIVIE_PTBA_RACCROCHAGE_FINANCIER')!=1 && $session->get('SESSION_SUIVIE_PTBA_RACCROCHAGE_QUANTITE')!=1)
    {
		...
		$data=$this->urichk();
		$libel="SELECT LIBELLE FROM execution_budgetaire  WHERE 1 AND EXECUTION_BUDGETAIRE_ID = ".$id;
		$libel='CALL `getList`("'.$libel.'")';
		$libelle_commentaire = $this->ModelPs->getRequeteOne( $libel);

		$output = array(
			"data123" => $libelle_commentaire['LIBELLE'],	
		);
		return $this->response->setJSON($output);
	}
