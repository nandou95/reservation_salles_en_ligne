...
class Rapport_Objectif_strategique_New extends BaseController
{
	protected $session;
	protected $ModelPs;
    ...
    if($session->get('SESSION_SUIVIE_PTBA_TABLEAU_BORD_PIP_OBJECTIF_STRATEGIQUE')!=1)
    {
      return redirect('Login_Ptba/homepage');
		...
		return view('App\Modules\dashboard\Views\Rapport_Objectif_strategique_New_View',$data);
	}
	...
	public function get_rapport()
	{
		...
		$pillier="SELECT statut.`ID_STATUT_PROJET` AS ID,statut.`DESCR_STATUT_PROJET` AS NAME,COUNT(DISTINCT pip.ID_DEMANDE_INFO_SUPP) AS NBRE FROM pip_statut_projet statut LEFT JOIN pip_demande_infos_supp pip ON statut.ID_STATUT_PROJET=pip.ID_STATUT_PROJET LEFT JOIN pip_cadre_mesure_resultat_livrable ON pip_cadre_mesure_resultat_livrable.ID_DEMANDE_INFO_SUPP=pip.ID_DEMANDE_INFO_SUPP LEFT JOIN cadre_mesure_resultat_valeur_cible ON cadre_mesure_resultat_valeur_cible.ID_CADRE_MESURE_RESULTAT_LIVRABLE=pip_cadre_mesure_resultat_livrable.ID_CADRE_MESURE_RESULTAT_LIVRABLE WHERE 1 ".$cond." ".$condition."  GROUP BY statut.`ID_STATUT_PROJET`,statut.`DESCR_STATUT_PROJET`";

		$nbr_pillier=$this->ModelPs->getRequete('CALL getTable("'.$pillier.'")');
		$pillier_tot="SELECT COUNT(DISTINCT pip.ID_DEMANDE_INFO_SUPP) AS NBRE FROM pip_statut_projet statut LEFT JOIN pip_demande_infos_supp pip ON statut.ID_STATUT_PROJET=pip.ID_STATUT_PROJET LEFT JOIN pip_cadre_mesure_resultat_livrable ON pip_cadre_mesure_resultat_livrable.ID_DEMANDE_INFO_SUPP=pip.ID_DEMANDE_INFO_SUPP LEFT JOIN cadre_mesure_resultat_valeur_cible ON cadre_mesure_resultat_valeur_cible.ID_CADRE_MESURE_RESULTAT_LIVRABLE=pip_cadre_mesure_resultat_livrable.ID_CADRE_MESURE_RESULTAT_LIVRABLE WHERE 1 ".$cond." ".$condition.""; 
    ...
    // print_r($projet_Statuts);die();
		...
		Highcharts.chart('container', {                    
			chart: {
				...
				title: {
					text: '<b>".lang("messages_lang.pip_rapport_institutio_statut")." :: ".number_format($data_total,0,'',' ')." BIF '
					},
					subtitle: {
						text: 'Le ".date('d-m-Y')." '
						},
												...
												events: {
													click: function(){
														...
														$(\"#mytableppppp\").DataTable({
															...
															\"oreder\":[],
															\"ajax\":{
																url:\"".base_url('dashboard/Rapport_Objectif_strategique_New/detail_strategique')."\",
																type:\"POST\",
																data:{
																	key:this.key,
   ...
   //function pour exporter le Rapport de suivie evaluation dans excel
      ...
      //boucle pour les institutions 
												...
												return redirect('dashboard/Rapport_Objectif_strategique');
											}
