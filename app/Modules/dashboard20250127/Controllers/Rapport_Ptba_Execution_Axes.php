...
class Dashboard_Ptba_Execution_Axes extends BaseController
{
	protected $session;
	protected $ModelPs;
    ...
    public function getBindParmsLimit($columnselect, $table, $where, $orderby, $Limit)
  {
    ...
    if($session->get('SESSION_SUIVIE_PTBA_TABLEAU_BORD_BUDGET')!=1)
    {
      return redirect('Login_Ptba/homepage');
		...
		return view('App\Modules\dashboard\Views\Rapport_Ptba_Execution_Axes_View',$data);
	}
    ...
    if($PAP_ACTIVITE_ID !=0)
		...
		return redirect('dashboard/Rapport_Ptba_Execution_Axes');
	}
...
$budget=("SELECT `AXE_PND_ID` as ID,`DESCR_AXE_PND` AS NAME,(SELECT ".$totaux." FROM ptba_tache ".$name_table." WHERE ptba_tache.ID_PILIER=ID) as vote,(SELECT SUM(execution_budgetaire.ENG_BUDGETAIRE) FROM ptba_tache JOIN execution_budgetaire ON execution_budgetaire.PTBA_TACHE_ID=ptba_tache.PTBA_TACHE_ID ".$name_table." WHERE ptba_tache.AXE_PND_ID=ID ".$cond." ".$cond_trim.") AS ENG_BUDGETAIRE,(SELECT SUM(execution_budgetaire.ENG_JURIDIQUE) FROM ptba_tache JOIN execution_budgetaire ON execution_budgetaire.PTBA_TACHE_ID=ptba_tache.PTBA_TACHE_ID ".$name_table." WHERE ptba_tache.AXE_PND_ID=ID ".$cond." ".$cond_trim.") AS ENG_JURIDIQUE,(SELECT SUM(execution_budgetaire.LIQUIDATION) FROM ptba_tache JOIN execution_budgetaire ON execution_budgetaire.PTBA_TACHE_ID=ptba_tache.PTBA_TACHE_ID ".$name_table." WHERE ptba_tache.AXE_PND_ID=ID ".$cond." ".$cond_trim.") AS LIQUIDATION,(SELECT SUM(execution_budgetaire.ORDONNANCEMENT) FROM ptba_tache JOIN execution_budgetaire ON execution_budgetaire.PTBA_TACHE_ID=ptba_tache.PTBA_TACHE_ID ".$name_table." WHERE ptba_tache.AXE_PND_ID=ID ".$cond." ".$cond_trim.") AS ORDONNANCEMENT,(SELECT SUM(execution_budgetaire.PAIEMENT) FROM ptba_tache JOIN execution_budgetaire ON execution_budgetaire.PTBA_TACHE_ID=ptba_tache.PTBA_TACHE_ID ".$name_table." WHERE ptba_tache.AXE_PND_ID=ID ".$cond." ".$cond_trim.") AS PAIEMENT,(SELECT SUM(execution_budgetaire.DECAISSEMENT) FROM ptba_tache JOIN execution_budgetaire ON execution_budgetaire.PTBA_TACHE_ID=ptba_tache.PTBA_TACHE_ID ".$name_table." WHERE ptba_tache.AXE_PND_ID=ID ".$cond." ".$cond_trim.") AS DECAISSEMENT FROM  `pnd_axe` WHERE 1 group by AXE_PND_ID,DESCR_AXE_PND");
   ...
   #########
	$rapp1="<script type=\"text/javascript\">
		...
		title: {
			text: '<b>Budget voté vs engagements par axes  </b><br>',
		},  
		subtitle: {
			text: ''
		},
	 ...
	 $(\"#myModal\").modal('show');
		...
		\"bDestroy\": true,
		\"ajax\":{
		url:\"".base_url('dashboard/Rapport_Ptba_Execution_Axes/detail_ptba_axes')."\",
		type:\"POST\",
		data:{
			key:this.key,
			key2:this.key2,
		...
		title: {
			text: '<b>Budget  exécuté par axes  </b><br>',
		},  
		subtitle: {
			text: ''
		},
	 ...
	 $(\"#myModal_deux\").modal('show');
		...
		\"bDestroy\": true,
		\"ajax\":{
		url:\"".base_url('dashboard/Rapport_Ptba_Execution_Axes/detail_ptba_axes_deux')."\",
		type:\"POST\",
		data:{
			key:this.key,
			key2:this.key2,
...
#########details
 public function detail_ptba_axe()
        {
        ...
        $order_by = '';
         ...
         $data = array();
               ...
               $sub_array[] ='<center><font color="#000000" size=2><label>'.$u.'</label></font> </center>';
                ...
                if (strlen($row->DESCR_AXE_PND) > 20) {
               $sub_array[] = mb_substr($row->DESCR_AXE_PND, 0, 18) .'...<a class="btn-sm" data-toggle="tooltip" title="'.$this->str_replacecatego($row->DESCR_AXE_PND).'"><i class="fa fa-eye"></i></a>';
                }else{
              $sub_array[] ='<font color="#000000" ><label>'.$this->str_replacecatego($row->DESCR_AXE_PND).'</label></font>';
                  }
               if (strlen($row->INTITULE_MINISTERE) > 20) {
               $sub_array[] = mb_substr($row->INTITULE_MINISTERE, 0, 18) .'...<a class="btn-sm" data-toggle="tooltip" title="'.$this->str_replacecatego($row->INTITULE_MINISTERE).'"><i class="fa fa-eye"></i></a>';
                   }else{
