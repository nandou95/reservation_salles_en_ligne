...
function exporter($TYPE_INSTITUTION_ID,$INSTITUTION_ID,$SOUS_TUTEL_ID,$PROGRAMME_ID,$ACTION_ID,$LIGNE_BUDGETAIRE,$IS_PRIVATE,$PAP_ACTIVITE_ID)
{
  ...
  $sheet->setCellValue('E1', 'LIGNE BUDGETAIRE');
  $sheet->setCellValue('F1', 'ACTIVITE');
  $sheet->setCellValue('G1', 'TACHES');
  ...
  $sheet->setCellValue('N1', 'GRANDE MASSE');
  $rows = 3;
  $i=1;
  foreach ($getData as $key)
  {
    ...
    $sheet->setCellValue('H' . $rows, $mont_budg);
    $sheet->setCellValue('I' . $rows, $mont_jur);
    $sheet->setCellValue('J' . $rows, $mont_liq);
    $sheet->setCellValue('K' . $rows, $mont_ordo);
    $sheet->setCellValue('L' . $rows, $mont_paie);
    $sheet->setCellValue('M' . $rows, $mont_decais);
    $sheet->setCellValue('N' . $rows, $key->DESCRIPTION_GRANDE_MASSE);
    $rows++;
    $i++;
  } 
               ...
               point:{
                events: {
                 click: function(){
                  ...
                  $colorv=$this->getcolor();
                $data_budget="{name:'".lang('messages_lang.labelle_montant')." ".lang('messages_lang.labelle_vote')." (<> BIF)',color:'".$colorv."',data:[";
                $data_total=0;
                ...
                foreach ($budg_vote as $value)
                {
                  ...
                  $data_total+=$mont;
                  $data_budget.="{name:'".trim(str_replace("'","\'", $value->gde_masse))."',y:".number_format(($mont*100)/$Tota_GM_VOTE,2,'.','').",key:'".$value->id."',key1:1,key2:".$KEY2.",color:'".$colorv."'},"; 
                }
               $data_total1=0;
               $colorex=$this->getcolor();
              $data_budget.="]},{name:'".lang('messages_lang.labelle_montant')." ".lang('messages_lang.labelle_execution')."(@ BIF)', color:'".$colorex."',data:[";
              
              foreach($budg_ex as $value){
                $color=$this->getcolor();
                $mont=$value->bug_exec>0 ? $value->bug_exec: 0;
                $data_total1=$data_total1+$mont;
                $data_budget.="{name:'".trim(str_replace("'","\'", $value->gde_masse))."',y:".number_format(($mont*100)/$Tot_Exec,2,'.','').",key:'".$value->id."',key1:2,color:'".$colorex."'},";
               }
              $data_budget.="]}";
