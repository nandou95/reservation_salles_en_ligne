     ...
     return redirect('Login_Ptba/homepage');
		...
		public function get_rapport()
		{
			...
			if(! empty($INSTITUTION_ID))
			{
				...
				$name_table= " JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=ptba_tache.INSTITUTION_ID JOIN inst_institutions_sous_tutel sous_tutel ON SUBSTRING(ptba_tache.CODE_NOMENCLATURE_BUDGETAIRE, 5,3) =sous_tutel.CODE_SOUS_TUTEL ";
				$format=" {point.y:.2f} %";
				$type="column";
        ...
        if(!empty($PAP_ACTIVITE_ID))
          {
			...
			$engage11=("SELECT ".$name_decl." AS name,".$id_decl." as ID,SUM(ENG_BUDGETAIRE) as engage  FROM `execution_budgetaire` JOIN  ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID  ".$name_table." WHERE 1 ".$cond_trim." ".$cond." ".$cond1." ".$cond_pri1." GROUP BY ".$name_decl.",".$id_decl."");

			$liquide11=("SELECT ".$name_decl." AS name,".$id_decl." as ID,SUM(LIQUIDATION) as liquide FROM `execution_budgetaire` JOIN  ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID ".$name_table." WHERE 1  ".$cond_trim."  ".$cond." ".$cond1." ".$cond_pri1." GROUP BY ".$name_decl.",".$id_decl."");

			$decaissement11=("SELECT ".$name_decl." AS name,".$id_decl." as ID, SUM(DECAISSEMENT) as decaissement FROM `execution_budgetaire` JOIN  ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID ".$name_table." WHERE 1  ".$cond_trim." ".$cond." ".$cond1." ".$cond_pri1." GROUP BY ".$name_decl.",".$id_decl."");
			$jurdique11=("SELECT ".$name_decl." AS name,".$id_decl." as ID,SUM(ENG_JURIDIQUE) as jurdique FROM `execution_budgetaire` JOIN  ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID ".$name_table." WHERE 1 ".$cond_trim." ".$cond." ".$cond1." ".$cond_pri1."  GROUP BY ".$name_decl.",".$id_decl."");
			$ordonence11=("SELECT ".$name_decl." AS name,".$id_decl." as ID,SUM(ORDONNANCEMENT) as ordonence FROM `execution_budgetaire` JOIN  ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID ".$name_table." WHERE 1 ".$cond_trim." ".$cond." ".$cond1." ".$cond_pri1."   GROUP BY ".$name_decl.",".$id_decl."");

			$paiement11=("SELECT ".$name_decl." AS name,".$id_decl." as ID,SUM(PAIEMENT) as paie FROM `execution_budgetaire` JOIN  ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID ".$name_table." WHERE 1 ".$cond_trim." ".$cond." ".$cond1." ".$cond_pri1."  GROUP BY ".$name_decl.",".$id_decl."");
			...
			foreach ($engage_req11 as $value)
			{
				...
				$taux2=("SELECT SUM(execution_budgetaire.ENG_BUDGETAIRE) as taux FROM execution_budgetaire JOIN  ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID LEFT JOIN execution_budgetaire_tache_detail ON execution_budgetaire_tache_detail.EXECUTION_BUDGETAIRE_ID=execution_budgetaire.EXECUTION_BUDGETAIRE_ID JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=ptba_tache.INSTITUTION_ID WHERE 1 ".$cond." ".$cond1." ".$cond_trim." ");
				$categorie.="'";
			...
			foreach ($jurdique_req11 as $value)
			{
				$pourcent=0;
				$taux=("SELECT ".$totaux." AS taux FROM `ptba_tache` JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=ptba_tache.INSTITUTION_ID WHERE 1 ".$cond." ");
				$taux2=("SELECT SUM(execution_budgetaire.ENG_BUDGETAIRE) as taux FROM execution_budgetaire JOIN  ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID LEFT JOIN execution_budgetaire_tache_detail ON execution_budgetaire_tache_detail.EXECUTION_BUDGETAIRE_ID=execution_budgetaire.EXECUTION_BUDGETAIRE_ID JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=ptba_tache.INSTITUTION_ID WHERE 1 ".$cond." ".$cond1." ".$cond_trim." ".$cond_pri1." ");
				$categorie.="'";
				$name = (!empty($value->name)) ? $value->name : "Autres";
				$rappel=$this->str_replacecatego($name);
				$categorie.= $rappel."',";
			...
			foreach ($liquide_req11 as $value)
			{
				$pourcent=0;
				$taux=("SELECT ".$totaux." AS taux FROM `ptba_tache` JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=ptba_tache.INSTITUTION_ID WHERE 1 ".$cond." ");
				$taux2=("SELECT SUM(execution_budgetaire.ENG_BUDGETAIRE) as taux FROM execution_budgetaire JOIN  ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID LEFT JOIN execution_budgetaire_tache_detail ON execution_budgetaire_tache_detail.EXECUTION_BUDGETAIRE_ID=execution_budgetaire.EXECUTION_BUDGETAIRE_ID JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=ptba_tache.INSTITUTION_ID WHERE 1 ".$cond." ".$cond1." ".$cond_pri1." ".$cond_trim." ");
				$categorie.="'";
				$name = (!empty($value->name)) ? $value->name : "Autres";
				$rappel=$this->str_replacecatego($name);
				$categorie.= $rappel."',";
...
function detail_valeur_dynamiques() 
{
	...
	$search = !empty($_POST['search']['value']) ? ("AND (
		DESCRIPTION_INSTITUTION LIKE '%$var_search%' OR inst_institutions_programmes.INTITULE_PROGRAMME LIKE '%$var_search%' OR inst_institutions_actions.LIBELLE_ACTION LIKE '%$var_search%' OR DESC_PAP_ACTIVITE LIKE '%$var_search%' OR RESULTAT_PAP_ACTIVITE LIKE '%$var_search%')") : ''; 
