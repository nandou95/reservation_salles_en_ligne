  ...
  */
  ...
  public function index($value='')
  {
    ...
    public function get_rapport()
    {
      ...
      $cond_pri.=' AND ptba_tache.INSTITUTION_ID IN ('.$code_inst.')';
      ...
      if(! empty($INSTITUTION_ID))
      {
        ...
        $name_table= " JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=ptba_tache.INSTITUTION_ID JOIN  inst_institutions_sous_tutel ON SUBSTRING(ptba_tache.CODE_NOMENCLATURE_BUDGETAIRE, 5,3)=inst_institutions_sous_tutel.CODE_SOUS_TUTEL";
        $format=" {point.y:.2f} %";
        $type="column";
...
if(!empty($PAP_ACTIVITE_ID))
   ...
   $KEY2=7;
      ...
      $engage11=("SELECT ".$name_decl." AS name,".$id_decl." as ID,SUM(ENG_BUDGETAIRE) as engage  FROM `execution_budgetaire` JOIN  ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID ".$name_table." WHERE 1 ".$cond_trim." ".$cond." ".$cond_exec." ".$cond_pri1." GROUP BY ".$name_decl.",".$id_decl."");
      $liquide11=("SELECT ".$name_decl." AS name,".$id_decl." as ID,SUM(LIQUIDATION) as liquide FROM `execution_budgetaire` JOIN  ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID  ".$name_table." WHERE 1  ".$cond_trim."  ".$cond." ".$cond_exec." ".$cond_pri1." GROUP BY ".$name_decl.",".$id_decl."");
      $decaissement11=("SELECT ".$name_decl." AS name,".$id_decl." as ID, SUM(DECAISSEMENT) as decaissement FROM `execution_budgetaire` JOIN  ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID  ".$name_table." WHERE 1  ".$cond_trim." ".$cond." ".$cond_exec." ".$cond_pri1." GROUP BY ".$name_decl.",".$id_decl."");
      $jurdique11=("SELECT ".$name_decl." AS name,".$id_decl." as ID,SUM(ENG_JURIDIQUE) as jurdique FROM `execution_budgetaire` JOIN  ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID  ".$name_table." WHERE 1 ".$cond_trim." ".$cond." ".$cond_exec." ".$cond_pri1."  GROUP BY ".$name_decl.",".$id_decl."");
      $ordonence11=("SELECT ".$name_decl." AS name,".$id_decl." as ID,SUM(ORDONNANCEMENT) as ordonence FROM `execution_budgetaire` JOIN  ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID  ".$name_table." WHERE 1 ".$cond_trim." ".$cond." ".$cond_exec." ".$cond_pri1."   GROUP BY ".$name_decl.",".$id_decl."");
      $paiement11=("SELECT ".$name_decl." AS name,".$id_decl." as ID,SUM(PAIEMENT) as paie FROM `execution_budgetaire` JOIN  ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID  ".$name_table." WHERE 1 ".$cond_trim." ".$cond." ".$cond_exec." ".$cond_pri1."  GROUP BY ".$name_decl.",".$id_decl."");
      ...
      foreach ($engage_req11 as $value)
      {
        $pourcent=0;
        $pourcent2=0;
        $taux=("SELECT ".$totaux." AS taux FROM `ptba_tache` JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=ptba_tache.INSTITUTION_ID JOIN execution_budgetaire ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID WHERE 1 ".$cond." ".$cond_exec." ");
        $taux2=("SELECT SUM(execution_budgetaire.ENG_BUDGETAIRE) as taux FROM execution_budgetaire JOIN execution_budgetaire_tache_detail detail ON execution_budgetaire.EXECUTION_BUDGETAIRE_ID=detail.EXECUTION_BUDGETAIRE_ID JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=ptba_tache.INSTITUTION_ID WHERE 1 ".$cond." ".$cond_exec." ".$cond_trim." ");
        $categorie.="'";
      ...
      foreach ($jurdique_req11 as $value)
      {
        $pourcent=0;
        $taux=("SELECT ".$totaux." AS taux FROM `ptba_tache` JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=ptba_tache.INSTITUTION_ID JOIN execution_budgetaire ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID WHERE 1 ".$cond." ".$cond_exec." ");
        $taux2=("SELECT SUM(execution_budgetaire.ENG_BUDGETAIRE) as taux FROM execution_budgetaire JOIN execution_budgetaire_tache_detail detail ON execution_budgetaire.EXECUTION_BUDGETAIRE_ID=detail.EXECUTION_BUDGETAIRE_ID JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=ptba_tache.INSTITUTION_ID WHERE 1  ".$cond." ".$cond_exec." ".$cond_trim." ".$cond_pri1." ");
        $categorie.="'";
      ...
      foreach ($liquide_req11 as $value)
      {
        $pourcent=0;
        $taux=("SELECT ".$totaux." AS taux FROM `ptba_tache` JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=ptba_tache.INSTITUTION_ID JOIN execution_budgetaire ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID WHERE 1 ".$cond." ".$cond_exec." ");
        $taux2=("SELECT SUM(execution_budgetaire.ENG_BUDGETAIRE) as taux FROM execution_budgetaire JOIN execution_budgetaire_tache_detail detail ON execution_budgetaire.EXECUTION_BUDGETAIRE_ID=detail.EXECUTION_BUDGETAIRE_ID JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=ptba_tache.INSTITUTION_ID WHERE 1  ".$cond." ".$cond_exec." ".$cond_pri1." ".$cond_trim." ");
        $categorie.="'";
        $name = (!empty($value->name)) ? $value->name : "Autres";
        $rappel=$this->str_replacecatego($name);
        $categorie.= $rappel."',";
      ...
      Highcharts.chart('container1', {
        ...
        $program_sect_req = $this->ModelPs->getRequete('CALL getTable("'.$program_sect.'")');
          ...
          if ($KEY2==7)
          {
            $critere=" AND ptba_tache.PAP_ACTIVITE_ID='".$KEY."'";
          }
...
// $engagement[] ='<center><font color="#000000" size=2><label>'.number_format($row->total,0,',',' ').'</label></font> </center>';
  ...
  function liste_institution_taux_votes() 
          ...
          $query_principal=" SELECT SUBSTRING(ptba_tache.CODE_NOMENCLATURE_BUDGETAIRE, 5,3) service,".$totaux." as vote,inst_institutions.DESCRIPTION_INSTITUTION AS INTITULE_MINISTERE,execution.PTBA_TACHE_ID, inst_institutions_programmes.INTITULE_PROGRAMME,inst_institutions_actions.LIBELLE_ACTION,pap_activites.DESC_PAP_ACTIVITE,pap_activites.RESULTAT_PAP_ACTIVITE,execution.ENG_BUDGETAIRE AS ENG_BUDGETAIRE,execution.LIQUIDATION AS LIQUIDATION, execution.ENG_JURIDIQUE AS ENG_JURIDIQUE, execution.DECAISSEMENT AS DECAISSEMENT,execution.ORDONNANCEMENT AS ORDONNANCEMENT,execution.PAIEMENT AS PAIEMENT FROM execution_budgetaire execution JOIN ptba_tache ON execution.PTBA_TACHE_ID=ptba_tache.PTBA_TACHE_ID LEFT JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=ptba_tache.INSTITUTION_ID JOIN inst_institutions_programmes ON inst_institutions_programmes.PROGRAMME_ID=ptba_tache.PROGRAMME_ID JOIN inst_institutions_actions ON inst_institutions_actions.ACTION_ID=ptba_tache.ACTION_ID JOIN pap_activites ON pap_activites.PAP_ACTIVITE_ID=ptba_tache.PAP_ACTIVITE_ID  WHERE 1  ".$cond." ".$cond_trim." ";
