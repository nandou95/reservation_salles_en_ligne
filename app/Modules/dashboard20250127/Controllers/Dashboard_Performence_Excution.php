...
* Adaptation au nouveau format ptba
* le 20/06/2024 
*/
...
class Dashboard_Performence_Excution extends BaseController
{
  ...
  */
  ...
  public function listing_dash_perform_exec() 
  {
    ...
    $query_principal="SELECT inst_institutions.DESCRIPTION_INSTITUTION AS  INTITULE_MINISTERE, inst_institutions_programmes.INTITULE_PROGRAMME,inst_institutions_actions.`LIBELLE_ACTION`,pap_activites.DESC_PAP_ACTIVITE,pap_activites.RESULTAT_PAP_ACTIVITE,execution.ENG_BUDGETAIRE,execution.LIQUIDATION,execution.DECAISSEMENT,execution.ENG_JURIDIQUE,execution.ORDONNANCEMENT,execution.PAIEMENT,ptba_tache.GRANDE_MASSE_ID,DESCRIPTION_GRANDE_MASSE FROM execution_budgetaire execution JOIN ptba_tache ON execution.PTBA_TACHE_ID=ptba_tache.PTBA_TACHE_ID JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=ptba_tache.INSTITUTION_ID JOIN inst_institutions_programmes ON inst_institutions_programmes.PROGRAMME_ID=ptba_tache.PROGRAMME_ID JOIN inst_institutions_actions ON inst_institutions_actions.ACTION_ID=ptba_tache.ACTION_ID JOIN inst_grande_masse ON inst_grande_masse.GRANDE_MASSE_ID = ptba_tache.GRANDE_MASSE_ID LEFT JOIN execution_budgetaire_tache_detail detail ON detail.EXECUTION_BUDGETAIRE_ID = execution.EXECUTION_BUDGETAIRE_ID JOIN pap_activites ON pap_activites.PAP_ACTIVITE_ID=ptba_tache.PAP_ACTIVITE_ID  WHERE 1 ".$cond." ".$cond_trim."";
    $limit = 'LIMIT 0,1000';
    if ($_POST['length'] != -1)
    {
    ...
    $order_by = '';
    $order_column = array(1,'inst_institutions.DESCRIPTION_INSTITUTION ','inst_institutions_programmes.INTITULE_PROGRAMME','inst_institutions_actions.LIBELLE_ACTION','DESC_PAP_ACTIVITE','DESCRIPTION_GRANDE_MASSE','execution.ENG_BUDGETAIRE','execution.ENG_JURIDIQUE','execution.LIQUIDATION','execution.ORDONNANCEMENT','execution.PAIEMENT','execution.DECAISSEMENT');
    ...
    $search = !empty($_POST['search']['value']) ? ("AND (inst_institutions.DESCRIPTION_INSTITUTION  LIKE '%$var_search%' OR inst_institutions_programmes.INTITULE_PROGRAMME LIKE '%$var_search%' OR inst_institutions_actions.LIBELLE_ACTION LIKE '%$var_search%' OR DESC_PAP_ACTIVITE LIKE '%$var_search%' OR RESULTAT_PAP_ACTIVITE LIKE '%$var_search%')") : '';
  ...
  function exporter($TYPE_INSTITUTION_ID='',$INSTITUTION_ID='',$PROGRAMME_ID='',$ACTION_ID='',$IS_PRIVATE='',$LIGNE_BUDGETAIRE='',$PAP_ACTIVITE_ID='')
  {
    ...
    if ($IS_PRIVATE==1) {
    $cond_trim=" AND (execution.TRIMESTRE_ID=1 or execution.TRIMESTRE_ID=0 )" ;
    }else if ($IS_PRIVATE==2) {
    $cond_trim=" AND execution.TRIMESTRE_ID=2" ;
    }else if ($IS_PRIVATE==3) {
    $cond_trim=" AND execution.TRIMESTRE_ID=3" ;
...
# fonction pour les details
    ...
    $var_search = addcslashes($var_search,"'");
 		...
 		$query_principal="SELECT SUBSTRING(ptba_tache.CODE_NOMENCLATURE_BUDGETAIRE, 5,3) service,".$totaux." AS total,inst_institutions.DESCRIPTION_INSTITUTION AS INTITULE_MINISTERE,inst_institutions_programmes.`INTITULE_PROGRAMME`,inst_institutions_actions.`LIBELLE_ACTION`, pap_activites.DESC_PAP_ACTIVITE, pap_activites.RESULTAT_PAP_ACTIVITE, execution.ENG_BUDGETAIRE, execution.LIQUIDATION, execution.DECAISSEMENT, execution.ENG_JURIDIQUE,execution.ORDONNANCEMENT,execution.PAIEMENT FROM execution_budgetaire execution JOIN ptba_tache ON execution.PTBA_TACHE_ID=ptba_tache.PTBA_TACHE_ID JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=ptba_tache.INSTITUTION_ID JOIN inst_institutions_programmes ON inst_institutions_programmes.PROGRAMME_ID=ptba_tache.PROGRAMME_ID JOIN inst_institutions_actions ON inst_institutions_actions.ACTION_ID=ptba_tache.ACTION_ID JOIN pap_activites ON pap_activites.PAP_ACTIVITE_ID=ptba_tache.PAP_ACTIVITE_ID  WHERE 1 ".$cond." ".$cond_trim."";
    ...
    $order_by = '';
    $order_column = array(1,'DESC_PAP_ACTIVITE','RESULTAT_PAP_ACTIVITE','inst_institutions_actions.LIBELLE_ACTION','inst_institutions_programmes.inst_institutions_programmes.`INTITULE_PROGRAMME`','inst_institutions.DESCRIPTION_INSTITUTION',1,1);

    $order_by = isset($_POST['order']) ? ' ORDER BY ' . $order_column[$_POST['order']['0']['column']] . '  ' . $_POST['order']['0']['dir'] : ' ORDER BY ptba_tache.PTBA_TACHE_ID ASC';
 	...
 	public function get_rapport()
  {
   ...
   $user_id=$inst_conn;
   $name_table= " JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=ptba_tache.INSTITUTION_ID JOIN inst_institutions_programmes ON inst_institutions_programmes.PROGRAMME_ID=ptba_tache.PROGRAMME_ID JOIN inst_institutions_actions ON inst_institutions_actions.ACTION_ID=ptba_tache.ACTION_ID";
   $cond_inst="";
   $cond_affectations1="";
   if (!empty($user_id))
   {
...
if(! empty($PROGRAMME_ID))
{
  if ($TYPE_INSTITUTION_ID==2) {
    ...
    $cond.=" AND inst_institutions_programmes.PROGRAMME_ID='".$PROGRAMME_ID."'";
...
if($ACTIVITE !='')
  {
  $cond.=" AND ptba_tache.CODE_NOMENCLATURE_BUDGETAIRE_ID=".$ACTIVITE.""; 
  }
 // si l'anné budgétaire est selectionnée
// if(!empty($ANNEE_BUDGETAIRE_ID))
// {
//   $cond.=" AND ptba_tache.ANNEE_BUDGETAIRE_ID=".$ANNEE_BUDGETAIRE_ID.""; 
// }
...
$engage=("SELECT SUM(execution_budgetaire.ENG_BUDGETAIRE) as engage FROM execution_budgetaire JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID ".$name_table."  WHERE 1 ".$cond_trim." ".$cond." ");


$liquide=("SELECT SUM(execution_budgetaire.LIQUIDATION) as liquide FROM execution_budgetaire JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID".$name_table."  WHERE 1 ".$cond_trim." ".$cond." ");
$decaissement=("SELECT SUM(execution_budgetaire.DECAISSEMENT) as decaissement FROM execution_budgetaire JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID".$name_table."  WHERE 1 ".$cond_trim." ".$cond."  ");
$jurdique=("SELECT SUM(execution_budgetaire.ENG_JURIDIQUE) as jurdique FROM execution_budgetaire JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID".$name_table."  WHERE 1 ".$cond_trim." ".$cond."  ");
$ordonence=("SELECT SUM(execution_budgetaire.ORDONNANCEMENT) as ordonence FROM execution_budgetaire JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID".$name_table."  WHERE 1 ".$cond_trim." ".$cond."  ");
$paiement=("SELECT SUM(execution_budgetaire.PAIEMENT) as paie FROM execution_budgetaire JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID".$name_table."  WHERE 1 ".$cond_trim." ".$cond."  ");
$tautaux=("SELECT ".$totaux." AS pourc FROM ptba_tache ".$name_table." WHERE 1 ".$cond." ");
...
foreach ($excute_grande_req as $value)
{
  if ($value->GRANDE_MASSE_ID==7) {
   $cond_gr=' AND ptba_tache.GRANDE_MASSE_ID  IN(6,7,8)';
 }else{
   $cond_gr= ' AND ptba_tache.GRANDE_MASSE_ID='.$value->GRANDE_MASSE_ID;	
 }
 $engage1=("SELECT SUM(execution_budgetaire.ENG_BUDGETAIRE) as engage FROM execution_budgetaire JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID ".$name_table."  WHERE 1 ".$cond_trim." ".$cond."  ".$cond_gr."");

 $liquide1=("SELECT SUM(execution_budgetaire.LIQUIDATION) as liquide FROM execution_budgetaire JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID ".$name_table."  WHERE 1 ".$cond_trim." ".$cond."  ".$cond_gr."");
 $decaissement1=("SELECT SUM(execution_budgetaire.DECAISSEMENT) as decaissement FROM  execution_budgetaire JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID ".$name_table."  WHERE 1 ".$cond_trim." ".$cond."  ".$cond_gr." ");
 $jurdique1=("SELECT SUM(execution_budgetaire.ENG_JURIDIQUE) as jurdique FROM  execution_budgetaire JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID ".$name_table."  WHERE 1 ".$cond_trim." ".$cond."  ".$cond_gr."");
 $ordonence1=("SELECT SUM(execution_budgetaire.ORDONNANCEMENT) as ordonence FROM  execution_budgetaire JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID ".$name_table."  WHERE 1 ".$cond_trim." ".$cond."   ".$cond_gr."");
 $paiement1=("SELECT SUM(execution_budgetaire.PAIEMENT) as paie FROM  execution_budgetaire JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID ".$name_table."  WHERE 1 ".$cond_trim." ".$cond."  ".$cond_gr."");

 $tautaux1=("SELECT ".$totaux." AS pourc FROM ptba_tache JOIN execution_budgetaire ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID ".$name_table." WHERE 1 ".$cond_gr." ".$cond."");
