...
class Rapport_Objectif_Source_Financement extends BaseController
{
	protected $session;
	protected $ModelPs;
    ...
    if($session->get('SESSION_SUIVIE_PTBA_TABLEAU_BORD_PIP_OBJECTIF_STRATEGIQUE')!=1)
    {
      return redirect('Login_Ptba/homepage');
		...
		return view('App\Modules\dashboard\Views\Rapport_Objectif_Source_Financement_View',$data);
	}
	...
	public function get_rapport()
	{
		...
		$pillier="SELECT pip_source_financement_bailleur.ID_SOURCE_FINANCE_BAILLEUR AS ID,pip_source_financement_bailleur.NOM_SOURCE_FINANCE AS NAME,SUM(`TOTAl_BIF`) AS NBRE  FROM `pip_demande_source_financement` JOIN pip_demande_infos_supp on pip_demande_infos_supp.ID_DEMANDE_INFO_SUPP=pip_demande_source_financement.ID_DEMANDE_INFO_SUPP JOIN pip_demande_source_financement_valeur_cible ON pip_demande_source_financement_valeur_cible.ID_DEMANDE_SOURCE_FINANCEMENT=pip_demande_source_financement.ID_DEMANDE_SOURCE_FINANCEMENT JOIN pip_source_financement_bailleur ON pip_source_financement_bailleur.ID_SOURCE_FINANCE_BAILLEUR=pip_demande_source_financement.ID_SOURCE_FINANCE_BAILLEUR WHERE 1  AND  pip_demande_infos_supp.IS_FINISHED=1 AND  pip_demande_infos_supp.IS_ANNULER=0  AND pip_demande_source_financement_valeur_cible.ANNEE_BUDGETAIRE_ID!=1 GROUP BY pip_source_financement_bailleur.ID_SOURCE_FINANCE_BAILLEUR,pip_source_financement_bailleur.NOM_SOURCE_FINANCE ".$condition." ".$cond."";
    ...
    // print_r($projet_Statuts);die();
		...
		Highcharts.chart('container', {                    
			chart: {
				...
				title: {
					text: '<b>".lang("messages_lang.pip_rapport_source_fina")." :: ".number_format($data_total,0,'',' ')." BIF</b>',
					},
					subtitle: {
						text: 'Le ".date('d-m-Y')." '
						},
												...
												events: {
													click: function(){
														...
														$(\"#mytableppppp\").DataTable({
															...
															\"oreder\":[],
															\"ajax\":{
																url:\"".base_url('dashboard/Rapport_Objectif_Source_Financement/detail_strategique')."\",
																type:\"POST\",
																data:{
																	key:this.key,
...
public function liste($value = 0)
  ...
  $search = !empty($_POST['search']['value']) ? (' AND (NOM_PROJET LIKE "%' . $var_search . '%" OR pip_demande_infos_supp.ID_DEMANDE_INFO_SUPP LIKE "%' . $var_search . '%" OR DESCRIPTION_INSTITUTION LIKE "%' . $var_search . '%" OR NOM_PROJET LIKE "%' . $var_search . '%" OR pip_demande_infos_supp.ID_DEMANDE_INFO_SUPP LIKE "%' . $var_search . '%" OR DESCR_AXE_INTERVATION_PND LIKE "%' . $var_search . '%")') : '';
  $conditions = $critaire . ' ' . $search . ' ' . $group . ' ' . $order_by . ' ' . $limit;
  $conditionsfilter = $critaire . ' ' . $search . ' ' . $group;
  $requetedebase = 'SELECT  pip_source_financement_bailleur.NOM_SOURCE_FINANCE,`TOTAl_BIF`,inst_institutions.CODE_INSTITUTION,pip_taux_echange.TAUX,pip_demande_source_financement.TAUX_ECHANGE_ID,`ID_DEMANDE_SOURCE_FINANCEMENT`,pip_demande_infos_supp.ID_DEMANDE_INFO_SUPP,DESCR_STATUT_PROJET,objectif_strategique.DESCR_OBJECTIF_STRATEGIC,DESCR_AXE_INTERVATION_PND,pip_demande_infos_supp.NOM_PROJET,inst_institutions.DESCRIPTION_INSTITUTION,pilier.DESCR_PILIER,inst_institutions_programmes.INTITULE_PROGRAMME FROM `pip_demande_infos_supp`  JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=pip_demande_infos_supp.INSTITUTION_ID  JOIN objectif_strategique ON objectif_strategique.ID_OBJECT_STRATEGIQUE=pip_demande_infos_supp.ID_OBJECT_STRATEGIQUE JOIN axe_intervention_pnd ON axe_intervention_pnd.ID_AXE_INTERVENTION_PND=pip_demande_infos_supp.ID_AXE_INTERVENTION_PND JOIN pip_statut_projet statut ON statut.ID_STATUT_PROJET=pip_demande_infos_supp.ID_STATUT_PROJET JOIN pilier on pilier.ID_PILIER=pip_demande_infos_supp.ID_PILIER JOIN inst_institutions_programmes ON
  inst_institutions_programmes.PROGRAMME_ID=pip_demande_infos_supp.ID_PROGRAMME  JOIN pip_demande_source_financement ON pip_demande_source_financement.ID_DEMANDE_INFO_SUPP=pip_demande_infos_supp.ID_DEMANDE_INFO_SUPP JOIN pip_source_financement_bailleur ON pip_source_financement_bailleur.ID_SOURCE_FINANCE_BAILLEUR=pip_demande_source_financement.ID_SOURCE_FINANCE_BAILLEUR JOIN pip_taux_echange ON pip_taux_echange.TAUX_ECHANGE_ID=pip_demande_source_financement.TAUX_ECHANGE_ID WHERE 1 AND  pip_demande_infos_supp.IS_FINISHED=1 AND  pip_demande_infos_supp.IS_ANNULER=0  '.$critere.' '.$critere1.'  ';
  ...
  foreach ($fetch_projets as $row)
  {
    ...
    {
      $baille[] ='-'.$baill->NOM_SOURCE_FINANCE." : ".number_format($baill->TOTAl_BIF, '0', ',', ' ')."</br>";
    }
    ...
    if (strlen($row->NOM_SOURCE_FINANCE) > 12) 
    {
      $sub_array[] = mb_substr($row->NOM_SOURCE_FINANCE, 0, 11) .'...<a class="btn-sm" data-toggle="tooltip" title="'.$row->NOM_SOURCE_FINANCE.'"><i class="fa fa-eye"></i></a>';
    }else{
      $sub_array[] ='<font color="#000000" ><label>'.$row->NOM_SOURCE_FINANCE.'</label></font>';
    }
    $sub_array[] = number_format($row->TOTAl_BIF, '0', ',', ' ');
    $anne1 = 0;
    $anne2 = 0;
    $anne3 = 0;
    if (isset($valeur_cible_anne[0])) {
      $anne1 = $valeur_cible_anne[0]->SOURCE_FINANCEMENT_VALEUR_CIBLE * $row->TAUX;
    }
    if (isset($valeur_cible_anne[1])) {
      $anne2 = $valeur_cible_anne[1]->SOURCE_FINANCEMENT_VALEUR_CIBLE * $row->TAUX;
    }
    if (isset($valeur_cible_anne[2]))
    {
      $anne3 = $valeur_cible_anne[2]->SOURCE_FINANCEMENT_VALEUR_CIBLE * $row->TAUX;
    }
  ...
  function exporter($INSTITUTION_ID,$ID_AXE_INTERVENTION_PND)
  {
    ...
    $getRequete="SELECT  pip_source_financement_bailleur.NOM_SOURCE_FINANCE,`TOTAl_BIF`,pip_taux_echange.TAUX,pip_demande_source_financement.TAUX_ECHANGE_ID,`ID_DEMANDE_SOURCE_FINANCEMENT`,pip_demande_infos_supp.ID_DEMANDE_INFO_SUPP,DESCR_STATUT_PROJET,objectif_strategique.DESCR_OBJECTIF_STRATEGIC,DESCR_AXE_INTERVATION_PND,pip_demande_infos_supp.NOM_PROJET,inst_institutions.DESCRIPTION_INSTITUTION,pilier.DESCR_PILIER,inst_institutions_programmes.INTITULE_PROGRAMME FROM `pip_demande_infos_supp`  JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=pip_demande_infos_supp.INSTITUTION_ID  JOIN objectif_strategique ON objectif_strategique.ID_OBJECT_STRATEGIQUE=pip_demande_infos_supp.ID_OBJECT_STRATEGIQUE JOIN axe_intervention_pnd ON axe_intervention_pnd.ID_AXE_INTERVENTION_PND=pip_demande_infos_supp.ID_AXE_INTERVENTION_PND JOIN pip_statut_projet statut ON statut.ID_STATUT_PROJET=pip_demande_infos_supp.ID_STATUT_PROJET JOIN pilier on pilier.ID_PILIER=pip_demande_infos_supp.ID_PILIER JOIN inst_institutions_programmes ON
       inst_institutions_programmes.PROGRAMME_ID=pip_demande_infos_supp.ID_PROGRAMME  JOIN pip_demande_source_financement ON pip_demande_source_financement.ID_DEMANDE_INFO_SUPP=pip_demande_infos_supp.ID_DEMANDE_INFO_SUPP JOIN pip_source_financement_bailleur ON pip_source_financement_bailleur.ID_SOURCE_FINANCE_BAILLEUR=pip_demande_source_financement.ID_SOURCE_FINANCE_BAILLEUR JOIN pip_taux_echange ON pip_taux_echange.TAUX_ECHANGE_ID=pip_demande_source_financement.TAUX_ECHANGE_ID WHERE 1 AND  pip_demande_infos_supp.IS_FINISHED=1 AND  pip_demande_infos_supp.IS_ANNULER=0 ".$criteres." ";
    ...
    $sheet->setCellValue('D1', 'BAILLEUR');
    $sheet->setCellValue('E1', 'MONTANT');
    $sheet->setCellValue('F1', 'BUDGET 2024-2025');
    $sheet->setCellValue('G1', 'BUDGET 2025-2026');
    $sheet->setCellValue('H1', 'BUDGET 2026-2027');
    $rows = 3;
    ...
    foreach ($getData as $key)
      ...
      $baille=array();
       foreach ($bailleurs as $baill) 
         {
      $baille[] =$baill->NOM_SOURCE_FINANCE." : ".number_format($baill->TOTAl_BIF, '0', ',', ' ')." ".",";
           }
       $baillee = implode(" ", $baille);
      $callpsreq= "CALL getRequete(?,?,?,?);";
      $table1 = "pip_demande_livrable";
      ...
      $anne3 = 0;
       if (isset($valeur_cible_anne[0])) {
        $anne1 = $valeur_cible_anne[0]->SOURCE_FINANCEMENT_VALEUR_CIBLE * $key->TAUX;
        }
      if (isset($valeur_cible_anne[1])) {
        $anne2 = $valeur_cible_anne[1]->SOURCE_FINANCEMENT_VALEUR_CIBLE * $key->TAUX;
       }
      if (isset($valeur_cible_anne[2]))
       {
        $anne3 = $valeur_cible_anne[2]->SOURCE_FINANCEMENT_VALEUR_CIBLE * $key->TAUX;
       }
    ...
    $sheet->setCellValue('D' . $rows, $key->NOM_SOURCE_FINANCE);
    $sheet->setCellValue('E' . $rows, $key->TOTAl_BIF);
    $sheet->setCellValue('F' . $rows, $anne1);
    $sheet->setCellValue('G' . $rows, $anne2);
    $sheet->setCellValue('H' . $rows, $anne3);
    $rows++;
      } 
    $writer = new Xlsx($spreadsheet);
    $writer->save('world.xlsx');
    return $this->response->download('world.xlsx', null)->setFileName('PIP par Source de Financement.xlsx');
    return redirect('dashboard/Rapport_Pip_Source_Financement');
  }
