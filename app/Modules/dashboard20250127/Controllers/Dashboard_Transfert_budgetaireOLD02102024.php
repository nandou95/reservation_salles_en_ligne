   ...
   */
    ...
    //fonction index
 	...
 	public function get_rapport()
 	{
 		...
 		// {
 		// 	$cond_budg=" AND ptba_tache.ANNEE_BUDGETAIRE_ID=".$ANNEE_BUDGETAIRE_ID."";
 		// }
 		...
 		foreach ($budget_req as $value)
 		{
 			...
 			$data_budget_req.="{name:'".$this->str_replacecatego($value->Name)." (".number_format($value->enga,0,',',' ')." BIF)', y:".$value->enga.",color:'#000080',key:'".$this->str_replacecatego($value->ID)."',key2:".$KEY2."},";
...
function listing()
{
	...
	// }
	$var_search = !empty($_POST['search']['value']) ? $_POST['search']['value'] : null; 
   	$query_principal="SELECT `PTBA_TACHE_ID_RECEPTION`,trimestre.DESC_TRIMESTRE,inst_institutions.DESCRIPTION_INSTITUTION as INTITULE_MINISTERE, inst_institutions_programmes.INTITULE_PROGRAMME,inst_institutions_actions.LIBELLE_ACTION,`DESC_TACHE`,ptba_tache.RESULTAT_ATTENDUS_TACHE as RESULTATS_ATTENDUS,REPLACE(RTRIM(transfert_historique_transfert.`MONTANT_TRANSFERT`),' ','') AS MONTANT_A_TRANSFERE, date_format(transfert_historique_transfert.DATE_ACTION,'%d-%m-%Y') as dat FROM `transfert_historique_transfert` LEFT JOIN trimestre ON trimestre.TRIMESTRE_ID=transfert_historique_transfert.TRIMESTRE_ID JOIN ptba_tache on transfert_historique_transfert.PTBA_TACHE_ID_TRANSFERT=ptba_tache.PTBA_TACHE_ID JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=ptba_tache.INSTITUTION_ID LEFT JOIN execution_budgetaire ON execution_budgetaire.PTBA_TACHE_ID=ptba_tache.PTBA_TACHE_ID LEFT JOIN execution_budgetaire_tache_detail sup ON sup.EXECUTION_BUDGETAIRE_ID=execution_budgetaire.EXECUTION_BUDGETAIRE_ID  JOIN inst_institutions_programmes ON inst_institutions_programmes.PROGRAMME_ID=ptba_tache.PROGRAMME_ID JOIN inst_institutions_actions ON inst_institutions_actions.ACTION_ID=ptba_tache.ACTION_ID  WHERE 1 ".$cond1."  ";
	$limit='LIMIT 0,10';
	if ($_POST['length'] != -1)
  {
    $limit = 'LIMIT ' . $_POST["start"] . ',' . $_POST["length"];
