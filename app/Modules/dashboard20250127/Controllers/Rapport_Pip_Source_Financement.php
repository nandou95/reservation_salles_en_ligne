...
class Rapport_Pip_Source_Financement extends BaseController
{
  protected $session;
  protected $ModelPs;
  ...
  { 
    ...
    return view('App\Modules\dashboard\Views\Rapport_Pip_Source_Financement_View',$data);
  }
...
\"oreder\":[],
\"ajax\":{
url:\"".base_url('dashboard/Rapport_Pip_Source_Financement/detail_inst')."\",
type:\"POST\",
data:{
key:this.key,         
...
public function get_lieux()
 ...
 $critere=" AND pip_valeur_nomenclature_livrable.ANNEE_BUDGETAIRE_ID=".$KEY;
 ...
 foreach ($fetch_data as $row) 
 {
  ...
  if (strlen($row->NOM_PROJET) > 10){
    $executio[] = mb_substr($row->NOM_PROJET, 0, 10) .'...<a class="btn-sm" data-toggle="tooltip" title="'.$row->NOM_PROJET.'"><i class="fa fa-eye"></i></a>';
  }else{
    $executio[] ='<font color="#000000"><label>'.$row->NOM_PROJET.'</label></font>';
  }
  if (strlen($row->DESCRIPTION_INSTITUTION) > 10){
    $executio[] = mb_substr($row->DESCRIPTION_INSTITUTION, 0, 10) .'...<a class="btn-sm" data-toggle="tooltip" title="'.$row->DESCRIPTION_INSTITUTION.'"><i class="fa fa-eye"></i></a>';
  }else{
  ...
  $executio[] ='<font color="#000000"><label>'.number_format($row->MONTANT_NOMENCALTURE,0,'',' ').'</label></font>';
  $data[] = $executio;        
}
