	  ...
	  public function getBindParms($columnselect, $table, $where, $orderby)
	  {
       ...
       $cond.=" AND p.PAP_ACTIVITE_ID=".$PAP_ACTIVITE_ID."";
        ...
        $cond.=" AND p.CODE_NOMENCLATURE_BUDGETAIRE_ID=".$LIGNE_BUDGETAIRE.""; 
	 			...
	 			$getRequete="SELECT racc.EXECUTION_BUDGETAIRE_ID,inst.DESCRIPTION_INSTITUTION,inst.CODE_INSTITUTION,pap_activites.DESC_PAP_ACTIVITE,inst_institutions_programmes.INTITULE_PROGRAMME,inst_institutions_ligne_budgetaire.LIBELLE_CODE_NOMENCLATURE_BUDGETAIRE,inst_institutions_actions.LIBELLE_ACTION,inst_institutions_actions.CODE_ACTION,p.DESC_TACHE,racc.TRIMESTRE_ID,inst.TYPE_INSTITUTION_ID,racc.ENG_BUDGETAIRE,racc.ENG_JURIDIQUE,racc.LIQUIDATION,racc.ORDONNANCEMENT,racc.PAIEMENT,racc.DECAISSEMENT FROM execution_budgetaire racc JOIN ptba_tache p ON racc.PTBA_TACHE_ID=p.PTBA_TACHE_ID JOIN inst_institutions inst ON p.INSTITUTION_ID=inst.INSTITUTION_ID  JOIN inst_institutions_programmes ON inst_institutions_programmes.PROGRAMME_ID=p.PROGRAMME_ID JOIN inst_institutions_actions ON inst_institutions_actions.ACTION_ID=p.ACTION_ID LEFT JOIN pap_activites ON pap_activites.PAP_ACTIVITE_ID=p.PAP_ACTIVITE_ID LEFT JOIN inst_institutions_ligne_budgetaire ON inst_institutions_ligne_budgetaire.CODE_NOMENCLATURE_BUDGETAIRE_ID=p.CODE_NOMENCLATURE_BUDGETAIRE_ID WHERE 1  ".$cond."  ORDER BY inst.CODE_INSTITUTION ASC";
     	...
     	# fonction pour les details
	 		function detail_Comparaison_Budget() 
	 		{
	 			...
	 			$var_search = !empty($_POST['search']['value']) ? $_POST['search']['value'] : null; 
	 			if ($KEY2==11) {
	 				$query_principal="SELECT p.PTBA_TACHE_ID as ID,".$totaux."-if((SELECT  COALESCE(raccro.ENG_BUDGETAIRE, 0) AS total  FROM execution_budgetaire AS raccro  JOIN ptba_tache  p ON p.PTBA_TACHE_ID=raccro.PTBA_TACHE_ID JOIN inst_institutions inst ON inst.INSTITUTION_ID=p.INSTITUTION_ID WHERE p.PTBA_TACHE_ID=ID ".$cond_trim." ".$cond1." ".$cond.")>=0,(SELECT  COALESCE(raccro.ENG_BUDGETAIRE, 0) AS total  FROM execution_budgetaire AS raccro  JOIN ptba_tache  p ON p.PTBA_TACHE_ID=raccro.PTBA_TACHE_ID JOIN inst_institutions inst ON inst.INSTITUTION_ID=p.INSTITUTION_ID WHERE p.PTBA_TACHE_ID=ID ".$cond_trim." ".$cond." ".$cond1."),0) AS non_realise,inst.DESCRIPTION_INSTITUTION,inst_institutions_programmes.INTITULE_PROGRAMME,inst_institutions_actions.LIBELLE_ACTION,p.DESC_TACHE, date_format(raccro.DATE_DEMANDE,'%d-%m-%Y') as dat  FROM ptba_tache AS p JOIN inst_institutions inst ON inst.INSTITUTION_ID=p.INSTITUTION_ID LEFT JOIN execution_budgetaire AS raccro ON raccro.PTBA_TACHE_ID=p.PTBA_TACHE_ID LEFT JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_ID=raccro.EXECUTION_BUDGETAIRE_ID JOIN inst_institutions_programmes ON inst_institutions_programmes.PROGRAMME_ID=p.PROGRAMME_ID JOIN inst_institutions_actions ON inst_institutions_actions.ACTION_ID=p.ACTION_ID WHERE 1 ".$cond." ".$cond1."";
	 			}else if ($KEY2==12 OR $KEY2==13) {
	 				$query_principal="SELECT  p.PTBA_TACHE_ID as ID, ".$totaux." AS monta,DESCRIPTION_INSTITUTION,inst_institutions_programmes.INTITULE_PROGRAMME, inst_institutions_actions.LIBELLE_ACTION,p.DESC_TACHE, date_format(raccro.DATE_DEMANDE,'%d-%m-%Y') as dat FROM  ptba_tache p JOIN inst_institutions inst ON inst.INSTITUTION_ID=p.INSTITUTION_ID LEFT JOIN execution_budgetaire raccro ON raccro.PTBA_TACHE_ID=p.PTBA_TACHE_ID LEFT JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_ID=raccro.EXECUTION_BUDGETAIRE_ID JOIN inst_institutions_programmes ON inst_institutions_programmes.PROGRAMME_ID=p.PROGRAMME_ID JOIN inst_institutions_actions ON inst_institutions_actions.ACTION_ID=p.ACTION_ID WHERE 1 ".$cond." ".$cond1." ".$cr_key."  ";
	 			}else{
	 				$query_principal="SELECT p.PTBA_TACHE_ID as ID,COALESCE(raccro.ENG_BUDGETAIRE, 0) AS total ,DESCRIPTION_INSTITUTION,inst_institutions_programmes.INTITULE_PROGRAMME,inst_institutions_actions.LIBELLE_ACTION,p.DESC_TACHE, date_format(raccro.DATE_DEMANDE,'%d-%m-%Y') as dat FROM execution_budgetaire AS raccro  JOIN ptba_tache  p ON p.PTBA_TACHE_ID=raccro.PTBA_TACHE_ID JOIN inst_institutions inst ON inst.INSTITUTION_ID=p.INSTITUTION_ID LEFT JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_ID=raccro.EXECUTION_BUDGETAIRE_ID JOIN inst_institutions_programmes ON inst_institutions_programmes.PROGRAMME_ID=p.PROGRAMME_ID JOIN inst_institutions_actions ON inst_institutions_actions.ACTION_ID=p.ACTION_ID WHERE 1 ".$cond."  ".$cond1."  ".$cr_key." ".$cond_trim." ";
	 			}
        ...
        $PAP_ACTIVITE_ID=$this->request->getVar('PAP_ACTIVITE_ID');
          ...
          $name_decl= "pap_activites.DESC_PAP_ACTIVITE"; 
	 			...
	 			$data_real='';
	 			$data_realnon='';
	 			$realisation_non=("SELECT SUM(COALESCE(`ENG_BUDGETAIRE`, 0)) AS realise FROM execution_budgetaire AS raccro  JOIN ptba_tache  p ON p.PTBA_TACHE_ID=raccro.PTBA_TACHE_ID JOIN inst_institutions inst ON inst.INSTITUTION_ID=p.INSTITUTION_ID JOIN inst_institutions_programmes ON inst_institutions_programmes.PROGRAMME_ID=p.PROGRAMME_ID JOIN inst_institutions_actions ON inst_institutions_actions.ACTION_ID=p.ACTION_ID JOIN inst_institutions_ligne_budgetaire ON inst_institutions_ligne_budgetaire.CODE_NOMENCLATURE_BUDGETAIRE_ID=p.CODE_NOMENCLATURE_BUDGETAIRE_ID JOIN pap_activites ON pap_activites.PAP_ACTIVITE_ID=p.PAP_ACTIVITE_ID WHERE 1 ".$cond." ".$cond_budg." ".$cond_inst." ".$cond_trim."");
	 			...
	 			foreach ($excute_grande_req as $value)
	 			{
	 				...
	 				$execution=("SELECT SUM(COALESCE(`ENG_BUDGETAIRE`, 0)) AS liquide FROM `execution_budgetaire` AS raccro JOIN ptba_tache p ON p.PTBA_TACHE_ID=raccro.PTBA_TACHE_ID JOIN inst_institutions inst ON inst.INSTITUTION_ID=p.INSTITUTION_ID JOIN inst_institutions_programmes ON inst_institutions_programmes.PROGRAMME_ID=p.PROGRAMME_ID JOIN inst_institutions_actions ON inst_institutions_actions.ACTION_ID=p.ACTION_ID JOIN inst_institutions_ligne_budgetaire ON inst_institutions_ligne_budgetaire.CODE_NOMENCLATURE_BUDGETAIRE_ID=p.CODE_NOMENCLATURE_BUDGETAIRE_ID JOIN pap_activites ON pap_activites.PAP_ACTIVITE_ID=p.PAP_ACTIVITE_ID WHERE 1 ".$cond_budg."  ".$cond_gr." ".$cond." ".$cond_trim." ".$cond_inst." ");
	 			...
	 			$monta_compare=("SELECT ".$id_decl." AS ID,".$name_decl." AS name,".$totaux." AS vote,COUNT(PTBA_TACHE_ID)ptb_vote,(SELECT SUM(COALESCE(raccro.ENG_BUDGETAIRE, 0)) AS realise FROM execution_budgetaire AS raccro  JOIN ptba_tache  p ON p.PTBA_TACHE_ID=raccro.PTBA_TACHE_ID JOIN inst_institutions inst ON inst.INSTITUTION_ID=p.INSTITUTION_ID WHERE ".$id_decl."=ID ".$cond." ".$cond_inst." ".$cond_trim.") AS exc,(SELECT COUNT(p.PTBA_TACHE_ID) FROM execution_budgetaire AS raccro  JOIN ptba_tache  p ON p.PTBA_TACHE_ID=raccro.PTBA_TACHE_ID JOIN inst_institutions inst ON inst.INSTITUTION_ID=p.INSTITUTION_ID WHERE ".$id_decl."=ID ".$cond." ".$cond_budg." ".$cond_inst." ".$cond_trim.") AS ptb_exc FROM `ptba_tache` AS p JOIN inst_institutions inst ON inst.INSTITUTION_ID=p.INSTITUTION_ID JOIN inst_institutions_programmes ON inst_institutions_programmes.PROGRAMME_ID=p.PROGRAMME_ID JOIN inst_institutions_actions ON inst_institutions_actions.ACTION_ID=p.ACTION_ID JOIN inst_institutions_ligne_budgetaire ON inst_institutions_ligne_budgetaire.CODE_NOMENCLATURE_BUDGETAIRE_ID=p.CODE_NOMENCLATURE_BUDGETAIRE_ID JOIN pap_activites ON pap_activites.PAP_ACTIVITE_ID=p.PAP_ACTIVITE_ID WHERE 1 ".$cond_inst." ".$cond."  GROUP BY ".$id_decl.",".$name_decl." ORDER BY vote DESC ");
