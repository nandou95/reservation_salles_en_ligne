...
// {
//   $cond.=" AND execution_budgetaire.IS_DOUBLE_COMMANDE=".$IS_DOUBLE_COMMANDE."";
// }

$engage11=("SELECT ".$name_decl." AS name,".$id_decl." as ID,COUNT(execution_budgetaire.EXECUTION_BUDGETAIRE_ID) as engage FROM `execution_budgetaire` JOIN ptba_tache ON ptba_tache.PTBA_TACHE_ID=execution_budgetaire.PTBA_TACHE_ID ".$name_table." WHERE ptba_tache.BUDGET_T1<execution_budgetaire.ENG_BUDGETAIRE ".$cond_trim." ".$cond." ".$cond_pri1." GROUP BY ".$name_decl.",".$id_decl." ORDER by inst_institutions.INSTITUTION_ID ASC");
...
{
 $u++;
 $date_engage=("SELECT DATE_DEMANDE,DATE_ENG_JURIDIQUE,DATE_LIQUIDATION,DATE_ORDONNANCEMENT,DATE_PAIEMENT,DATE_DECAISSENMENT FROM execution_budgetaire_tache_detail JOIN execution_budgetaire ON execution_budgetaire.EXECUTION_BUDGETAIRE_ID=execution_budgetaire_tache_detail.EXECUTION_BUDGETAIRE_ID WHERE execution_budgetaire.EXECUTION_BUDGETAIRE_ID=".$row->EXECUTION_BUDGETAIRE_ID."");
...
$var_search = !empty($_POST['search']['value']) ? $_POST['search']['value'] : null; 
$query_principal="SELECT inst_institutions.DESCRIPTION_INSTITUTION,EXECUTION_BUDGETAIRE_ID,ptba_tache.PTBA_TACHE_ID,inst_institutions_programmes.INTITULE_PROGRAMME,inst_institutions_actions.LIBELLE_ACTION,`DESC_TACHE`,activite.DATE_ENG_JURIDIQUE,activite.DATE_DEMANDE,`RESULTAT_ATTENDUS_TACHE`,BUDGET_T1,if(activite.`ENG_BUDGETAIRE`>0,activite.`ENG_BUDGETAIRE`,0) AS ENG_BUDGETAIRE FROM execution_budgetaire activite   JOIN ptba_tache ON activite.PTBA_TACHE_ID=ptba_tache.PTBA_TACHE_ID JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=ptba_tache.INSTITUTION_ID JOIN inst_institutions_programmes ON inst_institutions_programmes.PROGRAMME_ID=ptba_tache.PROGRAMME_ID JOIN inst_institutions_actions ON inst_institutions_actions.ACTION_ID=ptba_tache.ACTION_ID ".$name_table." WHERE ptba_tache.BUDGET_T1<activite.ENG_BUDGETAIRE ".$cond1."  ".$cond." ".$cond_trim." ";
...
//   $cond1=" AND ptba_tache.ANNEE_BUDGETAIRE_ID=".$ANNEE_BUDGETAIRE_ID;
// }
$getRequete="SELECT inst_institutions.DESCRIPTION_INSTITUTION,EXECUTION_BUDGETAIRE_ID,ptba_tache.PTBA_TACHE_ID,inst_institutions_programmes.INTITULE_PROGRAMME,inst_institutions_actions.LIBELLE_ACTION,`DESC_TACHE`,activite.DATE_ENG_JURIDIQUE,activite.DATE_DEMANDE,`RESULTAT_ATTENDUS_TACHE`,BUDGET_T1,if(activite.`ENG_BUDGETAIRE`>0,activite.`ENG_BUDGETAIRE`,0) AS ENG_BUDGETAIRE FROM execution_budgetaire activite   JOIN ptba_tache ON activite.PTBA_TACHE_ID=ptba_tache.PTBA_TACHE_ID JOIN inst_institutions ON inst_institutions.INSTITUTION_ID=ptba_tache.INSTITUTION_ID JOIN inst_institutions_programmes ON inst_institutions_programmes.PROGRAMME_ID=ptba_tache.PROGRAMME_ID JOIN inst_institutions_actions ON inst_institutions_actions.ACTION_ID=ptba_tache.ACTION_ID WHERE ptba_tache.BUDGET_T1<activite.ENG_BUDGETAIRE ".$cond1."  ".$cond." ".$cond_trim." ";
...
foreach ($getData as $key)
{
 ...
 $racc = "CALL `getTable`('" . $racc . "');";
 $raccrocher = $this->ModelPs->getRequeteOne($racc);
 $date_engage=("SELECT DATE_LIQUIDATION,DATE_ORDONNANCEMENT,DATE_PAIEMENT,DATE_DECAISSENMENT FROM execution_budgetaire_tache_detail WHERE EXECUTION_BUDGETAIRE_ID=".$key->EXECUTION_BUDGETAIRE_ID."");
 $date_engage_req=$this->ModelPs->getRequeteOne(' CALL getTable("'.$date_engage.'")');
