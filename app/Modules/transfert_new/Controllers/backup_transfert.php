...
class Transfert extends BaseController
{
	...
	public function index()
	{
		...
		$user = $this->getBindParms('USER_ID,INSTITUTION_ID,PROFIL_ID, IS_SOUS_TUTEL , SOUS_TUTEL_ID', 'user_users', 'USER_ID = '.$user_id.'' , ' USER_ID DESC');
		$getuser = $this->ModelPs->getRequeteOne($callpsreq, $user);
		
		$getInst  = $this->getBindParms('INSTITUTION_ID,CODE_INSTITUTION,DESCRIPTION_INSTITUTION', 'inst_institutions','INSTITUTION_ID='.$getuser ['INSTITUTION_ID'],' DESCRIPTION_INSTITUTION ASC');
		$data['institutions'] = $this->ModelPs->getRequeteOne($callpsreq, $getInst);
	...
	public function listing($value = 0)
	{
		...
		$requetedebase= 'SELECT LIBELLE,ENG_BUDGETAIRE,ENG_JURIDIQUE,LIQUIDATION,ORDONNANCEMENT,PAIEMENT,DECAISSEMENT,DATE_DEMANDE,IMPUTATION,EXECUTION_BUDGETAIRE_ID,IS_RACCROCHE,TRANSFERTS_CREDITS,CREDIT_APRES_TRANSFERT FROM execution_budgetaire_new WHERE IS_RACCROCHE = 0 AND IS_TRANSFERTS=1 AND IMPUTATION LIKE "'.$institution.'%"';
		...
		foreach ($fetch_data as $row)
		{
			...
			if ($userfiancier==1)
			{
				$sub_array[] = "<a  style='text-decoration:none'title='Transfert' href='".base_url("transfert_new/Transfert/getOne/".$row->EXECUTION_BUDGETAIRE_ID)."' >".$row->IMPUTATION."</a>";
			}
			else
			{
			...
			{	
				$BTN_TRAITE = "<a class='btn btn-primary btn-sm' title='Raccrocher' href='".base_url("transfert_new/Transfert/getOne/".$row->EXECUTION_BUDGETAIRE_ID)."' > <i class='fa fa-edit text-light'></i></a>";
			}
	...
	public function getOne($id)
	{
		...
		$columnselect='IMPUTATION,EXECUTION_BUDGETAIRE_ID,TRANSFERTS_CREDITS';
		$where="execution_budgetaire.EXECUTION_BUDGETAIRE_ID='".$id."'";
		...
		$table="ptba  JOIN execution_budgetaire_new ON execution_budgetaire_new.IMPUTATION = ptba.CODE_NOMENCLATURE_BUDGETAIRE";
		$columnselect='ACTIVITES,PTBA_ID,EXECUTION_BUDGETAIRE_ID,CODE_MINISTERE';
		...
		$ligne  = $this->getBindParms('CODE_NOMENCLATURE_BUDGETAIRE,PTBA_ID', 'ptba','1',' PTBA_ID ASC');
		...
		$table="ptba  JOIN execution_budgetaire_new ON execution_budgetaire_new.IMPUTATION = ptba.CODE_NOMENCLATURE_BUDGETAIRE JOIN inst_institutions on  inst_institutions.CODE_INSTITUTION=ptba.CODE_MINISTERE";
		$columnselect='INSTITUTION_ID';
		$where="ptba.CODE_NOMENCLATURE_BUDGETAIRE='".$imputation."'";
		$orderby=' CODE_NOMENCLATURE_BUDGETAIRE DESC';
		$where=str_replace("\'", "'", $where);
		$db = db_connect();
		...
		$tranche_params=$this->getBindParms('TRANCHE_ID,DESCRIPTION_TRANCHE','op_tranches', '1','TRANCHE_ID ASC');
		$data['tranches']= $this->ModelPs->getRequete($callpsreq, $tranche_params);
		return view('App\Modules\transfert\Views\Transfert_add_view',$data);		
	}
	...
	function get_montant($PTBA_ID)
	{
		...
		$callpsreq = "CALL `getRequete`(?,?,?,?);";
		$table="ptba";
		$columnselect='T1 as montant,CODE_MINISTERE,CODE_NOMENCLATURE_BUDGETAIRE,ACTIVITES,PTBA_ID,T1 as tranche,QT1 as qte,UNITE,CODE_ACTION,LIBELLE_ACTION,CODE_PROGRAMME,INTITULE_PROGRAMME';
	...
	public function get_code($ministere_id)
	{
		...
		$getcodeactivite = $this->getBindParms('CODE_NOMENCLATURE_BUDGETAIRE,PTBA_ID', 'ptba', 'CODE_NOMENCLATURE_BUDGETAIRE LIKE "'.$code.'%"', 'PTBA_ID  ASC');
	...
	public function get_activite($LIGNE_BUDG_ID)
	{
		...
		$tranche=$dataa['CODE_TRANCHE'];
		$callpsreq = "CALL `getRequete`(?,?,?,?);";		
		$getactive = $this->getBindParms('ACTIVITES,PTBA_ID,T1 as tranche','ptba','CODE_NOMENCLATURE_BUDGETAIRE='.$LIGNE_BUDG_ID,'PTBA_ID  ASC');
	...
	public function get_summ_activite($LIGNE_BUDG_ID)
	{
		...
		$get_summactivite = $this->getBindParms('sum(T1) as sommation','ptba','CODE_NOMENCLATURE_BUDGETAIRE='.$LIGNE_BUDG_ID,'PTBA_ID  ASC');
	...
	public function enregistre_tempo()
	{
		...
		$trimestre=$this->request->getPost('trim');
		...
		$datacolumsinsert= $type_operation_id.",".$user_id.",".$montant_transfer.",".$ptba.",".$mont_precise_transfert.",".$aactiv_transfert.",".$id.",'".$trimestre."',".$institustion_transfert.",".$ministere."";
	...
	public function save_transfert()
	{
		...
		{	
			...
			{
				return redirect('transfert/Transfert');
			}
			else
			{
