              ...
              <div style="box-shadow: rgba(100, 100, 111, 0.25) 0px 7px 29px 0px" class="card">
                <div class="card-header">
                  <div class="row col-md-12">
                    ...
                    <div class="col-md-3" style="float: right;">
                      <a href="<?php echo base_url('double_commande_new/Controles_Decaissement/correction_a_transmettre') ?>" style="float: right;margin-right: 20px" class="btn btn-primary"><i class="fa fa-list" aria-hidden="true"></i> <?= lang('messages_lang.list_transmission_du_bordereau') ?> </a>
                    </div>
               ...
               <br>
                ...
                <div class="card-body">
                  ...
                  <form id="my_form" action="<?= base_url('double_commande_new/Transmission_Brb_MinFin/save_transmission') ?>" method="POST" enctype="multipart/form-data">
             

                    <div class="row col-md-12">
                      <div class="col-md-6">
                        <br>
                        <label><?= lang('messages_lang.labelle_titre_dec') ?> <span style="color: red;">*</span></label>
                        <select class="form-control select2" multiple name="EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID[]" id="EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID">
                          ...
                          <?php  foreach ($get_info as $keys) { ?>
                            <?php if($keys->EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID==set_value('EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID')) { ?>
                              <option value="<?=$keys->EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID ?>" selected>
                                <?=$keys->TITRE_DECAISSEMENT ?></option>
                              <?php }else{?>
                          ...
                          <div class="col-md-6">
                            ...
                            <input type="date" class="form-control" name="DATE_TRANSMISSION" value="<?= $date_trans ?>" max="<?= date('Y-m-d') ?>" id="DATE_TRANSMISSION" onkeypress="return false" onblur="this.type='date'">
                            <?php if (isset($validation)) : ?>
            ...
            <div class="table-responsive  mt-3">
              <table class="table m-b-0 m-t-20">
                <tbody>
                  ...
                  <tr>
                    <td style="width:350px;"><i class="fa fa-cogs"></i> &nbsp;<strong><?= lang('messages_lang.labelle_titre_dec') ?></strong></td>
                    <td id="BON_ENGAGEMENT_VERIFY" class="text-dark"></td>
                  </tr>
                  <tr>
                    <td style="width:350px;"><i class="fa fa-calendar"></i> &nbsp;<strong><?= lang('messages_lang.date_transmission_transmission_Brb_MinFin') ?></strong></td>
...
<script type="text/javascript">
  ...
  if (DATE_TRANSMISSION === '') {
    statut = 1;
    $('#error_dat_trans').html("<?= lang('messages_lang.message_champs_obligatoire') ?>");
  } else {
    $('#error_dat_trans').html("");
  }
  if ($.isEmptyObject(EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID)) {
   $('#error_TD').html('<?= lang('messages_lang.message_champs_obligatoire') ?>');
   statut = 1;
 }
