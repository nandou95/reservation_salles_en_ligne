                    ...
                    <div class=" container " style="width:90%">
                      <form enctype='multipart/form-data' name="myEtape1" id="myEtape1" action="<?=base_url('double_commande_new/Introduction_Budget_Multi_Taches/save_etape1/')?>" method="post" >
                        <div class="container">
                            ...
                            <div class="col-md-7 mt-3 ml-2"style="margin-bottom:50px" >
                              <div class="row">
                                ...
                                <div class="col-md-6">
                                  ...
                                  <input type="text" class="form-control" id="NOTE_REFERENCE" name="NOTE_REFERENCE">
                                ...
                                <div class="col-md-3" >
                                  <div class="form-group " >
                                    <a onclick="valider_list()" id="btn_vqlider"  class="btn btn-primary" ><b id="loading_save"></b> <?= lang('messages_lang.btn_valider') ?><b id="loading_valider"></b></a>
                                  </div>
                                </div>
                              </div>
                              ...
                              <div class="row">
                                ...
                                <div class="col-md-6" id="id_res" hidden="true">
                                  <div class="form-froup">
                                    ...
                                    <input type="text" maxlength="20" class="form-control allownumericwithdecimal" name="resultat_attend" id="resultat_attend" placeholder="" value="<?=set_value('resultat_attend')?>" onpaste="return false;" min="1" onkeydown="qte();" onkeyup="qte();">
                          ...
                          <div class="table-responsive container" id="task_liste" style="display: none;">
                            <div></div>
                          ...
                          <div style="float: right;display: none;" class="col-md-2 mt-5 "  id="save_tasks">
                            <div class="form-group " >
  ...
  function save_tempo()
  {
    ...
    function (data) {
      ...
      if(data.status == true) 
      {
        ...
        $('#ENG_BUDGETAIRE').val('');
        $('#MONTANT_EN_DEVISE').val('');
        $('#TYPE_MONTANT_ID').val(data.TYPE_MONTANT_ID);
        if(data.TYPE_MONTANT_ID != 1)
        {
          $('#engagement_cous').prop('readonly', true);          
          $('#DATE_COUT_DEVISE').prop('readonly', true);
         
        }

        $('#COMMENTAIRE').prop('readonly', true);

        $('select[name="TYPE_MONTANT_ID"]').css({
          'pointer-events': 'none',
          'background-color': '#f4f4f4',
          'color': '#777'
        });

        $('#TYPE_ENGAGEMENT_ID').val('');
        $('#sous_act').val('');
        $('#fini').val('');
        $('#QTE').val('');
        $('#resultat_attend').val('');
...
<script type="text/javascript">
  function valider_list()
  {
    ...
    let url="<?=base_url('double_commande_new/Introduction_Budget_Multi_Taches/valider_liste')?>";
    $.ajax(
    {
      url:url,
      type:"POST",
      ...
      beforeSend:function() 
      {
        $('#loading_valider').html("<i class='fa fa-spinner fa-pulse fa-1x fa-fw'></i>");
      },
      success: function(data)
      {
        $('#loading_valider').html("<i class='fa fa-check'></i>");
        if(data.status === false) 
        {   
          $('#message').html(data.msg_aucun).fadeIn('slow').delay(3000).fadeOut('slow');
          $("#task_liste").hide();
          $('#save_tasks').hide();
          ...
          if(data.type_devise != 1)
          {  
            
            $('#TYPE_MONTANT_ID').val(data.type_devise);
            $('#engagement_cous').val(data.cours_devise);          
            $('#DATE_COUT_DEVISE').val(data.date_devise);
            money_devise();
            $('#engagement_cous').prop('readonly', true);          
            $('#DATE_COUT_DEVISE').prop('readonly', true);
          }
...
<script type="text/javascript">
  function liste() 
  {
    ...
    $("#mytable").DataTable({
      ...
      "serverSide": true,
      "ajax": {
        url: "<?= base_url('double_commande_new/Introduction_Budget_Multi_Taches/listing_tempo')?>",
        type: "POST",
        data: {
          INSTITUTION_ID : INSTITUTION_ID,
          NOTE_REFERENCE : NOTE_REFERENCE
