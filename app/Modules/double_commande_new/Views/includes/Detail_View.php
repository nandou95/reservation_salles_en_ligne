  ...
  <table class="table m-b-0 m-t-20">
    <tbody>
      <tr>
        ...
        <td><strong><font style="float:left;"><?= !empty($get_infoEBET[0]->CODE_INSTITUTION) ? $get_infoEBET[0]->CODE_INSTITUTION.'&nbsp;-&nbsp;' : 'N/A - ' ;?> <?= !empty($get_infoEBET[0]->DESCRIPTION_INSTITUTION) ? $get_infoEBET[0]->DESCRIPTION_INSTITUTION : 'N/A' ;?></font></strong></td>
      </tr>
      <?php if(!empty($get_infoEBET[0]->DESCRIPTION_SOUS_TUTEL)){ ?>
        <tr>
          <td style="width:250px ;"><font style="float:left;"><?=lang('messages_lang.label_sousTitre')?></font></td>
          <td><strong><font style="float:left;"><?= !empty($get_infoEBET[0]->CODE_SOUS_TUTEL) ? $get_infoEBET[0]->CODE_SOUS_TUTEL.'&nbsp;-&nbsp;' : 'N/A - ' ;?><?= $get_infoEBET[0]->DESCRIPTION_SOUS_TUTEL?></font></strong></td>
        </tr>
      <?php } ?>
      <tr>
        <td style="width:250px ;"><font style="float:left;"><?=lang('messages_lang.label_prog')?> </font></td>
        <td><strong><font style="float:left;"><?= !empty($get_infoEBET[0]->CODE_PROGRAMME) ? $get_infoEBET[0]->CODE_PROGRAMME.'&nbsp;-&nbsp;' : 'N/A - ' ;?><?=!empty($get_infoEBET[0]->INTITULE_PROGRAMME)?$get_infoEBET[0]->INTITULE_PROGRAMME:''?></font></strong></td>
      </tr>
      <tr>
        <td style="width:250px ;"><font style="float:left;"><?=lang('messages_lang.label_action')?> </font></td>
        <td><strong><font style="float:left;"><?= !empty($get_infoEBET[0]->CODE_ACTION) ? $get_infoEBET[0]->CODE_ACTION.'&nbsp;-&nbsp;' : 'N/A - ' ;?><?= !empty($get_infoEBET[0]->LIBELLE_ACTION)?$get_infoEBET[0]->LIBELLE_ACTION:''?></font></strong></td>
      </tr>

      <?php if(!empty($get_infoEBET[0]->CODE_NOMENCLATURE_BUDGETAIRE)){ ?>
        <tr>
          <td style="width:250px ;"><font style="float:left;"><?=lang('messages_lang.col_imputation')?></font></td>
          <td><strong><font style="float:left;"><?= $get_infoEBET[0]->CODE_NOMENCLATURE_BUDGETAIRE ?> - <?= $get_infoEBET[0]->LIBELLE_CODE_NOMENCLATURE_BUDGETAIRE ?></font></strong></td>
        </tr>
      <?php } ?>
      ...
      <tr>
        <td style="width:250px ;"><font style="float:left;"><?=lang('messages_lang.label_taches')?></font></td>
          <td><center><a href='javascript:void(0)'  class='btn btn-primary' onclick='get_task()'><?= count($get_infoEBET)?></a></center></strong></td>
      </tr>
   ...
   <!--    <--?php if(!empty($get_info['MONTANT_EN_DEVISE'])){ ?>
      ...
      <?php if(!empty($get_info['PATH_MODELE_PAIEMENT'])){?>
        <tr>
          <td>&nbsp;<strong><?=$get_info['DESC_MODELE']?></strong></td>
          <td>
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel"><?= lang('messages_lang.label_taches') ?></h5>
      ...
      <div class="modal-body overflow-auto" style="max-height: 400px">
        <div class="table-responsive  mt-3">
          <table class="table m-b-0 m-t-20">
            <!-- tableau des info tache -->
            <thead>
              <tr>
                <th>#</th>
                <th><?=lang('messages_lang.labelle_activites')?></th>
                <th><?=lang('messages_lang.label_taches')?></th>
                <th><?=lang('messages_lang.label_quantite')?></th>
                <th><?=lang('messages_lang.labelle_credit_vote_activite')?></th>
                <th><?=lang('messages_lang.labelle_credit_reserve')?></th>
                <th><?=lang('messages_lang.labelle_credit_restant_apres_engagement')?></th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <?php
                  ...
                  foreach ($get_infoEBET as $key)
                  {
                    ...
                    else if ($CODE_TRANCHE == 3)
                    {
                      ...
                      $restant=floatval($key->BUDGET_RESTANT_T3);
                    ...
                    else if ($CODE_TRANCHE == 4)
                    {
                      ...
                      $restant=floatval($key->BUDGET_RESTANT_T4); 
                    ...
                    {
                      $DESC_PAP_ACTIVITE='<center>--</center>';
                    }
                    echo '<tr><td>'.$u++.'</td>
                    <td>'.$DESC_PAP_ACTIVITE.'</td>
                    <td>'.$key->DESC_TACHE.'</td>
                    <td>'.$key->QTE_RACCROCHE.'</td>
                    <td>'.$creditVote.'</td>
                    <td>'.$reserve.'</td>
                    <td>'.$restant.'</td><tr>';
                  }
      ...
      <div class="modal-footer">
        <button type="button" id="mode1" class="btn btn-primary" data-dismiss="modal"> <i class="fa fa-close" aria-hidden="true"></i> <?= lang('messages_lang.label_ferm') ?></button>        
      </div>
    </div>
  </div>
</div>
                                    ...
                                    <?php if (!empty($get_info['PATH_RAPPORT_COMPTE_GENERAL_TRESOR'])) { ?>
                                      ...
                                      <?php if (!empty($get_info['PATH_BALANCE_COMPTE'])) { ?>
                                        <embed id="pdf16" style="display:none;" src="<?=base_url('uploads/double_commande_new/'.$get_info['PATH_BALANCE_COMPTE'])?>" type="application/pdf" width="100%" height="600px">
                                          ...
                                          <?php if (!empty($get_info['PATH_MODELE_PAIEMENT'])) { ?>
                                            <embed id="pdf24" style="display:none;" src="<?=base_url('uploads/double_commande_new/'.$get_info['PATH_MODELE_PAIEMENT'])?>" type="application/pdf" width="100%" height="600px">
                                          <?php } ?>
