              ...
              <div class="col-12">
                ...
                <div class="card-body">
                  ...
                  <div class=" container " style="width:100%">
                    ...
                    <form enctype='multipart/form-data' id="MyFormData" action="<?=base_url('double_commande_new/Ordonnancement/update_etape9/')?>" method="post" >
                      <div class="container">
                        ...
                        <div class="row" style="border:1px solid #ddd;border-radius:5px">
                          <div class="col-md-12 mt-3 ml-2"style="margin-bottom:50px" >
                            ...
                            <div class="col-md-6" id="date_ordo" style="display: none;">
                              ...
                              <input type="date" class="form-control" id="DATE_HEURE_ORDONNANCE" name="DATE_HEURE_ORDONNANCE"  value="<?= date('Y-m-d') ?>" min="<?=date('Y-m-d', strtotime($date_trans['DATE_TRANSMISSION']))?>" max="<?=date('Y-m-d')?>" onkeypress="return false" onblur="this.type='date'">
                              <?php if (isset($validation)) : ?>
                            ...
                            <div class="col-md-6" id="bordereau" style="display: none;">
                              <br>
                              <label><?=lang('messages_lang.label_bord_bon_eng')?><span style="color: red;">*</span></label>
...
<script type="text/javascript">
  function save()
  {
   ...
   if(OPERATION_ID=='')
   {
     $('#error_OPERATION_ID').html("<?=lang('messages_lang.champ_obligatoire')?>");
     status=1;
   }else{
    if(OPERATION_ID == 1 || OPERATION_ID == 3)
    {
      ...
      {
        $('#error_PATH_DOCUMENT').html("<?=lang('messages_lang.champ_obligatoire')?>");
        status=1;
      }else{

        if(PATH_DOCUMENT.files[0].size > maxFileSize)
        {
          $('#error_PATH_DOCUMENT').html('<?= lang('messages_lang.fichier_volumineux_transmission_du_bordereau') ?>');
          status = 1;
        }
      }
      ...
      {
        $('#error_DATE_HEURE_ORDONNANCE').html("<?=lang('messages_lang.champ_obligatoire')?>");
        status = 1;
      }
      ...
      if(TYPE_MONNAIE == 19)
      {
        ...
        else
        {
          ...
          {
            $('#error_MONTANT_EN_BIF').html('<?=lang('messages_lang.err_mont_inferieur')?>'+ MONTANT_LIQ +'.');
            status = 1;
          }
        }
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
          ...
          <table class="table m-b-0 m-t-20">
            <tbody>
              ...
              <tr id="showing_bordero">
                <td><i class="fa fa-file-pdf"></i>&nbsp;<strong><?=lang('messages_lang.label_bord_bon_eng')?></strong></td>
      ...
      <div class="modal-footer">
        ...
        <a onclick="confirm();hideButton()" id="ord_min" style="float: right;" class="btn btn-info"><i class="fa fa-check"></i><?=lang('messages_lang.bouton_confirmer')?></a>
      </div>
    </div>
  </div>
</div>
...
<script>
  function hideButton()
  {
    var element = document.getElementById("ord_min");
