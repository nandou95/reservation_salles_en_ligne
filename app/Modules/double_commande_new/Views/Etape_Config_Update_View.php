              ...
              <div style="box-shadow: rgba(100, 100, 111, 0.25) 0px 7px 29px 0px" class="card">
               ...
               <br>
                ...
                <div class="card-body">
                  ...
                  <form id="my_form" action="<?= base_url('double_commande_new/Etape_Double_Commande_Config/update') ?>" method="POST">

                    <div class="row col-md-12">
                      <input type="hidden" name="ETAPE_DOUBLE_COMMANDE_CONFIG_ID" id="ETAPE_DOUBLE_COMMANDE_CONFIG_ID" value="<?=$step_config['ETAPE_DOUBLE_COMMANDE_CONFIG_ID']?>">
                      <div class="col-md-6">
                        <br>
                        ...
                        <select class="form-control" name="ETAPE_DOUBLE_COMMANDE_ACTUEL_ID" id="ETAPE_DOUBLE_COMMANDE_ACTUEL_ID" onchange="next_step()">
                          ...
                          <?php  foreach ($etapes as $keys) { ?>
                            <?php if($keys->ETAPE_DOUBLE_COMMANDE_ID ==set_value('ETAPE_DOUBLE_COMMANDE_ACTUEL_ID') || $keys->ETAPE_DOUBLE_COMMANDE_ID == $step_config['ETAPE_DOUBLE_COMMANDE_ACTUEL_ID']) { ?>
                      ...
                      <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" id="LIEN" name="LIEN" value="<?= $step_config['LINK_ETAPE_DOUBLE_COMMANDE']?>" <?php if (isset($validation)) : ?> value="<?=set_value('LIEN')?>" <?php endif ?>>
                        <?php if (isset($validation)) : ?>
                      ...
                      <div class="col-md-6">
                        ...
                        <select class="form-control" name="CONTRAINTE_MONTANT_ID" id="CONTRAINTE_MONTANT_ID">
                          ...
                          <?php  foreach ($contr_mont as $keys) { ?>
                            <?php if($keys->CONTRAINTE_MONTANT_ID ==set_value('CONTRAINTE_MONTANT_ID') || $keys->CONTRAINTE_MONTANT_ID == $step_config['CONTRAINTE_MONTANT_ID']) { ?>
                      ...
                      <div class="col-md-6">
                        ...
                        <select class="form-control" name="IS_BENEFICIAIRE" id="IS_BENEFICIAIRE">
                          <option value=""><?= lang('messages_lang.labelle_selecte') ?></option>
                          <option value="1" <?= ($step_config['IS_FOURNISSEUR'] == 1) ? 'selected' : '' ?>><?= lang('messages_lang.label_oui') ?></option>
                          <option value="0" <?= ($step_config['IS_FOURNISSEUR'] == 0) ? 'selected' : '' ?>><?= lang('messages_lang.label_non') ?></option> 
                        </select>
                      ...
                      <div class="col-md-6">
                        ...
                        <select class="form-control" name="ETAPE_DOUBLE_COMMANDE_SUIVANT_ID" id="ETAPE_DOUBLE_COMMANDE_SUIVANT_ID" >
                          <option value=""><?= lang('messages_lang.labelle_selecte') ?></option>
                          <?php  foreach ($etap_suiv as $keys) { ?>
                            <?php if($keys->ETAPE_DOUBLE_COMMANDE_ID ==set_value('ETAPE_DOUBLE_COMMANDE_SUIVANT_ID') || $keys->ETAPE_DOUBLE_COMMANDE_ID == $step_config['ETAPE_DOUBLE_COMMANDE_SUIVANT_ID']) { ?>
                              <option value="<?=$keys->ETAPE_DOUBLE_COMMANDE_ID ?>" selected><?=$keys->DESC_ETAPE_DOUBLE_COMMANDE?></option>
                              <?php }else{?>
                               <option value="<?=$keys->ETAPE_DOUBLE_COMMANDE_ID ?>"><?=$keys->DESC_ETAPE_DOUBLE_COMMANDE?></option>
                              <?php } }?>
                    ...
                    <div id="SAVE" class="card-footer">
                      <div style="float:right;margin-bottom:5%">
                        <a onclick="save();" id="btn_save" class="btn btn-success" style="float:right;"><?= lang('messages_lang.bouton_modifier') ?></a>
                      </div>
                    </div>
