          ...
          <div style="box-shadow: rgba(100, 100, 111, 0.25) 0px 7px 29px 0px" class="card">
            <div class="col-12 d-flex">
              <div class="col-9" style="float: left;">
                <h1 class="header-title text-dark">Modifier Tâche/PTBA</h1>
              </div>
            </div>
            ...
            <div class="card-body">
              ...
              <form id="my_form" method="POST" action="<?= base_url('double_commande_new/Modifier_Tache/update') ?>" method="POST" enctype="multipart/form-data">
                <div class="card-body">
                  <div style="border:1px solid #ddd;border-radius:5px;margin: 5px">
                    <div class="row" style="margin :  5px">
                      ...
                      <div class="col-6">
                        <div class="">
                          <label>Tâches PTBA<span style="color: red;">*</span></label><span hidden id="PTBA_TACHE_ID_LOADING"><i class='fa fa-spinner fa-pulse fa-1x fa-fw'></i></span>
                          <select class="form-control select2" name="PTBA_TACHE_ID" id="PTBA_TACHE_ID" onchange="checkSelect(this); setTacheAModifier(this)" >
                            <option value=""><?=lang('messages_lang.labelle_selecte')?></option>
                            <?php
                            foreach($code_taches as $code_tache)
                            {
                              ?>
                              <option value="<?=$code_tache->PTBA_TACHE_ID?>"<?= $code_tache->PTBA_TACHE_ID == $PTBA_TACHE_ID ? 'selected' : '' ?>><?=$code_tache->DESC_TACHE?></option> 
                              <?php
                            }
                            ?>
                          </select>
                          ...
                          <?php endif ?>
                          <span id="error_PTBA_TACHE_ID" class="text-danger"></span>
                        </div>
                      </div>
          ...
          <table class="table m-b-0 m-t-20">
            <tbody>
              ...
              <tr>
                ...
                <td id="PTBA_TACHE_ID_VERIFY" class="text-dark"></td>
              </tr>
              <tr>
...
function save_data()
{
  ...
  if(PTBA_TACHE_ID == ""){
    $('#error_PTBA_TACHE_ID').html('<font style="color:red;size:2px;">Le champs est obligatoire</font>');
    isFormValid =  false;
  }
  else{
    if(MODIFIER_TACHE == ""){
  ...
  if(isFormValid)
  {
    ...
    $('#PTBA_TACHE_ID_VERIFY').text($('#PTBA_TACHE_ID  option:selected').text())
...
function get_taches()
{
  ...
  $.ajax(
  {
    ...
    beforeSend:function()
    {
      $('#PTBA_TACHE_ID_LOADING').attr("hidden",false);
      $('#MODIFIER_TACHE').val('');

    },
    success: function(data)
    {
      console.log(data.PTBA_TACHE_ID)
      $('#PTBA_TACHE_ID').html(data.PTBA_TACHE_ID)
      $('#PTBA_TACHE_ID_LOADING').attr("hidden",true);
    }
  });
}
