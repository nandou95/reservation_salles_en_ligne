              ...
              <div style="box-shadow: rgba(100, 100, 111, 0.25) 0px 7px 29px 0px" class="card">
                ...
                <div class="car-body">
                  <form id='MyFormData' enctype='multipart/form-data' action="<?=base_url('double_commande_new/Phase_comptable/Rec_Analyse_Decaisse_Transmettre/save') ?>" method="POST">
                    <input type="hidden" name="id_exec_titr_dec" id="id_exec_titr_dec" value="<?=$etape['EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID']?>">
                    <input type="hidden" name="id_detail" id="id_detail" value="<?=$etape['EXECUTION_BUDGETAIRE_DETAIL_ID']?>">
                    ...
                    <div class="card-body">
                      <div class="row">
                        ...
                        <?php if ($detai_taux_echange_id == 1) { ?>
                          <div class="col-md-6" id="show_mont_decais" style="display:none;">
                            <br>
                        ...
                        <?php } else { ?>
                          <div class="col-md-6" id="show_mont_dec_devise" style="display:none;"> <br>
                          ...
                          <div class="col-md-6" id="show_date_devise" style="display:none;">
                            <br>
                        ...
                        <div class="col-md-6" id="show_date_dec" style="display:none;"><br>
                          <label><?= lang('messages_lang.label_date_dec')?><span style="color: red;">*</span></label>
                          <input type="date" min="<?=date('Y-m-d', strtotime($date_trans['DATE_TRANSMISSION']))?>" class="form-control" name="DATE_DECAISSEMENT" id="DATE_DECAISSEMENT" value="<?= date('Y-m-d') ?>" max="<?=date('Y-m-d')?>" onkeypress="return false" onblur="this.type='date'">
                          <?php if (isset($validation)) : ?>
                        ...
                        <div class="col-md-6"> <br>
                          ...
                          <input type="date" min="<?=date('Y-m-d', strtotime($date_trans['DATE_TRANSMISSION']))?>" class="form-control" name="DATE_TRANSMISSION" value="<?= date('Y-m-d') ?>" max="<?=date('Y-m-d')?>" id="DATE_TRANSMISSION" onkeypress="return false" onblur="this.type='date'">
                          <?php if (isset($validation)) : ?>
...
<script type="text/javascript">
  function save()
  {
    ...
    if (ID_OPERATION==1 || ID_OPERATION==3) {
      ...
      statut = 1;
      $('#error_TYPE_ANALYSE_MOTIF_ID').html('<?= lang('messages_lang.validation_message')?>');
      }
    }
    else
    {
    ...
    if(statut == 2)
    {
      ...
      if(ID_OPERATION == 1 || ID_OPERATION == 3)
      {
        ...
        $('#motif_ret').show();
        $('#motif_retour_id_modal').show();

        $('#show_decais_date').hide();
        $('#show_montant_decais').hide();
        $('#show_montant_devise').hide();
        $('#show_date_dec_devise').hide();
      ...
      else
      {
        if(TAUX_ECHANGE_ID!=1)
        {
          $('#show_montant_decais').show();
          $('#show_montant_devise').show();
          $('#show_date_dec_devise').show();
        }
        else
        {
          $('#show_montant_decais').show();
...
<script>
  function get_rejet()
  {
    ...
    {
      $('#show_motif').hide();
      $('#show_mont_decais').hide();
      $('#show_mont_dec_devise').hide();
      $('#show_date_devise').hide();
      $('#show_date_dec').hide();
      ...
      if(OPERATION == 1 || OPERATION == 3)
      {
        ...
        $('#show_motif').hide();
        $('#show_mont_decais').show();
        $('#show_mont_dec_devise').show();
        $('#show_date_devise').show();
        $('#show_date_dec').show();
      }
