              ...
              <div class="col-12">
                <div style="box-shadow: rgba(100, 100, 111, 0.25) 0px 7px 29px 0px" class="card">
                  <div class="card-body">
                    <div class="card-body">
                      <div class="row">
                        ...
                        <div class="col-3" style="float: right;">
                          <a href="<?=base_url('double_commande_new/Liquidation/get_liquid_partiel')?>" style="float: right;margin-right: 90px;margin: 5px" class="btn btn-primary"><span class="fa fa-list pull-right"></span>&nbsp;<?=lang('messages_lang.link_list')?></a> 
                        </div>
                    ...
                    <form enctype='multipart/form-data' id="my_form" action="<?= base_url('double_commande_new/Liquidation/update_partielle') ?>" method="POST">
                      <div style="border:1px solid #ddd;border-radius:5px;margin: 5px">
                        <div class="row" style="margin :  5px">
                          <input hidden id="infoTaches" name="infoTaches">
                          ...
                          <input type="hidden" name="monnaie" id="monnaie" value="<?=$get_monnaie?>">
                          ...
                          <input type="hidden" name="LIQUIDATION_TOTALE" id="LIQUIDATION_TOTALE" value="<?=!empty($info['LIQUIDATION'])?$info['LIQUIDATION']:'0'?>">
                          ...
                          <div class="col-6">
                            <label for=""><?=lang('messages_lang.labelle_date_reception_demande')?>(CD)<span style="color: red;">*</span></label>
                          ...
                          <div id="DIV_TITRE_CREANCE" class="col-6">
                            ...
                            <input onkeyup="SetMaxLength(1)" autocomplete="off" value="<?=$info['TITRE_CREANCE']?>" type="text" name="TITRE_CREANCE" id="TITRE_CREANCE" class="form-control" >
                          ...
                          <div id="div_creance" class="col-6 mt-2">

                            <div id="div_taches" class="p-2" style="border: #ccc solid 1px;">
                              <label><?=lang('messages_lang.label_montant_tache')?> en <?=$info['DESC_DEVISE_TYPE']?></label>
                              <?php
                                $i=1;
                                foreach ($infoTaches as $infoTache) 
                                {
                                  echo '<label>'.$i++.'. '.$infoTache->DESC_TACHE.'</label>';
                                  echo '<input onkeyup="calculate(this)" placeholder="Entrez le montant de la tache" type="text" name="MONTANT_'.$infoTache->EBET_ID.'" id="MONTANT_'.$infoTache->EBET_ID.'" value="'.$infoTache->SINGLE_TACHE_LIQUIDATION.'" class="form-control">';
                                  echo '<font color="back" id="error_MONTANT_'.$infoTache->EBET_ID.'"></font>';
                                }
                              ?>
                            </div>

                            <label><?=lang('messages_lang.label_montant_titre_creance')?><span style="color: red;">*</span></label>
                            <input readonly value="<?=!empty($get_monnaie)?$get_monnaie:''?>" onkeyup="getSubstring(1);SetMaxLength(3);getCalculMontant()" autocomplete="off" type="text" name="MONTANT_CREANCE" id="MONTANT_CREANCE" class="form-control" >

                            <input value="<?=!empty($get_monnaie)?$get_monnaie:''?>" type="hidden" id="liqui_montant" class="form-control" >
                          ...
                          <div id="div_devise1" class="col-6 mt-2">

                            <div id="div_taches_devise" class="p-2" style="border: #ccc solid 1px;">
                              <label><?=lang('messages_lang.label_montant_tache')?> en <?=$info['DESC_DEVISE_TYPE']?></label>
                              <?php
                                $i=1;
                                foreach ($infoTaches as $infoTache) 
                                {
                                  echo '<label>'.$i++.'. '.$infoTache->DESC_TACHE.'</label>';
                                  echo '<input onkeyup="calculate_devise(this)" placeholder="Entrez le montant de la tache" type="text" name="MONTANT_DEVISE_'.$infoTache->EBET_ID.'" id="MONTANT_DEVISE_'.$infoTache->EBET_ID.'" value="'.$infoTache->SINGLE_TACHE_LIQUIDATION_DEVISE.'" class="form-control">';
                                  echo '<font color="back" id="error_MONTANT_DEVISE_'.$infoTache->EBET_ID.'"></font>';
                                }
                              ?>
                            </div>

                            <label><?=lang('messages_lang.label_montant_devise')?><span style="color: red;">*</span></label>
                            <input readonly value="<?=!empty($get_monnaie) ? $get_monnaie : ''?>" onkeyup="getSubstring(3);SetMaxLength(4);getCalculMontant()" autocomplete="off" type="text" name="MONTANT_DEVISE" id="MONTANT_DEVISE" class="form-control">
                          ...
                          <div class="col-6">
                            ...
                            <select class="form-control" name="EXONERATION" id="EXONERATION">
                              ...
                              elseif(set_value("EXONERATION")==0)
                              {
                                ...
                                <option value="0" selected>'.lang('messages_lang.label_non').'</option>';
                              }
                          ...
                          <div class="col-6">
                            <label><?=lang('messages_lang.label_date_trans_conf')?>(CD)<span style="color: red;">*</span></label>
                          ...
                          <div class="col-6">
                            ...
                            <select class="form-control select2" multiple  id="TYPE_ANALYSE_ID" name="TYPE_ANALYSE_ID[]">
                              <?php foreach ($get_verification2 as $key) {
                                if ($key->EXECUTION_BUDGETAIRE_ID==$info['EXECUTION_BUDGETAIRE_ID']) {
                                  ?>
                                  <option selected value="<?=$key->TYPE_ANALYSE_ID?>"><?=$key->DESC_TYPE_ANALYSE?></option>
                                  <?php
                                }
                              }
                          ...
                          if ($info['MARCHE_PUBLIQUE']==1)
                          {
                            ...
                            <div class="col-6">
                              ...
                              <font color="red" id="error_DATE_LIVRAISON_CONTRAT"><?=$validation->getError('DATE_LIVRAISON_CONTRAT');?></font><br>
  ...
  <div class='modal-dialog  modal-lg' style="max-width: 60%">
    <div class='modal-content'>
      ...
      <div class='modal-footer'>
        <button id="modif123" onclick="deleteFile();" class='btn btn-primary btn-md' data-dismiss='modal'><i class="fa fa-pencil"></i> <?=lang('messages_lang.labelle_mod')?> <span id="loading_delete"></span></button>
...
<script>
  $(document).ready(function() {
    $('#infoTaches').val(JSON.stringify(<?=$infoTachesJson?>));
  })
...
<script type="text/javascript">
  ...
  function getSubstring(id)
  {
    ...
    var liqui_montant = $('#liqui_montant').val();
...
<script type="text/javascript">
  ...
  {    
    ...
    var LIQUIDATION_TOTALE=$('#LIQUIDATION_TOTALE').val();
    ...
    { 
      ...
      var MONTANT_DEVISE_RESTANT = parseFloat(MONTANT_DEVISE_ENGAGEMENT_JURIDIQUE)-parseFloat(MONTANT_DEVISE_LIQUIDATION);

      if(MARCHE_PUBLIQUE==1)
      {
        var MONTANT_ENLEVE_DEVISE=$('#MONTANT_ENLEVE_DEVISE').val()
        MONTANT_DEVISE_RESTANT=MONTANT_ENLEVE_DEVISE-parseFloat(MONTANT_DEVISE_LIQUIDATION);
      }
      
      // var MONTANT_devise_liquide = parseFloat(liqui_devise)+parseFloat(MONTANT_DEVISE_RESTANT);

      if(parseFloat(MONTANT_DEVISE)>parseFloat(MONTANT_DEVISE_RESTANT))
      {
        $('#error_MONTANT_DEVISE_SUP').html("<?=lang("messages_lang.labelle_montant_devise_liquidation_pas_supeirieur_engag_juridik")?> ("+MONTANT_DEVISE_RESTANT+")");
        $('#LIQUIDATION').val('')
        return false;
      }
      else
    ...
    else
    {
      ...
      var MONTANT_CREANCE_RESTANT = parseInt(MONTANT_RACCROCHE_JURIDIQUE)-parseInt(LIQUIDATION_TOTALE);

      if(MARCHE_PUBLIQUE==1)
      {
        MONTANT_ENLEVE=$('#MONTANT_ENLEVE').val()
        MONTANT_CREANCE_RESTANT = MONTANT_ENLEVE-parseInt(LIQUIDATION_TOTALE);
      }

      if(parseInt(MONTANT_CREANCE)>parseInt(MONTANT_CREANCE_RESTANT))
      {
        $('#error_MONTANT_CREANCE_SUP').text("<?=lang("messages_lang.labelle_montant_titre_creance_pas_supeirieur_engag_juridik")?> ("+MONTANT_CREANCE_RESTANT+")");
        return false;
      }
      else
      {
...
<script type="text/javascript">
  function send_data()
  {
    ...
    if (TYPE_MONTANT_ID!=1)
    {
      ...
      var MONTANT_devise_liquide = parseInt(MONTANT_DEVISE_ENGAGEMENT_JURIDIQUE)-parseInt(MONTANT_DEVISE_LIQUIDATION);
      if(MARCHE_PUBLIQUE==1)
      {
        var MONTANT_ENLEVE_DEVISE=$('#MONTANT_ENLEVE_DEVISE').val()
        MONTANT_devise_liquide=MONTANT_ENLEVE_DEVISE-parseInt(MONTANT_DEVISE_LIQUIDATION)
      }

      if(parseInt(MONTANT_DEVISE)>parseInt(MONTANT_devise_liquide))
      {
        $('#error_MONTANT_DEVISE_SUP').html("<?=lang("messages_lang.labelle_montant_devise_liquidation_pas_supeirieur_engag_juridik")?> ("+MONTANT_DEVISE_ENGAGEMENT_JURIDIQUE+"), <?=lang("messages_lang.labelle_restant_engag_juridik")?> ("+MONTANT_devise_liquide+")");
        $('#LIQUIDATION').val('')
        return false;
      }
      else
    ...
    else
    {
      ...
      else
      {
        ...
        {
          MONTANT_ENLEVE=$('#MONTANT_ENLEVE').val()
          MONTANT_RESTANT_liquide = MONTANT_ENLEVE-parseInt(LIQUIDATION_TOTALE);
        }

        var MONTANT_RESTANT_liquide = parseInt(MONTANT_RACCROCHE_JURIDIQUE)-parseInt(LIQUIDATION_TOTALE);; 

        if(parseInt(MONTANT_CREANCE)>parseInt(MONTANT_RESTANT_liquide))
        {
          $('#error_MONTANT_CREANCE_SUP').text("<?=lang("messages_lang.labelle_montant_titre_creance_pas_supeirieur_engag_juridik")?> ("+MONTANT_RACCROCHE_JURIDIQUE+"), <?=lang("messages_lang.labelle_restant_engag_juridik")?> ("+MONTANT_RESTANT_liquide+")");
          return false;
        }
        else
        {
    ...
    if (TYPE_ANALYSE_ID == '')
    {
      $('#error_TYPE_ANALYSE_ID').text('<?=lang('messages_lang.labelle_selectionner_verification')?>');
      return false;
    }
    else
    {
    ...
    if(BUDGETAIRE_TYPE_DOCUMENT_ID==2)
    {
      ...
      if (intro_note == '')
      {
        $('#error_intro_note').text('<?=lang('messages_lang.message_champs_obligatoire')?>');
        statut = false;
      }
      else
      {
        $('#error_intro_note').text('');
      }
    }
...
<script>
  ...
  {
    getTaches=$('#infoTaches').val();
    taches = JSON.parse(getTaches);
    let totalLiquidPartiel = 0;

    taches.map((tache)=> {
      $('#MONTANT_'+tache.EBET_ID).val($('#MONTANT_'+tache.EBET_ID).val().replace(/[^0-9\.]/g, ''));
      let liquidTache = $('#MONTANT_'+tache.EBET_ID).val() ? parseFloat($('#MONTANT_'+tache.EBET_ID).val() ) : 0;

      if(liquidTache > (parseFloat(tache.TACHE_JURIDIQUE) - parseFloat(tache.TACHE_LIQUIDATION)))
      {
        //set le montant au total possible et afficher l'erreur
        $('#MONTANT_'+tache.EBET_ID).val(parseFloat(tache.TACHE_JURIDIQUE) - parseFloat(tache.TACHE_LIQUIDATION));
        liquidTache = parseFloat($('#MONTANT_'+tache.EBET_ID).val())

        $('#error_MONTANT_'+tache.EBET_ID).text('<?=lang('messages_lang.mount_sup_tache')?>: '+(tache.TACHE_JURIDIQUE  - tache.TACHE_LIQUIDATION));
      }
      else{
        $('#error_MONTANT_'+tache.EBET_ID).text('');
      }

      totalLiquidPartiel += liquidTache;

      $('#MONTANT_CREANCE').val(totalLiquidPartiel);
    });
    
  }

  function calculate_devise(ref)
  {
    getTaches=$('#infoTaches').val();
    taches = JSON.parse(getTaches);
    let totalLiquidPartiel = 0;
    ...
    taches.map((tache)=> {
      $('#MONTANT_DEVISE_'+tache.EBET_ID).val($('#MONTANT_DEVISE_'+tache.EBET_ID).val().replace(/[^0-9\.]/g, ''));
      let liquidTache = $('#MONTANT_DEVISE_'+tache.EBET_ID).val() ? parseFloat($('#MONTANT_DEVISE_'+tache.EBET_ID).val() ) : 0;
      if(liquidTache > (parseFloat(tache.TACHE_JURIDIQUE_DEVISE) - parseFloat(tache.TACHE_LIQUIDATION_DEVISE)))
      {
        //set le montant au total possible et afficher l'erreur
        $('#MONTANT_DEVISE_'+tache.EBET_ID).val(parseFloat(tache.TACHE_JURIDIQUE_DEVISE) - parseFloat(tache.TACHE_LIQUIDATION_DEVISE));
        liquidTache = parseFloat($('#MONTANT_DEVISE_'+tache.EBET_ID).val())

        $('#error_MONTANT_DEVISE_'+tache.EBET_ID).text('<?=lang('messages_lang.mount_sup_tache')?>: '+(tache.TACHE_JURIDIQUE_DEVISE  - tache.TACHE_LIQUIDATION_DEVISE));
      }
      else{
        $('#error_MONTANT_DEVISE_'+tache.EBET_ID).text('');
      }

      totalLiquidPartiel += liquidTache;

      $('#MONTANT_DEVISE').val(totalLiquidPartiel);
    });
