              ...
              <div style="box-shadow: rgba(100, 100, 111, 0.25) 0px 7px 29px 0px" class="card">
                ...
                <div class="card-body">
                  <div style="margin-left: 15px;" class="row">
                    <h1 class="header-title text-dark"><?=lang('messages_lang.titre_jur_dej_valide')?></h1>
                  </div>

                  <div style="margin-left: 15px;" class="row">
                    <div class="col-md-3">
                      <label for="Nom" class="form-label"><?=lang('messages_lang.Lab_jur_instit')?></label>
                      <select onchange="liste();vider(); get_sousTutel(this.value)" class="form-control" name="INSTITUTION_ID" id="INSTITUTION_ID">
                  ...
                  <div class="d-flex mt-3 ml-3">
                    <a id="exportExcel" onclick="exporterExcel()" type="button" class="btn btn-primary"><span><span class="fa fa-file-excel pull-center"></span> Excel</span></a>
                    <!-- <a id="exportPdf" onclick="exporterPdf()" type="button" class="btn btn-primary"><span><span class="fa fa-file-pdf"></span> Pdf</span></a> -->
                  </div>
...
<script type="text/javascript">
  function liste() 
  {
    ...
    $("#mytable").DataTable(
    {
      ...
      "ajax":
      {
        url: "<?= base_url('double_commande_new/Menu_Engagement_Juridique/listing_deja_valide')?>",
        type: "POST",
        data:
        {
...
<script>
  function exporterExcel()
  {
    ...
    if (DATE_FIN == '' || DATE_FIN == null) {DATE_FIN = 0}
            ...
            if(INSTITUTION_ID != 0){
              document.getElementById("exportExcel").href = "<?=base_url('double_commande_new/Menu_Engagement_Juridique/exporter_excel_deja_valide/')?>"+'/'+INSTITUTION_ID+'/'+SOUS_TUTEL_ID+"/"+DATE_DEBUT+"/"+DATE_FIN;
            }else{
...
<script>
  function exporterPdf()
  {
    ...
    document.getElementById("exportPdf").href = "<?=base_url('double_commande_new/Menu_Engagement_Juridique/exporter_pdf_deja_valide/')?>"+'/'+INSTITUTION_ID+'/'+SOUS_TUTEL_ID+"/"+DATE_DEBUT+"/"+DATE_FIN;
  }
</script>
