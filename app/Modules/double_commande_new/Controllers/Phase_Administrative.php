<?php
  ...
  class Phase_Administrative extends BaseController
  {
    ...
    function eng_juridique($EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID=0)
    {
      $session  = \Config\Services::session();
      $user_id ='';
      ...
      ################################ DETAILS D'INFOS ############################
      $getdemanande = "SELECT exec.EXECUTION_BUDGETAIRE_ID, 
                              tache.SOUS_TUTEL_ID, 
                              det.EXECUTION_BUDGETAIRE_DETAIL_ID,
                              exec.DEVISE_TYPE_HISTO_ENG_ID,
                              exec.NUMERO_BON_ENGAGEMENT,
                              exec.DATE_BON_ENGAGEMENT,
                              exec.ENG_BUDGETAIRE AS MONTANT_RACCROCHE,
                              ...
                              exec.DEVISE_TYPE_ID AS TAUX_ECHANGE_ID,
                              det.COUR_DEVISE,
                              exec.ENG_BUDGETAIRE_DEVISE AS MONTANT_RACCROCHE_DEVISE,
                              tache.INSTITUTION_ID,
                              docs.DESC_BUDGETAIRE_TYPE_DOCUMENT,
                              info.BUDGETAIRE_TYPE_DOCUMENT_ID,
                              ebtd.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID
                       FROM execution_budgetaire_titre_decaissement ebtd JOIN execution_budgetaire exec ON exec.EXECUTION_BUDGETAIRE_ID=ebtd.EXECUTION_BUDGETAIRE_ID JOIN execution_budgetaire_tache_info_suppl info ON info.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_DETAIL_ID=ebtd.EXECUTION_BUDGETAIRE_DETAIL_ID JOIN execution_budgetaire_execution_tache ebet ON ebet.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID JOIN inst_institutions inst ON inst.INSTITUTION_ID=exec.INSTITUTION_ID JOIN inst_institutions_ligne_budgetaire ligne ON ligne.CODE_NOMENCLATURE_BUDGETAIRE_ID=exec.CODE_NOMENCLATURE_BUDGETAIRE_ID JOIN ptba_tache tache ON tache.PTBA_TACHE_ID=ebet.PTBA_TACHE_ID LEFT JOIN pap_activites act ON act.PAP_ACTIVITE_ID=tache.PAP_ACTIVITE_ID JOIN inst_institutions_programmes progr ON progr.PROGRAMME_ID=tache.PROGRAMME_ID JOIN inst_institutions_actions actions ON actions.ACTION_ID=tache.ACTION_ID JOIN budgetaire_type_document docs ON docs.BUDGETAIRE_TYPE_DOCUMENT_ID=info.BUDGETAIRE_TYPE_DOCUMENT_ID 
    ...
    function corriger_juridique($EXEC_BUDGET_RAC_DET_ID=0)
    {
      ...
      if($session->get('SESSION_SUIVIE_PTBA_ENGAG_JURIDIQUE_CORRECTION')!=1)
      {
        return redirect('Login_Ptba/homepage'); 
      }
      ...
      ################################ DETAILS D'INFOS ############################
      $getdemanande = "SELECT exec.DEVISE_TYPE_HISTO_ENG_ID,
                              exec.EXECUTION_BUDGETAIRE_ID,
                              exec.ENG_JURIDIQUE AS MONTANT_RACCROCHE_JURIDIQUE,
                              exec.ENG_JURIDIQUE_DEVISE AS MONTANT_RACCROCHE_JURIDIQUE_DEVISE, 
                              det.EXECUTION_BUDGETAIRE_DETAIL_ID,
                              exec.NUMERO_BON_ENGAGEMENT,
                              exec.DATE_BON_ENGAGEMENT,
                              exec.ENG_BUDGETAIRE AS MONTANT_RACCROCHE,
                              ...
                              exec.MARCHE_PUBLIQUE,
                              exec.DEVISE_TYPE_ID AS TAUX_ECHANGE_ID,
                              exec.DEVISE_TYPE_ID AS TYPE_MONTANT_ID,
                              det.COUR_DEVISE,
                              exec.ENG_BUDGETAIRE_DEVISE AS MONTANT_RACCROCHE_DEVISE,
                              tache.INSTITUTION_ID,
                              tache.SOUS_TUTEL_ID,
                              PATH_PPM,PATH_PV_ATTRIBUTION,
                              PATH_AVIS_DNCMP,PATH_MODELE_PAIEMENT,
                              info.BUDGETAIRE_TYPE_DOCUMENT_ID,
                              ebtd.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID 
                       ...
                       LEFT JOIN execution_budgetaire_titre_decaissement ebtd ON ebtd.EXECUTION_BUDGETAIRE_ID = exec.EXECUTION_BUDGETAIRE_ID 
                       ...
                       JOIN execution_budgetaire_tache_info_suppl info ON info.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID 
                       ...
                       WHERE 1 AND MD5(ebtd.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID)='".$EXEC_BUDGET_RAC_DET_ID."'";
      ...
      if (!empty($getProfil))
      {
        foreach ($getProfil as $value)
        {
          ...
          if ($prof_id == $value->PROFIL_ID || $prof_id==1)
          {
            ...
            ################### INFOS A CORRIGER ############################
            $get_juridiq = "SELECT exec.EXECUTION_BUDGETAIRE_ID,
                                   det.EXECUTION_BUDGETAIRE_DETAIL_ID,
                                   exec.ENG_JURIDIQUE AS MONTANT_RACCROCHE_JURIDIQUE,
                                   exec.DATE_ENG_JURIDIQUE AS DATE_ENGAGEMENT_JURIDIQUE,
                                   info.MODELE_ID,exec.ENG_JURIDIQUE_DEVISE AS MONTANT_RACCROCHE_JURIDIQUE_DEVISE, 
                                   det.COUR_DEVISE, 
                                   info.REFERENCE, 
                                   info.TYPE_BENEFICIAIRE_ID, 
                                   info.PRESTATAIRE_ID, 
                                   exec.MARCHE_PUBLIQUE, 
                                   info.PATH_CONTRAT,
                                   ebtd.ETAPE_DOUBLE_COMMANDE_ID,
                                   exec.ENG_BUDGETAIRE AS MONTANT_RACCROCHE_DEVISE,
                                   info.DATE_DEBUT_CONTRAT,
                                   info.DATE_FIN_CONTRAT,
                                   PATH_MODELE_PAIEMENT,
                                   ebtd.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID 
                            FROM execution_budgetaire_tache_detail det 
                            JOIN execution_budgetaire exec ON exec.EXECUTION_BUDGETAIRE_ID=det.EXECUTION_BUDGETAIRE_ID 
                            LEFT JOIN execution_budgetaire_titre_decaissement ebtd ON ebtd.EXECUTION_BUDGETAIRE_ID = exec.EXECUTION_BUDGETAIRE_ID
            ...
            $bind_etap_prev = $this->getBindParms('ETAPE_DOUBLE_COMMANDE_ID AS ETAPE_ID','execution_budgetaire_tache_detail_histo',"MD5(EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID)='".$EXEC_BUDGET_RAC_DET_ID."'",'EXECUTION_BUDGETAIRE_DETAIL_HISTO_ID DESC');
            ...
            $motif_rejet  = "SELECT DISTINCT tpes.TYPE_ANALYSE_MOTIF_ID,tpes.DESC_TYPE_ANALYSE_MOTIF FROM budgetaire_type_analyse_motif tpes JOIN execution_budgetaire_histo_operation_verification_motif motif ON motif.TYPE_ANALYSE_MOTIF_ID=tpes.TYPE_ANALYSE_MOTIF_ID WHERE 1 AND MD5(EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID)='".$EXEC_BUDGET_RAC_DET_ID."' AND ETAPE_DOUBLE_COMMANDE_ID=".$etap_prev['ETAPE_ID']."";
            ...
            //Le min de la date de réception
            $bind_date_histo = $this->getBindParms('EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID,OBSERVATION,DATE_TRANSMISSION,DATE_INSERTION','execution_budgetaire_tache_detail_histo','MD5(EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID)="'.$EXEC_BUDGET_RAC_DET_ID.'"','DATE_INSERTION DESC');
            $bind_date_histo = str_replace('\\','',$bind_date_histo);
            $data['date_trans'] = $this->ModelPs->getRequeteOne($psgetrequete, $bind_date_histo);
    ...
    function confirmer_juridique($EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID=0)
    {
      $session  = \Config\Services::session();
      $user_id ='';
    ...
    //Enregistrement de l'étape 4 Engagement juridique
    function update_etape4()
    {
      ...
      if($MARCHE_PUBLIQUE == 1){
        ...
        $file_error = '';
        $file = $this->request->getFile('PATH_MODELE_PAIEMENT');
      ...
      if($this->validation->withRequest($this->request)->run())
      {
        ...
        if($MARCHE_PUBLIQUE == 0)
        {
          ...
          $next_step = $this->getBindParms('ETAPE_DOUBLE_COMMANDE_ACTUEL_ID,ETAPE_DOUBLE_COMMANDE_SUIVANT_ID','execution_budgetaire_etape_double_commande_config','ETAPE_DOUBLE_COMMANDE_ACTUEL_ID='.$ETAPE_ID.' AND IS_MARCHE=0 AND IS_SALAIRE=0','ETAPE_DOUBLE_COMMANDE_ACTUEL_ID ASC');
          ...
          if($TYPE_MONNAIE != 1)
          {
            ...
            $conditionsEBET='EXECUTION_BUDGETAIRE_ID ='.$EXEC_BUDGET_RAC_ID;
            $datatomodifieEBET= 'MONTANT_ENG_JURIDIQUE=MONTANT_ENG_BUDGETAIRE, MONTANT_ENG_JURIDIQUE_DEVISE=MONTANT_ENG_BUDGETAIRE_DEVISE';
          ...
          else
          {
            ...
            $tableEBET='execution_budgetaire_execution_tache';
            $conditionsEBET='EXECUTION_BUDGETAIRE_ID ='.$EXEC_BUDGET_RAC_ID;
            $datatomodifieEBET= 'MONTANT_ENG_JURIDIQUE=MONTANT_ENG_BUDGETAIRE';
            $this->update_all_table($tableEBET,$datatomodifieEBET,$conditionsEBET);
        ...
        elseif($MARCHE_PUBLIQUE == 1)
        {
          ...
          if ($path_avis->getSize() > $maxFileSize)
          {
            ...
            return eng_juridique(md5($EXEC_BUDGET_RAC_DET_ID));
          }
          ...
          if($TYPE_MONNAIE !=1)
          {
            ...
            $table_info='execution_budgetaire_tache_info_suppl';
            $conditions_info='EXECUTION_BUDGETAIRE_ID='.$EXEC_BUDGET_RAC_ID;
            $datatomodifie_info= 'MODELE_ID='.$MODEL_ID.', REFERENCE="'.$REFERENCE.'", DATE_DEBUT_CONTRAT="'.$DATE_DEBUT.'", DATE_FIN_CONTRAT="'.$DATE_FIN.'", NBRE_JR_CONTRAT='.$jours.', TYPE_BENEFICIAIRE_ID='.$TYPE_BENEFICIARE_ID.', PRESTATAIRE_ID='.$PRESTATAIRE_ID.',PATH_PPM="'.$PATH_PPM_DOC.'",PATH_PV_ATTRIBUTION="'.$PATH_PV_ATTRIBUTION.'",PATH_AVIS_DNCMP="'.$PATH_AVIS_DNCMP.'"';
            $this->update_all_table($table_info,$datatomodifie_info,$conditions_info);
          }
          else
          {
        ...
        $PATH_MODELE_PAIEMENT=$this->uploadFile('PATH_MODELE_PAIEMENT','double_commande_new','MODELE_PAIEMENT');

        $table_info='execution_budgetaire_tache_info_suppl';
        $conditions_info='EXECUTION_BUDGETAIRE_ID='.$EXEC_BUDGET_RAC_ID;
        $datatomodifie_info= 'PATH_MODELE_PAIEMENT="'.$PATH_MODELE_PAIEMENT.'"';
        $this->update_all_table($table_info,$datatomodifie_info,$conditions_info);
        
        //Enregistrement dans historique raccrochage
        $columsinsert_histo="EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID,USER_ID,ETAPE_DOUBLE_COMMANDE_ID,OBSERVATION,DATE_TRANSMISSION,DATE_RECEPTION";

        $datatoinsert_histo="".$EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID.",".$USER_ID_ENG.",".$ETAPE_ID.",'".str_replace("'", "\'", $COMMENTAIRE)."','".$DATE_TRANSMISSION."','".$DATE_RECEPTION."'";
      ...
      {
        return $this->eng_juridique(MD5($EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID));
      }
    }

    //Correction de l'étape 4 Engagement juridique
    function update_corriger_etape4()
    {
      ...
      if($this->validation->withRequest($this->request)->run())
      {
        ...
        if($MARCHE_PUBLIQUE == 0)
        {
          ...
          if($TYPE_MONNAIE != 1)
          {
            ...
            //update la table execution_budgetaire_titre_decaissement
            $whereEBTD='EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID='.$EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID;
            $insertEBTD='execution_budgetaire_titre_decaissement';
            $columEBTD='ETAPE_DOUBLE_COMMANDE_ID='.$NEXT_ETAPE_ID;
            $this->update_all_table($insertEBTD,$columEBTD,$whereEBTD);
        ...
        elseif($MARCHE_PUBLIQUE == 1)
        {
          ...
          else
          {
            if ($path_avis->getSize() > $maxFileSize)
            {
              $data=['message' => "".lang('messages_lang.pdf_max').""];
              session()->setFlashdata('alert', $data);
              return $this->corriger_juridique(md5($EXEC_BUDGET_RAC_DET_ID));
            }
            $PATH_AVIS_DNCMP=$this->uploadFile('path_avis','double_commande_new','AVIS_DNCMP');
          }
          ...
          if($TYPE_MONNAIE !=1)
          {
           ...
           //Enregistrement dans activite_detail
            ...
            $table_info='execution_budgetaire_tache_info_suppl';
            $conditions_info='EXECUTION_BUDGETAIRE_ID='.$EXEC_BUDGET_RAC_ID;
            $datatomodifie_info= 'MODELE_ID='.$MODEL_ID.', REFERENCE="'.$REFERENCE.'", PATH_PPM="'.$PATH_PPM_DOC.'",PATH_PV_ATTRIBUTION="'.$PATH_PV_ATTRIBUTION.'",PATH_AVIS_DNCMP="'.$PATH_AVIS_DNCMP.'", DATE_DEBUT_CONTRAT="'.$DATE_DEBUT.'", DATE_FIN_CONTRAT="'.$DATE_FIN.'", NBRE_JR_CONTRAT='.$jours.', TYPE_BENEFICIAIRE_ID='.$TYPE_BENEFICIARE_ID.', PRESTATAIRE_ID='.$PRESTATAIRE_ID;
            $this->update_all_table($table_info,$datatomodifie_info,$conditions_info);
          }
          else
          {
        ...
        if (!$file || !$file->isValid())
        {
          $PATH_MODELE_PAIEMENT=$this->request->getPost('PATH_MODELE_PAIEMENT_OLD');
        }
        else
        {
          $PATH_MODELE_PAIEMENT = $this->request->getPost('PATH_MODELE_PAIEMENT');
          $PATH_MODELE_PAIEMENT=$this->uploadFile('PATH_MODELE_PAIEMENT','double_commande_new','MODELE_PAIEMENT');            
        }

        $table='execution_budgetaire_tache_info_suppl';
        $conditions='EXECUTION_BUDGETAIRE_ID='.$EXEC_BUDGET_RAC_ID;
        $datatomodifie= 'PATH_MODELE_PAIEMENT="'.$PATH_MODELE_PAIEMENT.'"';
        $this->update_all_table($table,$datatomodifie,$conditions);

        //récuperer le EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID        
        $columsinsert_histo="EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID,USER_ID,ETAPE_DOUBLE_COMMANDE_ID,OBSERVATION,DATE_TRANSMISSION,DATE_RECEPTION";
      ...
      else
      {
        ...
        $getdemanande = "SELECT exec.DEVISE_TYPE_HISTO_ENG_ID,
                              ...
                              tache.SOUS_TUTEL_ID,
                              PATH_PPM,PATH_PV_ATTRIBUTION,
                              PATH_AVIS_DNCMP,PATH_BON_COMMANDE,
                              info.BUDGETAIRE_TYPE_DOCUMENT_ID,
                              ebtd.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID 
                       FROM execution_budgetaire_tache_detail det 
                       JOIN execution_budgetaire exec ON exec.EXECUTION_BUDGETAIRE_ID=det.EXECUTION_BUDGETAIRE_ID 
        ...
        if (!empty($getProfil))
        {
          foreach ($getProfil as $value)
          {
            ...
            //Le min de la date de réception
              $bind_date_histo = $this->getBindParms('EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID,OBSERVATION,DATE_TRANSMISSION,DATE_INSERTION','execution_budgetaire_tache_detail_histo','MD5(EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID)="'.$EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID.'"','DATE_INSERTION DESC');
              $bind_date_histo = str_replace('\\','',$bind_date_histo);
              $data['date_trans'] = $this->ModelPs->getRequeteOne($psgetrequete, $bind_date_histo);
    ...
    //Enregistrement de l'étape 5 Confirmation d'engagement juridique
      ...
      if($this->validation->withRequest($this->request)->run())
      {
        if($MARCHE_PUBLIQUE == 1)
        {
          if($OPERATION_ID == 1)
          {
            ...
            $columsinsert_histo="EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID,USER_ID,OBSERVATION,ETAPE_DOUBLE_COMMANDE_ID,DATE_RECEPTION,DATE_TRANSMISSION";

            $datatoinsert_histo="".$EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID.",".$USER_ID_ENG.",'".str_replace("'", "\'", $COMMENTAIRE)."',".$ETAPE_ID.",'".$DATE_RECEPTION."','".$DATE_TRANSMISSION."'";
          ...
          elseif($OPERATION_ID == 3)
          {
            ...
            //Vider les montants de l'engagement juridique
