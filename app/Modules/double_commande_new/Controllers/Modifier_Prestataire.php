...
class Modifier_Prestataire extends BaseController
{
	...
	public function get_view()
	{
		...
		// get BEs
		$bind_BEs=$this->getBindParms('exec.NUMERO_BON_ENGAGEMENT,td.EXECUTION_BUDGETAIRE_ID','execution_budgetaire_titre_decaissement td JOIN execution_budgetaire exec ON exec.EXECUTION_BUDGETAIRE_ID=td.EXECUTION_BUDGETAIRE_ID JOIN execution_budgetaire_tache_info_suppl supp ON supp.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID JOIN execution_budgetaire_execution_tache tache ON tache.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=tache.PTBA_TACHE_ID','NUMERO_BON_ENGAGEMENT IS NOT NULL AND ptba.INSTITUTION_ID IN (SELECT INSTITUTION_ID FROM user_affectaion WHERE USER_ID='.$user_id.') AND PRESTATAIRE_ID IS NOT NULL AND TYPE_BENEFICIAIRE_ID IS NOT NULL GROUP BY td.EXECUTION_BUDGETAIRE_ID','NUMERO_BON_ENGAGEMENT ASC');
