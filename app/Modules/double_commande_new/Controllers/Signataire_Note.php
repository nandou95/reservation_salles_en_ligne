...
class Signataire_Note extends BaseController
{
  ...
  function save_newPoste()
  {
    ...
    $DESC_POSTE_SIGNATAIRE = $this->request->getPost('DESC_POSTE_SIGNATAIRE');
    $DESC_POSTE_SIGNATAIRE=addslashes($DESC_POSTE_SIGNATAIRE);
    $table="poste_signataire";
    $columsinsert = "DESC_POSTE_SIGNATAIRE";
    $datacolumsinsert = "'{$DESC_POSTE_SIGNATAIRE}'";    
    $this->save_all_table($table,$columsinsert,$datacolumsinsert);

    $callpsreq = "CALL getRequete(?,?,?,?);";

      //récuperer les motifs
    $bind_poste = $this->getBindParms('POSTE_SIGNATAIRE_ID,DESC_POSTE_SIGNATAIRE','poste_signataire','1','POSTE_SIGNATAIRE_ID ASC');
    $poste = $this->ModelPs->getRequete($callpsreq, $bind_poste);

    $html="".lang('messages_lang.selectionner_transmission_du_bordereau')."";

    if(!empty($poste))
    {
      foreach($poste as $key)
      { 
        $html.= "<option value='".$key->POSTE_SIGNATAIRE_ID."'>".$key->DESC_POSTE_SIGNATAIRE."</option>";
      }
    }
    $html.='<option value="-1">'.lang('messages_lang.selection_autre').'</option>';
    $output = array('status' => TRUE ,'poste' => $html);
  ...
  public function get_view()
  {
    ...
    $cart->destroy();
    $bind_data = $this->getBindParms('INSTITUTION_SIGNATAIRE_ID_TEMPO,INSTITUTION_ID_TEMPO,DESCRIPTION_INSTITUTION,DESCRIPTION_SOUS_TUTEL,SOUS_TUTEL_ID_TEMPO,tempo.POSTE_SIGNATAIRE_ID,DESC_POSTE_SIGNATAIRE as DESC_POSTE_TEMPO,NOM_PRENOM_TEMPO','inst_institutions_signataires_notes_tempo tempo JOIN inst_institutions inst ON tempo.INSTITUTION_ID_TEMPO=inst.INSTITUTION_ID LEFT JOIN inst_institutions_sous_tutel st ON tempo.SOUS_TUTEL_ID_TEMPO=st.SOUS_TUTEL_ID JOIN poste_signataire poste ON tempo.POSTE_SIGNATAIRE_ID=poste.POSTE_SIGNATAIRE_ID','USER_ID='.$user_id,'1');
    ...
    $poste  = 'SELECT POSTE_SIGNATAIRE_ID,DESC_POSTE_SIGNATAIRE FROM poste_signataire WHERE 1';
    $poste = "CALL getTable('" .$poste. "');";
    $data['poste'] = $this->ModelPs->getRequete($poste);
    return view('App\Modules\double_commande_new\Views\Signataire_Note_Add_View',$data);
  }

   public function get_update_view($value=0)
  {
    $session  = \Config\Services::session();
    $callpsreq = "CALL `getRequete`(?,?,?,?);";
    if(!empty($session->get('SESSION_SUIVIE_PTBA_USER_ID')))
    ...
    $donnees  = 'SELECT inst.INSTITUTION_ID,INSTITUTION_SIGNATAIRE_ID,st.SOUS_TUTEL_ID,DESCRIPTION_SOUS_TUTEL,poste.POSTE_SIGNATAIRE_ID,NOM_PRENOM FROM inst_institutions_signataires_notes sign JOIN inst_institutions inst ON inst.INSTITUTION_ID=sign.INSTITUTION_ID LEFT JOIN inst_institutions_sous_tutel st ON st.SOUS_TUTEL_ID=sign.SOUS_TUTEL_ID JOIN poste_signataire poste ON poste.POSTE_SIGNATAIRE_ID=sign.POSTE_SIGNATAIRE_ID WHERE INSTITUTION_SIGNATAIRE_ID='.$value;
    $donnees = "CALL getTable('" .$donnees. "');";
    $data['donnees'] = $this->ModelPs->getRequeteOne($donnees);
    return view('App\Modules\double_commande_new\Views\Signataire_Note_Update_View',$data);
  }
  ...
  public function update_sign()
  {
    ...
    $INSTITUTION_ID=$this->request->getPost('INSTITUTION_ID');
    $POSTE_SIGNATAIRE_ID=$this->request->getPost('POSTE_SIGNATAIRE_ID');
    ...
    $INSTITUTION_SIGNATAIRE_ID=$this->request->getPost('INSTITUTION_SIGNATAIRE_ID');

    $where="INSTITUTION_SIGNATAIRE_ID=".$INSTITUTION_SIGNATAIRE_ID;
    $columsinsert="INSTITUTION_ID=".$INSTITUTION_ID.",SOUS_TUTEL_ID=".$SOUS_TUTEL_ID.",POSTE_SIGNATAIRE_ID=".$POSTE_SIGNATAIRE_ID.",NOM_PRENOM='".$NOM_PRENOM."'";
    $table='inst_institutions_signataires_notes';
    $this->update_all_table($table,$columsinsert,$where);
  ...
  public function save_tempo()
  {
    ...
    $columsinsert="INSTITUTION_ID_TEMPO,SOUS_TUTEL_ID_TEMPO,POSTE_SIGNATAIRE_ID,NOM_PRENOM_TEMPO,USER_ID";
    $datatoinsert= $INSTITUTION_ID.','.$SOUS_TUTEL_ID.',"'.$POSTE_SIGNATAIRE_ID.'","'.$NOM_PRENOM.'",'.$user_id;
  ...
  public function save()
  {
    ...
    foreach($bind_data as $val)
    {
      ...
      $columsinsert="INSTITUTION_ID,SOUS_TUTEL_ID,POSTE_SIGNATAIRE_ID,NOM_PRENOM";
      $datatoinsert= $val->INSTITUTION_ID_TEMPO.','.$SOUS_TUTEL_ID_TEMPO.',"'.$val->POSTE_SIGNATAIRE_ID.'","'.$val->NOM_PRENOM_TEMPO.'"';
      $table='inst_institutions_signataires_notes';
      $this->save_all_table($table,$columsinsert,$datatoinsert);
  ...
  function listing()
  {
    ...
    $requetedebase="SELECT INSTITUTION_SIGNATAIRE_ID,inst.CODE_INSTITUTION,inst.DESCRIPTION_INSTITUTION,st.CODE_SOUS_TUTEL,st.DESCRIPTION_SOUS_TUTEL,poste.POSTE_SIGNATAIRE_ID,DESC_POSTE_SIGNATAIRE AS DESC_POSTE,NOM_PRENOM FROM inst_institutions_signataires_notes sign JOIN inst_institutions inst ON inst.INSTITUTION_ID=sign.INSTITUTION_ID LEFT JOIN inst_institutions_sous_tutel st ON st.SOUS_TUTEL_ID=sign.SOUS_TUTEL_ID JOIN poste_signataire poste ON sign.POSTE_SIGNATAIRE_ID=poste.POSTE_SIGNATAIRE_ID WHERE 1";

    $order_column=array('DESCRIPTION_INSTITUTION','DESCRIPTION_SOUS_TUTEL','DESC_POSTE_SIGNATAIRE','NOM_PRENOM',1);
    ...
    $search = !empty($_POST['search']['value']) ?  (" AND (DESCRIPTION_INSTITUTION LIKE '%$var_search%' OR DESCRIPTION_SOUS_TUTEL LIKE '%$var_search%' OR DESC_POSTE_SIGNATAIRE LIKE '%$var_search%' OR NOM_PRENOM LIKE '%$var_search%')"):'';
