...
class Reception_borderau_brb extends BaseController
{
    ...
    if($session->get('SESSION_SUIVIE_PTBA_RECEPTION_BRB') !=1)
    {
      return redirect('Login_Ptba/homepage'); 
		...
		//-id detail -
		$callpsreq = "CALL getRequete(?,?,?,?);";
		$get_id_detail = $this->getBindParms('det.EXECUTION_BUDGETAIRE_DETAIL_ID,det.EXECUTION_BUDGETAIRE_ID,td.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID', 'execution_budgetaire_bordereau_transmission_bon_titre bon JOIN execution_budgetaire_titre_decaissement td ON td.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID=bon.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_DETAIL_ID = td.EXECUTION_BUDGETAIRE_DETAIL_ID', 'MD5(bon.BORDEREAU_TRANSMISSION_ID)="'.$bordereau_transmission_id.'"', 'det.EXECUTION_BUDGETAIRE_DETAIL_ID ASC');
		...
		$id_exec_titr_dec = md5($id_detail['EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID']);

		//Recuperation du statut operation bordereau transmission
		$requetebase3 = "SELECT STATUT_OPERATION_BORDEREAU_TRANSMISSION_ID,DESC_STATUT_OPERATION_BORDEREAU_TRANSMISSION FROM statut_operation_bordereau_transmission WHERE STATUT_OPERATION_BORDEREAU_TRANSMISSION_ID = 2";
		$fetch_data3 = $this->ModelPs->getRequeteOne("CALL `getTable` ('" . $requetebase3 . "')");
		$data['statut_operation_bordereau'] = $fetch_data3;

		// //Titre de la page
		$bindparamsetap = $this->getBindParms('td.ETAPE_DOUBLE_COMMANDE_ID,DESC_ETAPE_DOUBLE_COMMANDE,MONTANT_ORDONNANCEMENT','execution_budgetaire_etape_double_commande commande JOIN execution_budgetaire_titre_decaissement td ON td.ETAPE_DOUBLE_COMMANDE_ID=commande.ETAPE_DOUBLE_COMMANDE_ID JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_DETAIL_ID=td.EXECUTION_BUDGETAIRE_DETAIL_ID','md5(td.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID)="'.$id_exec_titr_dec.'"','1 DESC');
      ...
      if (!empty($getProfil))
      {
        foreach ($getProfil as $value)
        {
        	if ($prof_id == $value->PROFIL_ID || $prof_id==1)
          {
          	...
          	$requetebase1 = "SELECT trans.BORDEREAU_TRANSMISSION_ID, trans.NUMERO_BORDEREAU_TRANSMISSION,td.ETAPE_DOUBLE_COMMANDE_ID FROM execution_budgetaire_bordereau_transmission trans JOIN execution_budgetaire_bordereau_transmission_bon_titre bon_titre ON bon_titre.BORDEREAU_TRANSMISSION_ID =trans.BORDEREAU_TRANSMISSION_ID JOIN execution_budgetaire_titre_decaissement td ON td.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID=bon_titre.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_DETAIL_ID=td.EXECUTION_BUDGETAIRE_DETAIL_ID  WHERE md5(trans.BORDEREAU_TRANSMISSION_ID) = \'" . $bordereau_transmission_id . "\'";
          	...
          	$EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID = md5($id_detail['EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID']);
          	$requetebase2 = "SELECT DATE_TRANSMISSION,EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID FROM execution_budgetaire_tache_detail_histo WHERE 
          	md5(EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID) = \'" . $EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID . "\' ORDER BY EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID DESC";
          	...
          	$requetebase3 = "SELECT bon_titre.BORDEREAU_TRANSMISSION_BON_TITRE_ID,bon_titre.NUMERO_DOCUMENT,detail.EXECUTION_BUDGETAIRE_DETAIL_ID,td.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID FROM execution_budgetaire_bordereau_transmission_bon_titre bon_titre JOIN execution_budgetaire_titre_decaissement td ON td.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID=bon_titre.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID JOIN execution_budgetaire_tache_detail detail ON detail.EXECUTION_BUDGETAIRE_DETAIL_ID = td.EXECUTION_BUDGETAIRE_DETAIL_ID	WHERE md5(bon_titre.BORDEREAU_TRANSMISSION_ID) =\'" . $bordereau_transmission_id . "\' AND  ETAPE_DOUBLE_COMMANDE_ID=\'" . $ID_ETAPE_COMMANDE . "\'";
    ...
    if($session->get('SESSION_SUIVIE_PTBA_RECEPTION_BRB') !=1)
    {
      return redirect('Login_Ptba/homepage'); 
		...
		{
			$conditions_3 = 'BORDEREAU_TRANSMISSION_ID='.$BORDEREAU_ID.' AND EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID=' . $value;
			...
			$critere = "EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID=" . $value;
