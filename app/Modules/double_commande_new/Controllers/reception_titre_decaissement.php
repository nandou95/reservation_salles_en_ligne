...
class reception_titre_decaissement extends BaseController
{
  ...
  /**  page d'acceuil  */
  public function index($id_crypts)
  {
    $data = $this->urichk();
    ...
    $id_td_crypt = md5($etap_borderaux_transmission_id['EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID']);

    //--etape actuel --
    $callpsreq = "CALL `getRequete`(?,?,?,?);";
    $get_etape = $this->getBindParms('ETAPE_DOUBLE_COMMANDE_ID','execution_budgetaire_titre_decaissement','MD5(EXECUTION_BUDGETAIRE_DETAIL_ID)="'.$id_crypt.'"','EXECUTION_BUDGETAIRE_DETAIL_ID DESC');
    $get_etape = str_replace('\\', '', $get_etape);
    $etap_borderaux_transmission_id = $this->ModelPs->getRequeteOne($callpsreq, $get_etape);
    ...
    if(!empty($getProfil))
    {
      foreach ($getProfil as $value)
      {
        if ($prof_id == $value->PROFIL_ID || $prof_id==1)
        {
          ...
          $requetebase3 = "SELECT DISTINCT bon_titre.BORDEREAU_TRANSMISSION_BON_TITRE_ID,bon_titre.NUMERO_DOCUMENT,racc_detail.EXECUTION_BUDGETAIRE_DETAIL_ID,td.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID  FROM 
          execution_budgetaire_bordereau_transmission_bon_titre bon_titre JOIN execution_budgetaire_titre_decaissement td ON td.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID=bon_titre.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID JOIN
          execution_budgetaire_tache_detail racc_detail  ON racc_detail.EXECUTION_BUDGETAIRE_DETAIL_ID= td.EXECUTION_BUDGETAIRE_DETAIL_ID
          ...
          //--date transmission
          $callpsreq = "CALL `getRequete`(?,?,?,?);";
          $get_hist = $this->getBindParms('EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID,DATE_TRANSMISSION', 'execution_budgetaire_tache_detail_histo', 'MD5(EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID)="'.$id_td_crypt.'"', ' DATE_INSERTION DESC');
          $get_hist = str_replace('\\', '', $get_hist);
          $data['hist'] = $this->ModelPs->getRequeteOne($callpsreq, $get_hist);

          $callpsreq = "CALL `getRequete`(?,?,?,?);";
          $get_hist = $this->getBindParms('EXECUTION_BUDGETAIRE_ID','execution_budgetaire_tache_detail','MD5(EXECUTION_BUDGETAIRE_DETAIL_ID)="'.$id_crypt.'"','EXECUTION_BUDGETAIRE_DETAIL_ID DESC');
          $get_hist=str_replace('\\', '', $get_hist);
