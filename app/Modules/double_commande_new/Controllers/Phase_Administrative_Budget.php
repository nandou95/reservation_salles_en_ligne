...
class Phase_Administrative_Budget extends BaseController
{
  ...
  function save_etape1()
  {
    ...
    if($this->validation->withRequest($this->request)->run())
    {
      ...
      $MoneyRest  = 'SELECT ANNEE_BUDGETAIRE_ID,PTBA_TACHE_ID,BUDGET_RESTANT_T1,BUDGET_RESTANT_T2,BUDGET_RESTANT_T3,BUDGET_RESTANT_T4,BUDGET_UTILISE_T1,BUDGET_UTILISE_T2,BUDGET_UTILISE_T3,BUDGET_UTILISE_T4 FROM ptba_tache WHERE PTBA_TACHE_ID='.$PTBA_TACHE_ID.' AND ANNEE_BUDGETAIRE_ID='.$ANNEE_BUDGETAIRE_ID;
      $MoneyRest = "CALL `getTable`('" . $MoneyRest . "');";
      $RestPTBA = $this->ModelPs->getRequeteOne($MoneyRest);

      $existance = 'SELECT exec.EXECUTION_BUDGETAIRE_ID FROM execution_budgetaire exec JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID JOIN execution_budgetaire_execution_tache ebet ON ebet.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID JOIN execution_budgetaire_titre_decaissement ebtd ON ebtd.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID WHERE ebet.PTBA_TACHE_ID='.$PTBA_TACHE_ID.' AND NUMERO_BON_ENGAGEMENT IS NULL AND ebtd.ETAPE_DOUBLE_COMMANDE_ID <>42';
      ...
      if (empty($id_exist))
      {
        ...
        $col_intro='';
        $val_intro='';
        if($BUDGETAIRE_TYPE_DOCUMENT_ID==1)
        {
          $col_intro=',INTRODUCTION_NOTE';
          $val_intro=",'".$intro_note."'";
        }
        ...
        //insert into execution_budgetaire
        $EXECUTION_BUDGETAIRE_TYPE_EXECUTION_ID=1;
        $insertIntoExec='execution_budgetaire';
        $columExec="ANNEE_BUDGETAIRE_ID,INSTITUTION_ID,SOUS_TUTEL_ID,CODE_NOMENCLATURE_BUDGETAIRE_ID,TRIMESTRE_ID,ENG_BUDGETAIRE,ENG_BUDGETAIRE_DEVISE,DEVISE_TYPE_HISTO_ENG_ID,USER_ID,MARCHE_PUBLIQUE,COMMENTAIRE,EXECUTION_BUDGETAIRE_TYPE_EXECUTION_ID,DEVISE_TYPE_ID,TYPE_ENGAGEMENT_ID";
        $datacolumsExec=$ANNEE_BUDGETAIRE_ID.",".$INSTITUTION_ID.",".$SOUS_TUTEL_ID.",".$CODE_NOMENCLATURE_BUDGETAIRE_ID.",".$TRIMESTRE_ID.",".$MONTANT_RACCROCHE.",".$MONTANT_EN_DEVISE.",".$DEVISE_TYPE_HISTO_ID.",".$user_id.",".$MARCHE_PUBLIQUE.",'".$COMMENTAIRE."','".$EXECUTION_BUDGETAIRE_TYPE_EXECUTION_ID."',".$TYPE_MONTANT_ID.",".$TYPE_ENGAGEMENT_ID;
        $EXECUTION_BUDGETAIRE_ID =$this->save_all_table($insertIntoExec,$columExec,$datacolumsExec);

        //insert into execution_budgetaire_execution_tache
        $insertIntoExecBudgExecTache='execution_budgetaire_execution_tache';
        $columExecBudgExecTache="PTBA_TACHE_ID,QTE,EXECUTION_BUDGETAIRE_ID,EST_SOUS_TACHE,EST_FINI_TACHE,RESULTAT_ATTENDUS,OBSERVATION_RESULTAT,MONTANT_ENG_BUDGETAIRE,MONTANT_ENG_BUDGETAIRE_DEVISE";
        $datacolumsExecBudgExecTache=$PTBA_TACHE_ID.",'".$QTE_RACCROCHE."',".$EXECUTION_BUDGETAIRE_ID.",".$sous_act.",".$fini.",'".$resultat_attend."','".$observ_resultat."',".$MONTANT_RACCROCHE.",".$MONTANT_EN_DEVISE."";
        $EXECUTION_BUDGETAIRE_EXECUTION_TACHE_ID =$this->save_all_table($insertIntoExecBudgExecTache,$columExecBudgExecTache,$datacolumsExecBudgExecTache);

        $insertDetail = 'execution_budgetaire_tache_detail';
        
        $columDetail = "EXECUTION_BUDGETAIRE_ID,COMMENTAIRE,DATE_COUR_DEVISE,COUR_DEVISE".$col_intro;

        if ($DATE_COUT_DEVISE===null) {
          $datacolumsDetail = $EXECUTION_BUDGETAIRE_ID.",'".$COMMENTAIRE."',null,".$COUS_ECHANGE.$val_intro;
        } else {
          $datacolumsDetail = $EXECUTION_BUDGETAIRE_ID.",'".$COMMENTAIRE."',null,".$COUS_ECHANGE.$val_intro;
        }
        ...
        //insert into execution_budgetaire_titre_decaissement
        $insertIntoTitre_Dec = 'execution_budgetaire_titre_decaissement';
        $columTitre_Dec = "ETAPE_DOUBLE_COMMANDE_ID,EXECUTION_BUDGETAIRE_ID,EXECUTION_BUDGETAIRE_DETAIL_ID";
        $datacolumsTitre_Dec = $ETAPE_DOUBLE_COMMANDE_ID.",".$EXECUTION_BUDGETAIRE_ID.",".$EXECUTION_BUDGETAIRE_DETAIL_ID;

        $EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID = $this->save_all_table($insertIntoTitre_Dec, $columTitre_Dec, $datacolumsTitre_Dec);

        //insert into execution_budgetaire_tache_info_suppl
        $insertIntoSuppl='execution_budgetaire_tache_info_suppl';

        $columSuppl="EXECUTION_BUDGETAIRE_ID,BUDGETAIRE_TYPE_DOCUMENT_ID,PATH_LETTRE_TRANSMISSION,PATH_LISTE_PAIE,ID_TYPE_MARCHE";

        $datacolumsSuppl=$EXECUTION_BUDGETAIRE_ID.",".$BUDGETAIRE_TYPE_DOCUMENT_ID.",'".$PATH_LETTRE_TRANSMISSION."','".$PATH_LISTE_PAIE."',".$ID_TYPE_MARCHE;
        $this->save_all_table($insertIntoSuppl,$columSuppl,$datacolumsSuppl);

        $insertIntohist='execution_budgetaire_tache_detail_histo';
        $columhist="EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID,USER_ID,OBSERVATION,ETAPE_DOUBLE_COMMANDE_ID";
        $datacolumshist=$EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID .",".$USER_ID.",'".$COMMENTAIRE."',".$ETAPE_ID;
        ...
        else
        {
          ...
          if($TRIMESTRE_ID==1)
          {
            if($MONTANT_RACCROCHE > $RestPTBA['BUDGET_RESTANT_T1'])
            {
              $resultat1=$MONTANT_RACCROCHE-$RestPTBA['BUDGET_RESTANT_T1'];
              $restant_t1=0;
              $utilise_t1=$RestPTBA['BUDGET_RESTANT_T1']+$RestPTBA['BUDGET_UTILISE_T1'];
              $BUDGET_ENLEVE_T1=$RestPTBA['BUDGET_RESTANT_T1'];
              
              $columptba="BUDGET_RESTANT_T1=".$restant_t1.",BUDGET_UTILISE_T1=".$utilise_t1;
              $this->update_all_table($insertIntoptba,$columptba,$whereptba);
              if ($resultat1 > 0)
              {
                if($resultat1 > $RestPTBA['BUDGET_RESTANT_T2'])
                {
                  $resultat2=$resultat1-$RestPTBA['BUDGET_RESTANT_T2'];
                  $restant_t2=0;
                  $utilise_t2=$RestPTBA['BUDGET_RESTANT_T2']+$RestPTBA['BUDGET_UTILISE_T2'];
                  $BUDGET_ENLEVE_T2=$RestPTBA['BUDGET_RESTANT_T2'];
                  $columptba="BUDGET_RESTANT_T2=".$restant_t2.",BUDGET_UTILISE_T2=".$utilise_t2;
                  $this->update_all_table($insertIntoptba,$columptba,$whereptba);

                  if($resultat2 > 0)
                  {
                    if($resultat2 > $RestPTBA['BUDGET_RESTANT_T3'])
                    {
                      $resultat3=$resultat2-$RestPTBA['BUDGET_RESTANT_T3'];
                      $restant_t3=0;
                      $utilise_t3=$RestPTBA['BUDGET_RESTANT_T3']+$RestPTBA['BUDGET_UTILISE_T3'];
                      $BUDGET_ENLEVE_T3=$RestPTBA['BUDGET_RESTANT_T3'];
                      $columptba="BUDGET_RESTANT_T3=".$restant_t3.",BUDGET_UTILISE_T3=".$utilise_t3;
                      $this->update_all_table($insertIntoptba,$columptba,$whereptba);

                      if($resultat3 > 0)
                      {
                        if($resultat3 == $RestPTBA['BUDGET_RESTANT_T4'])
                        {
                          $resultat4=$resultat3-$RestPTBA['BUDGET_RESTANT_T4'];
                          $restant_t4=0;
                          $utilise_t4=$RestPTBA['BUDGET_RESTANT_T4']+$RestPTBA['BUDGET_UTILISE_T4'];
                          $BUDGET_ENLEVE_T4=$RestPTBA['BUDGET_RESTANT_T4'];
                          $columptba="BUDGET_RESTANT_T4=".$restant_t4.",BUDGET_UTILISE_T4=".$utilise_t4;
                          $this->update_all_table($insertIntoptba,$columptba,$whereptba);
                        }
                        else
                        {
                          $restant_t4=$RestPTBA['BUDGET_RESTANT_T4']-$resultat3;
                          $utilise_t4=$resultat3+$RestPTBA['BUDGET_UTILISE_T4'];
                          $BUDGET_ENLEVE_T4=$resultat3;
                          $columptba="BUDGET_RESTANT_T4=".$restant_t4.",BUDGET_UTILISE_T4=".$utilise_t4;
                          $this->update_all_table($insertIntoptba,$columptba,$whereptba);
                        }
                      }                      
                    ...
                    {
                      $restant_t3=$RestPTBA['BUDGET_RESTANT_T3']-$resultat2;
                      $utilise_t3=$resultat2+$RestPTBA['BUDGET_UTILISE_T3'];
                      $BUDGET_ENLEVE_T3=$resultat2;
                      $columptba="BUDGET_RESTANT_T3=".$restant_t3.",BUDGET_UTILISE_T3=".$utilise_t3;
                      $this->update_all_table($insertIntoptba,$columptba,$whereptba);
                    }                   
                  }
                ...
                {
                  $restant_t2=$RestPTBA['BUDGET_RESTANT_T2']-$resultat1;
                  $utilise_t2=$resultat1+$RestPTBA['BUDGET_UTILISE_T2'];
                  $BUDGET_ENLEVE_T2=$resultat1;
                  $columptba="BUDGET_RESTANT_T2=".$restant_t2.",BUDGET_UTILISE_T2=".$utilise_t2;
                  $this->update_all_table($insertIntoptba,$columptba,$whereptba);
                }               
              }
            ...
            {
              $restant_t1=$RestPTBA['BUDGET_RESTANT_T1']-$MONTANT_RACCROCHE;
              $utilise_t1=$MONTANT_RACCROCHE+$RestPTBA['BUDGET_UTILISE_T1'];
              $columptba="BUDGET_RESTANT_T1=".$restant_t1.",BUDGET_UTILISE_T1=".$utilise_t1;
              $BUDGET_ENLEVE_T1=$MONTANT_RACCROCHE;
              $this->update_all_table($insertIntoptba,$columptba,$whereptba);
            }
          }
          elseif($TRIMESTRE_ID==2)
          {
            if($MONTANT_RACCROCHE > $RestPTBA['BUDGET_RESTANT_T2'])
            {
              $resultat2=$MONTANT_RACCROCHE-$RestPTBA['BUDGET_RESTANT_T2'];
              $restant_t2=0;
              $utilise_t2=$RestPTBA['BUDGET_RESTANT_T2']+$RestPTBA['BUDGET_UTILISE_T2'];
              $BUDGET_ENLEVE_T2=$RestPTBA['BUDGET_RESTANT_T2'];
              $columptba="BUDGET_RESTANT_T2=".$restant_t2.",BUDGET_UTILISE_T2=".$utilise_t2;
            ...
            {
              $restant_t2=$RestPTBA['BUDGET_RESTANT_T2']-$MONTANT_RACCROCHE;
              $utilise_t2=$MONTANT_RACCROCHE+$RestPTBA['BUDGET_UTILISE_T2'];
              $BUDGET_ENLEVE_T2=$MONTANT_RACCROCHE;
              $columptba="BUDGET_RESTANT_T2=".$restant_t2.",BUDGET_UTILISE_T2=".$utilise_t2;
              $this->update_all_table($insertIntoptba,$columptba,$whereptba);
            }
          }
          ...
          {
            if($MONTANT_RACCROCHE > $RestPTBA['BUDGET_RESTANT_T3'])
            {
              $resultat3=$MONTANT_RACCROCHE-$RestPTBA['BUDGET_RESTANT_T3'];
              $restant_t3=0;
              $utilise_t3=$RestPTBA['BUDGET_RESTANT_T3']+$RestPTBA['BUDGET_UTILISE_T3'];
              $BUDGET_ENLEVE_T3=$RestPTBA['BUDGET_RESTANT_T3'];
              $columptba="BUDGET_RESTANT_T3=".$restant_t3.",BUDGET_UTILISE_T3=".$utilise_t3;
            ...
            {
              $restant_t3=$RestPTBA['BUDGET_RESTANT_T3']-$MONTANT_RACCROCHE;
              $utilise_t3=$MONTANT_RACCROCHE+$RestPTBA['BUDGET_UTILISE_T3'];
              $BUDGET_ENLEVE_T3=$MONTANT_RACCROCHE;
              $columptba="BUDGET_RESTANT_T3=".$restant_t3.",BUDGET_UTILISE_T3=".$utilise_t3;
              $this->update_all_table($insertIntoptba,$columptba,$whereptba);
            } 
          }
          ...
          {
            if($MONTANT_RACCROCHE == $RestPTBA['BUDGET_RESTANT_T4'])
            {
              $resultat4=$MONTANT_RACCROCHE-$RestPTBA['BUDGET_RESTANT_T4'];
              $restant_t4=0;
              $BUDGET_ENLEVE_T4=$RestPTBA['BUDGET_RESTANT_T4'];
              $utilise_t4=$RestPTBA['BUDGET_RESTANT_T4']+$RestPTBA['BUDGET_UTILISE_T4'];
              $columptba="BUDGET_RESTANT_T4=".$restant_t4.",BUDGET_UTILISE_T4=".$utilise_t4;
            ...
            {
              $restant_t4=$RestPTBA['BUDGET_RESTANT_T4']-$MONTANT_RACCROCHE;
              $utilise_t4=$MONTANT_RACCROCHE+$RestPTBA['BUDGET_UTILISE_T4'];
              $BUDGET_ENLEVE_T4=$MONTANT_RACCROCHE;
              $columptba="BUDGET_RESTANT_T4=".$restant_t4.",BUDGET_UTILISE_T4=".$utilise_t4;
              $this->update_all_table($insertIntoptba,$columptba,$whereptba);
            }
          }

          $insertIntohistbudg='execution_budgetaire_histo_budget_marche_public';
          $columhistbudg="EXECUTION_BUDGETAIRE_ID,BUDGET_ENLEVE_T1,BUDGET_ENLEVE_T2,BUDGET_ENLEVE_T3,BUDGET_ENLEVE_T4,ANNEE_BUDGETAIRE_ID";
          $datacolumshistbudg=$EXECUTION_BUDGETAIRE_ID.",".$BUDGET_ENLEVE_T1.",".$BUDGET_ENLEVE_T2.",".$BUDGET_ENLEVE_T3.",".$BUDGET_ENLEVE_T4.",".$ANNEE_BUDGETAIRE_ID;
          $this->save_all_table($insertIntohistbudg,$columhistbudg,$datacolumshistbudg);
        }
      }
      else
      {
  ...
  function etape1_prime($id=0)
  {
    ...
    $infoAffiche  = 'SELECT det.EXECUTION_BUDGETAIRE_DETAIL_ID,
                            ebtd.ETAPE_DOUBLE_COMMANDE_ID,
                            exec.EXECUTION_BUDGETAIRE_ID,
                            ab.ANNEE_DESCRIPTION,
                            inst.CODE_INSTITUTION,
                            ebtd.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID 
                     FROM execution_budgetaire_tache_detail det 
                     JOIN execution_budgetaire exec ON exec.EXECUTION_BUDGETAIRE_ID=det.EXECUTION_BUDGETAIRE_ID 
                     JOIN execution_budgetaire_execution_tache ebet ON ebet.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID 
                     ...
                     JOIN annee_budgetaire ab ON exec.ANNEE_BUDGETAIRE_ID=ab.ANNEE_BUDGETAIRE_ID 
                     JOIN execution_budgetaire_titre_decaissement ebtd ON ebtd.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID 
                     WHERE md5(ebtd.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID) = "'.$id.'"';
  ...
  function save_etape1_prime()
  {
    ...
    if($this->validation->withRequest($this->request)->run())
    {
      ...
      $insertIntoEBTD='execution_budgetaire_titre_decaissement';
      $columEBTD="ETAPE_DOUBLE_COMMANDE_ID = ".$ETAPE_DOUBLE_COMMANDE_ID;
      $this->update_all_table($insertIntoEBTD,$columEBTD,$whereEBTD);
      ...
      $columOp="EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID,ETAPE_DOUBLE_COMMANDE_ID,USER_ID,DATE_TRANSMISSION";
      $datacolumsOp=$EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID.",".$ETAPE_ID.",".$USER_ID.",'".$DATE_TRANSMISSION."'";
    ...
    {
      return $this->etape1_prime(md5($EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID));
    }
  }
  ...
  function corrige_etape1($id=0)
  {
    ...
    $infoAffiche  = 'SELECT ebtd.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID,det.EXECUTION_BUDGETAIRE_DETAIL_ID,exec.COMMENTAIRE,
                            ebet.QTE QTE_RACCROCHE,
                            ebtd.ETAPE_DOUBLE_COMMANDE_ID,
                            exec.EXECUTION_BUDGETAIRE_ID,
                            tache.CODE_NOMENCLATURE_BUDGETAIRE_ID,
                            tache.PTBA_TACHE_ID,
                            tache.DESC_TACHE,
                            act.PAP_ACTIVITE_ID,
                            ...
                            progr.INTITULE_PROGRAMME,
                            actions.ACTION_ID,
                            progr.CODE_PROGRAMME,
                            exec.DEVISE_TYPE_ID,
                            exec.DEVISE_TYPE_HISTO_ENG_ID,
                            actions.LIBELLE_ACTION,
                            inst.DESCRIPTION_INSTITUTION AS INTITULE_MINISTERE,
                            inst.CODE_INSTITUTION AS CODE_MINISTERE,
                            tache.SOUS_TUTEL_ID,
                            tache.INSTITUTION_ID,
                            exec.ENG_BUDGETAIRE,
                            exec.ENG_BUDGETAIRE_DEVISE,
                            exec.NUMERO_BON_ENGAGEMENT,
                            exec.MARCHE_PUBLIQUE,
                            inst.TYPE_INSTITUTION_ID,
                            exec.TRIMESTRE_ID
                     FROM execution_budgetaire_titre_decaissement ebtd JOIN execution_budgetaire exec ON exec.EXECUTION_BUDGETAIRE_ID=ebtd.EXECUTION_BUDGETAIRE_ID JOIN inst_institutions_ligne_budgetaire ligne ON ligne.CODE_NOMENCLATURE_BUDGETAIRE_ID=exec.CODE_NOMENCLATURE_BUDGETAIRE_ID JOIN inst_institutions inst ON inst.INSTITUTION_ID=exec.INSTITUTION_ID JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_DETAIL_ID=ebtd.EXECUTION_BUDGETAIRE_DETAIL_ID JOIN execution_budgetaire_execution_tache ebet ON ebet.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID
                     JOIN ptba_tache tache ON tache.PTBA_TACHE_ID=ebet.PTBA_TACHE_ID 
                     LEFT JOIN pap_activites act ON act.PAP_ACTIVITE_ID=tache.PAP_ACTIVITE_ID 
                     JOIN inst_institutions_programmes progr ON progr.PROGRAMME_ID=tache.PROGRAMME_ID 
                      
                     JOIN inst_institutions_actions actions ON actions.ACTION_ID=tache.ACTION_ID
                     WHERE md5(ebtd.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID) = "'.$id.'"';
    $infoAffiche = "CALL `getTable`('" .$infoAffiche."');";
    ...
    if (!empty($getProfil))
    {
      foreach ($getProfil as $value)
      {
        ...
        if ($prof_id == $value->PROFIL_ID || $prof_id==1)
        {
          ...
          $rej  = 'SELECT rej.DATE_TRANSMISSION,rej.MOTIF_REJET,rej.OBSERVATION FROM execution_budgetaire_tache_detail_histo rej WHERE rej.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID = "'.$data['get_info']['EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID'].'" ORDER BY EXECUTION_BUDGETAIRE_DETAIL_HISTO_ID  DESC';
          $rej = "CALL `getTable`('" .$rej. "');";
          $get_rej= $this->ModelPs->getRequeteOne($rej);

          $data['rejet'] = (!empty($get_rej)) ? $get_rej['MOTIF_REJET'] : '' ;

          $date_eng  = 'SELECT det.PATH_NOTE_A_LA_DCP,supp.PATH_LETTRE_TRANSMISSION,supp.PATH_LISTE_PAIE,supp.PATH_PPM,exec.NUMERO_BON_ENGAGEMENT,exec.PATH_BON_ENGAGEMENT,exec.DATE_BON_ENGAGEMENT AS DATE_ENGAGEMENT_BUDGETAIRE,exec.TYPE_ENGAGEMENT_ID,exec.ENG_BUDGETAIRE_DEVISE,det.COUR_DEVISE,det.DATE_COUR_DEVISE,exec.DEVISE_TYPE_ID AS TAUX_ECHANGE_ID,ebet.EST_SOUS_TACHE,ebet.EST_FINI_TACHE,RESULTAT_ATTENDUS,PATH_PV_ATTRIBUTION,PATH_AVIS_DNCMP,ID_TYPE_MARCHE,ebet.OBSERVATION_RESULTAT,BUDGETAIRE_TYPE_DOCUMENT_ID,INTRODUCTION_NOTE FROM execution_budgetaire_tache_info_suppl supp JOIN execution_budgetaire exec ON exec.EXECUTION_BUDGETAIRE_ID=supp.EXECUTION_BUDGETAIRE_ID JOIN execution_budgetaire_execution_tache ebet ON ebet.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID WHERE supp.EXECUTION_BUDGETAIRE_ID='.$data['get_info']['EXECUTION_BUDGETAIRE_ID'];   
          $date_eng = "CALL `getTable`('" . $date_eng . "');";
          $data['get_date_eng']= $this->ModelPs->getRequeteOne($date_eng);

          $callpsreq = "CALL `getRequete`(?,?,?,?);";     
          $bind_date_histo = $this->getBindParms('ETAPE_DOUBLE_COMMANDE_ID AS ETAPE_ID,EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID,OBSERVATION,DATE_TRANSMISSION','  execution_budgetaire_tache_detail_histo','EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID='.$data['get_info']['EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID'],'DATE_INSERTION DESC');
          ...
          $bind_etap_prev = $this->getBindParms('ETAPE_DOUBLE_COMMANDE_ID AS ETAPE_ID','execution_budgetaire_tache_detail_histo','EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID='.$data['get_info']['EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID'],'EXECUTION_BUDGETAIRE_DETAIL_HISTO_ID DESC');
  ...
  function etape1_correction()
  {
    ...
    if($this->validation->withRequest($this->request)->run())
    {
      ...
      $insertIntohist='execution_budgetaire_tache_detail_histo';
      $columhist="EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID,USER_ID,OBSERVATION,ETAPE_DOUBLE_COMMANDE_ID,DATE_TRANSMISSION,DATE_RECEPTION";
      $datacolumshist=$EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID.",".$USER_ID.",'".$COMMENTAIRE."',".$ETAPE_ID.",'".$DATE_TRANSMISSION."','".$DATE_RECEPTION."'";

      $this->save_all_table($insertIntohist,$columhist,$datacolumshist);

      $whereSuppl ="EXECUTION_BUDGETAIRE_ID = ".$EXECUTION_BUDGETAIRE_ID;
      $insertIntoSuppl='execution_budgetaire_tache_info_suppl';
      $columSuppl="BUDGETAIRE_TYPE_DOCUMENT_ID=".$BUDGETAIRE_TYPE_DOCUMENT_ID.", PATH_LETTRE_TRANSMISSION='".$trans_doc."', PATH_LISTE_PAIE='".$paie_doc."' ,ID_TYPE_MARCHE  =".$ID_TYPE_MARCHE;  
      $this->update_all_table($insertIntoSuppl,$columSuppl,$whereSuppl);

      $whereSuppl ="EXECUTION_BUDGETAIRE_ID = ".$EXECUTION_BUDGETAIRE_ID;
      $insertIntoSuppl='execution_budgetaire_execution_tache';
      $columSuppl="EST_SOUS_TACHE=".$sous_act.",EST_FINI_TACHE=".$fini.",RESULTAT_ATTENDUS='".$resultat_attend."',OBSERVATION_RESULTAT = '".$observ_resultat."',PTBA_TACHE_ID=".$PTBA_TACHE_ID.",QTE='".$QTE_RACCROCHE."'";  
      ...
      if($BUDGETAIRE_TYPE_DOCUMENT_ID==1)
      {
        $col_intro=',INTRODUCTION_NOTE="'.$intro_note.'"';  
      }

      $whereracc ="EXECUTION_BUDGETAIRE_ID = ".$EXECUTION_BUDGETAIRE_ID;
      $insertIntoracc='execution_budgetaire';
      $columracc="INSTITUTION_ID=".$INSTITUTION_ID.",SOUS_TUTEL_ID=".$SOUS_TUTEL_ID.",CODE_NOMENCLATURE_BUDGETAIRE_ID=".$CODE_NOMENCLATURE_BUDGETAIRE_ID.",TYPE_ENGAGEMENT_ID=".$TYPE_ENGAGEMENT_ID.",ENG_BUDGETAIRE=".$MONTANT_RACCROCHE.",ENG_BUDGETAIRE_DEVISE=".$MONTANT_EN_DEVISE.",DEVISE_TYPE_HISTO_ENG_ID=".$DEVISE_TYPE_HISTO_ID.",MARCHE_PUBLIQUE=".$MARCHE_PUBLIQUE.",COMMENTAIRE='".$COMMENTAIRE."',DEVISE_TYPE_ID=".$TYPE_MONTANT_ID.",NUMERO_BON_ENGAGEMENT='".$NUMERO_BON_ENGAGEMENT."'";
      ...
      if ($DATE_COUT_DEVISE === null)
      {
        $columDetail='COUR_DEVISE="'.$COUS_ECHANGE.'",COMMENTAIRE="'.$COMMENTAIRE.'",DATE_COUR_DEVISE=null'.$col_intro;
      }
      else
      {
        $columDetail='COUR_DEVISE="'.$COUS_ECHANGE.'",COMMENTAIRE="'.$COMMENTAIRE.'",DATE_COUR_DEVISE="'.$DATE_COUT_DEVISE.'"'.$col_intro;
      }
      ...
      if($MARCHE_PUBLIQUE==0)
      {
        $this->gestion_retour_ptba($EXECUTION_BUDGETAIRE_ID,$MONTANT_RACCROCHE);
      }
      else
      {
        $restant_t1=0;          
    ...
    {
      return $this->corrige_etape1(md5($EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID));
    }
  }
  ...
  function etape2($id=0)
  {
    ...
    $infoAffiche  = 'SELECT det.EXECUTION_BUDGETAIRE_DETAIL_ID,ebtd.ETAPE_DOUBLE_COMMANDE_ID,exec.EXECUTION_BUDGETAIRE_ID,exec.MARCHE_PUBLIQUE,BUDGETAIRE_TYPE_DOCUMENT_ID,ebtd.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID FROM execution_budgetaire_titre_decaissement ebtd JOIN execution_budgetaire exec ON exec.EXECUTION_BUDGETAIRE_ID=ebtd.EXECUTION_BUDGETAIRE_ID JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_DETAIL_ID=ebtd.EXECUTION_BUDGETAIRE_DETAIL_ID JOIN execution_budgetaire_tache_info_suppl suppl ON suppl.EXECUTION_BUDGETAIRE_ID=ebtd.EXECUTION_BUDGETAIRE_ID WHERE md5(ebtd.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID) = "'.$id.'"';
    ...
    if (!empty($getProfil))
    {
      foreach ($getProfil as $value)
      {
        if ($prof_id == $value->PROFIL_ID || $prof_id==1)
        {
          ...
          $date_eng  = 'SELECT exec.TYPE_ENGAGEMENT_ID FROM execution_budgetaire_titre_decaissement ebtd JOIN execution_budgetaire exec ON exec.EXECUTION_BUDGETAIRE_ID=ebtd.EXECUTION_BUDGETAIRE_ID WHERE ebtd.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID = "'.$data['info']['EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID'].'"';
          $date_eng = "CALL `getTable`('" . $date_eng . "');";
          $data['get_date_eng']= $this->ModelPs->getRequeteOne($date_eng);
  ...
  function save_etape2()
  {
    ...
    if($this->validation->withRequest($this->request)->run())
    {
      ...
      if ($nbre_analyse != $verifier )
      {
        ...
        session()->setFlashdata('alert', $data);
        return $this->etape2(md5($EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID));
      }
      else
      {
      ...
      if ($BUDGETAIRE_TYPE_DOCUMENT_ID == 1 AND $ID_OPERATION==2)
      {
        ...
        $typ_exec = $this->ModelPs->getRequeteOne($callpsreq, $typ_exec);
        $provenance=1;
        $url ='';
  ...
  // trouver  tous montants a partir de la tache choisie (cas de la correction)
