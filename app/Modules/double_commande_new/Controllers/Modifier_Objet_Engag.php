...
class Modifier_Objet_Engag extends BaseController
{
	...
	public function index($value='')
	{
		...
		// get BEs
		$bind_BEs=$this->getBindParms('exec.NUMERO_BON_ENGAGEMENT,td.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID','execution_budgetaire_titre_decaissement td JOIN execution_budgetaire exec ON exec.EXECUTION_BUDGETAIRE_ID=td.EXECUTION_BUDGETAIRE_ID JOIN execution_budgetaire_tache_detail det ON det.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID JOIN execution_budgetaire_execution_tache tache ON tache.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=tache.PTBA_TACHE_ID','NUMERO_BON_ENGAGEMENT IS NOT NULL AND ptba.INSTITUTION_ID IN (SELECT INSTITUTION_ID FROM user_affectaion WHERE USER_ID='.$user_id.') AND (exec.COMMENTAIRE IS NOT NULL OR det.TITRE_CREANCE IS NOT NULL) GROUP BY td.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID','NUMERO_BON_ENGAGEMENT ASC');
	...
	public function save()
	{
		...
		if ($this->validation->withRequest($this->request)->run())
		{
			...
			$callpsreq = "CALL getRequete(?,?,?,?);";
			$exec=$this->getBindParms('EXECUTION_BUDGETAIRE_ID,EXECUTION_BUDGETAIRE_DETAIL_ID','execution_budgetaire_titre_decaissement td','td.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID='.$EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID,'1');
			$exec= $this->ModelPs->getRequeteOne($callpsreq,$exec);

			//update dans detail
			$table1='execution_budgetaire_tache_detail';
			$conditions1='EXECUTION_BUDGETAIRE_DETAIL_ID='.$exec['EXECUTION_BUDGETAIRE_DETAIL_ID'];
	...
	public function get_info($EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID)
	{
		$callpsreq = "CALL getRequete(?,?,?,?);";
		$getdata = $this->getBindParms('exec.COMMENTAIRE,TITRE_CREANCE,MOTIF_LIQUIDATION','execution_budgetaire_titre_decaissement td JOIN execution_budgetaire exec ON exec.EXECUTION_BUDGETAIRE_ID=td.EXECUTION_BUDGETAIRE_ID JOIN execution_budgetaire_tache_detail ON execution_budgetaire_tache_detail.EXECUTION_BUDGETAIRE_ID=exec.EXECUTION_BUDGETAIRE_ID','td.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID='.$EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID,'exec.EXECUTION_BUDGETAIRE_ID ASC');
		$getdata = $this->ModelPs->getRequeteOne($callpsreq, $getdata);

		$TITRE_CREANCE=!empty($getdata['TITRE_CREANCE'])?$getdata['TITRE_CREANCE']:'null';
		$COMMENTAIRE=!empty($getdata['COMMENTAIRE'])?$getdata['COMMENTAIRE']:'null';
		$MOTIF_LIQUIDATION=!empty($getdata['MOTIF_LIQUIDATION'])?$getdata['MOTIF_LIQUIDATION']:'null';
