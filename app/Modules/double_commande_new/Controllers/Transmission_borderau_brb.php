...
class Transmission_borderau_brb extends BaseController
{
    ...
    $callpsreq = "CALL getRequete(?,?,?,?);";
  		...
  		foreach ($getProfil as $value)
      {
        ...
        {
        	$psgetrequete = "CALL `getRequete`(?,?,?,?);";
					$step = $this->getBindParms('td.EXECUTION_BUDGETAIRE_ID,det.EXECUTION_BUDGETAIRE_DETAIL_ID,td.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID,td.ETAPE_DOUBLE_COMMANDE_ID','execution_budgetaire_tache_detail det JOIN execution_budgetaire_titre_decaissement td ON td.EXECUTION_BUDGETAIRE_DETAIL_ID=det.EXECUTION_BUDGETAIRE_DETAIL_ID','td.ETAPE_DOUBLE_COMMANDE_ID ='. $etape_actuel, 'td.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID DESC');
					...
					$data['etape'] = $this->ModelPs->getRequeteOne($psgetrequete, $step);
					...
					$requetebase2 = "SELECT DATE_TRANSMISSION FROM execution_budgetaire_tache_detail_histo WHERE 
					EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID =" .$data['etape']['EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID'] . " ORDER BY DATE_INSERTION DESC";
					...
					FROM statut_operation_bordereau_transmission WHERE STATUT_OPERATION_BORDEREAU_TRANSMISSION_ID = 1";
					...
					$requetebase4 = "SELECT TYPE_DOCUMENT_ID,DESCR_DOCUMENT FROM type_document WHERE TYPE_DOCUMENT_ID = 2";
					...
					FROM statut_document_bordereau_transmission WHERE STATUT_DOCUMENT_BORDEREAU_TRANSMISSION_ID = 1";
					...
					$requetebase3 = "SELECT td.EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID,td.TITRE_DECAISSEMENT FROM execution_budgetaire_titre_decaissement td WHERE td.ETAPE_DOUBLE_COMMANDE_ID=".$ETAPE_DOUBLE_COMMANDE_ID;
	                    ...
	                    <tbody>
	                       ...
	                       <tr>
							...
							<tr>
								...
								<td><strong><font style="float:left;">' .$EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID. '</font></strong></td>
							</tr>
							<tr>
		...
		if($session->get('SESSION_SUIVIE_PTBA_TRANSMISSION_BRB') !=1)
    {
      return redirect('Login_Ptba/homepage'); 
		...
		foreach ($EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID as $key) {			
			//Recuperation titre_dec_id
			$maRequete5 = 'SELECT EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID,TITRE_DECAISSEMENT FROM execution_budgetaire_titre_decaissement
			WHERE EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID ='.$key .' ORDER BY EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID DESC';
			$fetch_data5 = $this->ModelPs->getRequeteOne("CALL `getTable`('" . $maRequete5 . "')");
			
			$NUMERO_DOCUMENT = $fetch_data5['TITRE_DECAISSEMENT'];
			...
			$columsinsert2 = "BORDEREAU_TRANSMISSION_ID,TYPE_DOCUMENT_ID,EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID,NUMERO_DOCUMENT, USER_ID, STATUT_DOCUMENT_BORDEREAU_TRANSMISSION_ID";

			$datacolumsinsert2 = $EXECUTION_BORD_TRANSMISSION_ID . ", " . $TYPE_DOCUMENT_ID . ", " . $key . ", '" . $NUMERO_DOCUMENT . "', " . $USER_ID . ", " . $STATUT_DOCUMENT_BORDEREAU_TRANSMISSION_ID . " ";
			...
			$columsinsert5 = "EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID,USER_ID, ETAPE_DOUBLE_COMMANDE_ID,DATE_RECEPTION,DATE_TRANSMISSION";
			$datacolumsinsert5 = $key . ", " . $USER_ID . ", " . $ETAPE_ID . ",'" . $DATE_RECEPTION . "', '" . $DATE_TRANSMISSION . "'";
			$this->save_all_table($table5, $columsinsert5, $datacolumsinsert5);


			//mise à jour dans la table execution_budgetaire_titre_decaissement
			$psgetrequete = "CALL `getRequete`(?,?,?,?);";
			$step = $this->getBindParms('ETAPE_DOUBLE_COMMANDE_ACTUEL_ID ,ETAPE_DOUBLE_COMMANDE_SUIVANT_ID', 'execution_budgetaire_etape_double_commande_config', 'ETAPE_DOUBLE_COMMANDE_ACTUEL_ID ='. $ETAPE_ID.' AND IS_SALAIRE=0', 'ETAPE_DOUBLE_COMMANDE_ACTUEL_ID ASC');
			$get_step = $this->ModelPs->getRequeteOne($psgetrequete, $step);
			$NEXT_ETAPE_ID =  $get_step['ETAPE_DOUBLE_COMMANDE_SUIVANT_ID']; // Etape suivante

			$table6 = 'execution_budgetaire_titre_decaissement';
			$where6 = 'EXECUTION_BUDGETAIRE_TITRE_DECAISSEMENT_ID='.$key;
