...
* Dashbord :: Tableau croisé dynamique en valeur
 le 29/08/2023 au 19 
...
$requetedebase = "SELECT ligne.CODE_NOMENCLATURE_BUDGETAIRE service,".$totaux." as vote,inst.DESCRIPTION_INSTITUTION AS INTITULE_MINISTERE,ligne.CODE_NOMENCLATURE_BUDGETAIRE,ptba.DESC_TACHE,ptba.PTBA_TACHE_ID, programme.INTITULE_PROGRAMME,actions.LIBELLE_ACTION,pap_activites.DESC_PAP_ACTIVITE,pap_activites.RESULTAT_PAP_ACTIVITE,exec.ENG_BUDGETAIRE AS ENG_BUDGETAIRE,exec.LIQUIDATION AS LIQUIDATION, exec.ENG_JURIDIQUE AS ENG_JURIDIQUE, exec.DECAISSEMENT AS DECAISSEMENT,exec.ORDONNANCEMENT AS ORDONNANCEMENT,exec.PAIEMENT AS PAIEMENT FROM execution_budgetaire exec LEFT  JOIN execution_budgetaire_execution_tache exec_tache ON exec.EXECUTION_BUDGETAIRE_ID=exec_tache.EXECUTION_BUDGETAIRE_ID  left JOIN ptba_tache ptba ON exec_tache.PTBA_TACHE_ID=ptba.PTBA_TACHE_ID LEFT JOIN inst_institutions inst ON inst.INSTITUTION_ID=ptba.INSTITUTION_ID LEFT JOIN inst_institutions_ligne_budgetaire ligne ON ligne.CODE_NOMENCLATURE_BUDGETAIRE_ID =ptba.CODE_NOMENCLATURE_BUDGETAIRE_ID LEFT JOIN inst_institutions_programmes programme ON programme.PROGRAMME_ID=ptba.PROGRAMME_ID LEFT JOIN inst_institutions_actions actions ON actions.ACTION_ID = ptba.ACTION_ID LEFT JOIN pap_activites ON pap_activites.PAP_ACTIVITE_ID=ptba.PAP_ACTIVITE_ID WHERE 1 ".$cond."";
    ...
    $data = array();
	  ...
	  ######liste des entites responsable
		function liste_institution_engage_votes() 
			...
			$var_search = !empty($_POST['search']['value']) ? $_POST['search']['value'] : null; 
			$query_principal="SELECT  ligne.CODE_NOMENCLATURE_BUDGETAIRE as service,".$totaux." as vote,inst.DESCRIPTION_INSTITUTION AS INTITULE_MINISTERE,ptba.DESC_TACHE,ptba.PTBA_TACHE_ID,programme.INTITULE_PROGRAMME,actions.LIBELLE_ACTION,pap_activites.DESC_PAP_ACTIVITE,pap_activites.RESULTAT_PAP_ACTIVITE, exec.ENG_BUDGETAIRE AS ENG_BUDGETAIRE,exec.LIQUIDATION AS LIQUIDATION, exec.ENG_JURIDIQUE AS ENG_JURIDIQUE, exec.DECAISSEMENT AS DECAISSEMENT,exec.ORDONNANCEMENT AS ORDONNANCEMENT,exec.PAIEMENT AS PAIEMENT FROM  execution_budgetaire exec JOIN execution_budgetaire_execution_tache exec_tache ON exec.EXECUTION_BUDGETAIRE_ID=exec_tache.EXECUTION_BUDGETAIRE_ID JOIN ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec_tache.PTBA_TACHE_ID JOIN inst_institutions inst ON inst.INSTITUTION_ID=ptba.INSTITUTION_ID JOIN inst_institutions_ligne_budgetaire ligne ON ligne.CODE_NOMENCLATURE_BUDGETAIRE_ID =ptba.CODE_NOMENCLATURE_BUDGETAIRE_ID JOIN inst_institutions_programmes programme ON programme.PROGRAMME_ID=ptba.PROGRAMME_ID JOIN inst_institutions_actions actions ON actions.ACTION_ID = ptba.ACTION_ID JOIN pap_activites ON pap_activites.PAP_ACTIVITE_ID=ptba.PAP_ACTIVITE_ID WHERE 1 ".$cond." ".$cond_trim."";
			...
			foreach ($fetch_data as $row) 
			{
				...
				$engagement[] ='<center><font color="#000000" size=2><label>'.mb_substr($row->LIBELLE_ACTION, 0, 5).'...<a class="btn-sm" data-toggle="tooltip" title="'.$row->LIBELLE_ACTION.'"><i class="fa fa-eye"></i></a></label></font> </center>';
                 ...
                 $engagement[] ='<center><font color="#000000" size=2><label>'.mb_substr($row->service, 0, 20).'...<a class="btn-sm" data-toggle="tooltip" title="'.$row->service.'"><i class="fa fa-eye"></i></a></label></font> </center>';
                     }
                   if (strlen($row->DESC_PAP_ACTIVITE) < 13){
                   $engagement[] ='<center><font color="#000000" size=2><label>'.$row->DESC_PAP_ACTIVITE.'</label></font> </center>';
                         }else{
		...
		function exporter($TYPE_INSTITUTION_ID='',$INSTITUTION_ID='',$SOUS_TUTEL_ID='',$IS_PRIVATE='',$PROGRAMME_ID='',$ACTION_ID='',$PAP_ACTIVITE_ID='',$LIGNE_BUDGETAIRE='')
		{
			...
			if(! empty($INSTITUTION_ID))
			{
				...
				if (! empty($cond_sy_req['INSTITUTION_ID']))
				{
					$cond1=' AND inst.INSTITUTION_ID='.$cond_sy_req['INSTITUTION_ID'];
					$cond_program=' AND `inst_institutions`.`INSTITUTION_ID`='.$cond_sy_req['INSTITUTION_ID'];
	...
	//Fonction pou appel des series et hichart
        public function get_rapport()
        {
        	...
        	$votes=("SELECT ".$totaux." as vote FROM  ptba_tache ptba JOIN inst_institutions inst ON inst.INSTITUTION_ID=ptba.INSTITUTION_ID  ".$cond." ".$cond22." ".$cond_pri1." ");
        	...
        	$engage=("SELECT SUM(ENG_BUDGETAIRE) as engage FROM `execution_budgetaire` exec JOIN execution_budgetaire_execution_tache exec_tache ON exec.EXECUTION_BUDGETAIRE_ID=exec_tache.EXECUTION_BUDGETAIRE_ID JOIN  ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec_tache.PTBA_TACHE_ID JOIN inst_institutions inst ON inst.INSTITUTION_ID=ptba.INSTITUTION_ID WHERE 1 ".$cond_trim." ".$cond."  ".$cond_pri." ");
        	...
        	$decaissement=("SELECT SUM(DECAISSEMENT) as decaissement FROM `execution_budgetaire` exec JOIN execution_budgetaire_execution_tache exec_tache ON exec.EXECUTION_BUDGETAIRE_ID=exec_tache.EXECUTION_BUDGETAIRE_ID JOIN  ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec_tache.PTBA_TACHE_ID JOIN inst_institutions inst ON inst.INSTITUTION_ID=ptba.INSTITUTION_ID WHERE 1 ".$cond_trim." ".$cond."  ".$cond_pri." ");
        	$jurdique=("SELECT SUM(ENG_JURIDIQUE) as jurdique FROM `execution_budgetaire` exec JOIN execution_budgetaire_execution_tache exec_tache ON exec.EXECUTION_BUDGETAIRE_ID=exec_tache.EXECUTION_BUDGETAIRE_ID JOIN  ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec_tache.PTBA_TACHE_ID JOIN inst_institutions inst ON inst.INSTITUTION_ID=ptba.INSTITUTION_ID WHERE 1 ".$cond_trim." ".$cond."  ".$cond_pri."");
        	...
        	$paiement=("SELECT SUM(PAIEMENT) as paie FROM `execution_budgetaire` exec JOIN execution_budgetaire_execution_tache exec_tache ON exec.EXECUTION_BUDGETAIRE_ID=exec_tache.EXECUTION_BUDGETAIRE_ID JOIN  ptba_tache ptba ON ptba.PTBA_TACHE_ID=exec_tache.PTBA_TACHE_ID JOIN inst_institutions inst ON inst.INSTITUTION_ID=ptba.INSTITUTION_ID WHERE 1 ".$cond_trim." ".$cond."  ".$cond_pri."");
