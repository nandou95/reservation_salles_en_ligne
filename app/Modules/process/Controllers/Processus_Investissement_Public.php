...
class Processus_Investissement_Public extends BaseController
{
		...
		if($session->get('SESSION_SUIVIE_PTBA_PIP_EXECUTION') !=1)
    {
      return redirect('Login_Ptba/homepage');
		...
		$order_by = '';
		$order_column = '';
		$order_column = array('DESCR_ETAPE', 'DESCR_ETAPE', 'COMMENTAIRE', 'proc_etape.DESCR_ETAPE', 'EMAIL', 1, 1, 1);
	...
	public function save_compilation($columsinsert, $datacolumsinsert)
	{
		...
		$idcomp = $this->ModelPs->createUpdateDelete($insertReqAgence, $bindparms);
		return $idcomp;
	}
		...
		if($session->get('SESSION_SUIVIE_PTBA_PIP_EXECUTION') !=1)
    {
      return redirect('Login_Ptba/homepage');
		...
		$file_compiler = $this->request->getFile('file_compiler');
		if (empty($file_compiler)) {
			$data = ['message' => lang('messages_lang.msg_mettre_doc')];
			session()->setFlashdata('alert', $data);
			return redirect('pip/Projet_Pip_A_Compiler/liste_pip_compiler');
		}
      ...
      return redirect('Login_Ptba/homepage');
		...
		if (!empty($getProfil)) {
			foreach ($getProfil as $value) {
				...
				$columnselect = "DISTINCT IS_CORRECTION_PIP,ACTION_ID,doc_pip.ETAPE_ID,DESCR_ACTION,GET_FORM,MOVETO,IS_COMPILE,prof_etapes.PROFIL_ID";
